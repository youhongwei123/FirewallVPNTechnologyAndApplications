...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |	1. 所有 RELATED、ESTABLISHED 状态流量放行；2. 入接口 veth-gwa-lan、出接口 wg0，源 10.1.0.0/24、目的 10.2.0.2、TCP 8080 端口新建连接放行 |A-TO-B-DENY: |DROP |
| `gwB` |1. 所有 RELATED、ESTABLISHED 状态流量放行；2. 入接口 wg0、出接口 veth-gwb-lan，源 10.1.0.0/24、目的 10.2.0.2、TCP 8080 端口新建连接放行 |VPN-S2S-DENY: |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功 |无 A-TO-B-DENY 拦截日志 |
| `siteA -> siteB:2222` | 失败 |失败 |存在 A-TO-B-DENY 日志，TCP DPT=2222 |
| `siteA -> siteB ping` | 失败 |失败 |存在多条 A-TO-B-DENY ICMP 类型 8 拦截日志 |
...
| `latest handshake` |9 seconds ago |30 seconds ago |
| `transfer` 计数 |received 180 B, sent 716 B |received 156 B, sent 212 B |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |siteA -> siteB:8080：gwA FORWARD 2 号规则（放行 A 内网发往隧道、目标 8080 新建 TCP 连接）、gwB FORWARD 2 号规则（放行隧道流入 B 内网、目标 8080 新建 TCP 连接） |
| `siteA -> siteB:2222` |失败 |siteA -> siteB:2222：gwA FORWARD 3 号 LOG 审计规则、4 号 REJECT 拒绝规则，流量在 gwA 出站侧直接拦截，不会抵达 gwB。 |
| `siteA -> siteB ping` |失败 |siteA -> siteB ping：gwA FORWARD 3 号 LOG 审计规则、4 号 REJECT 拒绝规则，流量在 gwA 出站侧直接拦截，不会抵达 gwB。 |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay | 192.0.2.1 ↔ 192.0.2.2|UDP 51820 |WireGuard 外层加密封装包，公网 underlay 地址传输加密流量 |
| `wg0` |	10.1.0.2 ↔ 10.2.0.2 |TCP（8080） |解密后的内层原始业务流量，站点内网真实 IP 通信 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP（8080） |解密、防火墙放行后，转发到 siteB 服务器的明文业务流量 |
...
答：1.使用场景不同：远程接入VPN是单个远程客户端设备接入企业内网，属于 “单设备访问整个内网”；站点到站点VPN是两个完整私有网段互通，实现两个办公/机房内网全段互访。2.AllowedIPs配置逻辑不同：远程接入：网关配置客户端隧道单IP，客户端配置完整内网网段；站点到站点：两端网关互相填写对方完整内网网段。3.路由需求不同：远程接入仅客户端配置指向隧道的内网路由；站点到站点两个网关都会自动生成对端全网段隧道路由，两侧内网主机均需默认网关指向本地VPN网关。4.拓扑结构不同：远程接入拓扑：客户端→VPN网关→内网服务器；
站点到站点拓扑：站点A内网↔gwA↔WireGuard隧道↔gwB↔站点B内网。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：AllowedIPs拥有自动路由下发和报文安全校验两个核心作用，填写对端内网网段才能满足站点互通需求。填写对端完整内网网段后，wg-quick会自动添加路由，所有去往该内网段的流量都会转发到wg0隧道接口，实现跨站点流量自动走VPN隧道；如果只填隧道单点IP，内网跨段流量不会进入隧道转发。同时WireGuard会校验隧道收到的内层报文源IP，只有源地址匹配peer的AllowedIPs网段才会解封装接收，填写完整对端子网可以拦截伪造内网IP的恶意隧道报文，提升隧道安全性。
站点到站点需要两个内网所有主机互相访问，因此不能只填写单个隧道地址，必须配置完整对端内网子网段。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：gwA和gwB都承担三层跨接口流量转发工作，ip_forward关闭时Linux内核会直接丢弃跨接口IP数据包。gwA需要转发两类流量：siteA内网发往隧道的出站流量，以及隧道解封装后发回siteA内网的回程流量；gwB同理，需要转发隧道到siteB内网、siteB内网回隧道的双向流量。只要任意一个网关没有开启ip转发，流量转发链路就会断裂，跨站点通信直接完全中断，因此两个网关都必须开启该参数。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：站点内普通主机本身没有对端内网网段的静态路由，访问跨站点网段的流量会匹配主机的默认路由。
主机将默认网关设为本地VPN网关后，访问异地内网的数据包会先发送到gwA/gwB，再由网关依靠WireGuard自动生成的对端网段路由送入隧道转发。如果主机默认网关没有指向本地VPN网关，访问对端网段的数据包会转发到错误出口设备，无法进入WireGuard隧道，跨站点访问直接失败。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：第一步先检查两个网关net.ipv4.ip_forward是否等于1，转发未开启是握手正常但不通的高频故障点。第二步查看两端网关路由表，确认存在对端内网网段、出接口为wg0的路由，AllowedIPs配置错误会缺失这条关键路由。第三步查看siteA、siteB主机路由，确认主机默认网关正确指向本地VPN网关，主机路由错误流量无法送达网关。第四步分层抓包定位故障：underlay抓包看加密UDP报文双向收发、wg0抓包看内层业务报文封装解封装、站点内网lan抓包看报文能否抵达目标服务器。第五步查看网关iptables FORWARD链，检查是否存在DROP/REJECT拦截规则、默认策略是否阻断流量。第六步通过journalctl查询A-TO-B-DENY、VPN-S2S-DENY内核拦截日志，确认流量是否被防火墙拒绝。最后检查siteB目标服务是否正常监听对应端口，服务器本地防火墙是否拦截入站流量。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：两边网关都配置防火墙规则可以实现纵深防御，gwA作为出站第一道防线，gwB作为入站第二道防线，即使gwA规则配置出错，gwB依然能保护siteB内网，避免内网直接暴露。同时双向规则可以分别管控两个站点主动发起的访问流量，后续放开siteB主动访问siteA时，gwB管控出站、gwA管控入站，精细化控制双向访问权限。两端独立的日志前缀能区分流量是在出站侧还是入站侧被拦截，故障排查时可以快速定位拦截位置，审计日志更完整清晰。只在单侧写规则功能上能实现访问控制，但存在明显安全缺陷，非法流量会完整走完WireGuard隧道才被丢弃，浪费隧道带宽；且仅单侧有拦截日志，故障溯源不完整，生产环境不推荐仅配置单侧规则。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答：WireGuard通过封装加密，在underlay底层隐藏了内网所有业务相关信息：隐藏站点真实内网IP（10.1.0.2、10.2.0.2）、内层TCP业务端口、ICMP ping报文、HTTP业务数据载荷，所有内层报文整体加密，底层抓包无法解析业务内容。underlay抓包依旧会暴露部分基础信息：两端网关的underlay公网IP地址、WireGuard固定UDP 51820端口；可以观测隧道的数据包数量、传输时长、流量大小，能识别存在VPN通信行为；外层UDP头部无加密，第三方可以识别该流量属于WireGuard协议。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答：WireGuard两端的AllowedIPs已经配置了完整对端内网网段，路由天然支持双向流量转发，无需修改隧道配置，首先要在gwB的FORWARD 链 LOG、REJECT规则上方添加出站放行iptables规则，匹配siteB内网网段发起、访问siteA目标服务端口的新建TCP连接，允许站点B流量转发进入WireGuard隧道，接着在gwA的FORWARD链LOG、REJECT规则上方添加入站放行iptables规则，匹配从wg0隧道流入、目标为siteA对应服务端口的新建TCP连接，让解封装后的流量能够转发至siteA内网，实验中原有的RELATED、ESTABLISHED全局放行规则无需改动，会自动放行TCP连接建立后的双向回程应答报文，两条新增放行规则分别管控出站与入站流量，形成双向纵深防护，同时必须保证放行规则写在日志、拒绝规则之前，否则流量会先被拦截导致访问失效。
