...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            |client 端：10.20.0.0/24；vpn 网关对 client 的 Peer 配置：10.10.10.1/32|
...
| `latest handshake` 是否还在      |在|
| 去 `10.20.0.0/24` 的路由是否消失 |已消失|
| 是否还能访问 `10.20.0.2:8080`    |不能|
| 失败的直接原因                   |client 路由表没有去往10.20.0.0/24的隧道路由，访问 10.20.0.2 的数据包不会被转发进 wg0 隧道，无法封装走 VPN 链路|

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
原因：WireGuard 握手依赖Endpoint的底层 underlay 地址（192.0.2.x UDP）协商密钥，该链路不受 AllowedIPs 配置影响，所以 wg show 握手正常；
业务流量依赖AllowedIPs 生成系统路由（出方向）+ 对端入站源 IP 白名单（入方向），AllowedIPs 改错后，目标网段没有指向 wg0 的路由，报文无法进入隧道转发。
路径结论：握手和业务数据是两套完全独立路径。
握手：底层公网 UDP（underlay 192.0.2.x 地址）；
业务数据：内层 VPN 私网 IP（10.x.x.x），受 AllowedIPs 路由管控。
...
| 协议                     |UDP|ICMP|
| 是否能看到 ICMP/HTTP 内容 |不能|可以|
| 为什么内外两层看到的不同 |报文经过 WireGuard 加密、UDP 封装，变成外层 UDP 数据包，只使用底层 underlay 地址传输|VPN 网关收到 UDP 包后，剥离外层 UDP 头部、解密数据，还原出原始明文 ICMP 数据包，从 wg0 虚拟网卡转发，因此两层报文地址、协议、载荷完全不一样|

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
对应关系：outer (underlay) 外层数据 = 信封（只记录信封收发双方的外部地址，看不到内部信件内容）；inner (wg0) 内层数据 = 信纸（原始真实通信内容、真实内网 IP 地址）。
安全意义：
① 隐藏通信主体隐私：公网链路只能看见两端底层公网 IP，中间人无法获知真实内网通信地址（10 段隧道 IP 与服务器内网 IP）；
② 传输数据加密防窃听：原始报文整体加密封装，链路抓包只能获取密文，没有 WireGuard 私钥无法解密还原业务明文，防止中间人窃取传输数据。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：两件作用：①出方向路由控制：wg-quick 依据 AllowedIPs 网段写入系统路由，去往该网段的数据包路由至 wg0 隧道接口；②入方向访问白名单：对端报文解密后，源 IP 不在 AllowedIPs 范围则直接丢弃。
   原因：wg-quick 启动时优先依据 AllowedIPs 自动生成操作系统路由表，路由决定数据包能否进入隧道，报文进入 wg 解密后才执行源 IP 校验，因此优先作用于路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手依靠 Endpoint 配置的 underlay 底层公网 UDP 地址完成密钥协商，不受 AllowedIPs 参数管控；业务报文收发依赖 AllowedIPs 生成隧道路由与入站白名单，受 AllowedIPs 配置约束。二者使用两套寻址路径，因此握手链路正常不代表业务可以连通。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：①本机所有流量全部导入 VPN 隧道，本地全部上网流量经由网关转发，造成流量泄露；②网关侧配置 0.0.0.0/0 会关闭入站源 IP 白名单校验，客户端失陷后攻击者可借助隧道横向遍历、渗透内网全部网段。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 使用对端公钥校验报文签名，vpn 网关存储客户端公钥，客户端需使用自身私钥对握手报文签名；攻击者缺少 client 私钥，无法生成合法签名，网关校验失败拒绝建连；vpn 私钥仅用于解密接收报文，无法伪造客户端身份。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2；
原因：原始内层 IP 报文被整体加密后封装在 UDP 载荷中传输，链路抓取的只有加密密文，无对应私钥无法解密，因此无法查看内层 IP 与业务数据。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①地址隐私防护：外层仅暴露底层公网 IP，隐藏两端真实内网隧道地址，中间人无法追踪真实通信主体；
   ②传输数据加密：原始业务报文加密封装传输，链路窃听者无法获取明文数据，防范中间人窃听窃取内容。
