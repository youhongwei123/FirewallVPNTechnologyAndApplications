...
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0x6f77                |
| 返回码（Response Code）      |0 No error（解析成功）                |
| 回答记录数（Answers）       | 5               |
| 返回的 IP 地址               |20.53.130.158                |
| TTL（若可见）                |Time to live: 213 (3 minutes, 33 seconds)                |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 用于将域名解析为对应的 IP 地址。计算机网络通信只能识别 IP 地址，不能直接使用域名，因此访问网站前必须先通过 DNS 解析得到目标服务器的 IP，才能建立连接。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致。Transaction ID 用于匹配 DNS 请求与响应，保证客户端收到的响应是对应自己发出的查询，避免多个 DNS 报文混淆。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录是 IPv4 地址记录，用于查询域名对应的 IPv4 地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用 UDP 协议，端口 53。当响应数据超过 512 字节、进行区域传输或需要可靠传输时，会使用 TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：1. 不支持 IPv6，只能解析 IPv4；2. 线程不安全，多线程环境下易出错；3. 错误处理方式老旧，功能有限。


6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：说明该域名启用了负载均衡，通过多个服务器 IP 分散访问压力，提高服务稳定性和访问速度。
