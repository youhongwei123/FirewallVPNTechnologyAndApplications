...
| 测试项 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `client -> 192.0.2.1` | 成功 |成功（ping 通，无丢包） |
| `server -> 10.20.0.1` | 成功 |成功（ping 通，无丢包） |
| `client -> 10.20.0.2` | 失败 |失败，提示 Network is unreachable |
...
| client 端 `AllowedIPs` |10.10.10.1/32, 10.20.0.0/24 |
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0、10.20.0.0/24 dev wg0 |
| 是否看到 `latest handshake` |是，已握手 46 秒 ago |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |ctstate RELATED,ESTABLISHED，所有接口、源 / 目的 |放行所有已建立 / 相关的连接（服务端回包、响应流量），保障双向通信正常 |
| 2 |ACCEPT |tcp dpt:8080 ctstate NEW，入接口 wg0，出接口 veth-vpn-lan，源 10.10.10.1，目的 10.20.0.2 |仅允许客户端发起访问服务端 8080 端口的新 TCP 连接 |
| 3 |LOG |入接口 wg0，日志前缀 VPN-DENY: ，日志级别 4 |对后续被拒绝的流量打日志，方便后续排查被拦截的源 / 目的地址和端口 |
| 4 |REJECT |入接口 wg0，返回 icmp-port-unreachable |拒绝所有从 wg0 进入的其他流量（如 ping、2222 端口），并返回明确的错误响应 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |行 1（状态规则）、行 2（8080 放行规则） |是（行 1 和行 2 的 pkts/bytes 都有增加） |
| `client -> server:2222` |成功 |行 3（LOG 规则）、行 4（REJECT 规则） |是（行 3 和行 4 的 pkts/bytes 都有增加） |
| `client -> server ping` |失败 |行 3（LOG 规则）、行 4（REJECT 规则） |是（行 3 和行 4 的 pkts/bytes 都有增加） |
...
| 日志前缀 |VPN-DENY |
| `IN=` |wg0（流量从 VPN 隧道进入） |
| `OUT=` |veth-vpn-lan（流量被转发到内网服务器） |
| `SRC=` |10.10.10.1（客户端隧道地址） |
| `DST=` |10.20.0.2（目标服务器地址） |
| `PROTO=` |TCP（访问 2222 端口） / ICMP（ping 请求） |
| `DPT=` |2222（TCP 端口） / 无（ICMP） |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |成功（无规则时可正常访问） |失败（连接超时 / 被拒绝） |是（日志已捕获） |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量是 “穿过” VPN 网关（从 wg0 进、veth-vpn-lan 出），目的地是内网 server，属于转发流量 |
| `client -> server:2222` | `FORWARD` |同上，仅端口未被允许，仍属于转发流量 |
| `client -> vpn:9090` | `INPUT` |流量目的地是 VPN 网关本机的 9090 端口（本机进程），未离开 VPN 网关，属于本机入站流量 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有已建立的连接（如 server:8080 的响应包）被防火墙丢弃，访问失败 |状态规则被删除后，服务端回包无法通过 FORWARD 链，双向通信中断 |重新添加状态规则：iptables -A FORWARD -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |无法捕获被拒绝流量的日志（如 2222 端口的请求） |LOG 规则不会中断数据包匹配，但如果放在 REJECT 后面，数据包会先被 REJECT 丢弃，无法触发 LOG 记录 |调整规则顺序，将 LOG 规则放在 REJECT 规则前面 |
| 管理面规则错写到 `FORWARD` |访问 vpn:9090 仍然成功，FORWARD 链的规则计数器不增加 |访问 VPN 网关本机的流量走 INPUT 链，不会经过 FORWARD 链，因此 FORWARD 链的规则对其无效 |将管理面拦截规则从 FORWARD 链移动到 INPUT 链 |
...
| 是否看到 WireGuard 握手 |是（wg show 显示 latest handshake） |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |FORWARD 链规则允许 8080 端口的新连接 |
| `client -> server:2222` |成功 |FORWARD 链的 REJECT 规则拦截了 2222 端口的请求 |
| `client -> server ping` |失败 |FORWARD 链的默认策略为 DROP，ICMP 请求被拦截 |
...
| underlay 抓包看到的协议 |UDP 51820（WireGuard 外层加密包） |
| `wg0` 抓包看到的源地址 |10.10.10.1（client 隧道地址） |
| 内网接口是否能看到被拒绝的 2222 连接 |否（请求被 FORWARD 链拦截，无法到达内网） |
...
| `client -> vpn:9090` 经过的链 |INPUT 链（访问网关本机） |
...
| 内层 TCP 连接目的地址 |10.20.0.2 |
| 内层 TCP 目标端口 |8080 |
| 内层连接状态 |ESTABLISHED（已放行） / SYN_SENT（被拒绝） |
| 外层 UDP 源地址 |192.0.2.2 |
| 外层 UDP 目的地址 |192.0.2.1 |
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |wg show（conntrack 跟踪的是业务连接，不是隧道本身） |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有已建立的连接（如 server:8080 的响应包）被丢弃，访问失败 |状态规则被删除后，服务端回包无法通过 FORWARD 链，双向通信中断 |重新添加状态规则：iptables -A FORWARD -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |无法捕获被拒绝流量的日志（如 2222 端口的请求） |LOG 规则不会中断数据包匹配，但如果放在 REJECT 后面，数据包会先被 REJECT 丢弃，无法触发 LOG 记录 |调整规则顺序，将 LOG 规则放在 REJECT 规则前面 |
| 管理面规则错写到 `FORWARD` |访问 vpn:9090 仍然成功，FORWARD 链的规则计数器不增加 |访问 VPN 网关本机的流量走 INPUT 链，不会经过 FORWARD 链，因此 FORWARD 链的规则对其无效 |将管理面拦截规则从 FORWARD 链移动到 INPUT 链 |
...
答：AllowedIPs：控制哪些内层 IP 流量可以通过 WireGuard 隧道转发，属于 “路由层面” 的访问控制。
iptables：控制流量的端口、协议和状态，属于 “防火墙层面” 的访问控制。
不能只靠 AllowedIPs：它只能限制 IP 范围，无法限制端口、协议和连接状态，也无法记录日志和拒绝特定请求。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：因为访问内网服务器的流量是 “穿过” VPN 网关的（从 wg0 接口进、veth-vpn-lan 接口出），目的地不是网关本机，因此走 FORWARD 链，而不是 INPUT 链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：访问会失败。服务端的响应包无法通过 FORWARD 链，双向通信中断，表现为连接超时或被拒绝。
4. 为什么 LOG 必须放在 REJECT 前面？
答：LOG 规则不会中断数据包匹配，只会记录日志；如果放在 REJECT 后面，数据包会先被 REJECT 丢弃，无法触发 LOG 记录，导致日志为空。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：意味着 WireGuard 的内层业务数据被加密，公网环境下无法窃听和解析，安全性高。
仅能暴露 WireGuard 隧道的存在和公网 IP，无法获取业务内容。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：可能导致用户访问未授权的敏感服务（如数据库、管理后台），扩大攻击面，一旦用户设备被攻陷，内网安全将受到严重威胁。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client -> vpn:9090：走 INPUT 链，因为目的地是 VPN 网关本机的 9090 端口。
client -> server:8080：走 FORWARD 链，因为目的地是内网服务器，需要网关转发。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：conntrack 跟踪的是业务连接状态，而 WireGuard 隧道本身是加密的 UDP 连接，不涉及 NAT 转换，因此不能视为 NAT 映射。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：仅允许指定的管理 IP 或内网地址访问，拒绝所有其他来源。
在 INPUT 链添加 LOG 规则，记录所有被拒绝的 SSH 访问请求，方便排查攻击行为。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答：①检查 server 服务是否正常监听端口；
②检查 vpn 网关的路由表，确保内网路由可达；
③检查 FORWARD 和 INPUT 链的防火墙规则；
④查看 iptables 规则计数器，确认请求是否命中；
⑤查看 dmesg 或 journalctl 中的日志，定位拒绝原因。
---
