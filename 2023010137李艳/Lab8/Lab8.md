...
| `wan` 无法回复的原因 |wan 的路由表中没有去往 10.0.0.0/24 的回程路由，无法将 ICMP 回复包发回内网主机，因此只有请求包、没有回复包 |
...
| NAT 把源地址改成了什么 |改为防火墙外网口地址 192.0.2.1 |
...
conntrack 额外记录了 NAT 地址映射关系（原始内网地址 10.0.0.2 和 NAT 后的外网地址 192.0.2.1）。这说明状态检测和 NAT 都依赖 conntrack 提供的连接状态与地址映射信息，两者共用同一套底层连接跟踪机制，因此家用路由器会把这两个功能集成在一起。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会用不同的源端口（或 ICMP 的 id 字段）来区分不同连接，把内网主机的地址 + 端口映射成不同的外网地址 + 端口，回复包根据端口号就能对应回正确的内网主机。
...
| TCP 38070 访问是否成功 |成功 |
| ping 是否成功 |失败 |
| ping 失败的原因 |防火墙 FORWARD 链默认拒绝所有流量，仅放行 TCP 38070 的新建及已建立连接，未放行 ICMP 协议，因此 ping 包被规则拦截 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址是 IANA 预留的内网专用地址段，互联网上的路由设备不会转发目的地址为私有网段的数据包，公网服务器也没有到这些网段的路由，因此内网主机无法被公网直接访问。NAT 在内网出口设备上，将内网主机的私有地址转换为出口设备的公网 IP 地址，使外网服务器认为请求来自出口设备；外网的回复包也只会发回出口设备，再由 NAT 根据连接跟踪记录还原为内网地址转发给对应主机，从而解决私有地址无法公网路由的问题。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：两者都能修改数据包的源 IP，但适用场景不同：
MASQUERADE 会自动获取出口接口的当前 IP 作为转换后的源地址，适合外网 IP 动态变化的场景，如家用宽带拨号，IP 变动时无需修改规则。
SNAT --to-source <IP> 需手动指定固定的转换 IP，适合外网 IP 固定的场景，如企业专线，规则更稳定，性能略高。
本实验中出口 IP 固定，两者效果相同，但实际场景中需根据 IP 是否固定选择。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：会给应用层载荷中内嵌 IP 地址或端口信息的协议带来麻烦，典型如 FTP、SIP、H.323 等。原因是 NAT 仅修改网络层的 IP 头部，不会解析和修改应用层载荷中的 IP / 端口信息。当这些协议的控制报文中包含内网地址时，外网设备收到后无法识别，导致连接建立失败，这类协议通常需要额外的 ALG（应用层网关）支持才能在 NAT 环境下正常工作。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：状态检测需要 conntrack 记录连接的状态信息，以判断数据包是否属于合法连接；NAT 需要 conntrack 记录地址 / 端口映射关系，以正确还原回复包的目的地址。两者共用同一套连接跟踪表，集成实现可以减少系统开销，提升转发效率，同时方便统一管理连接状态，因此家用路由器会将防火墙状态检测和 NAT 功能集成在同一设备中。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在 fw 命名空间的 filter 表 FORWARD 链中，添加放行 ICMP 协议的规则即可：sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT该规则会允许所有 ICMP 流量通过防火墙，使 ping 命令可以正常执行，同时保留原有的 TCP 38070 状态检测规则。
