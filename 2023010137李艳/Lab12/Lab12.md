...
| client 端 `AllowedIPs` |10.20.0.0/24, 10.10.10.1/32 |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |所有入 / 出网卡，所有源目地址，连接状态为 RELATED、ESTABLISHED | 放行已建立连接或相关应答流量，保障回程数据包正常通行|
| 2 |ACCEPT | 入网卡 wg0，出网卡 veth-vpn-lan，源 10.10.10.1，目的 10.20.0.2，TCP 目的端口 8080，新建连接|允许 VPN 客户端访问 server 的 8080 端口 TCP 服务 |
| 3 |LOG |入网卡 wg0，所有源目地址，限流 5 包 / 分钟，日志前缀 VPN-DENY |记录所有从 VPN 隧道进入、未匹配放行规则的流量日志 |
| 4 |REJECT | 入网卡 wg0，所有源目地址|拒绝所有未匹配放行规则的 VPN 流量，返回端口不可达 ICMP 报文 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 2 行 ACCEPT（wg0 入、veth-vpn-lan 出、TCP 8080 新建连接） | 是|
| `client -> server:2222` |失败 |第 3 行 LOG + 第 4 行 REJECT |是 |
| `client -> server ping` |失败 |第 3 行 LOG + 第 4 行 REJECT |是 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |client -> vpn:9090 |可正常连通（无拦截、无日志） |是（产生 VPN-MGMT-DENY 日志） |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |数据包源 client、目的 server，二者是不同主机，需要 vpn 命名空间转发，走 FORWARD 转发链 |
| `client -> server:2222` | `FORWARD` |流量跨两台主机，需要 vpn 网关转发，匹配 FORWARD 链过滤规则 |
| `client -> vpn:9090` | `INPUT` |数据包目的 IP 是 vpn 网关本机（10.10.10.1），目标为本机进程，不走转发，进入 INPUT 入站链处理 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 server:8080 超时 / 连接失败 | 客户端请求被放行，但服务端回程响应包无匹配规则，被 FORWARD 默认策略丢弃|在 FORWARD 链首行重新添加 ESTABLISHED,RELATED 放行规则 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但无法抓取对应日志 | iptables 按顺序匹配规则，数据包命中 REJECT 后直接终止流程，后方 LOG 规则不会执行| 调整规则顺序，将 LOG 规则放置在 REJECT 规则之前|
| 管理面规则错写到 `FORWARD` | client -> vpn:9090 依旧可以正常访问，规则计数器不增长、无日志| 访问网关本机的流量走 INPUT 链，FORWARD 链规则无法拦截本机入站流量| 删除 FORWARD 链上错误规则，将拦截 + 日志规则写入 INPUT 链|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |匹配 FORWARD 链放行规则，允许该流量转发 |
| `client -> server:2222` |失败 |未匹配放行规则，依次命中 LOG、REJECT 规则被拦截 |
| `client -> server ping` |失败 |未匹配放行规则，依次命中 LOG、REJECT 规则被拦截 |
...
| :--- | :--- |
| `VPN-DENY` 日志条数 | 3 条 |
| 日志中被拒绝的目标端口 | 2222（TCP）/ 无（ICMP ping）|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 8080 超时、无法连通 | 回程响应包被默认 DROP，连接无法完整建立| 重新在 FORWARD 首行添加状态放行规则|
| `LOG` 放在 `REJECT` 后面 |访问被拒，无对应日志输出 | 规则顺序错误，REJECT 提前终止数据包处理| 把 LOG 规则调整到 REJECT 之前|
| 管理面规则错写到 `FORWARD` |9090 端口正常访问，规则无计数、无日志 |访问本机流量走 INPUT 链，FORWARD 规则不生效 | 删除错误规则，将规则写入 INPUT 链|
...
AllowedIPs：WireGuard 隧道配置项，作用是路由控制，仅决定哪些 IP 网段的流量会被送入 VPN 隧道加密转发，不区分端口、协议，也不做流量拦截。
iptables：Linux 防火墙，作用是访问控制与流量过滤，可基于 IP、端口、协议、连接状态精细化放行 / 拒绝流量，同时支持日志审计。
原因：AllowedIPs 只能控制流量是否进入隧道，一旦流量进入隧道，就无法限制客户端访问内网的具体端口与服务，会造成权限过大，存在安全风险，因此必须配合 iptables 实现最小权限管控。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
INPUT 链仅处理目标为本机进程的入站数据包；而客户端访问内网服务器的流量，目的地址并非 VPN 网关本机，VPN 网关仅承担路由转发功能，这类跨主机转发流量由 FORWARD 链处理，因此规则需要配置在 FORWARD 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端发起的 TCP SYN 请求可以被放行并到达服务器，但服务器返回的应答包属于 ESTABLISHED/RELATED 状态，因无对应放行规则，会被 FORWARD 链默认策略丢弃。最终表现为连接超时、访问失败。

4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则按照从上到下顺序匹配执行。REJECT 会直接终止数据包的处理流程，若 LOG 放在后方，数据包不会走到 LOG 规则，无法生成日志。LOG 仅记录日志，不会终止流量，因此必须将 LOG 置于 REJECT 之前。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
安全意义：WireGuard 对内层业务流量全程加密，公网（underlay）抓包无法获取内网 IP、端口、业务数据，有效防止数据窃听与流量分析。
可暴露信息：外层公网 IP、WireGuard 默认端口 51820、两端通信时长、流量大小，攻击者可基于这些信息识别 VPN 服务，并针对性发起端口扫描、流量攻击。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
权限失控：VPN 用户可访问内网所有服务器、数据库、管理后台，极易造成数据泄露、越权操作；
横向渗透：若客户端被入侵，攻击者可借助 VPN 隧道对内网进行横向移动，攻陷整个内网；
缺少审计：全量放行无法针对性拦截恶意流量，安全事件难以追溯；
网络拥塞：无限制访问会增加内网与 VPN 网关的负载。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> vpn:9090：走 INPUT 链，流量目标是 VPN 网关本机的应用进程，属于本机入站流量。
client -> server:8080：走 FORWARD 链，流量需要经 VPN 网关转发至另一台主机，属于跨主机转发流量。
核心区别：数据包的最终目标不同，iptables 根据路由结果分配至不同规则链处理。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
工作层级不同：WireGuard 是应用层 / 网络层加密隧道，负责数据包加密、解密与隧道封装；conntrack 是内核连接状态跟踪模块，仅记录连接五元组与状态，不做加密 / 封装。
功能不同：NAT 仅修改 IP / 端口，不改变数据内容；WireGuard 会对整个内层数据包加密封装，和 NAT 原理完全不同。
状态维护不同：WireGuard 隧道握手、保活由 wg 组件独立维护，不依赖 conntrack；conntrack 只跟踪解密后的内层业务连接与外层 UDP 通信。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
允许访问来源：仅允许企业内网固定 IP、运维管理终端 IP，禁止公网与 VPN 客户端网段访问。
日志配置：在 INPUT 链针对 SSH 端口（22）添加 LOG 规则，自定义日志前缀（如 SSH-ACCESS:），记录访问源 IP、时间；对非信任源 IP 添加 REJECT 规则并单独标记拒绝日志，便于安全审计与告警。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
标准排查顺序（由近到远、由易到难）：
检查服务监听：确认目标服务器端口正常监听，服务未宕机；
检查路由表：确认客户端、VPN 网关、服务端路由配置正确，可达目标网段；
检查防火墙规则与计数器：查看 iptables 规则顺序、默认策略，观察规则计数器判断流量是否命中；
查看系统 / 防火墙日志：通过 dmesg/journalctl 查看拒绝日志、报错信息，定位拦截点。
---
