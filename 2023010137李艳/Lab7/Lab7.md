...
| :--- | :--- |
| 只有第一条规则时是否成功 |失败 |
| 第一条规则放行的是什么方向的包 |服务端 → 客户端（响应包） |
| 失败的原因 |防火墙默认策略是 DROP，仅放行请求包但未放行响应包，TCP 三次握手无法完成，导致请求超时 |
| 第二条规则匹配的是什么 |客户端 → 服务端的响应包 |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 | 2条|
...
这条无状态规则仅基于端口放行数据包，无法区分 “响应包” 和 “主动发起的连接包”，存在安全漏洞，会导致外网主机可以主动利用该端口发起对内部网络的连接请求。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
场景：内网用户主动访问外网服务，同时需要阻止外网主动向内网发起连接
无状态规则困境：
若要允许内网访问外网的响应包通过，就必须开放对应端口，这会同时允许外网主机主动从该端口向内网发起连接，无法区分 “合法响应” 和 “主动攻击”。
有状态规则解决方式：
使用 iptables -A FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT 这类规则，仅放行已建立连接或相关的响应包，不允许任何外网主动发起的新连接，在保证内网用户正常访问外网的同时，完全阻止了外网对内网的主动探测和攻击。
![无状态规则](stateless.png)
...
| 是否需要手动考虑返回方向 |需要 |不需要 |
| 能否区分"新连接"和"响应包" |不能 | 能|
| `wan` 能否主动发起到 `lan` 的连接 |可以 | 不可以|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为有状态防火墙通过连接跟踪（conntrack）模块，会自动记录并维护连接状态。当 lan 主动向 wan 发起连接请求时，fw 会将该连接标记为 NEW 状态；当 wan 回复响应包时，该包会被识别为 RELATED 或 ESTABLISHED 状态，直接匹配有状态规则中 --ctstate RELATED,ESTABLISHED -j ACCEPT 这一条，因此即使没有手动配置返回方向的规则，响应包也能被放行。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
无状态规则：--sport 38070 仅基于源端口放行数据包，不区分连接状态。wan 侧主机只要从 38070 端口发起连接，其 SYN 包就会被该规则放行，因此无法阻止主动连接。
有状态规则：通过 --ctstate NEW 限制，仅放行 lan 主动发起的新连接；同时通过 --ctstate RELATED,ESTABLISHED 仅放行已有连接的响应包。wan 主动发起的连接请求属于 NEW 状态，且不匹配 lan 主动发起的规则，因此会被防火墙默认策略拒绝，从而阻止了主动连接。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:46140 |
...
防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的记录，而非 IP/TCP 包头中的字段。
两者的核心区别：
conntrack 表记录这是系统维护的一个 “连接状态表”，里面记录了每一条 TCP 连接的完整状态（如 NEW、ESTABLISHED、RELATED 等），是动态的、由内核维护的状态信息。防火墙的 --ctstate ESTABLISHED 规则，正是通过查询这张表，判断该包是否属于一个已建立的、活跃的连接。
包头字段IP/TCP 包头中并没有一个叫 ESTABLISHED 的字段，它只有 SYN、ACK、FIN、RST 等控制位。虽然 ACK 包是连接建立后常见的包，但仅靠 ACK 标志无法区分 “合法响应包” 和 “伪造的攻击包”。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤仅根据数据包的源 / 目的地址、端口、协议等静态包头信息进行过滤，不维护连接上下文，无法区分 “新连接请求” 和 “已有连接的响应包”。
有状态检测防火墙通过 conntrack 模块维护连接状态表，记录连接的完整生命周期（NEW/ESTABLISHED/RELATED 等），能基于连接状态动态放行合法响应包，无需手动配置双向规则。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：顺序意义：ESTABLISHED,RELATED 规则放在最前面，可以优先放行已有连接的响应包，减少后续规则匹配次数，提高效率；同时确保合法响应包不会被 NEW 规则阻挡。
能否调换：不能调换。如果将 NEW 放在前面，所有包（包括已有连接的响应包）都会先匹配 NEW 规则，响应包因不满足 NEW 状态而被默认策略拒绝，导致正常通信中断。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被识别为 NEW 状态。
防火墙会将其视为新的连接请求，如果 NEW 规则未放行，则该包会被防火墙默认策略拒绝；若 NEW 规则放行，会创建一条新的 conntrack 条目，但原连接已中断，通信无法继续。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已有连接相关但独立的辅助连接流量，这些流量不是原连接本身的包，但与原连接有逻辑关联。
典型场景：FTP 协议的主动模式下，数据通道是与控制通道相关的独立连接，防火墙会将其标记为 RELATED 状态；或者 ICMP 错误消息（如 port unreachable），它们属于已有连接的相关反馈流量。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答："默认拒绝，按需放行" 的核心优势是最小权限原则：所有流量默认被阻断，仅放行明确允许的服务，攻击面被严格限制，未授权流量即使存在漏洞也无法进入网络。
"默认允许，事后补堵" 会开放所有端口和服务，攻击者可利用任何未被及时发现的漏洞发起攻击，事后封堵存在滞后性和安全风险。
