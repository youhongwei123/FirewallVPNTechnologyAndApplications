...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |	放行所有 RELATED、ESTABLISHED 状态连接；放行 veth-gwa-lan 入、wg0 出，源 10.1.0.0/24 访问 10.2.0.2 的 TCP 8080 新建流量 |A-TO-B-DENY: |DROP |
| `gwB` |放行所有 RELATED、ESTABLISHED 状态连接；放行 wg0 入、veth-gwb-lan 出，源 10.1.0.0/24 访问 10.2.0.2 的 TCP 8080 新建流量 |VPN-S2S-DENY: | DROP|
...
| `siteA -> siteB:2222` | 失败 |失败 |	存在 A-TO-B-DENY: 日志，无 VPN-S2S-DENY: |
| `siteA -> siteB ping` | 失败 |失败 |存在 A-TO-B-DENY: 日志，无 VPN-S2S-DENY: |
...
| underlay 是否连通 |是（gwA 与 gwB 可互相 ping 通） |
...
| `latest handshake` |实时更新（如 10 seconds ago） |实时更新（如 10 seconds ago） |
| `transfer` 计数 |接收 / 发送均有数值（如 1.23 KiB received, 1.12 KiB sent） | 接收 / 发送均有数值（如 1.12 KiB received, 1.23 KiB sent）|

### C. 路由表

| 节点 | 远端网段路由 | 是否指向 wg0 |
| :--- | :--- | :--- |
| `gwA` | `10.2.0.0/24` | 是（10.2.0.0/24 dev wg0 scope link）|
| `gwB` | `10.1.0.0/24` 是（10.1.0.0/24 dev wg0 scope link） |

### D. 策略测试

| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |gwA：允许 ESTABLISHED,RELATED + 允许 8080 新建连接；gwB：允许 ESTABLISHED,RELATED + 允许 8080 新建连接 |
| `siteA -> siteB:2222` |失败 |gwA：匹配 LOG 规则（A-TO-B-DENY）+ REJECT 规则；gwB 无命中（流量未到达） |
| `siteA -> siteB ping` |失败 |gwA：匹配 LOG 规则（A-TO-B-DENY）+ REJECT 规则；gwB 无命中（流量未到达） |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 ↔ 192.0.2.2 |UDP 51820 |WireGuard 外层加密包，仅暴露 underlay 地址和 VPN 端口 |
| `wg0` |10.1.0.2 ↔ 10.2.0.2	 |TCP 8080 |解密后的内层业务包，显示真实内网地址和业务端口 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 | 转发到 B 站点内网的明文业务包|
...
远程接入 VPN 是单个远程客户端接入企业内网，AllowedIPs 客户端填内网、网关填客户端隧道地址；站点到站点 VPN 是两个完整私有网段互通，两端网关互相填写对方内网网段，面向整网段流量互通。

2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
AllowedIPs 一方面指定去往该网段的流量通过对应 WireGuard 对等端转发，另一方面校验隧道收到数据包的源地址合法性；站点互通需要把对端整个内网网段加入，才能实现两端内网互相转发流量。

3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
IP 转发关闭时网关只能处理本机流量，无法在不同接口之间转发数据包；gwA 需要把内网流量转发进隧道，gwB 需要把隧道流量转发到内网，因此两个网关都必须开启 IP 转发。

4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
siteA、siteB 主机的路由表没有对端网段路由，访问跨站点地址时不知道如何转发，配置默认网关指向本地 VPN 网关后，所有非本地网段流量都会交给网关，由网关通过 VPN 隧道转发到对端。

5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
先检查 gwA、gwB 是否开启 IP 转发；再核对 WireGuard 两端 AllowedIPs 和隧道路由是否正确；接着检查 siteA/siteB 主机默认路由是否配置；然后临时清空防火墙测试是否被拦截；最后分别在 underlay、wg0、内网接口抓包定位丢包位置，同时排查 MTU 分片问题。

6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
双网关配置防火墙可以实现纵深防御，一端规则失效时另一端仍能防护内网，同时提前在 gwA 拦截无效流量，节省隧道带宽；只在单侧配置规则技术上可以实现，但安全性差，无效流量会全部占用隧道带宽，存在内网暴露风险。

7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
VPN 隐藏了两端内网 IP、内网网段、业务端口、内层数据包协议等内网拓扑信息；但会暴露网关互联 underlay 地址、WireGuard 固定 UDP 端口、数据包长度与收发时序等外层传输信息。

8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
在 gwB 新增规则放行 siteB 内网去往 siteA 目标端口的新建流量，在 gwA 新增规则放行隧道流入、访问 siteA 目标端口的新建流量，两条规则均插入在连接状态放行规则之后、日志拒绝规则之前，仅限定对应源网段、目标主机与端口，遵循最小权限原则。
---
