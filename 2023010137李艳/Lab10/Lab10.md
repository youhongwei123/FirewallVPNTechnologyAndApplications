...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |curl 提示无法连接 / 网络不可达，无响应 |
| `office → 10.0.3.2:8080` |成功 |返回 HTTP 目录列表（HTML 代码），说明服务正常 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate ESTABLISHED,RELATED |放行已建立连接 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 ctstate NEW | 放行 8080 新连接|
...
> 答：203.0.113.1（防火墙的外网 IP）。因为 conntrack 会自动把 DMZ 发出来的包的源 IP 从 10.0.3.2 还原成 203.0.113.1，所以 wan 侧完全感知不到内网的存在。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条连接的后续包仍然可以正常转发。因为 conntrack 条目已经记录了该连接的双向地址映射关系，即使删除 DNAT 规则，只要连接还没超时，防火墙依然能根据现有条目完成地址改写，只有新发起的连接会受规则删除的影响。
...
> 答：说明 LOG 规则是在 DNAT 改写之后 触发的。因为 FORWARD 链在 nat 表的 PREROUTING 链（DNAT 改写）之后执行，所以日志里看到的是改写后的内网地址 10.0.3.2。
如果要记录改写前的原始目标地址，需要把 LOG 规则放在 nat 表的 PREROUTING 链，在 DNAT 规则之前执行，这样就能记录到未改写的公网 IP 203.0.113.1。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以发现。
识别方法：在短时间内，来自同一个源 IP（如 203.0.113.2）的日志中，出现大量不同的 DPT（目的端口）字段，说明该 IP 正在对防火墙进行端口扫描。例如日志中连续出现 DPT=8080、DPT=2222、DPT=3306 等多个不同端口的连接请求，就是典型的端口扫描行为。
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat |	PREROUTING | dport 8080，目标 IP 203.0.113.1| DNAT，映射到 10.0.3.2:8080|
| DNAT 2222 |nat | 	PREROUTING|dport 2222，目标 IP 203.0.113.1 |DNAT，映射到 10.0.3.2:2222 |
| FORWARD 8080 |filter | FORWARD|dport 8080，目标 IP 10.0.3.2，状态 NEW |ACCEPT |
| FORWARD 2222 |filter |FORWARD | dport 2222，目标 IP 10.0.3.2，状态 NEW| ACCEPT|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT（改写目标地址）发生在 nat 表的 PREROUTING 链（数据包进入防火墙后、路由决策前）。
SNAT（改写源地址）发生在 nat 表的 POSTROUTING 链（路由决策后、数据包离开防火墙前）。
角色分工：
PREROUTING：处理入站方向的 DNAT，将外网请求的目标 IP / 端口改写为内网服务地址，使数据包能路由到 DMZ。
POSTROUTING：处理出站方向的 SNAT，将内网服务器访问外网的源 IP 改写为防火墙公网 IP，确保响应包能正确返回。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先 “抢到” 包。因为 PREROUTING 链的处理在 INPUT 链之前，DNAT 会先把目标地址改写为内网 IP，数据包会被路由转发到 DMZ，不会进入 fw 本地服务。
共存方法：
把本地服务改监听不同端口（如 8081），外网通过 203.0.113.1:8081 访问；
或者在 DNAT 规则中，通过源 IP / 接口区分流量，例如来自内网的请求访问本地服务，来自外网的请求执行 DNAT。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 会为每个连接维护一个完整的五元组映射记录（源 IP、源端口、目标 IP、目标端口、协议）。当外网请求 203.0.113.1:8080 时，conntrack 记录映射关系为 203.0.113.1:8080 → 10.0.3.2:8080；请求 203.0.113.1:8081 时，记录映射关系为 203.0.113.1:8081 → 10.0.3.3:8080。
当内网服务器回复时，防火墙根据 conntrack 表中的反向映射，自动将回复包的源 IP / 端口还原为对应的公网 IP / 端口，确保外网客户端收到正确的响应。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能回到 wan。因为 conntrack 连接跟踪机制会自动维护反向地址映射，将内网服务器回复包的源 IP 从 10.0.3.2 还原为防火墙的公网 IP 203.0.113.1，外网客户端收到的响应源 IP 就是公网 IP，不需要额外配置 SNAT 规则。
补充说明：如果 dmz 服务器主动访问外网（非端口映射触发的连接），则必须配置 SNAT 规则，否则外网响应包无法路由回内网私有地址。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：不能正常工作。当 conntrack 表满时，防火墙无法记录新的连接映射关系，新的连接请求会被丢弃，导致端口映射和 NAT 功能失效，合法用户也无法访问服务。
后果：服务中断、拒绝服务攻击（DoS）成功。
防护措施：
增大 conntrack 表的最大容量（调整 net.netfilter.nf_conntrack_max 参数）；
缩短无效连接的超时时间（调整 nf_conntrack_tcp_timeout_established 等参数）；
配置防火墙规则限制单 IP 的并发连接数；
启用状态检测，丢弃无效的半开连接。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础：连接跟踪（conntrack）。所有功能都依赖 conntrack 来维护连接状态，实现状态检测、地址转换、流量审计和端口映射。
安保系统类比：
Lab7 有状态防火墙：对应大楼的门禁系统，只允许合法人员（已建立连接的流量）进出，拦截未授权的访问；
Lab8 NAT：对应大楼的内部员工通道，将内部员工的身份（私有 IP）统一转换为大楼对外的身份（公网 IP），隐藏内部网络结构；
Lab9 分区审计：对应大楼的监控系统，记录所有进出的人员和行为，便于事后追溯；
Lab10 端口映射：对应大楼的访客服务窗口，将外部访客的请求（公网端口）转发到内部的服务部门（内网服务器），只开放指定的服务入口。
