...
| :--- | :--- |
| 只有第一条规则时是否成功 |否|
| 第一条规则放行的是什么方向的包 |lan→wan 的请求包（目的端口 38070）|
| 失败的原因 |wan→lan 的响应包目的端口是临时端口（不是 38070），不匹配第一条规则（--dport 38070），且此时没有其他放行规则，因此被默认 DROP 丢弃|
| 第二条规则匹配的是什么 |源端口 38070 的包（wan→lan 的响应包）|
| 加第二条规则后是否成功 |是|
| 当前规则共几条 |2|
...
答：无状态规则只检查单个包的头部字段（如源端口），无法区分这个包是属于已建立的连接还是全新的连接请求。


2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：FTP 协议的数据通道连接（端口 20）由控制通道（端口 21）动态协商产生，无状态防火墙无法提前知道该放行哪个临时端口，有状态防火墙可通过 RELATED 状态识别并自动放行。
...
| 是否需要手动考虑返回方向 |是|否|
| 能否区分"新连接"和"响应包" |否|是|
| `wan` 能否主动发起到 `lan` 的连接 |能（如果源端口是 38070）|不能|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
答：因为响应包属于已经建立的连接。当 lan 发出的 SYN 包（NEW）被放行后，conntrack 在表中记录了这条连接。响应包（SYN+ACK）到达 fw 时，conntrack 将其识别为 ESTABLISHED 状态，匹配第一条规则 --ctstate ESTABLISHED,RELATED -j ACCEPT，因此被放行。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态规则的判断依据是 conntrack 表中的连接记录。wan 主动发起的 SYN 包在 conntrack 表中没有对应的记录，状态为 NEW，但它不匹配第二条规则（因为方向是 wan→lan，而第二条规则匹配的是 dport 38070，即 lan→wan 方向），因此被默认 DROP。无状态规则只看源端口是否为 38070，不关心这个包是否属于已有连接，所以无法区分是响应还是新连接。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:38040|
| 条目中的目的地址和目的端口 |192.0.2.2:38070|
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |ESTABLISHED（还有 SYN_RECV, FIN_WAIT, LAST_ACK, TIME_WAIT）|

简答题：

1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
答：依据的是 conntrack 表里的记录。区别在于：包头字段是每个包独立携带的信息（如源/目的 IP、端口），而 conntrack 记录是防火墙维护的关于连接的状态信息，记录了双向流量的上下文，防火墙通过查表判断这个包属于哪条连接及其当前状态。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤对每个包独立判断，只检查包头字段（如 IP、端口、协议）；有状态检测防火墙维护连接状态表，将每个包放到所属连接的上下文中判断，能区分 NEW、ESTABLISHED、RELATED、INVALID 等状态。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：不能调换。因为大多数包（响应、数据传输）都是 ESTABLISHED 状态，放在第一条可以快速匹配放行，提高效率。如果调换，NEW 规则放在前面，每个包都要先检查是否 NEW（大多数不匹配），再继续检查 ESTABLISHED，效率降低。另外从安全逻辑上，先放行已建立的流量更合理。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会变成 INVALID 状态。防火墙如果配置了 DROP INVALID 规则（或在默认 DROP 策略下），则会丢弃该包。这可以防止攻击者发送与已超时连接相关的伪造包。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 描述与已有连接相关但不属于同一连接的流量。常见场景：FTP 的数据通道（由控制通道协商端口）、ICMP 错误回应（如端口不可达）、主动模式的 FTP 数据传输等。本实验是简单的 HTTP 请求，没有衍生子连接，所以没有出现 RELATED 状态。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：默认拒绝遵循最小权限原则，只开放明确需要的端口/服务，减少攻击面，任何未明确允许的流量都被拒绝。默认允许则容易遗漏关闭不应开放的服务，规则是"补丁式"的，容易产生安全漏洞，且新增的服务可能无意中被暴露。
