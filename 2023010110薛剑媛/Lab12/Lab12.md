...
| 测试项 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `client -> 192.0.2.1` | 成功 |成功（0% packet loss）|
| `server -> 10.20.0.1` | 成功 |成功（0% packet loss）|
| `client -> 10.20.0.2` | 失败 |失败（网络不可达）|
...
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1与10.20.0.0/24 dev wg0 scope link|
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功 |成功（返回目录列表）|
| `client -> server:2222` | 成功 |成功（返回目录列表）|
| `client -> server ping` | 成功 |成功（0% loss）|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT|ctstate RELATED,ESTABLISH| 放行已建立连接和关联连接的回复包|
| 2 |ACCEPT|-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 -p tcp --dport 8080 ctstate NEW|只允许 VPN 客户端（10.10.10.1）访问内网服务器（10.20.0.2）的 8080 端口（新连接）|
| 3 |LOG|-i wg0 配合 limit（5/min）|记录所有其他来自 VPN 的转发流量，日志前缀 "VPN-DENY: "，限速避免日志泛滥|
| 4 |REJECT|-i wg0|拒绝其他所有来自 VPN 的转发流量，返回 icmp-port-unreachable|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功|规则2 (ACCEPT)|是（从 1→2）|
| `client -> server:2222` |失败|规则3+4 (LOG+REJECT)|是（从 4→8）|
| `client -> server ping` |失败|规则3+4 (LOG+REJECT)|是|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量经过 vpn 网关转发到内网服务器（目的地址不是 vpn 本机）|
| `client -> server:2222` | `FORWARD` |流量经过 vpn 网关转发到内网服务器（目的地址不是 vpn 本机）|
| `client -> vpn:9090` | `INPUT` |流量目的地是 vpn 网关本机的 9090 端口，属于发给 vpn 自身进程的包|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 server:8080 失败（curl 超时）|新连接请求能通过 ACCEPT 规则到达 server，但 server 返回的响应包不匹配任何允许规则，被默认 DROP。缺少 ESTABLISHED,RELATED 规则导致响应包无法返回|重新添加 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 规则到 FORWARD 链最前面|
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但日志中没有 WRONG-ORDER 记录|iptables 按顺序匹配。包先命中 REJECT 规则，处理立即结束，后面的 LOG 规则没有机会执行|把 LOG 规则放在 REJECT 前面|
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 仍然成功，FORWARD 链中 WRONG-CHAIN 规则计数器为 0|目的地是 vpn 本机进程的包走 INPUT 链，不走 FORWARD 链。即使 FORWARD 写了拒绝规则也拦不住|管理面规则必须写在 INPUT 链
|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功|命中 FORWARD 链规则2（ACCEPT），允许访问 8080 端口|
| `client -> server:2222` |失败|命中 FORWARD 链规则3+4（LOG+REJECT），2222 端口被拒绝|
| `client -> server ping` |失败|命中 FORWARD 链规则3+4（LOG+REJECT），ICMP 被拒绝|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 server:8080 失败（curl 超时）|新连接请求能通过 ACCEPT 规则到达 server，但 server 返回的响应包不匹配任何允许规则，被默认 DROP|重新添加 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 规则到 FORWARD 链最前面|
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但日志中没有 WRONG-ORDER 记录|iptables 按顺序匹配，包先命中 REJECT 规则，处理立即结束，后面的 LOG 规则没有机会执行|把 LOG 规则放在 REJECT 前面|
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 仍然成功，FORWARD 链中规则计数器为 0|目的地是 vpn 本机进程的包走 INPUT 链，不走 FORWARD 链|管理面规则必须写在 INPUT 链|
...
答：AllowedIPs 解决的是：哪些目标地址的流量应该进入 VPN 隧道（路由控制），以及对端发来的包源地址是否被接受（入口白名单）。iptables 解决的是：哪些流量允许通过（端口、协议、连接状态等细粒度访问控制）。为什么不能只靠 AllowedIPs：AllowedIPs 只能控制到网段级别，无法区分端口（如 8080 vs 2222）、协议（TCP vs ICMP）。它只能决定"能否访问这个 IP 范围"，不能决定"能访问这个 IP 范围的哪个服务"。iptables 可以提供精细的端口级和协议级控制。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：INPUT 链：处理目的地是 VPN 网关本机的流量。FORWARD 链：处理穿过 VPN 网关转发到其他设备（内网服务器）的流量。client 访问 server（10.20.0.2）时，包的目的地是 server，不是 vpn 本机，所以包会走 FORWARD 链，不会进入 INPUT 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：访问会失败。原因：client 发出的新连接请求（SYN 包）能命中 --dport 8080 的 ACCEPT 规则，成功到达 server，server 返回的响应包方向是 veth-vpn-lan → wg0，这个包是已建立连接的一部分（ESTABLISHED 状态），删除 ESTABLISHED,RELATED 规则后，响应包不匹配任何允许规则，被默认 DROP，导致 client 收不到响应，curl 超时。

4. 为什么 LOG 必须放在 REJECT 前面？
答：iptables 规则按顺序从上到下匹配。一旦匹配到 REJECT 或 DROP 规则，包会被立即丢弃，处理结束，后面的规则不会再执行。如果 LOG 放在 REJECT 后面，包在被拒绝之前不会触发 LOG，日志就永远不会产生。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：对安全的意义：中间人无法看到内层真实通信内容（HTTP 请求、ICMP 类型等），也无法知道内层源和目的地址，实现了内容保密和通信关系隐藏。还能暴露的信息：外层 UDP 包仍会暴露：两个 VPN 节点在通信（源 IP 192.0.2.2，目的 IP 192.0.2.1），使用 WireGuard 协议（UDP 端口 51820），通信的时间模式、包大小（可能推断出流量模式），但看不到具体内容。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：横向移动风险：攻击者攻陷一个 VPN 客户端后，可以扫描和攻击整个内网。敏感数据泄露：VPN 用户可以访问数据库、财务系统等敏感服务。权限过大：普通员工可能访问本不应接触的系统。违规操作：无法按最小权限原则控制。难以审计：访问范围太广，难以追踪异常行为。DDoS 风险：恶意用户可能从内网发起攻击。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client -> vpn:9090 经过 INPUT 链，因为目的地是 VPN 网关本机的 9090 端口，包需要交给本机进程处理；client -> server:8080 经过 FORWARD 链，因为目的地是内网服务器（10.20.0.2），包需要被 VPN 网关转发到其他设备，不交给本机。简单说：给本机走 INPUT，穿过本机走 FORWARD。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：WireGuard 隧道不是 NAT 映射，因为 NAT 是修改 IP 地址/端口的转换关系，而 WireGuard 是完整的加密隧道——外层 UDP 包是隧道传输层，内层 IP 包是独立的上层数据，两者之间没有地址转换关系，而是完整的加密封装。conntrack 跟踪的是解密后内层连接的状态（用于 iptables 的 ESTABLISHED 匹配），而 WireGuard 隧道自身的状态由 wg show 独立维护，两者各司其职。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：应该只允许特定的管理 IP 地址或 VPN 隧道内的特定管理员地址（如 10.10.10.100/32）访问 SSH 端口，不应允许所有 VPN 用户访问。日志写法：先用 -I INPUT 1 添加允许规则，再用 -I INPUT 2 添加 LOG 规则（带 --log-prefix "SSH-DENY: " 和 limit 限速），最后用 -I INPUT 3 添加 REJECT 规则，注意 LOG 必须在 REJECT 前面才能记录被拒绝的访问。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答：检查顺序：首先确认目标服务是否在监听（如 ss -lntp | grep 端口），然后检查 client 路由表是否有到目标网段且指向 wg0 的路由，再检查 vpn 的路由表，接着查看 iptables FORWARD 和 INPUT 规则及计数器（-v 参数看 pkts 列判断哪条规则命中），之后检查日志（dmesg | grep "VPN-DENY"），最后用 conntrack 确认内层连接状态。通常业务访问失败的原因是路由表缺失（AllowedIPs 配置错误）或 iptables 规则阻止（如缺少 ESTABLISHED 规则），而 wg show 显示握手成功只说明底层隧道正常，不代表业务能通。
