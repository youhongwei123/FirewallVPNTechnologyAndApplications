...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS将用户输入的域名（如www.yxnu.edu.cn）转换为对应的IP地址（如202.203.86.93），访问网站时需要先进行 DNS 解析，因为浏览器和网络设备需要通过 IP 地址才能定位到目标服务器并建立连接，而域名本身无法在底层的 IP 网络中直接路由。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：是一致的。Transaction ID 是一个 16 位的标识符，用于将 DNS 查询与对应的响应进行精确匹配。由于 UDP 协议是无连接的，服务器在响应时通过回传完全相同的 Transaction ID，使得客户端能够确认这个响应是针对自己先前发出的特定查询的。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录是 DNS 记录中的一种类型，它将一个域名直接映射到一个 IPv4 地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 作为传输层协议，端口是 53。当响应数据包过大需要使用TCP进行可靠传输时。某些安全或可靠性要求高的场景，因为 TCP 能提供有序、可靠的连接。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：这个函数是较老的接口，不支持 IPv6，线程安全性也较差，在实际工程中通常更推荐使用 `getaddrinfo()`。但在入门阶段，`gethostbyname()` 更容易帮助理解“域名解析”这件事是如何发生的。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：当一个用户请求访问某个域名时,域名服务器会根据预先设置的负载均衡策略,随机或轮询地返回多个对应的 IP 地址。 这些 IP 地址通常指向同一个网站,但分布在不同的服务器上。 通过这种方式,可以避免单台服务器承担过重的访问压力,提高整个系统的性能和可用性。
