...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败|curl: (7) Could not connect to server|
| `office → 10.0.3.2:8080` |成功|返回了 HTML 目录列表|
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED|（放行已建立连接）|
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 ctstate NEW|（放行新连接）|
---
...
> 答：203.0.113.1（fw 的外网接口 IP）。wan 完全感知不到 DNAT 的存在，它以为自己在和 203.0.113.1:8080 通信。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：仍然能正常工作。因为 conntrack 已经记录了映射关系，后续包（包括回复包）会直接根据 conntrack 表项进行地址转换，不再需要重新匹配 iptables 规则。只有当连接完全结束后，conntrack 条目被清除，新连接才会因为没有 DNAT 规则而失败。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |成功（返回 HTML）|有|
| `wan → 203.0.113.1:2222` |成功（返回 HTML）|有|
| `wan → 203.0.113.1:3306` |失败（connection refused）|无|

![多端口映射测试](multi_port.png)
---
...
| 日志中 `DST=` 字段是哪个地址 |10.0.3.2（DNAT 改写后的地址）|
...
> 答：这说明 LOG 规则在 PREROUTING 链的 DNAT 改写之后 触发（即 FORWARD 链上）。因为 DNAT 在 PREROUTING 链完成地址改写，而 FORWARD 链在路由决策之后执行，此时目标地址已经被改写为 10.0.3.2。如果要记录原始目标地址（改写前），应该把 LOG 规则放在 nat 表的 PREROUTING 链、在 DNAT 规则之前。因为 PREROUTING 链在路由决策之前执行，DNAT 规则也在这个链上，所以把 LOG 规则放在 DNAT 规则之前就能看到原始地址 203.0.113.1。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：能发现。端口扫描的特征是：短时间内从同一个源 IP（SRC=...）向多个不同的目标端口（DPT=）发起 NEW 连接请求，但这些连接大多不会完成完整的 TCP 三次握手。识别方法：一是按源 IP 分组统计，如果某个 IP 在短时间内有大量不同端口的 LOG 记录，且没有对应的 ESTABLISHED 连接后续包，很可能是端口扫描。二是使用命令分析：sudo journalctl -k | grep "DNAT-TO-DMZ" | awk '{print $14}' | cut -d'=' -f2 | sort | uniq -c | sort -nr 可以统计每个源 IP 的访问次数。三是如果日志中出现连续的、递增的目标端口号（如 DPT=8080, 8081, 8082...），也是典型扫描行为。
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 发生在 PREROUTING 链（路由决策之前），SNAT 发生在 POSTROUTING 链（路由决策之后）。如果 DMZ 既对外提供服务，又需要访问外网：PREROUTING 负责 DNAT（外部访问进来的请求，改写目标地址）POSTROUTING 负责 SNAT（内部访问外网时，改写源地址），两者可能同时存在，conntrack 会分别记录两种映射关系。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先抢到。因为 PREROUTING 链在本地路由决策之前执行，DNAT 规则先改写目标地址，包就不再被认为是"发往本机"，而是被转发。要让两者共存，可以：修改 DNAT 规则的匹配条件，增加 ! -d 203.0.113.1 或使用不同的公网 IP或者让本地服务监听不同的端口（如 8081）。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 表记录了完整的五元组映射（源IP、源端口、目标IP、目标端口、协议）。当 dmz 服务器回复时，conntrack 会根据原始方向的记录，把回复包的源地址和源端口还原为对应的公网 IP 和端口。因为 conntrack 条目中保存了 sport 和 dport 的对应关系，所以能精确还原。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能。因为 conntrack 在 DNAT 时会自动建立逆向映射。当 dmz 回复包（src=10.0.3.2）经过 fw 时，conntrack 识别出这是之前 DNAT 连接的回复，自动把源地址还原为 203.0.113.1。这就是 conntrack 的"状态追踪"功能——不需要额外的 SNAT 规则。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：当 conntrack 表满时，新连接无法建立，防火墙性能急剧下降甚至拒绝服务，但已有连接不受影响。防护措施包括：调大 conntrack 表容量（如 `net.netfilter.nf_conntrack_max=655360`）、缩短连接超时时间（如 `net.netfilter.nf_conntrack_tcp_timeout_established=600`）、对高流量端口使用 `NOTRACK` 跳过 conntrack，以及部署 DDoS 防护设备。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础是 conntrack（连接追踪） 和 Netfilter 框架。安保系统类比：Lab7 有状态防火墙 = 安保人员记住"谁进来了"，只放行有记录的对话，Lab8 NAT（SNAT） = 访客接待处，把内部人员身份转换成统一的对外身份，Lab9 分区审计 = 各区域门禁 + 监控摄像头，记录谁违规闯入，Lab10 端口映射（DNAT） = 大楼总机，外部来电转接到内部对应分机。
