...
| 测试 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> 10.1.0.1` | 成功 |成功(0% packet loss)|
| `siteB -> 10.2.0.1` | 成功 |成功(0% packet loss)|
| `gwA -> 192.0.2.2` | 成功 |成功(0% packet loss)|
| `siteA -> 10.2.0.2` | 失败 |失败(Destination Net Unreachable)|
...
| `siteA -> siteB ping` | 成功 |成功(0% packet loss)|
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |-s 10.1.0.0/24 -d 10.2.0.2 -p tcp --dport 8080 -j ACCEPT|A-TO-B-DENY:|DROP|
| `gwB` |-s 10.1.0.0/24 -d 10.2.0.2 -p tcp --dport 8080 -j ACCEPT|VPN-S2S-DENY:|DROP|
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功|无（被 ACCEPT）|
| `siteA -> siteB:2222` | 失败 |失败（Could not connect）|有（A-TO-B-DENY）|
| `siteA -> siteB ping` | 失败 |失败（Destination Port Unreachable）|有（A-TO-B-DENY）|
...
| `latest handshake` |10 seconds ago|19 seconds ago|
| `transfer` 计数 |180 B received, 272 B sent|124 B received, 180 B sent|
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功|gwA 规则2（ACCEPT，pkts=1）和 gwB 规则2（ACCEPT，pkts=1）|
| `siteA -> siteB:2222` |失败|gwA 规则3（LOG，pkts=4）和规则4（REJECT，pkts=4）|
| `siteA -> siteB ping` |失败|gwA 规则3（LOG，pkts=4）和规则4（REJECT，pkts=4）|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1:51820 ↔ 192.0.2.2:51820|UDP 51820|外层加密包（WireGuard 封装）|
| `wg0` |10.1.0.2 ↔ 10.2.0.2:8080|TCP 8080|解密后的内层包（HTTP 请求/响应）|
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2:8080|TCP 8080|转发到 B 站点内网的包|
...
答：远程接入 VPN 是单个远程用户通过隧道访问企业内网，网关配置客户端隧道地址作为 AllowedIPs，客户端配置内网网段；站点到站点 VPN 是两个私有网络通过互联网安全互通，两端网关都配置对端内网网段作为 AllowedIPs，实现网段间的双向通信。本实验属于站点到站点 VPN，siteA 和 siteB 两个网段通过 gwA 和 gwB 之间的隧道互通。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：AllowedIPs 在 WireGuard 中既用于路由（发往这些 IP 的包走隧道），也用于加密过滤（只接受来自这些 IP 的包）。在站点到站点 VPN 中，gwA 需要把发往 siteB 内网（10.2.0.0/24）的包通过隧道发给 gwB，所以 gwA 上 peer gwB 的 AllowedIPs 写 10.2.0.0/24；同理 gwB 上写 10.1.0.0/24。这样 WireGuard 会自动添加对应的路由表条目。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：IP 转发允许网关在不同网络接口之间转发 IP 包。gwA 需要在 veth-gwa-lan（内网）和 wg0（隧道）之间转发包，gwB 需要在 wg0（隧道）和 veth-gwb-lan（内网）之间转发包。如果某个网关没有开启转发，包到达后不会被转发到另一个接口，跨站点通信就会失败。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：siteA 的默认网关指向 gwA（10.1.0.1），siteB 的默认网关指向 gwB（10.2.0.1）。这样 siteA 发往 siteB（10.2.0.2）的包会被路由到 gwA，gwA 再通过 WireGuard 隧道转发给 gwB，最终到达 siteB。如果没有配置默认网关，siteA 不知道如何到达 10.2.0.0/24 网段，通信就会失败。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：排查顺序：首先确认 wg show 中 latest handshake 是最近的（几秒内），确认隧道已建立；然后检查两个网关的 IP 转发是否开启（sysctl net.ipv4.ip_forward）；再检查两个网关的路由表是否正确，是否有对端内网网段指向 wg0 的路由；接着检查两个站点内网主机的默认网关是否指向本地网关；最后检查 iptables FORWARD 链是否放行了相关流量，特别是 ESTABLISHED,RELATED 规则是否存在。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：两边都写规则实现了纵深防御。在 gwA 写规则是第一道防线，在 gwB 写规则是第二道防线——即使 gwA 配置错误（例如 ACCEPT 规则被误写成允许所有流量），gwB 仍能拦截不符合策略的流量。本实验中 gwB 的 LOG 计数器为 0 就证明了流量在 gwA 已被正确拦截。只在一边写规则也可以，但存在单点风险：如果那台网关的规则被误删除或绕过，整个站点就失去了保护。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答：VPN 隐藏了内网真实地址（10.1.0.2 和 10.2.0.2）、内网拓扑结构、具体应用层协议内容（在加密包内）。但暴露了通信双方网关的公网地址（192.0.2.1 和 192.0.2.2）、使用的协议（UDP）、端口（51820）、包大小和时间模式。攻击者虽然不知道内网具体在传输什么，但可以观察到两个站点之间有持续的加密流量。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答：需要在 gwA 增加一条反向允许规则：-A FORWARD -i wg0 -o veth-gwa-lan -s 10.2.0.0/24 -d 10.1.0.2 -p tcp --dport <服务端口> -m conntrack --ctstate NEW -j ACCEPT，同时在 gwB 增加对应的出口允许规则：-A FORWARD -i veth-gwb-lan -o wg0 -s 10.2.0.0/24 -d 10.1.0.2 -p tcp --dport <服务端口> -m conntrack --ctstate NEW -j ACCEPT。由于 ESTABLISHED,RELATED 规则已存在，返回流量会自动放行。
