...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |all  --  * 0.0.0.0/0  |ctstate RELATED,ESTABLISHED |
| 2 |ACCEPT |cp  --  wg0    veth-vpn-lan  10.10.10.1  |tcp dpt:8080 ctstate NEW |
| 3 |LOG |all  --  wg0    *   0.0.0.0/0 |limit: avg 5/min burst 10 LOG flags 0 level 4 prefix "VPN-DENY: " |
| 4 |REJECT |all  --  wg0    *       0.0.0.0/0 |reject-with icmp-port-unreachable |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 2 行 ACCEPT |是 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量途经 VPN 网关转发，源目都不是网关本机 |
| `client -> server:2222` | `FORWARD` |流量途经 VPN 网关转发，源目都不是网关本机 |
| `client -> vpn:9090` | `INPUT` |目的地址是网关本机监听的服务，数据包交付本机上层应用 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |8080 端口 curl 超时、无法访问 |客户端 SYN 能转发，但服务器回程 ACK 无放行规则被 DROP，TCP 握手无法完成 |将该规则重新插入 FORWARD 链首行 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但无对应审计日志 |iptables 顺序匹配，命中 REJECT 直接终止规则遍历，LOG 不会执行 |调换顺序，LOG 在前、REJECT 在后 |
| 管理面规则错写到 `FORWARD` |client 依旧能正常访问 vpn:9090，规则计数器不变、无日志 |访问本机服务走 INPUT 链，FORWARD 链规则永远不会匹配 |删除 FORWARD 内错误规则，在 INPUT 链添加日志 + 拒绝规则 |
...
| client wg0 地址 |10.10.0.1/24 |
| vpn wg0 地址 |10.10.0.1/24 |
| 是否看到 WireGuard 握手 |10.10.0.2/24 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |匹配专属端口 ACCEPT 转发规则 |
| `client -> server:2222` |失败 |无放行规则，被 LOG+REJECT 拦截 |
| `client -> server ping` |失败 |ICMP 无放行规则，默认拒绝转发 |
...
| 内网接口是否能看到被拒绝的 2222 连接 |不能，数据包在网关 FORWARD 链直接丢弃 |
...
| :--- | :--- |
| 内层 TCP 连接源地址 |10.10.10.1 |
| 内层 TCP 连接目的地址 |10.20.10.2 |
| 内层 TCP 目标端口 |8080 |
| 内层连接状态 |ESTABLISHED |
| 外层 UDP 源地址 |192.0.2.2 |
| 外层 UDP 目的地址 |192.0.2.1 |
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |用wg show |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端访问 server:8080 出现 curl 超时、TCP 握手无法完成，访问失败 |服务器返回的 ACK 应答回程流量无匹配的放行规则，被 FORWARD 链默认 DROP，导致 TCP 三次握手中断 |将 ESTABLISHED,RELATED 规则重新插入到 FORWARD 链的首行，优先放行回程应答与关联连接 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝的流量无对应审计日志，日志计数器无增长，无法留存违规访问记录 |数据包命中 REJECT 规则后会直接丢弃报文、终止规则匹配流程，后置的 LOG 规则永远没有机会执行 |调换两条规则的顺序，将 LOG 规则前置、REJECT 规则后置 |
| 管理面规则错写到 `FORWARD` |客户端依然能正常访问 VPN 网关的 9090 管理端口，规则计数器无增长、无对应拒绝日志 |访问 VPN 网关本机运行的服务的流量，会进入 INPUT 链处理，不会经过负责转发流量的 FORWARD 链 |删除 FORWARD 链内的错误规则，在 INPUT 链中重新添加管理端口的日志记录与拒绝规则 |
...
答：AllowedIPs：WireGuard 路由控制，仅决定哪些网段流量进入加密隧道封装转发，是粗粒度路由规则，只管 “走不走隧道”。
iptables：四层精细化访问控制，隧道解密后对源 IP、目的 IP、协议、端口做放行 / 拒绝，实现最小权限。
若仅依靠 AllowedIPs 缺陷，流量一旦进入隧道，内网所有端口、ICMP、后台服务全部放开，无端口隔离，接入 VPN 即可横向遍历内网，权限完全失控。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：client→vpn→server 流量只是途经 VPN 网关转发，数据包源、目的都不是网关本机进程，经过 FORWARD 链；INPUT 链仅处理目的地是网关本机运行程序的数据包，转发流量不会进入 INPUT 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：客户端 SYN 新建连接可以命中 ACCEPT 规则发往内网服务器；但服务器回程应答数据包无匹配放行规则，被 FORWARD 默认 DROP。TCP 三次握手无法完成，curl 长时间超时，访问失败。

4. 为什么 LOG 必须放在 REJECT 前面？
答：iptables 规则自上而下逐条匹配；数据包命中 REJECT 后直接丢弃报文，规则匹配流程终止，后置 LOG 永远不会执行，无法留存审计日志。LOG 仅打印日志不阻断流量，必须前置。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：意味着内层内网 IP、业务端口、HTTP 载荷、访问目标全部加密封装，中间人无法窃听业务数据、无法测绘内网拓扑，传输安全性高；会泄露VPN 网关公网 IP、客户端出口公网 IP、WireGuard 固定 UDP 51820 端口；攻击者可扫描该端口尝试暴力破解 WG 密钥。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：VPN 接入终端中毒或被劫持后，攻击者可横向扫描整个内网，遍历全部业务主机；无端口隔离，可直接访问数据库、运维后台、备份服务器、监控节点等高敏感服务；单点 VPN 接入失守，等同于整个企业内网直接暴露在公网。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client→server:8080：FORWARD 链；流量穿越网关转发给内网服务器，目的地不是网关本机。
client→vpn:9090：INPUT 链；数据包终点是 VPN 网关本机监听的 Web 服务，交付本机上层应用，不走转发。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：NAT 会修改 IP / 端口做地址转换；WireGuard 仅做外层 UDP 封装加密，不修改内层 IP 地址，不存在地址映射关系。
WireGuard 隧道握手、在线状态由 wg 独立维护；conntrack 只跟踪解密后的内层 TCP/ICMP 业务连接，不管理加密隧道本身。
二者层级完全独立，封装加密 ≠ NAT 地址转换。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：放行来源：仅固定运维办公公网 IP、内网运维管理网段；绝对禁止 wg0 隧道内 VPN 客户端访问 SSH。
示例规则：
# 拒绝VPN侧SSH访问并记录日志
iptables -A INPUT -i wg0 -p tcp --dport 22 -m limit --limit 5/min -j LOG --log-prefix "VPN-SSH-DENY:"
iptables -A INPUT -i wg0 -p tcp --dport 22 -j REJECT

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答：检查两端 wg 接口 IP、AllowedIPs 是否自动生成对应路由；检查 VPN 网关内核 IP 转发net.ipv4.ip_forward=1是否开启；查看 iptables FORWARD/INPUT 规则计数器，判断流量是否被 DROP；dmesg/journalctl 检索拒绝日志，定位拦截规则；分别在 wg0、内网接口 tcpdump 抓包，判断报文是否抵达内网服务器；检查内网服务监听地址是否为 0.0.0.0，服务器自身防火墙是否拦截入站流量。
