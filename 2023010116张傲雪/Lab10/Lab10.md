...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |不能连接到服务器 |
| `office → 10.0.3.2:8080` |成功 |正常返回 HTTP 目录列表页面 |

![拓扑与基本连通性](topolopy.png)
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED |放行流量 |
| `filter` | `FORWARD` | 2 | ACCEPT |tcp dpt:8080 ctstate NEW |放行新连接 |
...
> 答：源 IP 是 203.0.113.1（防火墙外网 IP）

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：连接跟踪条目依然生效，已有连接的后续数据包仍可正常通信；但新的连接无法再完成 DNAT 映射，新建连接会失败
...
| 日志中 `DST=` 字段是哪个地址 |10.0.3.2（DMZ 内网地址） |
...
> 答：LOG 规则配置在 filter 表 FORWARD 链，记录改写前原始目标地址，需要将 LOG 规则配置在 nat 表 PREROUTING 链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：会发现。端口扫描行为特征：同一源 IP 短时间内连续访问大量不同目标端口，日志会出现大量同 SRC、不同 DPT 的记录，据此可判定端口扫描
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |NAT |PREROUTING |目的地址 203.0.113.1，TCP，目的端口 8080 |DNAT --to 10.0.3.2:8080 |
| DNAT 2222 |NAT |PREROUTING |目的地址 203.0.113.1，TCP，目的端口 2222 |DNAT --to 10.0.3.2:2222 |
| FORWARD 8080 |FILTER |FORWARD |目的地址 10.0.3.2，TCP，目的端口 8080，新连接 |ACCEPT |
| FORWARD 2222 |FILTER |FORWARD |目的地址 10.0.3.2，TCP，目的端口 2222，新连接 |ACCEPT |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT（改目标地址）工作在 nat 表 PREROUTING 链（路由决策之前）；SNAT（改源地址）工作在 nat 表 POSTROUTING 链（路由决策之后、数据包出网卡前）：ROUTING：负责入站流量 DNAT，把外网访问转发到内网服务；POSTROUTING：负责内网出站流量 SNAT，把内网地址转换为公网地址访问外网。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 优先：PREROUTING 执行早于本地 INPUT 链，数据包先被 DNAT 改写目标地址并转发到内网，本地服务无法收到该端口请求；方案：对使用不同端口映射，本地服务与映射端口区分开；使用 --to-destination 指定不同 IP + 端口，或配合路由策略区分流量。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 会五元组（源 IP、源端口、目的 IP、目的端口、协议） 完整记录双向映射关系。外部不同端口对应内网不同服务器 / 端口，回复包回来时，连接跟踪表根据五元组匹配，自动将内网源地址 + 端口还原为防火墙公网 IP + 对应端口，精准回包。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：可以正常回包。原因：本实验是单向 DNAT 场景，外部客户端主动发起连接，conntrack 记录了会话状态。dmz 回复包到达防火墙后，conntrack 根据会话记录自动逆向还原源地址，将 10.0.3.2 改为 203.0.113.1 再发往 wan，无需额外 SNAT

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：后果：无法新建连接跟踪条目，新 TCP/UDP 连接全部失败，NAT、端口映射、有状态防火墙全部失效；常用于 DoS/DDoS 攻击；防护措施：调大 conntrack 最大连接数；缩短无效连接超时时间等

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础：Linux netfilter/iptables 框架 + conntrack 连接跟踪，所有防火墙、访问控制、NAT、端口映射、日志审计都基于该内核模块实现；安保类比：有状态防火墙：大楼门禁，只放行合法进出人员；SNAT（内网访外网）：内部人员外出，统一使用大楼对外身份；分区审计：大楼区域隔离 + 出入登记；端口映射（DNAT）：外部访客通过指定接待窗口访问内部服务区，不直接接触内部区域。
