...
| :--- | :--- |
| 只有第一条规则时是否成功 |否 |
| 第一条规则放行的是什么方向的包 |正向（请求）包：lan → wan，目标端口 38070 的 TCP 包 |
| 失败的原因 |只放行了请求包，没有放行响应包（wan → lan，源端口 38070），防火墙看到响应包没有匹配规则，按默认策略 DROP 了。 |
| 第二条规则匹配的是什么 |反向（响应）包：wan → lan，源端口 38070 的 TCP 包 |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 |2 |
...
答：无状态防火墙只看"包长什么样"（端口号），不看"这个包从哪来"（连接上下文），所以无法区分"真正的响应"和"伪装成响应的主动连接"。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：FTP 协议的数据连接：
无状态防火墙：FTP 数据连接使用动态端口，无法提前写好端口规则。如果开放所有高端口，等于敞开大门；如果只开放 20 号端口，主动模式的出站数据连接又无法正常工作。
有状态防火墙：conntrack 模块能识别 FTP 的控制连接（端口 21）和数据连接之间的 RELATED 关系，自动放行动态端口上的数据连接，无需手动开放端口范围。
...
答：因为 conntrack 模块会记忆连接状态。当 lan → wan 的 SYN 包通过时，防火墙在连接跟踪表中创建了一条记录。wan → lan 的响应包到达时，防火墙查表发现这个包属于已有连接，直接放行。它记得"这个连接是我允许建立的"，所以不需要手动写反向规则

2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：无状态防火墙只看端口号，分不清方向。当 wan 发送 dpt=38070 的 SYN 包时，它会匹配规则 --dport 38070 -j ACCEPT，误以为这是合法的请求从而放行。
有状态防火墙能通过 conntrack 判断方向：这个 SYN 包没有对应的反向连接记录，是一个"反方向"的 NEW 连接，而规则只放行从 lan 发起的 NEW 连接，所以直接丢弃。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2：54998 |
| 条目中的目的地址和目的端口 |192.0.2.2：38070  |
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |[UPDATE] tcp      6 432000 ESTABLISHED src=10.0.0.2 dst=192.0.2.2 sport=54998 dport=38070 src=192.0.2.2 dst=10.0.0.2 sport=38070 dport=54998 [ASSURED]|
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态防火墙每个数据包独立检查，只根据当前包的头部信息做决策，不知道前后文；有状态防火墙维护连接跟踪表，能记住一条连接的完整状态（NEW、ESTABLISHED、RELATED），根据连接上下文判断是否放行。本质区别就是有无"记忆"能力。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：不能调换。因为网络上大部分流量属于已建立的连接（响应包、数据传输），把 ESTABLISHED 放第一条可以让这些包快速匹配放行，提高效率。如果调换顺序，每个包都要先检查 NEW，而大部分包不是 NEW，造成不必要的匹配开销。功能上可能还能工作，但性能下降，逻辑也不清晰。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包到达时 conntrack 表里找不到记录，会被视为 NEW 状态。防火墙的处理取决于规则配置：如果存在匹配的 NEW 规则就放行并重建连接跟踪记录；如果没有匹配规则或默认策略是 DROP，则直接丢弃。对于 TCP 连接，这种超时后出现的包通常被认为是非法包。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 描述与已有连接相关但不是同一连接的流量。典型场景包括：FTP 协议的数据通道（控制连接在端口 21，数据连接被认为与之相关）；ICMP 错误消息（如"端口不可达"回应某个 TCP 连接）；某些多媒体流协议的控制通道和数据通道之间的关系。本实验只是简单的 TCP 回声服务，没有派生子连接，所以看不到 RELATED。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：本质优势是遵循最小权限原则，保证安全策略的完整性。默认拒绝下，所有流量初始被阻断，只需开放明确需要的服务，未知流量自动被拒绝，配置遗漏会导致服务不可用（能快速发现问题）。默认允许下，所有流量初始被放行，需要不断封堵新发现的威胁，配置遗漏会直接产生安全漏洞且可能长期不被发现。前者是"先假设都恶意，再放行信任的"；后者是"先假设都善意，再封堵恶意的"——安全上默认拒绝明显更优。
