...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |   0xd172       |
| 查询域名（Query Name）       | www.cnki.net    |
| 查询类型（Type）             | A (1) (Host Address) |
| 目的端口                     |   53             |

**嵌入截图：**

DNS 请求包截图![alt text](dns_req.png)
---
...
| 回答记录数（Answers）        |        2        |
| 返回的 IP 地址               |  115.31.65.10   |
| TTL（若可见）                |      128         |

**嵌入截图：**

DNS 响应包截图![alt text](dns_resp.png)
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名系统）的作用是将人类易记的域名（如 www.yxnu.edu.cn）转换为机器可读的 IP 地址，因为在访问网站时网络通信必须通过 IP 地址定位服务器，而用户无法记住大量数字 IP，所以需要先进行 DNS 解析来建立连接。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：Transaction ID 是一致的，这个字段用于匹配请求和对应的响应，确保客户端能正确地将收到的 DNS 响应与之前发出的请求对应起来，避免响应错乱或伪造响应的干扰。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：'A' 记录表示将域名解析为 IPv4地址，是 DNS 中最基本的记录类型，用于实现域名到 32 位 IPv4 地址的映射。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认通常使用UDP协议和53端口；当响应数据包超过 512 字节（如 DNSSEC、大量记录）或需要进行区域传输（主从 DNS 同步）时，会改用TCP协议。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：1、它不支持 IPv6 地址解析，仅能处理 IPv4；
   2、该函数非线程安全，内部使用静态缓冲区，在多线程环境下可能产生数据竞争或结果被覆盖，现代编程应使用支持 IPv6 且线程安全的 getaddrinfo()替代。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：说明该网站采用了负载均衡或高可用性部署，通过将流量分散到多台服务器上，来提高网站的并发处理能力和容错性。
