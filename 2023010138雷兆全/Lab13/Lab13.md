...
| :--- | :--- | :--- |
| `gwA` | 10.2.0.0/24|10.2.0.0/24 dev wg0 proto wireguard |
| `gwB` |10.1.0.0/24 |10.1.0.0/24 dev wg0 proto wireguard |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |放行10.1.0.0/24访问10.2.0.2:8080，放行 RELATED、ESTABLISHED 连接 |A-TO-B-DENY: | DROP|
| `gwB` | 放行10.1.0.0/24访问10.2.0.2:8080，放行 RELATED、ESTABLISHED 连接| VPN-S2S-DENY:|DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功 |无拦截日志，流量被 ACCEPT 规则放行，未触发 LOG 规则 |
| `siteA -> siteB:2222` | 失败 | 成功|产生A-TO-B-DENY前缀内核日志，记录源10.1.0.2访问目的10.2.0.2的 TCP 2222 端口数据包 |
| `siteA -> siteB ping` | 失败 |失败 |产生多条A-TO-B-DENY前缀 ICMP 协议拦截日志，记录两次 ping 数据包被网关拦截拒绝 |
...
| underlay 是否连通 | 连通，gwA 可 ping 通 192.0.2.2，gwB 可 ping 通 192.0.2.1|
...
| `latest handshake` |存在最新握手记录（隧道正常建立） | 存在最新握手记录（隧道正常建立）|
| `transfer` 计数 | 	有收发数据包统计|	有收发数据包统计 |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功| 1. 已建立连接放行规则；2. 放行10.1.0.0/24访问10.2.0.2:8080新建 TCP 连接的 ACCEPT 规则|
| `siteA -> siteB:2222` |失败 | 1. 匹配 LOG 日志记录规则；2. REJECT 拒绝规则，产生A-TO-B-DENY拦截日志|
| `siteA -> siteB ping` |失败 |1. 匹配 LOG 日志记录规则；2. REJECT 拒绝规则，产生A-TO-B-DENY拦截日志 |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |源192.0.2.1、目的192.0.2.2 | UDP| 原始内网数据包被 WireGuard 加密封装，在底层公网以 UDP 报文传输，只能看到两端公网 underlay 地址|
| `wg0` |源10.1.0.2、目的10.2.0.2 |	TCP/ICMP | 隧道内为原始明文内网 IP 数据包，未加密封装，可看到两端业务内网地址|
| B 站点 LAN |源10.1.0.2、目的10.2.0.2 |TCP/ICMP | 数据包经过网关路由转发后，以原始内网地址在 siteB 局域网传输|
...
远程接入 VPN（SSL/IPsec 客户端 VPN）：面向个人用户，员工用电脑 / 手机客户端接入企业内网，单个终端获取内网 IP，实现个人访问企业内网；一端是用户终端，一端是企业网关。
站点到站点 VPN（S2S VPN，如本次 WireGuard）：面向两个局域网，两端都是网关设备，两个内网整体互通，站点内所有主机无需单独安装客户端，网关之间建立加密隧道，实现两个网段全量互通。

2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？

AllowedIPs 两个核心作用：
第一：路由作用，WireGuard 会自动为配置的网段生成路由，去往该网段的流量自动从 wg0 隧道转发；
第二：访问控制作用，只允许源 IP 属于该网段的数据包通过隧道收发，过滤非法流量。
如果只写对端 wg 隧道地址，只能网关之间互通，无法让站点内所有主机跨网段访问；填写对端完整内网网段，才能让整个局域网流量走 VPN 隧道。

3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？

网关本质是三层路由器，ip_forward=1 开启内核 IP 数据包转发功能：
siteA 主机的数据包到达 gwA 后，需要从 LAN 网卡转发到 wg0 隧道网卡；
数据包通过 VPN 隧道到达 gwB 后，需要从 wg0 网卡转发到 gwB 的 LAN 网卡，才能送达 siteB 主机；
不开启转发，内核只会接收目的为网关自身 IP 的数据包，跨网卡的转发流量会直接丢弃，两端内网无法互通。

4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？

普通主机没有跨网段路由，访问非本网段的10.2.0.0/24时，数据包会发给默认网关；
只有把默认网关设置为本地 VPN 网关（gwA/gwB），跨网段流量才能被转发进入 WireGuard 加密隧道；
如果不指向本地网关，跨网段数据包会直接丢弃，无法通过 VPN 传输。

5. `wg show` 里有握手但业务不通，排查顺序应该是什么？

检查两端内核ip_forward是否开启；
检查AllowedIPs配置是否为对端内网网段，确认 wg0 自动生成的路由是否存在、出接口是否为 wg0；
检查两端 iptables/firewalld 防火墙 FORWARD 链策略是否放行跨网卡转发流量；
检查两端主机默认网关是否正确指向本地 VPN 网关；
抓包排查：分别在 wg0、物理网卡抓包，定位数据包在哪一段被丢弃；
核对 WireGuard 公私钥、Endpoint 公网 IP、监听端口配置是否正确。

6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？

双向安全防护：既可以管控 A 站点主动访问 B 站点的流量，也可以管控 B 站点主动访问 A 站点的流量，实现双向细粒度访问控制；
就近拦截：流量在进入 VPN 隧道前就被 gwA 拦截，不用消耗 VPN 带宽传输非法流量，减少加密开销；
故障隔离，两端独立策略，方便分别审计、记录双向访问日志。

7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？

隐藏的信息：
两端业务内网 IP 地址、传输的原始四层协议（TCP/ICMP、源目的端口）；
应用层传输的明文业务数据、访问的服务类型；
内网的网络拓扑、主机数量、业务访问行为。
暴露的信息：
两端 underlay 公网 IP 地址、WireGuard 监听 UDP 端口；
隧道两端有持续的 UDP 流量，可以识别出使用了 WireGuard 类 VPN 加密隧道；
数据包的传输带宽、报文长度、收发时间等流量特征。


8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？

WireGuard 配置侧：gwA 的AllowedIPs已经包含10.2.0.0/24、gwB 包含10.1.0.0/24，路由层面已放行，无需修改；
gwB 防火墙规则：在 FORWARD 链放行源网段10.2.0.0/24、去往 siteA 目标服务端口的新建 TCP 流量，同时配置日志 + 拒绝兜底规则；
gwA 防火墙规则：在 FORWARD 链放行从 wg0 入网卡、访问 siteA 目标服务端口的新建 TCP 流量，配合已有的RELATED,ESTABLISHED放行规则；
确保两端 IP 转发保持开启，站点主机默认网关配置正常。
