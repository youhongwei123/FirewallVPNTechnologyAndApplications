...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |wan 到 10.0.3.2 网络不可达，无法建立 TCP 连接，HTTP 访问请求无法正常发起，最终访问失败 |
| `office → 10.0.3.2:8080` |成功 |office 到 10.0.3.2 网络连通正常，HTTP 请求可正常到达 dmz 侧的 8080 端口服务，服务端返回 200 状态码，访问成功 |
...
| `filter` | `FORWARD` | 1 | ACCEPT | 所有协议、所有源目地址，连接状态为 ESTABLISHED,RELATED|允许匹配的数据包通过防火墙转发 |
| `filter` | `FORWARD` | 2 | ACCEPT |TCP 协议，目标地址为 10.0.3.2、目标端口为 8080，连接状态为 NEW |允许匹配的新连接数据包通过防火墙转发 |
...
> 答：203.0.113.1
原理：DNAT 对客户端完全透明，客户端全程只和防火墙外网 IP 203.0.113.1 通信。防火墙在收到内网 DMZ 服务器 10.0.3.2 的回复包时，会通过连接追踪（conntrack）自动将回复包的源 IP 反向还原为 203.0.113.1，再发给 wan 侧的客户端，因此客户端 curl 感知不到 DNAT 的存在，收到的响应源 IP 始终是防火墙外网 IP。


2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条连接的后续数据包仍然会被正常转发，DNAT 转换不受影响
原理：Linux iptables 的 NAT 转换完全依赖连接追踪（conntrack）机制，而非规则的实时匹配。
连接的第一个包匹配 DNAT 规则后，转换结果会被永久记录在 conntrack 条目中；
后续属于这条连接的所有数据包，会直接匹配 conntrack 条目完成地址转换与转发，不再重新遍历 NAT 规则链；
因此即使删除 DNAT 规则，只要 conntrack 条目未超时被清除，这条连接的后续包依然会正常完成地址转换和通信，不会被中断。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |成功，正常返回 HTTP 目录列表页面 | 有，已配置 DNAT 将 8080 端口映射到内网 DMZ 服务|
| `wan → 203.0.113.1:2222` | 成功，正常返回 HTTP 目录列表页面| 有，已新增配置 2222 端口的 DNAT 映射规则|
| `wan → 203.0.113.1:3306` |失败，无法建立连接 |无，未配置 3306 端口的 DNAT 映射规则 |
...
> 答：这一现象说明 LOG 规则在 DNAT 转换完成之后触发，此时数据包的目标地址已经被防火墙改写为内网服务器地址 10.0.3.2，因此日志记录的是转换后的地址。
如果要记录 DNAT 改写前的原始目标地址，需要把 LOG 规则放在 nat 表的 PREROUTING 链。因为 PREROUTING 链是数据包进入防火墙后第一个被处理的链，此时 DNAT 转换尚未执行，数据包的目标地址还是客户端访问的原始外网 IP 203.0.113.1，可以完整记录原始访问信息。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：LOG 日志完全可以帮助安全人员发现端口扫描攻击。
识别端口扫描行为的核心特征：
源 IP 高度集中：短时间内，同一个源 IP 地址，向防火墙的大量不同目标端口发起了连接请求，且都触发了 LOG 日志记录；
端口访问特征：日志中出现大量连续、随机的目标端口号，而非正常业务的固定端口（如 8080、2222）；
时间特征：请求时间高度集中，在几秒到几分钟的极短时间内，发起了几十甚至上百个端口的访问请求；
连接特征：日志中的数据包大多是 TCP SYN 包（端口扫描的典型特征），没有后续的完整连接交互记录。
...
| :--- | :------- |
| `wan` 地址 |203.0.113.2 |
| `fw` 四个接口地址 | 外网接口 203.0.113.1、DMZ 接口 10.0.3.1、Office 内网接口 10.0.1.1、回环接口 127.0.0.1|
| `wan → 203.0.113.1` ping 结果 | 成功，2 包发送 2 包接收，0% 丢包，同一外网直连网段|
| `wan → 10.0.3.2` ping 结果 | 失败，返回 Network is unreachable，wan 无到内网的路由|
| `office → 10.0.3.2` ping 结果 |成功，2 包发送 2 包接收，0% 丢包，经 fw 转发连通 |
| `office → 203.0.113.1` ping 结果 |成功，2 包发送 2 包接收，0% 丢包，访问 fw 自身接口 |

### B. 端口映射配置

| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat | PREROUTING	|目标地址 203.0.113.1、TCP 协议、目标端口 8080 | DNAT --to-destination 10.0.3.2:8080|
| DNAT 2222 | 	nat|PREROUTING | 目标地址 203.0.113.1、TCP 协议、目标端口 2222|DNAT --to-destination 10.0.3.2:2222 |
| FORWARD 8080 |	filter | FORWARD| 目标地址 10.0.3.2、TCP 协议、目标端口 8080、连接状态 NEW|ACCEPT |
| FORWARD 2222 |filter |	FORWARD |目标地址 10.0.3.2、TCP 协议、目标端口 2222、连接状态 NEW |ACCEPT |

### C. 端口映射测试

| 测试项 | 结果 |
| :----- | :--- |
| `wan → 203.0.113.1:8080` |成功，正常返回 HTTP 目录列表，DNAT 规则生效 |
| `wan → 203.0.113.1:2222` |成功，正常返回 HTTP 目录列表，DNAT 规则生效 |
| `wan → 203.0.113.1:3306` |失败，无对应 DNAT 规则，无法建立连接 |
| `wan → 10.0.3.2:8080` | 失败，wan 无到内网的路由，网络不可达|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT（目标地址转换）：发生在数据包进入防火墙后的 PREROUTING 阶段，在路由决策之前执行，用于修改数据包的目标地址，将外网访问请求转发到内网服务器。
SNAT（源地址转换）：发生在数据包离开防火墙前的 POSTROUTING 阶段，在路由决策之后执行，用于修改数据包的源地址，让内网服务器的外网访问流量能以防火墙外网 IP 的身份出去。
对于 DMZ 双向访问场景：
PREROUTING 链：负责处理入站流量，执行 DNAT 规则，将外网访问防火墙外网 IP 的请求，转换为内网 DMZ 服务器的 IP，实现端口映射。
POSTROUTING 链：负责处理出站流量，执行 SNAT 规则，将 DMZ 服务器访问外网的流量，源 IP 转换为防火墙外网 IP，让外网能正常回包给防火墙。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先 "抢到" 这个包。因为 nat 表的 PREROUTING 链在数据包进入防火墙的最早期执行，优先级高于本地服务的接收流程，匹配 DNAT 规则的包会被直接转发到内网，不会送到防火墙本地的 Web 服务。
要让两者共存，有两种常用方案：
端口拆分：给防火墙本地 Web 服务换一个监听端口（如 8081），外网访问时用不同端口区分：203.0.113.1:8080 访问内网 DMZ 服务，203.0.113.1:8081 访问防火墙本地服务。
匹配条件细化：给 DNAT 规则增加更精细的匹配条件（如限制源 IP 范围、协议类型），让特定流量走 DNAT 转发，其余流量送到本地服务；或使用 REDIRECT 规则，将特定端口的流量重定向到本地服务。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：防火墙的 conntrack 连接追踪表，会为每一条连接记录完整的五元组映射关系：
外部 8080 端口的请求：记录 (客户端IP, 客户端端口, TCP, 203.0.113.1, 8080) ↔ (客户端IP, 客户端端口, TCP, 10.0.3.2, 8080)
外部 8081 端口的请求：记录 (客户端IP, 客户端端口, TCP, 203.0.113.1, 8081) ↔ (客户端IP, 客户端端口, TCP, 10.0.3.3, 8080)
当内网服务器回复包到达防火墙时，会直接匹配 conntrack 表中的对应条目，根据回复包的源 IP、源端口、目的 IP、目的端口，找到对应的原始连接记录，自动将源 IP 和源端口逆向还原为公网 IP 和公网端口，全程不需要重新匹配 DNAT 规则，因此能精准还原到正确的公网端口。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能回到 wan，核心机制是DNAT 双向匹配 + conntrack 状态追踪：
客户端发起到 203.0.113.1:8080 的请求，防火墙通过 DNAT 将目标地址改为 10.0.3.2，并在 conntrack 表中记录这条连接的映射关系；
DMZ 服务器收到请求后，以源地址 10.0.3.2 给客户端 203.0.113.2 回复，包先到达防火墙；
防火墙匹配 conntrack 表，识别到这是已建立连接的回复包，自动将源地址从 10.0.3.2 逆向还原为 203.0.113.1，再发给客户端；
客户端收到的回复包源 IP 是 203.0.113.1，完全感知不到内网 IP 的存在，通信正常完成。
这种场景不需要 SNAT 规则，因为客户端和 DMZ 服务器的通信全程由防火墙中转，回复包会被 conntrack 自动还原。只有当 DMZ 服务器主动发起对外网的访问时，才需要配置 SNAT 规则，否则外网无法给内网 IP 回包。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：防火墙无法正常工作。
会出现的后果：
新的连接无法被追踪，NAT 转换完全失效，端口映射、源地址转换功能全部无法使用；
有状态防火墙的 ESTABLISHED/RELATED 规则失效，已建立的连接也可能被中断，网络完全瘫痪；
防火墙性能急剧下降，出现大量丢包、连接超时，甚至内核崩溃。
防护措施：
增大 conntrack 表容量：调整内核参数 net.netfilter.nf_conntrack_max，提升最大连接追踪条目数；
缩短连接超时时间：调整 net.netfilter.nf_conntrack_tcp_timeout_* 等参数，让无效连接更快释放；
限流防护：使用 iptables 的 limit 模块，限制单 IP 的并发连接数，防止攻击打满连接表；
优化规则：减少不必要的连接追踪，对不需要状态检测的流量，使用 NOTRACK 标记跳过追踪；
部署攻击防护：在防火墙前端部署 DDoS 防护设备，过滤掉大量伪造的连接攻击流量。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：四次实验的共同基础是 Linux Netfilter 框架和连接追踪（conntrack）机制，所有功能都基于内核的 Netfilter 钩子点实现，依赖 conntrack 记录连接状态，实现有状态的流量控制、地址转换和日志审计。
安保系统类比：
Lab7 有状态防火墙：对应大楼的门禁系统，只允许合法的人员进出，拒绝陌生访客，同时允许已登记的人员自由往返；
Lab8 NAT：对应大楼的前台转接，大楼内部的人员对外联系时，统一使用大楼的对外号码，隐藏内部分机号，保护内部隐私；
Lab9 分区审计：对应大楼的监控系统，对不同区域（办公区、服务区）的进出人员进行全程录像和记录，发生安全事件时可追溯；
Lab10 端口映射：对应大楼的服务专线，外部访客拨打大楼的对外号码时，自动转接到对应的内部服务窗口（如快递、前台、会议室），让外部能精准访问内部服务。
