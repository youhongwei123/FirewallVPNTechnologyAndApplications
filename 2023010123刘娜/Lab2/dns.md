...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS的作用是实现域名与IP地址的映射,作为互联网的“电话簿”方便记忆和使用；访问网站时必须先进行DNS解析，因为底层网络协议（TCP/IP）只认识IP地址，浏览器只有通过解析拿到目标服务器的IP后，才能建立连接并加载网页。



2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致，均为0x77f3。该字段用于匹配DNS请求与响应，确保客户端能识别对应的应答，避免与其他查询混淆。


3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A记录是DNS中的地址记录，用于将域名映射到IPv4地址，是互联网中最基础、最常用的记录类型，负责提供主机对应的32位IPv4地址，供网络层实现寻址通信。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS解析默认使用传输层UDP协议和端口53，以保证查询的快速与高效；当响应数据量较大超过UDP 512字节限制、进行DNS服务器区域同步、使用DNSSEC或需要可靠传输时，会改用TCP协议继续在端口53上进行通信。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：(1)不支持IPv6：该函数设计于IPv4时代，无法解析AAAA记录（IPv6 地址），在IPv6已广泛部署的今天缺乏兼容性。
   (2)线程不安全、设计老旧:缺乏对现代网络配置、错误信息返回和扩展功能的支持，同时也被更安全、更灵活的getaddrinfo()等标准接口替代。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常是为了实现负载均衡（分散流量）和提高可用性（一台挂了还能访问其他服务器），也可能是通过CDN或全局调度让用户就近访问以降低延迟。
