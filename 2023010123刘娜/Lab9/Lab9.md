...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功 |curl 命令返回 HTTP 目录列表，dmz 端的 python3 -m http.server 日志中记录 10.0.1.2 - - "GET / HTTP/1.1" 200 |
| `guest -> dmz:8080` |失败 |curl 命令返回 Failed to connect to 10.0.3.2 port 8080: Couldn't connect to server，无 HTTP 访问日志 |
| `guest -> office` |失败 |curl 命令返回 Failed to connect to 10.0.1.2 port 8080: Couldn't connect to server，无 HTTP 访问日志 |
...
> 答：不会被命中。iptables的规则是按顺序从上到下逐条匹配的，当数据包匹配到REJECT规则后，会立即执行拒绝动作（丢弃数据包并返回ICMP错误），同时终止后续所有规则的匹配流程。因此，放在REJECT规则之后的LOG规则永远没有机会被匹配到，数据包在被拒绝前不会经过LOG规则，也就无法生成日志记录。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |REJECT |是 | GUEST-TO-DMZ |
| `guest -> office` |REJECT |是 | GUEST-TO-OFFICE |
| `office -> dmz:8080` |ACCEPT | 否|无 |

**从终端 D 复制一条实际日志行：**
May 21 09:02:30 LN kernel: GUEST-TO-OFFICE: IN=veth-fw-guest OUT=veth-fw-office MAC=92:57:f5:49:96:a2:76:39:85:3d:e0:ed:08:00 SRC=10.0.2.2 DST=10.0.1.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=33669 DF PROTO=TCP SPT=42466 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```text
...
> 答：这是合理的。当前配置的LOG规则仅与REJECT规则绑定，只有被拒绝的流量才会触发日志；而office到dmz的流量匹配了ACCEPT规则，直接被放行，因此不会生成日志。若要记录所有被放行的流量，需要在对应的ACCEPT规则之前，添加一条匹配条件完全一致的LOG规则，或在FORWARD链的末尾添加默认LOG规则，对所有放行流量进行记录。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：从日志中可以完整还原访问行为。日志中的SRC、DST、PROTO、DPT等字段，分别记录了源IP、目的IP、协议类型和目的端口，结合日志前缀，可以清晰还原出 “谁在访问谁的什么服务”。例如GUEST-TO-DMZ 日志，就能还原出guest网段的主机正在访问dmz网段主机的8080端口服务。
...
> 答：--log-prefix + grep组合相当于给不同事件打上标签，在真实场景中，这种方式能实现快速的日志分类与筛选，便于运维人员快速定位不同类型的访问行为（如异常访问、违规访问），大幅提升故障排查和安全审计的效率；同时也能为后续的日志统计、告警规则配置提供明确的标识依据，让不同安全事件的处理流程更清晰、更自动化。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：如果防火墙每秒拦截上千个包，不加限制地写LOG，会导致日志量暴增，占用大量磁盘空间，甚至引发日志文件膨胀耗尽磁盘；同时高频的日志写入会消耗大量CPU和I/O资源，影响防火墙本身的转发和处理性能；此外，海量日志也会增加后续日志分析和排查的难度，降低运维效率。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |允许初始突发最多 5 条，之后降到 --limit 速率 |
...
> 答：LOG和REJECT在逻辑上是独立的串行关系，二者互不影响--limit仅限制了LOG规则的匹配频率，未命中LOG规则的数据包，依然会继续向下匹配并触发后续的REJECT规则。LOG只是一个日志记录动作，不会改变数据包的流向，也不会中断规则链的匹配过程，因此后续的REJECT规则对所有匹配的数据包依然生效。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：二者有本质区别。“没记录就当没发生” 意味着事件未被处理，而 “仍然拦截但不记录” 则表示事件被处理了，只是没有留下日志痕迹。在安全审计中，这个区别至关重要：前者会导致安全事件被遗漏，无法追溯攻击行为，无法评估安全策略的有效性；而后者虽然日志不完整，但数据包的拦截动作已实际发生，安全策略依然生效，只是审计数据不完整。因此，日志限流应合理配置，避免因过度限制导致关键安全事件无法被审计。
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT |所有流量，状态为 RELATED,ESTABLISHED | 无|
| 2 |ACCEPT |10.0.1.0/24 → 10.0.3.2:8080（TCP） |无 |
| 3 |LOG |10.0.2.0/24 → 10.0.3.2:8080（TCP） |GUEST-TO-DMZ: |
| 4 |REJECT |10.0.2.0/24 → 10.0.3.2:8080（TCP） |无 |
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：如果dmz服务和office混在同一网段，防火墙无法通过IP/网段区分二者，就无法实现本实验中 “仅允许office访问dmz服务、拒绝guest访问dmz和office”的精细化访问控制。一旦dmz的服务被攻陷，攻击者可以直接横向移动到同网段的办公网，造成更大范围的数据泄露

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：guest网络通常包含不受控的外部访客设备，安全状态不可信。如果不与办公网隔离，guest网络中的恶意软件或攻击者可以直接访问办公网内的业务系统、敏感数据，给企业带来极大的安全风险。本实验中，防火墙直接配置了拒绝guest访问office的规则，正是为了阻断这种风险路径。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：必须保留该规则，因为它允许已建立连接的响应数据包通过防火墙。TCP通信是双向的，当office主机主动访问dmz服务时，dmz的响应包属于RELATED/ESTABLISHED状态，需要被放行。如果去掉这条规则，响应包会被默认DROP策略丢弃，导致TCP三次握手无法完成office对dmz的访问也会失败。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG target不会改变数据包的流向，仅记录日志；而 ACCEPT/DROP/REJECT会直接决定数据包的最终命运。LOG必须放在 REJECT前面，因为iptables是按顺序匹配规则的，如果REJECT在前，数据包会被直接拒绝，后续的LOG规则永远不会被命中，也就无法记录被拦截的访问行为。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：这是合理的，不是疏漏。本实验中仅为被拦截的guest访问配置了LOG规则，目的是记录违规访问行为，便于安全审计；而office到dmz的访问是被允许的正常业务流量，无需额外记录日志，避免日志冗余。如果需要记录放行流量，需要单独为ACCEPT规则配置LOG规则。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：二者有本质区别。超出限制的包仍然被REJECT，说明防火墙的安全策略依然生效，数据包被正常拦截，只是日志记录被限流，审计信息不完整；而 “只要不记录就当没发生” 意味着数据包既没有被拦截，也没有被记录，安全策略失效，存在完全未被监控的风险。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：VPN用户应该先接入dmz区域。从网络分区角度，dmz是对外提供服务的非军事区，VPN用户接入dmz后，需要通过防火墙的访问控制规则才能进一步访问office办公网，避免直接暴露内部网络；从日志审计角度，VPN用户的所有访问行为都可以在dmz区域的防火墙上被统一记录和监控，便于后续安全审计和溯源。
