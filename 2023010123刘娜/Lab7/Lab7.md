...
| :--- | :--- |
| 只有第一条规则时是否成功 |否 |
| 第一条规则放行的是什么方向的包 |从lan到wan、目标端口为38070的TCP请求包（出站连接请求） |
| 失败的原因 |防火墙默认策略为DROP，仅放行了目标端口38070的出站请求，却没有放行wan侧服务端返回的响应包，导致请求被阻断 |
| 第二条规则匹配的是什么 |从wan到lan、源端口为38070的TCP响应包（入站回复） |
| 加第二条规则后是否成功 |是 |
| 当前规则共几条 |2条 |
...
答：这个问题的本质是：无状态的iptables规则无法区分 “合法连接的响应包” 和 “主动发起的恶意连接请求”，仅基于端口放行的规则会引入安全漏洞，无法控制连接的发起方向。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：一个无状态规则无法解决、但有状态规则能解决的场景：企业内网只允许内网主机主动访问外部的HTTP/HTTPS网站，不允许外部主机主动向内网发起任何连接请求。无状态规则只能基于端口放行，无法区分入站包是“内网访问请求的响应”还是“外部主动发起的攻击连接”；而有状态规则（如iptables的conntrack模块）可以通过--state RELATED,ESTABLISHED仅放行已建立连接的响应包，彻底阻断外部主动发起的未授权连接，同时不影响内网正常访问。
![无状态规则](stateless.png)
...
| 是否需要手动考虑返回方向 |是 |否 |
| 能否区分"新连接"和"响应包" |否 |是 |
| `wan` 能否主动发起到 `lan` 的连接 |能 |否 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
答：有状态规则没有写任何关于“返回方向”的规则，wan的响应包仍然能通过fw，是因为iptables的conntrack状态跟踪模块会自动识别已建立连接的数据包。当lan向wan发起TCP连接时，防火墙会记录这个连接的状态为NEW；当wan返回响应包时，这些包会被识别为ESTABLISHED或RELATED状态，被--ctstate ESTABLISHED,RELATED -j ACCEPT规则放行，无需额外配置返回方向的规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：有状态规则能阻止wan主动发起到lan的连接，而无状态规则的--sport 38070做不到，原因是两者的匹配逻辑完全不同。无状态规则仅基于源端口放行，只要wan侧的进程从38070端口发起连接，--sport 38070规则就会放行，无法区分这是合法响应还是主动发起的新连接；而有状态规则中，--ctstate NEW规则仅允许lan侧发起的、目标端口为38070的新连接，wan主动发起的连接无法匹配这条规则，同时ESTABLISHED,RELATED规则只放行已建立连接的响应包，不会放行新的主动连接，因此能有效阻止外部主动发起的未授权访问。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:41672 |
...
答：防火墙放行一个ESTABLISHED包时，依据的是conntrack表里的连接记录，而不是仅依赖 IP 包头部的字段。IP包头部仅包含源 / 目的IP、端口、TCP标志位等信息，无法区分一个包是新连接的请求还是已建立连接的响应；而conntrack会在连接建立时记录五元组（源IP、目的IP、源端口、目的端口、协议）和连接状态，当收到响应包时，会匹配表里的记录判断它属于已建立的连接，因此ESTABLISHED规则能精准放行合法响应包，同时拒绝外部主动发起的未授权连接，这也是有状态防火墙相比无状态防火墙的核心优势。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤仅基于IP包的头部信息（源IP、目的IP、端口、协议等）匹配规则，无法识别连接的上下文和状态，无法区分新连接请求与已建立连接的响应包；而有状态检测防火墙通过conntrack模块维护连接状态表，能够识别连接的完整生命周期（NEW/ESTABLISHED/RELATED等），区分主动发起的连接和已建立连接的响应，因此能实现更细粒度、更安全的访问控制。


2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：规则顺序决定了匹配优先级，将ESTABLISHED,RELATED放在第一条，能优先放行已建立连接的响应包和关联流量，减少不必要的规则匹配开销，同时确保合法的响应包不会被后续规则拦截。
不能调换顺序。如果将NEW放在第一条，会导致ESTABLISHED状态的响应包需要先匹配NEW规则，若NEW规则未放行该流量，响应包就会被丢弃，导致正常的连接无法完成。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：当conntrack条目因超时被删除后，属于该连接的后续包会被防火墙识别为NEW状态的包。
如果防火墙规则中仅允许特定端口的NEW连接，或默认策略为DROP，这些后续包会被当作新连接请求处理，若无法匹配放行规则就会被丢弃，导致连接中断。


4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED状态描述的是与已有连接相关联、但本身不是该连接直接会话的流量，它是独立的连接，但和已有的会话存在关联关系。
典型场景包括：FTP协议的主动 / 被动模式中，数据通道连接是控制通道的关联流量；或者ICMP错误报文（如 “端口不可达”“超时”），它们是已建立TCP连接的关联响应流量，会被标记为RELATED状态。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：“默认拒绝，按需放行”的本质是最小权限原则，默认阻断所有流量，仅放行明确允许的服务，攻击面被严格限制，即使存在配置疏漏也不会直接暴露风险；而“默认允许，事后补堵”默认开放所有流量，再通过规则限制，攻击面极大，一旦有规则遗漏或被绕过，攻击者就能直接访问未授权服务，存在大量潜在安全漏洞，且事后修补的成本和风险远高于事前控制。
