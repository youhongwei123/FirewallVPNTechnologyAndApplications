...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |curl命令返回Failed to connect to 10.0.3.2 port 8080: Couldn't connect to server，无法建立连接。 |
| `office → 10.0.3.2:8080` |成功 |curl命令返回了HTTP响应的HTML页面内容（目录列表），成功访问到了DMZ服务器的8080端口。 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED（所有已建立或相关的连接） |允许通过 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 ctstate NEW（目的地址为 10.0.3.2、目的端口为 8080 的新 TCP 连接） |允许通过 |
...
| :--- | :--- | :--- |
| 请求包的源地址 |203.0.113.2:51206	 |203.0.113.2:51206	 |
| 请求包的目的地址 |203.0.113.1:8080 |10.0.3.2:8080 |
| 回复包的源地址 |203.0.113.1:8080 |10.0.3.2:8080 |
| 回复包的目的地址 |203.0.113.2:51206	 |203.0.113.2:51206	 |
...
| 连接状态 |TIME_WAIT（同时显示为 ASSURED） |

**简答题：**

1. `wan` 的 curl 收到的 HTTP 响应，源 IP 是什么？（提示：`wan` 无感知 DNAT 存在）

> 答：wan的curl收到的HTTP响应，源IP是203.0.113.1（防火墙的外网接口地址）。
原因：虽然DMZ服务器的真实地址是10.0.3.2，但在数据包返回防火墙时，会被自动进行地址转换，将源地址从10.0.3.2 改写回203.0.113.1。这个转换对客户端是透明的，因此客户端看到的源IP始终是它最初请求的公网IP，完全感知不到DNAT的存在。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条连接的后续包依然可以正常通信。
原因：DNAT规则仅在连接建立的第一个包（NEW 状态）时生效，负责改写目标地址并生成conntrack条目。一旦连接被跟踪，后续的数据包会直接根据conntrack表中的状态信息进行地址转换，不再匹配nat表的规则。因此，即使删除了DNAT规则，只要conntrack条目存在，已建立连接的后续数据包依然能被正确转发和转换，不会受到影响。
...
| `DNAT-TO-DMZ:8080` 日志条数 |1条 |
| `DNAT-TO-DMZ:2222` 日志条数 |1条 |

**简答题：**

1. 日志中的 `DST=10.0.3.2` 而不是 `DST=203.0.113.1`。这一现象说明了 LOG 规则在哪个阶段触发？如果要记录原始目标地址（改写前），应该把 LOG 规则放在哪个表、哪个链？

> 答：日志中的DST=10.0.3.2说明LOG规则是在DNAT地址改写之后 触发的。因为数据包经过PREROUTING链的DNAT规则后，目标地址已经从203.0.113.1被改写为10.0.3.2，此时再经过LOG规则记录的就是改写后的地址。如果要记录原始目标地址（改写前），应该把LOG规则放在nat表的PREROUTING链上，且顺序在DNAT规则之前，这样就能在地址改写前记录下原始的目的IP和端口。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：LOG 日志能够帮助安全人员发现端口扫描行为。
可以通过以下方式识别：1.短时间内来自同一源IP的大量不同目的端口的日志记录：端口扫描的典型特征是单个IP在极短时间内尝试连接多个不同端口（如 8080、2222、3306 等），日志中会出现大量同一SRC、不同DPT的记录。2.无对应服务的端口访问日志：扫描通常会尝试所有端口，包括未配置服务或DNAT规则的端口，这些访问行为也会被日志记录下来，形成大量无效连接的记录。3.日志条目数量异常激增：正常访问只会产生少量日志，而端口扫描会在短时间内产生大量连续的日志条目，形成明显的异常模式。
...
| `fw` 四个接口地址 |10.0.1.1/24（veth-fw-office）；10.0.2.1/24（veth-fw-guest）；10.0.3.1/24（veth-fw-dmz）；203.0.113.1/24（veth-fw-wan） |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT（目标地址转换）发生在nat表的PREROUTING链，此时数据包刚进入防火墙，还未进行路由决策；SNAT（源地址转换）发生在nat表的POSTROUTING链，此时数据包已经过路由决策，即将离开防火墙。在入站方向，PREROUTING链的DNAT规则负责将公网IP改写为内网服务器IP，让数据包能被正确转发到DMZ；在出站方向，POSTROUTING链的SNAT规则负责将内网服务器的源IP改写为防火墙公网IP，让外网能识别回复包的来源，确保双向通信正常建立。


2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT规则会先 “抢到” 这个包。因为 DNAT 发生在PREROUTING链，早于路由决策和INPUT链的本地投递流程，只要匹配DNAT规则，目标地址会被改写为内网地址，数据包会被转发到DMZ，防火墙本机的Web服务永远收不到这个包。要让两者共存，需要修改DNAT规则，为不同来源或端口设置区分：比如仅对来自外网（iif=veth-fw-wan）的请求执行DNAT，而本机发起的请求直接访问本地服务；或者为防火墙本地服务配置不同端口，避免端口冲突。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：防火墙通过conntrack连接跟踪表来完成逆向还原。当外网请求8080端口时，DNAT规则会将其映射到 10.0.3.2:8080，并在conntrack表中记录一条状态条目，包含 “公网地址/端口↔内网地址/端口” 的映射关系；当 10.0.3.2 的回复包返回防火墙时，conntrack 会根据连接状态匹配到这条条目，自动将源地址/端口还原为客户端最初请求的公网地址和端口。同理，8081端口的映射会生成独立的conntrack条目，两者互不干扰，因此能精准还原到对应的公网端口。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：在本实验中，DMZ服务器的回复包能回到wan。虽然没有配置显式的SNAT规则，但DNAT会在conntrack表中记录连接状态，当回复包从DMZ发往外网时，防火墙会根据conntrack中的映射关系，自动将源地址从10.0.3.2改写为203.0.113.1，这个过程被称为 “隐含SNAT”，无需手动配置规则。但如果DMZ服务器主动发起访问外网的请求，而非回应DNAT转发的请求，就需要配置SNAT/MASQUERADE规则，否则外网无法识别内网地址，回复包无法返回。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：如果conntrack表被占满，防火墙将无法记录新的连接状态，所有新的TCP连接请求都会被丢弃，防火墙无法正常建立NAT映射和状态检测，网络通信会中断。常见后果包括合法用户无法建立新连接、服务完全不可用。防护措施包括：调大conntrack表的最大条目数、缩短超时时间清理无效连接、配置防火墙规则限制单IP的并发连接数、启用SYN Cookies防御SYN Flood攻击，以及部署前置防护设备缓解连接压力。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：这四次实验的共同基础是conntrack连接跟踪机制和iptables状态检测框架，所有功能都依赖对数据包连接状态的识别与处理。如果把防火墙比作大楼安保系统：Lab7的有状态防火墙对应 “门禁系统”，只允许已授权的出入（已建立/相关连接）；Lab8的 NAT 对应 “访客登记系统”，将内网地址伪装为公网地址，隐藏内部结构；Lab9的分区审计对应 “楼层区域隔离”，限制不同区域（如办公区、访客区）之间的直接访问；Lab10的端口映射对应 “快递代收点”，将公网端口的请求转发到内部指定的服务器，仅开放必要服务。
