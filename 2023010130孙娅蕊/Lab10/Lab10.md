...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |Network is unreachable |
| `office → 10.0.3.2:8080` | 成功|返回目录列表 |
...
| :--- | :--- | :--- |
| 请求包的源地址 |203.0.113.2:45208 |203.0.113.2:45208 |
| 请求包的目的地址 |203.0.113.1:8080 |10.0.3.2:8080 |
| 回复包的源地址 |203.0.113.1:8080 |10.0.3.2:8080 |
| 回复包的目的地址 |203.0.113.2:45208 |203.0.113.2:45208 |
...
> 答：源 IP 是 203.0.113.1

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：后续包仍可正常通信，因为连接已被 conntrack 记录，NAT 改写依赖会话条目而非规则本身。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |成功 |有（DNAT 映射到 10.0.3.2:8080） |
| `wan → 203.0.113.1:2222` | 成功|有（DNAT 映射到 10.0.3.2:2222） |
| `wan → 203.0.113.1:3306` |失败 |无（无对应 DNAT 规则，且防火墙本机未监听 3306 端口） |
...
> 答：LOG 规则在 DNAT 改写后触发。所以记录的是内网地址；要记录原始地址，需将 LOG 规则放在 nat 表的 PREROUTING 链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以发现。识别方式：同一源 IP 短时间内访问大量不同端口，且多为无后续交互的 SYN 包。
...
| :--- | :------- |
| `wan` 地址 |203.0.113.2 |
| `fw` 四个接口地址 |203.0.113.1（wan）、10.0.3.1（dmz）、10.0.0.1（office）、127.0.0.1（lo） |
| `wan → 203.0.113.1` ping 结果 |成功 |
| `wan → 10.0.3.2` ping 结果 | 失败（无路由）|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 nat/PREROUTING（入站前）改写目标地址；SNAT 在 nat/POSTROUTING（出站后）改写源地址。
PREROUTING 负责入站 DNAT；POSTROUTING 负责出站 SNAT。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先 “抢到” 包。需通过 --dport 更精准匹配，或使用端口映射到非本机监听端口，或修改 DNAT 规则的优先级。
