...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |匹配 ESTABLISHED、RELATED 状态连接 |放行内网服务器回包，保证双向通信 |
| 2 |ACCEPT|入接口wg0、出接口veth-vpn-lan，源10.10.10.1、目的10.20.0.2，TCP 8080 新连接 |仅允许 VPN 客户端访问内网 8080 端口服务 |
| 3 | LOG|入接口wg0，限速 5 次 / 分钟，日志前缀VPN-DENY: |记录所有被拒绝的 VPN 转发流量 |
| 4 |REJECT |入接口wg0 |拒绝所有未被放行的 VPN 转发流量 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |成功 | 失败 | 是（VPN-MGMT-DENY） |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量经过 VPN 网关转发至其他主机，不属于网关本机流量 |
| `client -> server:2222` | `FORWARD` |跨主机转发流量|
| `client -> vpn:9090` | `INPUT` |流量目的为 VPN 网关本机进程，属于入站本机流量 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 8080 超时 / 失败，请求能发出但无回包 | 客户端请求被放行，但服务器回包无匹配规则，被 FORWARD 默认 DROP|重新添加ESTABLISHED,RELATED放行规则至 FORWARD 链首行 |
| `LOG` 放在 `REJECT` 后面 | 访问被拒绝，但无对应日志输出 |iptables 规则顺序执行，命中 REJECT 后直接终止流程，后续 LOG 规则不执行| 调整规则顺序，将 LOG 规则放在 REJECT 规则之前|
| 管理面规则错写到 `FORWARD` |客户端仍可正常访问 vpn:9090，计数器与日志均无变化 | 访问网关本机的流量走 INPUT 链，FORWARD 链规则无法拦截本机入站流量| 删除 FORWARD 链错误规则，将拦截 / 日志规则写入 INPUT 链|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |iptables FORWARD 规则专门放行该端口流量|
| `client -> server:2222` |失败 | 未配置放行规则，被 LOG+REJECT 拒绝|
| `client -> server ping` |失败 |ICMP 流量无放行规则，被 LOG+REJECT 拒绝 |
...
答：AllowedIPs：控制流量是否进入 WireGuard 隧道（路由层面）；iptables：控制隧道内流量的端口 / 协议访问（防火墙层面）。AllowedIPs 只能限制 IP 段，无法限制端口和协议，无法实现细粒度权限控制。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：访问内网服务器的流量是经过 VPN 网关转发的，不是访问网关本机，因此走FORWARD链；INPUT链只处理目的为网关本机的流量。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：请求报文能被放行，但服务器的响应报文因无ESTABLISHED,RELATED规则而被丢弃，最终表现为连接超时、访问失败。

4. 为什么 LOG 必须放在 REJECT 前面？
答：iptables 规则按顺序匹配，REJECT会直接终止匹配流程；若LOG在REJECT之后，REJECT先命中，LOG规则不会执行，无法记录拒绝日志。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：安全上：内层业务数据全程加密，底层抓包无法窃取内容，保障传输安全；但仍会暴露两端的外网 IP、UDP 端口 51820，攻击者可借此识别 WireGuard 服务。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：会导致权限失控，VPN 用户可访问内网所有端口 / 服务，一旦客户端被入侵，攻击者可横向渗透整个内网，大幅提升数据泄露和系统被攻陷的风险。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client -> vpn:9090 走INPUT链（访问网关本机服务）；client -> server:8080 走FORWARD链（流量经网关转发到内网），区别在于流量的最终目的地是否为网关本机。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：WireGuard 是加密隧道，负责数据包的封装 / 解密，不修改 IP / 端口；conntrack NAT 映射是地址转换，且隧道生命周期由密钥 / 握手控制，和单次业务连接无关，因此不能等同。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：仅允许企业内网固定管理 IP / 运维终端 IP 访问，禁止公网和 VPN 客户端 IP；配置LOG规则记录所有访问，非白名单 IP 的 SSH 访问添加LOG+REJECT，自定义日志前缀便于审计。
