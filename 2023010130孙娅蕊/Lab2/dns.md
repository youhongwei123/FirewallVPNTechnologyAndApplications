...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0x13f3|
| 查询域名（Query Name）       |www.yxnu.edu.cn|
| 查询类型（Type）             |A（Host Address，值为 1）
|
| 目的端口                     |53  |
...
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0x13f3|
| 返回码（Response Code）      |No error（对应 0x0000）|
| 回答记录数（Answers）        |1|
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用是将域名解析为对应的 IP 地址，实现人类易记的域名到机器可识别的 IP 地址的映射。访问网站时必须先进行 DNS 解析，因为网络通信基于 IP 地址，浏览器 / 操作系统无法直接通过域名建立 TCP 连接。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：DNS 请求包和响应包中的 Transaction ID一致。该字段用于匹配请求与响应，防止响应混淆与伪造。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录表示IPv4 地址记录，用于将域名映射到 IPv4 地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用UDP 协议，端口 53。当响应报文超过 512 字节、进行区域传输或需要可靠传输时，会改用 TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：gethostbyname()仅支持 IPv4，不兼容 IPv6，无法适配双栈网络。它不是线程安全的，返回静态缓冲区，多线程场景存在数据竞争风险。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：一个域名返回多个 IP 地址通常说明该服务启用了DNS 轮询、负载均衡、CDN 分发或多活部署，用于提升可用性、并发能力和访问速度。
