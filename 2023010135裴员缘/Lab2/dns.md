...
| 项目             | 你的填写内容 |
| :--------------- | :----------- |
| 你查询的域名     |www.tencent.com|
| 程序输出的 IP 地址 |103.220.64.100|
...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0x82e0|
| 查询域名（Query Name）       |www.tencent.com|
| 查询类型（Type）             |A|
| 目的端口                     |53|
...
| 回答记录数（Answers）        |3|
| 返回的 IP 地址               |103.220.64.100、182.247.248.128|
| TTL（若可见）                | 606               |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名系统）的核心作用是建立人类可读的域名与机器可读的 IP 地址之间的映射关系，充当互联网的 “电话簿” 实现域名解析功能；访问网站时必须先进行 DNS 解析，是因为 TCP/IP 网络通信依赖 IP 地址定位服务器，而便于记忆和传播的域名无法被计算机直接识别，只有通过解析将域名转换为对应 IP 地址，才能建立网络连接并加载网页内容，同时 DNS 解析也为网站服务的灵活调度与变更提供了保障

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：是一致的，该字段的作用是用于标识和匹配一次 DNS 查询与对应的响应，确保客户端能准确识别服务器返回的响应归属哪一次请求，避免在并发或多次查询中出现响应混淆，保证通信的可靠性与有序性。


3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：是 DNS 中的地址记录，用于将域名映射到对应的 IPv4 地址，是最基础的 DNS 记录类型，通过查询 A 记录可以获取域名所绑定的服务器 IPv4 地址，从而实现对网站服务器的定位与访问

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 协议，端口号为 53；当 DNS 响应报文长度超过 512 字节，或者进行区域传输（AXFR）时，会改用 TCP 协议，同样使用端口 53

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：gethostbyname() 仅支持 IPv4 解析，不兼容 IPv6；且线程不安全，返回数据存储在静态缓冲区，多线程场景下易出错，因此现代工程中不再推荐使用。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：这通常说明该网站采用了负载均衡技术，通过将同一个域名解析到多个不同的 IP 地址，将用户请求分散到多台服务器上，以此提高网站的访问速度、处理能力和可用性，避免单台服务器压力过大或故障导致服务不可用
