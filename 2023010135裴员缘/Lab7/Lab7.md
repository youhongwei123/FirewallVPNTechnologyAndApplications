...
| `lan` 的默认网关 |10.0.0.1（即 fw 的内侧 IP） |
| 初始访问是否成功 |成功 |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 只有第一条规则时是否成功 |失败（请求超时） |
| 第一条规则放行的是什么方向的包 |lan → wan 方向，目的端口为 38070 的 TCP 请求包 |
| 失败的原因 |防火墙默认策略为 DROP，仅放行请求包但拦截了 wan → lan 方向的响应包，导致 TCP 连接无法完成三次握手 |
| 第二条规则匹配的是什么 |wan → lan 方向，源端口为 38070 的 TCP 响应包 |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 |2 |

简答题：

1. 第二条规则 `--sport 38070 -j ACCEPT` 放行的是"所有源端口为 38070 的 TCP 包"，不区分这个包是响应还是新发起的连接。如果 `wan` 侧有主机想主动连接 `lan`，只需要让程序从端口 38070 发起连接，这条规则就会放行它的 SYN 包。用一句话概括这个问题的本质：无状态防火墙仅按端口放行数据包，无法区分 “响应包” 和 “主动发起的连接包”，导致存在安全漏洞，无法有效阻止外部主动连接内网

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
场景：内网用户访问外网的 Web 服务，同时需要阻止外网主动发起的所有连接。
无状态规则：要让内网访问外网 Web 服务的响应包回来，就必须放行外网 80/443 端口的所有入站包，这会导致外网主机也能主动通过这些端口向内网发起连接，存在安全隐患。
有状态规则：只需要放行内网发起的 “已建立 / 相关” 连接，自动允许响应包返回；同时拒绝所有来自外网的新连接请求，既满足业务需求，又保障了内网安全。
![无状态规则](stateless.png)
...
有状态防火墙通过 conntrack 模块跟踪 TCP 连接状态。当 lan 主动发起连接时，内核会将该连接标记为 NEW 并建立状态表；当 wan 返回响应包时，内核会识别出该包属于一个已建立的连接（状态为 ESTABLISHED），匹配 --ctstate ESTABLISHED,RELATED 规则，因此即使没有写专门的返回方向规则，响应包也能被自动放行
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
无状态规则的 --sport 38070 仅按源端口放行数据包，不区分连接状态，因此 wan 侧主机只要从 38070 端口发起连接，就会被放行；而有状态规则仅允许 NEW 状态的包（即由 lan 主动发起的连接请求），所有来自 wan 的主动连接请求都是 NEW 状态，但不匹配规则，因此会被拒绝，从而有效阻止外部主动连接。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） | 10.0.0.2（lan 主机 IP），随机端口（如 43336）|
| 条目中的目的地址和目的端口 |192.0.2.2（wan 主机 IP），38070（服务端端口） |
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |ESTABLISHED（三次握手完成后）、FIN_WAIT、LAST_ACK、TIME_WAIT（连接关闭过程） |

简答题：

1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的连接状态记录，而不是单纯的包头字段。
区别：仅靠包头字段（如端口、地址）无法区分包是新连接请求还是已有连接的响应；而 conntrack 会记录连接的完整状态（NEW/ESTABLISHED/RELATED 等），只有属于已建立连接的响应包才会被放行，能有效防止外部主动发起的连接攻击。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：最本质的区别在于是否跟踪连接状态：
无状态包过滤仅根据数据包的源 / 目的地址、端口等包头字段进行匹配，不区分 “新连接请求” 和 “已有连接的响应包”，无法识别完整的连接上下文。
有状态检测防火墙通过 conntrack 模块维护连接状态表，能识别 NEW/ESTABLISHED/RELATED 等连接状态，基于连接的上下文进行放行控制，安全性更高。


2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：顺序的意义在于提高匹配效率和避免逻辑错误：
ESTABLISHED,RELATED 规则匹配已建立连接的响应包和关联流量，这类包数量占比高，放在第一条可以快速匹配放行，减少后续规则的处理压力。
NEW 规则仅匹配新连接请求，数量相对较少，放在第二条。
不能调换：如果把 NEW 放在前面，会导致所有包都先被匹配是否为新连接，响应包也会先被错误地判定为 NEW 状态，可能导致正常响应包被拦截，或被外部利用发起非法连接。


3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被视为 NEW 状态（内核找不到对应的连接记录，认为这是一个新的连接请求）。
如果防火墙规则仅允许 lan 主动发起的 NEW 连接，且默认策略为 DROP，那么超时后的后续包会被当成外部主动发起的连接请求而被拒绝，导致连接中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已建立连接相关联、但独立于该连接本身的辅助流量，比如 FTP 协议的数据连接、ICMP 错误报文（如端口不可达、超时等）。
本实验使用的是简单的 HTTP 通信，没有这类关联流量，因此未观察到。
典型场景：主动模式 FTP 中，客户端与服务器建立控制连接后，服务器主动发起的数据连接会被标记为 RELATED 状态，由有状态防火墙自动放行。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：默认拒绝，按需放行” 采用白名单机制，只放行明确允许的流量，其余所有流量都被默认拦截，能最大程度缩小攻击面，即使存在配置疏漏也不会直接暴露风险。
“默认允许，事后补堵” 采用黑名单机制，先放行所有流量，再手动拦截恶意流量，容易因遗漏规则、新漏洞出现或配置错误导致安全漏洞，无法有效抵御未知威胁。
