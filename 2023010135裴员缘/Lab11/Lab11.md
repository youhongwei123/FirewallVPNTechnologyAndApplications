...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |是，client 为 12 秒前、vpn 为 21 秒前握手|
| 是否看到 `transfer` 计数               |是，两端均有收发字节统计|
...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            |客户端：10.20.0.0/24；VPN 网关：10.10.10.1/32|
| 是否能访问 `10.20.0.2:8080`  |可以正常访问，curl 能够列出服务端目录内容、ping 连通无丢包|
...
| `latest handshake` 是否还在      |在，12 秒前完成握手，隧道底层 UDP 连通正常|
| 去 `10.20.0.0/24` 的路由是否消失 |已消失，路由表只有10.30.0.0/24 dev wg0，无 10.20 网段隧道路由|
| 是否还能访问 `10.20.0.2:8080`    |不能访问，ping 提示 Network is unreachable，连接超时失败|
| 失败的直接原因                   |AllowedIPs 修改后 wg-quick 自动更新内核路由，去往 10.20.0.0/24 没有指向 wg0 的路由，流量无法进入 WireGuard 加密隧道转发          |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
隧道握手与业务传输是两套不同路径：
握手：依靠两端 underlay 底层公网 UDP（192.0.2.x:端口），只受 Endpoint 配置影响，和 AllowedIPs 无关，因此配置修改后仍能正常握手；
业务流量：由 AllowedIPs 控制内核路由，只有目标网段在 AllowedIPs 内才会被导入 wg0 加密隧道，本次改为10.30.0.0/24后，10.20.0.0/24流量不走隧道，访问失败。
...
| 协议                     | UDP                                   |      ICMP    |
| 是否能看到 ICMP/HTTP 内容 |           不能，内层数据被加密，仅 UDP 密文载荷                         |    可以，完整明文 ICMP/HTTP 数据      |
| 为什么内外两层看到的不同 |        WireGuard 做 UDP 封装 + 加密：原始内网报文被整体加密封装进 UDP 数据包在底层公网传输；到达 vpn 后 wg0 自动解密拆封装，还原原始内网报文                            |          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
outer（underlay）对应信封：只记录收发方外层公网地址，信封内部信件（原始业务数据）被加密无法查看；
inner（wg0）对应信纸：解密后露出原始内网地址、ICMP/HTTP 真实业务内容；
安全性意义：公网链路传输全程密文，中间人抓底层数据包无法窃取内网通信内容，实现数据加密保密传输。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：① 路由下发：wg-quick 启动时自动向内核添加路由，目标网段流量转发至 wg0 隧道网卡；
② 入站访问控制：收到报文解密后，校验源 IP 是否在 AllowedIPs 网段，不在则丢弃报文。
优先影响路由原因：业务流量是否进隧道由内核路由决定，路由是数据转发前置条件，因此配置改动首先变更路由，表现为先影响连通，看起来像访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手与业务传输是两套独立路径：
握手：基于两端 underlay 底层 UDP（公网 IP: 端口），仅依赖 Endpoint 与公私钥校验，不受 AllowedIPs 路由影响，因此 wg show 显示隧道在线；
业务传输：依赖 AllowedIPs 生成的内核路由，网段配置错误会缺失隧道路由，流量无法进入 wg0，导致 curl 访问失败，二者互相独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0代表所有 IP 全量路由进隧道，风险：
客户端全量流量全部经过 VPN 转发，易造成网关带宽耗尽；
失去内网访问控制，客户端可通过隧道访问 VPN 侧所有内网网段，扩大内网攻击面；
若 VPN 节点被攻陷，客户端全部上网流量会被劫持监听。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 是非对称密钥双向鉴权：
建立隧道需要客户端私钥签名、服务端公钥验签，同时服务端私钥、客户端公钥完成反向校验；攻击者仅有 VPN 私钥，没有客户端私钥无法完成客户端身份签名，无法冒充客户端建立隧道。


5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2（client 底层 underlay 公网 IP）；
抓包看不到内层内容原因：WireGuard 对内层原始 IP 报文做高强度加密后，整体封装进 UDP 报文在公网传输，底层抓包只能拿到加密后的 UDP 密文，无明文内层 IP 与应用数据。


6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：地址隐私保护：外层仅暴露公网 underlay 地址，内网真实源 / 目的 IP 被封装隐藏，公网抓包无法获知内网拓扑与内网通信地址；
传输数据加密：内层原始报文整体加密后封装在外层 UDP 中，公网链路抓包只能获取密文，无法解析 ICMP、HTTP 等原始业务明文数据，防止数据窃听泄露。
