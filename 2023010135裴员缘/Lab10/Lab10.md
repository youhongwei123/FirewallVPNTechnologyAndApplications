...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |执行 curl 命令后返回curl: (7) Failed to connect to 10.0.3.2 port 8080: Couldn't connect to server，外网节点无法直接访问 DMZ 区域的内网服务 |
| `office → 10.0.3.2:8080` |成功 |执行 curl 命令后成功返回 Web 服务的 HTML 目录列表，内网节点可正常访问 DMZ 区域的 Web 服务 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |所有ctstate RELATED,ESTABLISHED的流量 |放行已建立连接、相关连接的所有转发流量 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 ctstate NEW |放行目标为 dmz 主机 8080 端口的新 TCP 连接 |
...
> 答：源 IP 是防火墙 fw 的外网公网 IP 203.0.113.1。因为 DNAT 端口映射对外网 wan 客户端完全透明，内网 dmz 主机的响应包会被防火墙自动改写源 IP 为防火墙公网 IP，wan 节点全程只和公网 IP 通信，完全感知不到 DNAT 的存在和内网 IP

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条连接的后续数据包仍然可以正常转发通信。因为 conntrack 连接跟踪表已经记录了该连接的双向地址映射关系，数据包进入防火墙时，会优先匹配 conntrack 表的已有条目，直接执行地址改写，不会再重新匹配 DNAT 规则；只有新建连接的首包才需要匹配 DNAT 规则，已建立连接的后续包完全依赖 conntrack 表实现转发
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |访问成功，正常返回 HTML 目录列表	 |是 |
| `wan → 203.0.113.1:2222` |访问成功，正常返回 HTML 目录列表	 |是 |
| `wan → 203.0.113.1:3306` |访问失败，返回连接错误 |否 |
...
| 日志中 `DST=` 字段是哪个地址 |内网 DMZ 主机的 IP 地址 10.0.3.2（DNAT 改写后的目标地址） |
...
> 答：这一现象说明，当前的 LOG 规则是在DNAT 地址改写完成之后才触发的，因此日志中记录的是改写后的内网目标 IP 10.0.3.2，而非改写前的公网 IP。如果要记录原始目标地址（改写前），需要将 LOG 规则放在 nat 表的 PREROUTING 链中，因为nat表的PREROUTING链是 DNAT 地址改写的发生位置，在这个位置记录日志，就能捕获到改写前的原始目标地址

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：LOG 日志完全可以帮助安全人员发现端口扫描攻击。识别端口扫描的方法：
短时间内，同一个源 IP，向目标主机的大量不同端口发起大量 SYN 新连接请求，生成大量带不同端口号的日志记录，是端口扫描的核心特征；
日志中出现大量来自同一源 IP 的、目标端口连续递增 / 随机的 SYN 包记录，且绝大多数端口没有对应服务响应，即可判定为端口扫描行为；
正常访问只会针对特定端口生成日志，而端口扫描会在短时间内生成大量不同端口的日志，通过日志的端口分布、源 IP 统计、时间间隔即可快速识别
...
| :--- | :------- |
| `wan` 地址 |203.0.113.2 |
| `fw` 四个接口地址 |10.0.1.1（office口）、10.0.2.1（guest口）、10.0.3.1（dmz口）、203.0.113.1（wan口） |
| `wan → 203.0.113.1` ping 结果 |通，能正常 ping 通防火墙外网口 |
| `wan → 10.0.3.2` ping 结果 |不通，外网 wan 无法直接 ping 通内网 DMZ 主机 |
| `office → 10.0.3.2` ping 结果 |通，内网 office 主机能正常 ping 通 DMZ 主机 |
| `office → 203.0.113.1` ping 结果 |通，内网 office 主机能正常 ping 通防火墙外网口 |

### B. 端口映射配置

| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat |PREROUTING |目标 IP 203.0.113.1、TCP 8080 端口 |DNAT --to-destination 10.0.3.2:8080 |
| DNAT 2222 |nat |PREROUTING |目标 IP 203.0.113.1、TCP 2222 端口 |DNAT --to-destination 10.0.3.2:2222 |
| FORWARD 8080 |filter |FORWARD |目标 IP 10.0.3.2、TCP 8080 端口、新连接 |ACCEPT |
| FORWARD 2222 |filter |FORWARD |目标 IP 10.0.3.2、TCP 2222 端口、新连接 |ACCEPT |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 改写目标地址发生在数据包进入防火墙后的 PREROUTING 阶段，在路由决策之前完成；SNAT 改写源地址发生在数据包离开防火墙前的 POSTROUTING 阶段，在路由决策之后完成。
在 DMZ 双向 NAT 场景中：
PREROUTING 链负责处理入站流量，将外网访问防火墙公网 IP 的流量，通过 DNAT 改写为内网 DMZ 主机的 IP，实现对外服务的端口映射；
POSTROUTING 链负责处理出站流量，将 DMZ 主机访问外网的流量，通过 SNAT 改写为防火墙的公网 IP，实现内网主机的外网访问。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先抢到这个包。因为 DNAT 规则配置在nat表的PREROUTING链，是数据包进入防火墙的第一处理阶段，会在路由决策之前就改写目标地址，数据包会被转发到内网 DMZ 主机，而不会被路由到防火墙本地的 Web 服务。
要让两者共存，有两种方案：
端口区分：将防火墙本地 Web 服务改为监听其他端口（如 8081），DNAT 规则映射 8080 端口到内网主机，实现端口隔离；
源 IP 区分：通过匹配源 IP 地址，将特定源 IP 的流量放行给本地服务，其余流量执行 DNAT 转发，实现访问主体的隔离。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：防火墙通过 conntrack 连接跟踪表 来实现回复包的逆向还原。
当外网客户端访问203.0.113.1:8080时，DNAT 规则将其改写为10.0.3.2:8080，conntrack 表会记录这条连接的双向映射关系：203.0.113.1:8080 ↔ 10.0.3.2:8080；
当外网客户端访问203.0.113.1:8081时，DNAT 规则将其改写为10.0.3.3:8080，conntrack 表会记录另一条独立的映射关系：203.0.113.1:8081 ↔ 10.0.3.3:8080。
当内网服务器返回响应包时，防火墙通过 conntrack 表，根据响应包的源 IP + 源端口，匹配到对应的原始连接记录，就能精准地将内网 IP + 端口，逆向还原为对应的公网 IP + 端口，保证回复包能正确返回给外网客户端。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能回到 wan。
机制：本实验中，wan 客户端发起的入站请求经过 DNAT 改写后，防火墙的 conntrack 连接跟踪表会记录这条连接的双向映射关系。当 DMZ 服务器回复包到达防火墙时，防火墙会匹配 conntrack 表中的对应记录，自动将回复包的源地址10.0.3.2逆向还原为公网 IP203.0.113.1，再转发给 wan 客户端，全程不需要额外配置 SNAT 规则。
补充说明：如果是 DMZ 服务器主动发起的出站访问（非 wan 客户端发起的入站连接），则必须配置 SNAT 规则，否则内网 IP 的数据包无法在公网路由，无法回到 wan

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：防火墙无法正常工作。
后果：conntrack 表满后，新的连接无法被记录，NAT 映射、状态防火墙的连接追踪都会失效，新的访问请求会被直接丢弃，导致服务中断；同时，大量无效连接会占用防火墙的 CPU、内存资源，导致防火墙性能急剧下降，甚至出现卡死、重启等故障。
防护措施：
调大 conntrack 表的最大容量，提升连接承载能力；
配置连接超时时间，及时清理无效的空闲连接，释放表空间；
配置连接速率限制，对单个源 IP 的并发连接数、连接速率做限制，抵御连接洪水攻击；
开启 SYN 代理、连接跟踪黑名单等防护功能，过滤恶意攻击流量。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：四次实验的共同基础是连接跟踪（conntrack）机制，所有功能都依赖 conntrack 来记录、追踪数据包的连接状态，实现状态匹配、地址映射、日志审计等核心能力。
安保系统类比：
有状态防火墙：对应大楼的门禁系统，只允许已授权、已登记的人员进出，拦截非法闯入；
NAT：对应大楼的前台接待系统，将外来访客的请求，转发到对应办公室的内部人员，同时隐藏内部办公区的真实地址；
分区审计：对应大楼的监控系统，记录所有进出人员的时间、身份、行为，实现安全事件的追溯；
端口映射：对应大楼的专线接待通道，将指定的对外服务，映射到内部的对应服务窗口，既对外提供服务，又保护内部网络。
