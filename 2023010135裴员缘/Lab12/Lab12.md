...
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1 和 10.20.0.0/24 dev wg0 scope link |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |--ctstate RELATED,ESTABLISHED（所有接口、所有地址） |放行所有已建立或关联的连接（内网服务器回包） |
| 2 |ACCEPT |-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 -p tcp --dport 8080 --ctstate NEW |仅允许 VPN 客户端发起的、访问内网服务器 8080 端口的新 TCP 连接 |
| 3 |LOG |-i wg0 -m limit --limit 5/min --limit-burst 10 --log-prefix "VPN-DENY: " --log-level 4 |对所有来自 VPN 隧道的被拒绝流量进行限流日志记录 |
| 4 |REJECT |-i wg0（所有地址、所有协议） |拒绝所有来自 VPN 隧道、未匹配前两条规则的流量 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量目的地是内网服务器，需要经过 VPN 网关转发，因此匹配 FORWARD 链规则 |
| `client -> server:2222` | `FORWARD` |流量目的地是内网服务器，需要经过 VPN 网关转发，因此匹配 FORWARD 链规则 |
| `client -> vpn:9090` | `INPUT` |流量目的地是 VPN 网关本机进程，直接进入 INPUT 链，不走转发流程 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |业务访问失败，客户端 curl 超时，服务器无响应 |内网服务器的回包无法被 FORWARD 链放行，连接无法完成三次握手 |重新添加规则：iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |被拒绝的流量不会生成 VPN-DENY 日志，journalctl 查不到记录 |数据包被 REJECT 直接丢弃，无法匹配到后面的 LOG 规则 |将 LOG 规则移动到 REJECT 规则之前：iptables -I FORWARD [LOG规则行号] -i wg0 -m limit ... -j LOG |
| 管理面规则错写到 `FORWARD` |客户端仍能访问 vpn:9090，规则不生效；内网流量也被错误拦截 |访问网关本机的流量走 INPUT 链，不会匹配 FORWARD 链规则 |将规则从 FORWARD 链移动到 INPUT 链：iptables -I INPUT 1 -i wg0 -p tcp --dport 9090 -j REJECT |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |FORWARD 链中明确放行 wg0 接口访问 10.20.0.2:8080 的 TCP 流量，规则匹配通过 |
| `client -> server:2222` |失败 |未在 FORWARD 链中添加放行 2222 端口的规则，流量被默认 DROP 策略拦截 |
| `client -> server ping` |失败 |FORWARD 链未允许 ICMP 流量，且默认策略为 DROP，因此 ping 请求被拒绝 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端访问 server:8080 失败，连接超时，服务器收不到回包 |内网服务器的响应包无法通过 FORWARD 链，TCP 三次握手无法完成 |重新添加规则：iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |被拒绝的流量不会生成 VPN-DENY 日志，journalctl 查不到记录 |数据包被 REJECT 直接丢弃，无法匹配后面的 LOG 规则 |将 LOG 规则移动到 REJECT 规则之前：iptables -I FORWARD [LOG规则行号] -i wg0 -m limit ... -j LOG |
| 管理面规则错写到 `FORWARD` |客户端仍能访问 vpn:9090，规则不生效；部分内网流量可能被错误拦截 |访问网关本机的流量走 INPUT 链，不会匹配 FORWARD 链规则 |将规则从 FORWARD 链移动到 INPUT 链：iptables -I INPUT 1 -i wg0 -p tcp --dport 9090 -j REJECT |
...
AllowedIPs 是 WireGuard 本身的路由 / 准入控制，决定哪些网段的流量可以通过隧道进出，同时作为对端认证的一部分；iptables 是在操作系统层面做的细粒度访问控制，可按端口、协议、状态、方向等过滤流量。仅靠 AllowedIPs 无法控制同一网段内不同端口 / 协议的访问权限，也无法实现连接状态跟踪、日志审计和动态策略，因此不能替代 iptables。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
因为 client -> server:8080 的流量目的地是内网服务器，VPN 网关仅做转发，不接收该流量，因此只会经过 FORWARD 链；INPUT 链仅处理目的地是网关本机的流量，不会匹配转发流量的规则。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
TCP 三次握手的 SYN 包能到达服务器并得到响应，但服务器返回的 ACK/SYN 包在回传时会被 FORWARD 链默认策略拦截，导致客户端连接超时，无法完成握手，访问失败。
4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则是按顺序匹配的，一旦流量匹配到 REJECT 规则就会被直接丢弃，不会继续匹配后续规则；只有将 LOG 规则放在前面，才能在流量被拒绝前完成日志记录。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
这意味着业务流量已被 WireGuard 加密，抓包无法获取明文内容，通信数据保密性得到保障；但仍可暴露隧道两端的公网 IP、隧道端口、流量大小与频率，攻击者可据此识别 VPN 隧道、进行流量分析或 DDoS 攻击
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
会导致用户可访问内网所有网段，一旦用户设备被攻陷，攻击者可横向渗透整个内网；同时会扩大攻击面，用户可访问高风险服务（如数据库、运维端口），缺乏细粒度控制和审计，安全风险极高
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> vpn:9090 经过 INPUT 链，因为流量目的地是 VPN 网关本机；client -> server:8080 经过 FORWARD 链，因为流量目的地是内网服务器，VPN 网关仅做转发，因此两条流量经过的链不同。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
因为 WireGuard 隧道是端到端的加密隧道，不是 NAT；外层 UDP 包仅用于承载加密的内层流量，不修改 IP / 端口，也不维护 NAT 会话状态，与 conntrack 记录的 NAT 映射在原理和功能上完全不同
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
应仅允许可信 IP（如企业办公网段、管理员固定 IP）访问，禁止 VPN 隧道内的用户访问；日志可在INPUT链添加规则，对访问 SSH 端口的流量打MGMT-SSH:前缀日志，便于审计和溯源
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
先检查服务是否正常监听并在服务器本地可访问，再检查 VPN 网关和服务器的路由表是否配置正确，接着检查 iptables 规则是否放行流量，再查看内核日志是否有拒绝记录，最后检查 iptables 计数器验证规则是否匹配
