...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` | 放行所有 RELATED、ESTABLISHED 回程流量，允许源 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |A-TO-B-DENY: | DROP |
| `gwB` | 放行所有 RELATED、ESTABLISHED 回程流量，允许源 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |VPN-S2S-DENY:	 |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |curl 正常返回网站 HTML 目录页面，访问连通 |无拦截日志 |
| `siteA -> siteB:2222` | 失败 |curl 连接超时，无法建立 TCP 连接 |产生带A-TO-B-DENY前缀的内核拦截日志，记录 TCP 2222 报文 |
| `siteA -> siteB ping` | 失败 |ping 全部无应答，100% 丢包 |产生带A-TO-B-DENY前缀的内核拦截日志，记录 ICMP 请求报文
...
| `latest handshake` |15 seconds ago |15 seconds ago |
| `transfer` 计数 |接收 180B，发送 272B |接收 124B，发送 180B |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |访问成功，返回网页 HTML |1. RELATED,ESTABLISHED 放行；2. 允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |
| `siteA -> siteB:2222` |连接超时，访问失败 | 1. LOG 记录流量（前缀 A-TO-B-DENY）；2. REJECT 拒绝数据包|
| `siteA -> siteB ping` |无应答，100% 丢包 |1. LOG 记录 ICMP 流量（前缀 A-TO-B-DENY）；2. REJECT 拒绝数据包 |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 <-> 192.0.2.2	 |UDP 51820 |WireGuard 外层加密封装报文，看不到内网业务 IP 与端口，载荷为密文 |
| `wg0` |10.1.0.2 <-> 10.2.0.2 |TCP 8080 |隧道解密后的原始内网明文 HTTP 流量，无外层 UDP 头部 |
| B 站点 LAN |10.1.0.2 <-> 10.2.0.2 |TCP 8080 |抵达目标内网的明文业务报文，和 wg0 抓包内容一致 |
...
远程接入 VPN 是单台终端远程接入企业内网，一端为个人设备一端为网关；站点到站点 VPN 是两个完整内网网关互联，两端均是局域网，实现内网全互通。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
AllowedIPs 用于匹配流量转发隧道，填写对端内网网段能让网关识别去往对方内网的流量，自动封装进 WireGuard 隧道转发，非匹配流量不会走 VPN。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
开启内核 IP 转发功能，站点间流量需要网关跨网卡转发内网与隧道流量，关闭则无法跨接口传递数据包
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
普通主机访问对端内网时，数据包需先交给本地 VPN 网关处理封装转发，默认网关指向 VPN 网关才能让跨站点流量路由至隧道
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
先核对两端 AllowedIPs 与内核路由是否生成远端网段路由，再检查网关 ip 转发是否开启，接着排查两端防火墙 FORWARD 转发规则，最后核对站点主机路由与连通性
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
两端配置防火墙规则形成纵深防御，单一侧防护失效仍能拦截非法流量；只在一边写规则可以，但流量会完整穿越 VPN 隧道才被拦截，浪费隧道带宽且安全性下降
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
VPN 隐藏内网源目 IP、业务端口、传输明文内容，仅暴露两端底层公网 IP、WireGuard 固定 UDP 端口 51820 与数据包长度
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
gwA 的 FORWARD 链新增放行 10.2.0.0/24 访问对应服务端口的新建 TCP 规则，gwB 的 FORWARD 链同步放行对应出站流量，同时两端 AllowedIPs 保留双向内网网段路由。
