...
| `wan` 无法回复的原因 |已删除 wan 上指向 10.0.0.0/24 的回程路由，无 NAT 地址转换时，wan 无法将 ICMP 回复包路由回内网主机 10.0.0.2 |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 加 NAT 后 ping 是否成功 |成功（0% 丢包，可正常收到回复） |
...
在 Lab7 中，conntrack 帮助防火墙判断包是否属于合法连接（状态检测）；在 Lab8 中，它还额外记录了NAT 地址映射关系（内网真实地址 ↔ NAT 转换后的公网地址）。这说明 conntrack 是 Linux 网络状态管理的核心模块，既支撑防火墙的有状态过滤，也支撑 NAT 的地址转换与还原，是实现有状态网络服务的基础
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会通过 ** 五元组（源 IP、源端口、目的 IP、目的端口、协议）** 来区分连接：
当两台内网主机同时访问同一个外网服务器的同一个端口时，NAT 会为它们分配不同的外网源端口（即使源 IP 相同，端口也会不同）。
回复包到达时，NAT 会根据 conntrack 记录的五元组映射关系，找到对应的内网主机，将包转发回去
...
| TCP 38070 访问是否成功 |成功 |
| ping 是否成功 |失败 |
| ping 失败的原因 |防火墙 FORWARD 规则仅放行 TCP 38070 端口的新连接和已建立 / 相关连接，未放行 ICMP 协议的流量，因此 ping 请求被防火墙拒绝转发 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址段（如 10.0.0.0/8、192.168.0.0/16）仅在局域网内有效，互联网路由器不会为这些网段配置路由，因此外网无法将数据包直接路由到内网主机。
NAT（网络地址转换）通过网关设备将内网主机的私有地址转换为网关的公网地址对外通信，外网回复包也先到达网关，再由 NAT 根据 conntrack 记录的映射关系转发给对应的内网主机，从而解决了私有地址无法被外网直接访问的问题。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source <IP>：适用于公网 IP 固定的场景（如企业专线、静态公网 IP）。它会直接将内网地址转换为指定的公网 IP，性能更稳定，无需额外接口信息，但 IP 变更时需要手动修改规则。
MASQUERADE：适用于公网 IP 动态变化的场景（如家庭宽带、DHCP 拨号环境）。它会自动获取出口接口的当前 IP 进行转换，无需手动配置，适配 IP 频繁变化的网络环境，灵活性更强。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT 会给在应用层数据中携带 IP 地址 / 端口信息的协议带来麻烦，典型包括：
FTP（主动模式）：控制通道中会协商数据通道的 IP 和端口，NAT 无法修改应用层数据，导致服务器无法主动连接内网主机。
SIP/VoIP：信令包中携带的媒体地址为内网地址，外网服务器无法根据该地址建立媒体流连接。
IPSec VPN：NAT 修改 IP 头会导致 IPSec 校验失败，需要额外的 NAT-T（NAT 穿越）机制才能正常工作。
部分 P2P 协议：依赖双方直接暴露 IP 地址建立连接，NAT 隐藏内网地址后，会导致 P2P 打洞失败。
根本原因是：NAT 仅修改 IP / 传输层头部，无法感知并修改应用层数据中的地址信息，导致外网设备无法根据应用层数据建立有效连接。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：共享核心模块，提升效率：状态检测需要 conntrack 记录连接状态，NAT 需要 conntrack 记录地址映射关系，两者共享同一个连接跟踪表，减少资源占用，提升处理效率。
逻辑联动，简化安全策略：NAT 改写源地址后，状态检测可基于转换后的公网地址 / 端口识别合法连接，两者配合实现 “内网主动访问外网可放行，外网主动访问内网默认拦截” 的安全策略，无需配置复杂的独立规则。
降低实现成本：将 conntrack 作为核心支撑模块，同时提供状态检测和 NAT 功能，可简化路由器的软件架构，减少开发和维护成本，更适合家用设备的资源限制

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在保留现有规则的基础上，执行以下命令允许 ICMP 流量通过 FORWARD 链：sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
如需精细化控制，仅允许内网发起的 ping 请求与回复：sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -j ACCEPTsudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-reply -j ACCEPT
