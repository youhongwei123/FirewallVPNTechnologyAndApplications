...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 | ACCEPT|所有接口，已建立 / 相关连接（ctstate RELATED,ESTABLISHED） |允许响应流量回包，保障已建立连接正常通信 |
| 2 | ACCEPT|入接口 wg0、出接口 veth-vpn-lan，源 10.10.10.1，目的 10.20.0.2，TCP 端口 8080，新连接（ctstate NEW） |仅允许客户端访问服务端的 8080 端口 |
| 3 |LOG |入接口 wg0，所有流量，限制速率 5/min（burst 10） |记录所有被拒绝的 VPN 流量，日志前缀为 VPN-DENY: |
| 4 |REJECT | 入接口 wg0，所有其他流量| 拒绝所有未匹配前面规则的 VPN 流量，返回 ICMP 端口不可达|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |规则 2（ACCEPT 10.10.10.1 -> 10.20.0.2:8080 NEW） |是（pkts 从 0→1，bytes 从 0→60） |
| `client -> server:2222` |失败 |规则 3（LOG）+ 规则 4（REJECT） | 是（pkts 从 0→4，bytes 从 0→312）|
| `client -> server ping` | 失败|规则 3（LOG）+ 规则 4（REJECT） | 是（pkts 从 0→4，bytes 从 0→312）|
...
| `SRC=` | 10.10.10.1|
| `DST=` | 10.20.0.2|
| `PROTO=` |ICMP（ping 日志）/ TCP（2222 端口日志） |
| `DPT=` |无（ICMP 无端口） / 2222（TCP 日志） |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量是从客户端通过 VPN 网关转发到内网服务端，属于跨网络转发流量，命中 FORWARD 链的允许规则，因此可以正常访问 |
| `client -> server:2222` | `FORWARD` |流量同样是转发流量，但命中了 FORWARD 链的拒绝规则，因此访问失败 |
| `client -> vpn:9090` | `INPUT` |流量的目的地址是 VPN 网关自身（10.10.10.2），属于进入本机的流量，命中 INPUT 链的拒绝规则，因此访问失败并产生 VPN-MGMT-DENY 日志 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有已建立的连接被重置，业务访问失败 |状态匹配规则丢失，服务端无法响应客户端请求 |添加 iptables -A FORWARD -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |无法产生 VPN-DENY/VPN-MGMT-DENY 日志 |流量被 REJECT 规则提前匹配，不会走到 LOG 规则 |将 LOG 规则调整到 REJECT 规则前面 |
| 管理面规则错写到 `FORWARD` |客户端可以访问 vpn:9090，无法被拒绝 |FORWARD 链不处理目的为本机的流量，规则不生效 |将管理面拒绝规则从 FORWARD 链移动到 INPUT 链 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 | 命中 FORWARD 链的 ACCEPT 规则，允许访问|
| `client -> server:2222` | 失败| 命中 FORWARD 链的 LOG + REJECT 规则，被拒绝|
| `client -> server ping` | 失败| 命中 FORWARD 链的 LOG + REJECT 规则，被拒绝|

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |2 条（1 条 ICMP ping、1 条 TCP 2222） |
| 日志中被拒绝的目标端口 | 2222（TCP）、无（ICMP）|
| underlay 抓包看到的协议 |UDP 51820（WireGuard 隧道） |
| `wg0` 抓包看到的源地址 |10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 |否（流量在 FORWARD 链被拒绝，无法到达内网接口） |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` | 所有已建立的业务连接被重置，访问失败| 状态匹配规则丢失，服务端响应流量被默认 DROP 策略丢弃| 添加 iptables -A FORWARD -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |无法产生 VPN-DENY 日志 |流量被 REJECT 提前匹配，不会走到 LOG 规则 | 将 LOG 规则调整到 REJECT 规则前面|
| 管理面规则错写到 `FORWARD` |客户端可以正常访问 vpn:9090 |FORWARD 链不处理目的为本机的流量，规则不生效|将规则从 FORWARD 链移动到 INPUT 链 |
...
AllowedIPs 控制哪些流量进入隧道，iptables 控制隧道内的精细化访问权限；不能只靠 AllowedIPs，因为它只能粗粒度放行，无法做端口级控制和日志记录。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
访问内网服务器的流量是转发流量，目的不是 VPN 网关本身，所以走 FORWARD 链；只有访问网关自身的流量才走 INPUT 链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端能发请求，但服务端的响应流量会被丢弃，导致访问失败。
4. 为什么 LOG 必须放在 REJECT 前面？
iptables 按顺序匹配规则，LOG 必须在 REJECT 前，才能在拒绝前记录日志。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
说明 WireGuard 隧道加密了业务流量，外层只能看到加密的 UDP 包，业务内容被隐藏，提升了安全性。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
用户可以访问整个内网，一旦账号泄露，攻击者能横向移动、窃取数据，风险极高。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> vpn:9090 走 INPUT，client -> server:8080 走 FORWARD：因为前者目的地址是 VPN 网关本身，后者目的地址是内网服务器。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
外层是 WireGuard 隧道的 UDP 连接，内层是解密后的业务 TCP 连接；WireGuard 隧道不是 NAT，是加密隧道，所以不能按 NAT 理解。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
应仅允许信任 IP 访问（如内网管理段），并配置日志记录，避免公网直接暴露。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
① wg show 看握手是否正常 → ② 服务监听是否正常 → ③ 路由表是否正确 → ④ iptables 规则和计数器 → ⑤ 内核日志是否有拒绝记录。
---
