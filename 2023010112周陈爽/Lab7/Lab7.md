...
| :--- | :--- |
| 只有第一条规则时是否成功 |否 |
| 第一条规则放行的是什么方向的包 | 内网 lan → 外网 wan 的出站 TCP 请求包，目的端口为 38070|
| 失败的原因 |防火墙默认策略为 DROP，仅放行出站请求，未放行 wan 服务器返回的响应包（源端口 38070），回包被丢弃，导致 TCP 连接无法完成 |
| 第二条规则匹配的是什么 |外网 wan → 内网 lan 的入站 TCP 响应包，源端口为 38070 |
| 加第二条规则后是否成功 |是 |
| 当前规则共几条 | 2条|
...
无状态的端口匹配规则无法区分 “内网主动连接的响应包” 和 “外网主动发起的新连接请求”，导致防火墙在放行合法响应流量的同时，也暴露了端口，引入了安全风险。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
场景：内网主机需要主动访问外网的 HTTPS 服务，同时禁止外网主机主动向内网发起任何连接。
无状态规则：为了让内网收到服务器的响应，必须放行源端口为 443 的 TCP 包，这会导致外网可以主动从 443 端口发起连接，内网端口直接暴露，无法实现 “仅允许响应、拒绝主动访问” 的需求。
有状态规则：可以通过 --state ESTABLISHED,RELATED 匹配已建立连接的响应包，只放行内网主动发起连接的回包，同时拒绝所有外网主动发起的 NEW 状态连接，既保证了正常访问，又实现了内网端口的隐藏与安全防护。
![无状态规则](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2条 |2条 |
| 是否需要手动考虑返回方向 | 是，必须单独放行响应包 |否，状态规则自动匹配响应包 |
| 能否区分"新连接"和"响应包" |不能，仅按端口匹配，无法区分连接状态 |能，通过 ctstate 区分 NEW 和 ESTABLISHED |
| `wan` 能否主动发起到 `lan` 的连接 | 能，只要从 38070 端口发起就能被放行 |不能，只有 lan 主动发起的连接的响应包才会被放行 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为状态防火墙通过 conntrack 模块维护了一张连接状态表，记录了 lan 主动发起的 TCP 连接信息。wan 返回的响应包属于 ESTABLISHED 或 RELATED 状态，会被第一条 --ctstate ESTABLISHED,RELATED 规则自动放行，无需手动添加返回方向的规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
无状态的 --sport 38070 仅匹配源端口，无法区分包是 “响应包” 还是 “主动发起的新连接”，因此 wan 从 38070 端口发起的 SYN 包也会被放行，存在安全漏洞。
有状态规则中，NEW 状态仅匹配 lan 主动发起到 wan 38070 端口的新连接请求，而 wan 主动发起的 NEW 状态连接请求会被默认策略 DROP 拒绝，因此无法建立连接，实现了更精准的访问控制。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:35658 |
| 条目中的目的地址和目的端口 |192.0.2.2:38070 |
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |SYN_SENT → SYN_RECV → ESTABLISHED → FIN_WAIT → LAST_ACK → TIME_WAIT |

简答题：

1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
依据的是 conntrack 表里的记录，而不是 IP/TCP 包头里的字段。
区别：
包头字段只能看到源 / 目的 IP、端口、TCP 标志位等静态信息，无法判断这个包是否属于一个已经建立的合法连接，也无法区分响应包和伪造的主动请求包。
conntrack 表会记录完整的连接状态（如 NEW/ESTABLISHED/RELATED），防火墙会根据表中的连接状态来放行响应包，只有之前由内网主动发起的连接的回包才会被放行，有效防止了伪造包和外网主动攻击。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤仅基于单个数据包的 IP、端口、协议等静态字段进行匹配，不记录连接状态，无法区分响应包和伪造的主动请求包，需要手动放行双向流量，存在安全漏洞。
有状态检测防火墙通过 conntrack 模块维护连接状态表，记录每一条连接的状态（如 NEW/ESTABLISHED/RELATED），能识别并放行合法连接的响应包，拒绝所有未建立连接的主动请求，既简化了规则配置，又提升了安全性。
本质区别：是否能识别并维护连接状态，而非仅基于单个数据包的静态字段进行过滤。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：顺序意义：ESTABLISHED,RELATED 匹配已建立连接的响应包和关联流量，这类流量占比高、匹配效率高，放在第一条可以快速放行合法流量，减少后续规则匹配开销；NEW 匹配新连接请求，放在第二条可以先处理响应包，避免不必要的状态检查。
能否调换：不能调换。如果把 NEW 放在第一条，ESTABLISHED,RELATED 放在第二条，那么 NEW 规则无法匹配已建立连接的响应包，这些包会被默认策略 DROP 拒绝，导致连接无法正常通信。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被识别为 NEW 状态，因为 conntrack 表中已经没有对应的连接记录，防火墙无法识别这些包属于之前的连接。
防火墙会将其视为新连接请求，匹配 NEW 状态的规则。如果 NEW 规则未放行，这些包会被默认策略 DROP 拒绝，导致连接中断；如果 NEW 规则放行，会被当作新连接处理，但由于三次握手不完整，通信也会失败。


4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已建立连接相关的、独立的辅助连接流量，这些流量不是原连接的响应包，而是为了支持原连接而发起的新连接。
常见场景：FTP 协议的数据连接（控制连接已建立，数据连接由服务器主动发起，属于 RELATED 状态）、ICMP 错误报文（如目标不可达、超时等，属于原连接的关联流量）。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答："默认拒绝，按需放行" 的核心是 “最小权限原则”，所有流量默认被拒绝，仅放行明确允许的流量，攻击面最小，即使存在配置疏漏，也不会暴露额外端口或服务，安全性更高。
"默认允许，事后补堵" 的核心是 “先开放后限制”，所有流量默认被放行，再通过规则禁止部分流量，攻击面大，容易因规则遗漏或配置错误导致安全漏洞，且事后补堵往往无法覆盖所有攻击场景。
本质优势：通过缩小攻击面、遵循最小权限原则，从根本上降低了被攻击的风险，安全策略更严谨、更可控。
