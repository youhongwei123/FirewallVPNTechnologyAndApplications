...
| `filter` | `FORWARD` | 1 | ACCEPT | | |
| `filter` | `FORWARD` | 2 | ACCEPT | | |
...
| `wan → 203.0.113.1:80` | | 没有 80 端口的 DNAT 规则 |
| `wan → 10.0.3.2:8080` | | wan 没有到内网的路由，也没有默认路由 |
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` | | |
| `wan → 203.0.113.1:2222` | | |
| `wan → 203.0.113.1:3306` | | |
...
| 日志中 `OUT=` 字段的值 | |
| 日志中 `DST=` 字段是哪个地址 | |
| `DNAT-TO-DMZ:8080` 日志条数 | |
| `DNAT-TO-DMZ:2222` 日志条数 | |
...
| `wan` 地址 | |
| `fw` 四个接口地址 | |
| `wan → 203.0.113.1` ping 结果 | |
| `wan → 10.0.3.2` ping 结果 | |
| `office → 10.0.3.2` ping 结果 | |
| `office → 203.0.113.1` ping 结果 | |
