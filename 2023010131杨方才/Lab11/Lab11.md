...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            | client 端：10.20.0.0/24；vpn 端：10.10.10.1/32         |
| 是否能访问 `10.20.0.2:8080`  | 是         |
| `ip route` 中 wg0 相关的条目 |      10.20.0.0/24 dev wg0 scope link；10.10.10.0/24 dev wg0 proto kernel ...    |
...
| 失败的直接原因                   |  client 路由表中无 10.20.0.0/24 指向 wg0 的路由，包无法进入隧道        |
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |                 	client 的 underlay 地址（192.0.2.2）                   |     client 的 wg0 地址（10.10.10.1）     |
| 看到的目的地址           |           vpn 的 underlay 地址（192.0.2.1）                         |       server 内网地址（10.20.0.2）   |
| 协议                     |   UDP（51820 端口）                                 |    ICMP（或 HTTP）      |
| 是否能看到 ICMP/HTTP 内容 |    否（仅十六进制加密内容）                                |     是     |
| 为什么内外两层看到的不同 |                                    |          |
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：出口路由：wg-quick 自动把网段写入系统路由表，目标地址匹配该网段的数据包从 wg0 发往隧道对端；
入口白名单：收到对端解密后的报文，校验报文源 IP，不在 AllowedIPs 则直接丢弃。
原因：wg-quick 自动基于 AllowedIPs 生成系统路由，所以配置最先改变路由表，看似访问控制实则优先管控路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手依靠外层 underlay UDP（Endpoint 公网 IP+51820），只要底层网络互通、密钥合法就能完成握手；
业务数据依靠AllowedIPs 生成的路由 + 入站白名单转发内层 IP 报文。两套寻址体系相互独立，因此隧道握手在线，但路由 / 白名单错误就会访问失败。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：客户端所有流量全部强制走 VPN 隧道，上网、内网访问全部经过 VPN 网关；
VPN 网关收到客户端任意源 IP 的解密报文都会放行，失去源 IP 准入过滤，被入侵后可利用隧道转发全量流量，造成内网泄露、跳板攻击。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。
WireGuard 校验依靠对端公钥，VPN 配置里 peer 段写入的是合法客户端公钥；攻击者仅有 VPN 私钥，没有客户端私钥无法签名报文，VPN 网关使用客户端公钥校验数据包签名会失败，无法冒充客户端接入。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层抓包源地址：192.0.2.2（client 底层物理地址）。
原因：原始内层 IP 报文被整体加密封装为 UDP 报文，内层 IP、应用数据全部密文存储，底层链路只能读取外层 UDP 头部，无法解密查看内层真实信息。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：隐藏真实通信地址：公网链路看不到隧道内网 IP，规避 IP 溯源；
全报文加密：内层业务数据全程密文传输，中间人抓包无法窃取 HTTP/ICMP 等明文业务内容。
