...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |curl报错Could not connect to server，连接超时，外网无法直接访问内网/DMZ服务 |
| `office → 10.0.3.2:8080` |成功 |正常返回HTTP目录列表，内网主机可正常访问DMZ服务器的8080端口 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |所有流量，状态为 RELATED,ESTABLISHED |允许已建立/相关的连接通过 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080，状态为 NEW |允许外网访问 203.0.113.1:8080转发到 10.0.3.2:8080 的新连接 |
...
| :--- | :--- | :--- |
| 请求包的源地址 |203.0.113.2（客户端公网地址） |203.0.113.2（源地址不变） |
| 请求包的目的地址 |203.0.113.1（防火墙外网 IP） |10.0.3.2（DMZ 服务器真实 IP） |
| 回复包的源地址 |203.0.113.1（防火墙外网 IP） |10.0.3.2（DMZ 服务器真实 IP） |
| 回复包的目的地址 |203.0.113.2（客户端公网地址） |203.0.113.2（客户端公网地址） |
...
| 连接状态 |TIME_WAIT/[ASSURED]|

**简答题：**

1. `wan` 的 curl 收到的 HTTP 响应，源 IP 是什么？（提示：`wan` 无感知 DNAT 存在）

> 答：源IP是203.0.113.1（防火墙的外网 IP），因为DNAT反向转换后，服务器真实IP 10.0.3.2 被隐藏，外网只看到防火墙的公网IP。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：后续包依然能正常转发。因为连接跟踪（conntrack）表中已存在该会话的双向映射，防火墙会根据会话记录继续进行地址转换，无需重新匹配 DNAT 规则，直到会话超时被删除。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |成功返回HTTP 目录列表 |有DNAT规则 |
| `wan → 203.0.113.1:2222` |成功返回HTTP 目录列表 |有DNAT规则 |
| `wan → 203.0.113.1:3306` |curl: (7) Failed to connect（连接失败） |无DNAT规则 |
...
| 日志中 `DST=` 字段是哪个地址 |DNAT 转换后的内网地址 10.0.3.2 |
...
> 答：说明LOG规则在DNAT转换之后 的阶段触发，此时目标地址已被改写为内网地址。要记录原始目标地址（改写前），LOG规则应放在nat表的 PREROUTING 链（DNAT 发生之前）

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：能发现。端口扫描行为会表现为：来自同一源 IP 的大量日志，DPT（目标端口）字段快速、连续地遍历多个不同端口号，且多为 TCP SYN 包。
...
| `wan → 203.0.113.1` ping 结果 |通（0% 丢包，有响应） |
| `wan → 10.0.3.2` ping 结果 |不通（网络不可达） |
| `office → 10.0.3.2` ping 结果 |通（0% 丢包，有响应） |
| `office → 203.0.113.1` ping 结果 |通（0% 丢包，有响应） |
...
| `wan → 203.0.113.1:3306` |失败，无法连接（curl: (7) Failed to connect） |
| `wan → 10.0.3.2:8080` |失败，无法连接（网络不可达） |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT：发生在nat表的PREROUTING链，路由决策前。SNAT：发生在 nat 表的 POSTROUTING 链，路由决策后、数据包出站前。角色分工：PREROUTING 负责处理入站连接，把公网 IP + 端口映射到内网服务器；POSTROUTING 负责处理出站连接，把内网地址伪装成防火墙公网 IP，让回复包能回到防火墙。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：谁先抢到：PREROUTING 链的 DNAT 规则先处理，请求会被转发到内网，防火墙本地服务收不到包。共存方案：方法 1：修改本地服务监听地址，改为 127.0.0.1:8080，避免监听公网 IP。方法2：使用 --to-destination 时保留端口，或用 REDIRECT 把公网端口重定向到本地不同端口（如 8081）。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：防火墙的 conntrack 会记录完整的五元组映射关系：外部请求：公网IP:8080 → 内网A:8080、公网IP:8081 → 内网B:8080。回复包到达时，防火墙会根据 conntrack 表中的连接记录，将内网服务器的源端口还原为对应的公网端口，再转发给客户端。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：结果：能回到 WAN。机制：回复包从 dmz 发出后，目标地址是客户端的公网 IP，防火墙的路由表知道如何转发到 wan 侧；同时 conntrack 会识别这是 DNAT 连接的回复包，自动将目标地址还原为客户端地址，直接转发，无需额外 SNAT 规则。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：影响：无法新建连接，防火墙拒绝处理新请求，表现为服务不可用。后果：正常用户无法访问，网络瘫痪。防护措施：1. 调大 net.netfilter.nf_conntrack_max 表容量。2. 缩短超时时间（如 nf_conntrack_tcp_timeout_time_wait）。3. 限制单 IP 并发连接数，防止攻击占满表项。4. 使用无状态规则处理非关键流量，减少表项占用。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础：conntrack 连接跟踪，所有功能都依赖它识别连接状态、匹配 NAT 映射和过滤规则。 安保类比： Lab7（有状态防火墙）：大楼门禁，只允许合法出入。Lab8（NAT）：访客登记处，统一用前台身份对外交流。Lab9（分区审计）：楼层安检，不同区域单独管控和记录。Lab10（端口映射）：快递收发点，外部包裹按指定房间号转递。
