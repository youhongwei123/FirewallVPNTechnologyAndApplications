...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |放行所有 ESTABLISHED、RELATED 回程连接放行10.1.0.0/24访问10.2.0.2 TCP 8080 新建流量 |A-TO-B-DENY: |DROP |
| `gwB` |放行所有 ESTABLISHED、RELATED 回程连接放行隧道进来10.1.0.0/24访问10.2.0.2 TCP 8080 新建流量 |VPN-S2S-DENY: |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功 |无拦截日志（流量匹配放行规则，不会触发 LOG） |
| `siteA -> siteB:2222` | 失败 |失败 |存在A-TO-B-DENY前缀内核日志，记录源 10.1.0.2、目的 10.2.0.2、TCP DPT=2222 拦截流量 |
| `siteA -> siteB ping` | 失败 |失败 |存在A-TO-B-DENY前缀内核日志，记录 ICMP 类型拦截流量（本次截图仅展示 2222 的日志，执行 ping 后会新增对应 ICMP 拦截日志） |
...
| underlay 是否连通 |连通（0% 丢包，双向 ping 通） |
...
| `latest handshake` |12 seconds ago |12 seconds ago |
| `transfer` 计数 |接收 180 B，发送 272 B |接收 124 B，发送 180 B |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |gwA/gwB 放行 TCP 8080 新建连接规则 + ESTABLISHED/RELATED 回程放行规则 |
| `siteA -> siteB:2222` |失败 |gwA LOG 记录规则 + REJECT 拒绝规则（第一道防线拦截） |
| `siteA -> siteB ping` |失败 |gwA LOG 记录规则 + REJECT 拒绝规则（ICMP 流量被拦截） |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 ↔ 192.0.2.2 |UDP 51820 |WireGuard 外层加密封装流量，underlay 公网地址互访，原始业务数据包被加密封装在 UDP 51820 载荷内 |
| `wg0` |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |解密后的原始内网业务流量，能直接看到两端站点内网 IP 与 HTTP 服务端口，无加密封装 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |经隧道解密 + gwB 防火墙放行后，转发至 B 站点内网的原生业务 TCP 流量，到达目标服务主机 |
...
答:远程接入：单用户终端接入总部内网，一端是个人设备、一端是网关；站点到站点：两个完整内网互通，两端均为站点网关，全站点主机互访。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答:WireGuard 根据 AllowedIPs 生成路由，匹配目标网段流量才走隧道封装；仅填对端内网，避免普通公网流量误走隧道。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答:网关需要转发跨三层流量：本地内网 ↔ WireGuard 隧道，关闭转发则跨网段数据包丢弃。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答:主机访问对端内网的数据包，需发给本地网关，由网关通过隧道转发至对端站点。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答:核对两端 AllowedIPs 网段配置；
查看网关路由表，确认对端网段路由指向 wg0；
检查 iptables FORWARD 转发放行规则；
抓包 wg0、内网接口验证数据包流转；
核对主机默认网关配置。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答:好处：纵深防御，单侧配置出错也能拦截非法流量，双向访问均可审计；
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答:隐藏：两端内网 IP、业务端口、传输明文、内网拓扑；
暴露：两端公网 underlay 地址、WireGuard 固定 UDP 51820 端口、外层 UDP 封装流量。

8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答:gwA 防火墙：放行 wg0 流入、去往 siteA 内网对应端口的新建 TCP 流量；
gwB 防火墙：放行 siteB 内网发往隧道、访问 siteA 网段对应端口的新建流量；
双向保留 ESTABLISHED/RELATED 回程放行规则。
