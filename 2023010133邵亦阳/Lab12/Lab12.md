...
| client 路由表中的 `wg0` 路由 | 10.20.0.0/24、10.10.10.2/32 网段均通过 wg0 接口转发|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 | 	ACCEPT| 所有协议，任意入 / 出接口，任意源目地址；连接状态为 RELATED、ESTABLISHED|放行已有连接的回应数据包，允许内网服务器主动回复客户端流量 |
| 2 |ACCEPT |TCP 协议，入接口 wg0，出接口 veth-vpn-lan；源地址 10.10.10.1，目的 10.20.0.2，目的端口 8080，新建连接 NEW | 仅放行 VPN 客户端 (10.10.10.1) 访问服务器 10.20.0.2 的 8080 端口 TCP 流量|
| 3 |LOG | 所有协议，入接口 wg0，任意出接口，任意源目；限速 5 包 / 分钟，日志级别 4，前缀VPN-DENY:| 对所有从 wg0 进入、未匹配前面放行规则的数据包做日志记录，方便排查被拦截流量|
| 4 |	REJECT |所有协议，入接口 wg0，任意出接口，任意源目；拒绝时返回 icmp-port-unreachable | 拦截所有从 wg0 进入、未匹配放行规则的流量，返回端口不可达 ICMP 报错|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 2 条 ACCEPT 规则 | 是|
| `client -> server:2222` |失败 | 第 3 条 LOG、第 4 条 REJECT|是 |
| `client -> server ping` | 失败|第 3 条 LOG、第 4 条 REJECT | 是|
...
| `OUT=` |任意接口（*） |
| `SRC=` |任意源地址（0.0.0.0/0） |
| `DST=` |任意目的地址（0.0.0.0/0） |
| `PROTO=` |all（所有协议，TCP/ICMP 等） |
| `DPT=` | 无固定匹配端口，所有目的端口|
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |无放行规则，INPUT 链默认拦截，数据包被丢弃 |匹配VPN-MGMT-DENY LOG 规则记录日志，随后拒绝 |是 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` | 数据包源地址10.10.10.1、目的地址10.20.0.2，源与目标分属不同网段，需要 vpn 命名空间主机转发，因此经过 FORWARD 转发链|
| `client -> server:2222` | `FORWARD` | 数据包源地址10.10.10.1、目的地址10.20.0.2，跨网段流量需要 vpn 主机转发，走 FORWARD 转发链，无放行规则最终被 REJECT 丢弃|
| `client -> vpn:9090` | `INPUT` |数据包目的 IP 为 vpn 本机10.10.10.2，流量是发给 vpn 主机自身进程，不需要转发，进入 INPUT 本机入站链处理 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有回程响应流量全部被 FORWARD 默认 DROP 阻断，已建立连接无法收到回复，所有访问全部失败 |第 1 条 ACCEPT 规则负责放行已有连接的回程报文，删除后无规则允许应答包转发 |恢复 FORWARD 链第一条规则：-A FORWARD -m ctstate --ctstate RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |匹配 REJECT 的流量不会执行 LOG，无对应拦截日志输出 | iptables 规则从上到下匹配，命中 REJECT 后直接终止流程，不会继续执行后方 LOG 规则| 将 LOG 规则移动到对应 REJECT 规则前面，先记录日志再丢弃流量|
| 管理面规则错写到 `FORWARD` |访问 vpn 本机 9090 端口的流量匹配 FORWARD 链规则，无日志输出（journalctl -k --grep "WRONG-CHAIN"无条目），访问直接失败 |目标 IP10.10.10.2是 vpn 本机地址，本机入站流量走INPUT链，不会经过FORWARD转发链，FORWARD 里的 9090 规则完全不会被匹配 |1. 删除 FORWARD 链中 tcp 9090 的 LOG/REJECT 规则；
2. 在INPUT链添加针对wg0入站、tcp 9090 的 LOG+REJECT 规则 |
...
| 项目 | 你的填写 |
| :--- | :--- |
| client underlay 地址 |192.0.2.1 |
| vpn underlay 地址 |192.0.2.2 |
| server 地址 |10.20.0.2 |
| client wg0 地址 |10.10.10.1/32 |
| vpn wg0 地址 |10.10.10.2/32 |
| 是否看到 WireGuard 握手 |是 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |匹配 FORWARD 第 2 条 ACCEPT 规则，允许10.10.10.1访问10.20.0.2:8080新建 TCP 流量 |
| `client -> server:2222` | 失败|不匹配放行规则，先 LOG 再 REJECT，返回 ICMP 端口不可达 |
| `client -> server ping` | 失败| ICMP 不匹配 TCP 放行规则，被 LOG+REJECT 拦截，100% 丢包|
...
| 外层 UDP 目的地址 | 192.0.2.1|
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |wg show；conntrack 仅跟踪四层流量连接，无法反映 WG 加密握手、保活、密钥状态 |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` | 所有已建立连接无回程应答，访问全部超时失败|该规则放行服务器回程响应包，删除后响应流量匹配默认 DROP 策略 | 恢复 FORWARD 首条规则：-A FORWARD -m ctstate --ctstate RELATED,ESTABLISHED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 | 被拦截流量无日志输出|iptables 从上至下匹配，命中 REJECT 直接终止处理，不会执行后置 LOG |将 LOG 规则写在对应 REJECT 规则前方，先记录日志再丢弃 |
| 管理面规则错写到 `FORWARD` |访问网关 9090 端口完全不匹配规则，无WRONG-CHAIN日志，直接被默认策略拦截 |访问本机的流量走 INPUT 链，不会经过 FORWARD 转发链，FORWARD 中 9090 规则永远不会触发 |1. 删除 FORWARD 链 9090 相关 LOG/REJECT 规则；2. 在 INPUT 链添加 wg0 入站、TCP 9090 的 LOG+REJECT 规则 |
...
AllowedIPs：路由层控制，决定哪些网段流量会被封装进 WireGuard 隧道，只控制 “能不能走隧道”；
iptables：四层访问控制，在网关转发 / 入站时精细限制 IP、端口、协议、连接状态；
仅靠 AllowedIPs 的缺陷：粒度只能到网段，无法限制单个端口 / 协议；无法区分访问内网服务器、访问网关本机；无日志审计；无法放行回程响应流量。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
客户端流量目标是内网服务器10.20.0.2，不是 VPN 网关本机，流量需要网关跨网卡转发，属于转发流量，由 FORWARD 链处理；INPUT 链仅处理目标 IP 为网关本机的入站流量。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端能发出 TCP 握手 SYN 包并匹配 ACCEPT 放行，服务器回复的 SYN-ACK 回程包无匹配规则，被 FORWARD 默认 DROP；客户端收不到响应，连接超时失败。

4. 为什么 LOG 必须放在 REJECT 前面？
ptables 规则顺序从上到下匹配，一旦匹配 REJECT，数据包会直接被丢弃，后续规则不再执行；LOG 前置才能在流量被拦截前完成日志记录，用于审计排查。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
安全意义：内层业务流量全程加密，底层抓包无法获取源 / 目的内网 IP、端口、传输内容，具备流量加密隐私保护；
可暴露信息：两端公网 IP、WireGuard 固定 UDP 端口、数据包长度 / 发包频率（流量特征可被流量分析识别 VPN 行为）。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
横向渗透风险：VPN 用户一旦设备中毒，可遍历内网所有服务器，入侵数据库、业务系统；
数据泄露：可随意访问文件共享、内部运维管理平台；
无最小权限管控：无法隔离开发、生产、办公网段，单点失守全网暴露。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> vpn:9090：INPUT 链，目标 IP 是 VPN 网关本机，属于本机入站流量；
client -> server:8080：FORWARD 链，目标是内网其他主机，需要网关转发跨网卡流量。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
NAT 是修改 IP / 端口做地址转换；WireGuard 是加密封装隧道，不会修改内层 IP 地址；
conntrack 仅跟踪四层连接状态，WireGuard 有独立的密钥握手、保活、加密会话，生命周期和四层连接完全独立；
NAT 不加密流量，WireGuard 全程加密内层载荷。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
允许来源：仅企业固定运维公网 IP，禁止 VPN 客户端、公网任意地址访问；
日志写法：INPUT 链最前端添加 LOG 规则，前缀区分SSH-ADMIN-ACCESS，限制日志速率，放行合法 IP，拒绝其余所有 IP 并记录拒绝日志。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
路由表：确认客户端 / 网关存在目标内网网段路由；
防火墙规则与计数器：查看 iptables pkts 计数，确认流量是否匹配放行 / 拦截规则；
内核拦截日志（journalctl/dmesg）：查看 VPN-DENY 类拒绝日志定位拦截原因；
内网服务监听状态：确认目标服务器端口正常监听、服务正常运行；
抓包辅助验证：wg0 / 内网网卡抓包，确认流量是否转发到内网。
