...
| `ip route` 中 wg0 相关的条目 |     1. 10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1
...
| 去 `10.20.0.0/24` 的路由是否消失 |      已消失，路由表无10.20.0.0/24 dev wg0条目    |
| 是否还能访问 `10.20.0.2:8080`    |     不能，curl 连接超时失败     |
| 失败的直接原因                   |      1. Peer 的 AllowedIP 仅允许去往10.30.0.0/24；2. 内核路由删除了10.20.0.0/24隧道路由，访问 10.20 网段没有匹配路由，无法通过 WireGuard 隧道转发报文    |

>简答：为什么 wg show 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？

WireGuard 握手（底层 UDP 连通）和业务 IP 转发由AllowedIPs + 系统三层路由两套规则管控：
latest handshake 是两端 ** 底层 UDP 报文（underlay 192.0.2.x: 端口）** 互通，只负责密钥协商保活，不受 AllowedIPs 约束；
业务10.20.0.0/24访问依赖两点：①路由存在指向 wg0 ②AllowedIPs 包含目标网段；现在 AllowedIP 改为10.30.0.0/24、且删除了10.20.0.0/24隧道路由，业务 IP 数据包无法匹配 WG 转发规则，不能封装进隧道，所以业务不通，但 UDP 握手通道完好。
路径区分：两套逻辑路径
握手保活报文（UDP）：同一套底层 UDP 链路（underlay），始终走两端192.0.2.x的 UDP 端口，不受 AllowedIP 影响；
业务 IP 流量：两套管控：握手的 UDP 通道物理链路不变，但业务 IPv4 数据包能否进隧道由 AllowedIPs + 内核路由控制，AllowedIP 是 WireGuard 内部加密路由，和底层 UDP 握手解耦。
截图：
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |   192.0.2.2（client 底层 IP）               |    10.10.10.1      |
| 看到的目的地址           |  	192.0.2.1（vpn 底层IP）                                  |      10.20.0.2    |
| 协议                     |                 UDP（WireGuard 封装报文，底层传输协议）                   |     ICMP     |
| 是否能看到 ICMP/HTTP 内容 |                  不能                   |    可以，明文 ICMP 原始报文      |
| 为什么内外两层看到的不同 |                underlay 是外层封装 IP+UDP 包头，业务报文被加密封装在 UDP 载荷；wg0 是内层解封后的原始明文 IP 报文，WireGuard 做了 IP 包加密封装 / 解封装                              |
> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？

outer (underlay 外层)：对应信封
信封上写收发寄件地址（192.0.2.2 <->192.0.2.1），信封内部信纸被密封加密，外人拆开信封也读不到信纸内容。
inner (wg0 内层)：对应信纸
信纸是原始业务报文（ICMP/HTTP，10.10.10.1<->10.20.0.2），只有拆封解密后才能读取。
安全意义
传输链路抓包只能获取外层 UDP 和底层 IP，原始业务 IP、端口、通信内容全被加密，无法窃听、篡改业务数据；
隐藏内网拓扑（内网网段、终端 IP 不会暴露在公网链路），提升传输保密性。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：① 加密路由（转发规则）：决定本机哪些 IP 流量会被加密封装进 WireGuard 隧道发给对应 peer；② 入站访问控制 ACL：对端发来的解密报文，只有源 IP 匹配 AllowedIPs 才允许进入本机协议栈，丢弃不匹配报文。原因：内核收到 IP 报文时，优先依靠 AllowedIPs 自动生成内核隧道路由，系统路由直接决定报文从 wg0 转发，因此配置 AllowedIPs 最先改变路由表，表现为先影响路由、再实现访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手：是两端底层UDP 保活报文（underlay 192.0.2.x 端口），仅做密钥协商、保活连通，不受 AllowedIPs、内网隧道路由限制，只要底层 UDP 通就能持续握手；
业务传输：依赖内核三层路由 + AllowedIPs 加密转发规则，目标网段无隧道路由、AllowedIP 不包含目标 IP 时，业务 IP 包无法封装进隧道；
二者传输逻辑、校验规则分离，因此握手在线但业务不通。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0代表本机所有 IPv4 流量全部默认从 WireGuard 隧道转发：
本地上网、访问公网全部走 VPN 隧道，消耗 vpn 服务器带宽；
一旦 vpn 节点被攻陷，client 全部流量被劫持、窃听；
内网异常流量、恶意流量意外通过隧道穿透到对端内网，扩大攻击面。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 是双向认证：握手建立隧道需要两端公私钥互相验证，vpn 私钥仅能证明 vpn 自身身份；缺少 client 私钥无法签名伪装 client 身份，对端 vpn 校验客户端签名失败，拒绝握手建连。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2（client 底层 underlay 地址）；内层业务报文被 WireGuard 高强度对称加密，底层链路抓包只能捕获外层 UDP 封装头，载荷是密文，没有解密密钥无法还原 ICMP/HTTP 明文内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：数据保密性：内层业务报文全加密，链路窃听无法窃取通信内容；
内网拓扑隐藏：底层链路只暴露外层 underlay 公网 IP，内网10.x.x.x网段、主机地址被封装隐藏，防止内网网段与设备信息泄露。
