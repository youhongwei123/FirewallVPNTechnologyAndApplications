...
| 项目 | 你的填写 |
| :--- | :--- |
| client `wg0` 地址 |10.10.10.2/24 |
| vpn `wg0` 地址 |10.10.10.1/24 |
| client 端 `AllowedIPs` |10.10.10.0/24,10.20.0.2/32 |
| client 路由表中的 `wg0` 路由 |去往 10.10.10.0/24、10.20.0.2/32 的路由出口为 wg0 接口 |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |	all，ctstate RELATED,ESTABLISHED| 放行已建立、关联连接回程流量|
| 2 |ACCEPT |tcp，in wg0，out veth-vpn-lan，源 10.10.10.2，目的 10.20.0.2，dport http-alt，ctstate NEW |仅放行 VPN 客户端新建访问 8080 端口流量 |
| 3 | LOG|all，in wg0，limit avg 5/min burst 10，LOG level warn，前缀 "VPN-DENY:" | 限流记录被拒绝的 VPN 访问日志|
| 4 |REJECT |all，in wg0，reject-with icmp-port-unreachable |拒绝其余所有 VPN 入站转发流量 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` | 成功| FORWARD 第 1、2 条 ACCEPT 规则|是 |
| `client -> server:2222` |失败|FORWARD 第 3 条 LOG、第 4 条 REJECT 规则 |是 |
| `client -> server ping` |失败 |	FORWARD 第 3 条 LOG、第 4 条 REJECT 规则 | 是|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |目标是后端另一台主机，流量需要 VPN 网关转发，走转发链 FORWARD |
| `client -> server:2222` | `FORWARD` | 目标是后端另一台主机，流量需要 VPN 网关转发，走转发链 FORWARD|
| `client -> vpn:9090` | `INPUT` |目标是 VPN 网关本机 IP 与本机服务，无需转发，进入本机入站 INPUT 链 |
...
| 观察项 | 你的填写 |
| :--- | :--- |
| 内层 TCP 连接源地址 |10.10.10.2 |
| 内层 TCP 连接目的地址 |	10.20.0.2 |
| 内层 TCP 目标端口 |8080 |
| 外层 UDP 源地址 |192.0.2.1 |
| 外层 UDP 目的地址 | 192.0.2.2|
| 外层 UDP 目标端口 | 51820|
...
| `wg0` | 10.10.10.2 ↔ 10.20.0.2| TCP/ICMP | 解密后的内层包 |
| `veth-vpn-lan` | 10.10.10.2 → 10.20.0.2| TCP 8080 | 被允许转发的内网包 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |ATED	访问 8080 端口超时失败，请求能发出但收不到响应 | 后端回程应答包无放行规则，被 FORWARD 默认策略 DROP 丢弃|在 FORWARD 链首行重新插入 ESTABLISHED,RELATED 放行 ACCEPT 规则 |
| `LOG` 放在 `REJECT` 后面 | 目标端口被拒绝，但无法抓取对应拦截日志|iptables 从上至下顺序匹配，命中 REJECT 后处理流程终止，后置 LOG 不会执行 |将 LOG 规则调整到 REJECT 规则前面 |
| 管理面规则错写到 `FORWARD` |仍可正常访问 VPN 网关本机管理端口，规则计数器无增长、无拦截日志 |访问网关本机流量走 INPUT 链，不会经过 FORWARD 转发链，规则完全不生效 |删除 FORWARD 链内错误规则，把 LOG+REJECT 拦截规则配置到 INPUT 链 |
...
| server 地址 | 10.20.0.2|
| client wg0 地址 |10.10.10.2 |
| vpn wg0 地址 | 10.10.10.1|
| 是否看到 WireGuard 握手 |是 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |iptables 放行该端口新建 TCP 流量，回程 ESTABLISHED 流量允许 |
| `client -> server:2222` |失败 | FORWARD 链默认 DROP，无放行规则，同时触发日志记录|
| `client -> server ping` | 失败| ICMP 无放行规则，被 FORWARD 链拒绝|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |访问 8080 端口超时失败，请求能发出但收不到响应| 后端回程应答包无放行规则，被 FORWARD 默认策略 DROP 丢弃| 在 FORWARD 链首行重新插入 ESTABLISHED,RELATED 放行 ACCEPT 规则|
| `LOG` 放在 `REJECT` 后面 | 目标端口被拒绝，但无法抓取对应拦截日志|iptables 从上至下顺序匹配，命中 REJECT 后处理流程终止，后置 LOG 不会执行 | 将 LOG 规则调整到 REJECT 规则前面|
| 管理面规则错写到 `FORWARD` |仍可正常访问 VPN 网关本机管理端口，规则计数器无增长、无拦截日志 |访问网关本机流量走 INPUT 链，不会经过 FORWARD 转发链，规则完全不生效 |删除 FORWARD 链内错误规则，把 LOG+REJECT 拦截规则配置到 INPUT 链 |
...
>答：（1）AllowedIPs 是 WireGuard 隧道层面的路由控制配置，仅用来指定哪些流量会被封装进 VPN 隧道转发、哪些流量走本地路由，只能实现网段级别的路由放行；iptables 是四层防火墙访问控制工具，能够在隧道解密后针对内网 IP、端口、传输协议、连接状态做精细化放行、拦截，还能生成访问审计日志。（2）只依靠 AllowedIPs 无法做细粒度权限管控，只要网段被放行，VPN 客户端就能访问该网段内所有 IP 与全部端口，无法限制仅开放指定业务端口，也不能拦截扫描、暴力访问行为，同时缺少日志审计能力，安全边界无法收紧，因此不能只靠它做权限控制。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
>答：VPN 用户访问内网服务器时，流量是从 VPN 客户端经由 VPN 网关转发至后端内网服务器，流量只是途经网关、最终目的地并非 VPN 网关本机进程。INPUT 链仅处理目标为本机 IP、交付本机进程的入站流量，FORWARD 链专门处理需要本机路由转发的过境流量，因此转发控制规则必须配置在 FORWARD 链，不能写在 INPUT 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
>答：删除 ESTABLISHED,RELATED 规则后，客户端发起访问 8080 端口的新建 TCP 请求包能够命中放行规则，正常发送到后端服务器；但服务器回复的应答数据包属于 ESTABLISHED 已建立连接状态，不再匹配 NEW 状态的端口放行规则，FORWARD 链默认策略为 DROP，回程响应包会被丢弃。最终客户端 curl 请求长时间超时，无法获取网页返回内容，业务访问失败。

4. 为什么 LOG 必须放在 REJECT 前面？
>答：iptables 规则是自上而下依次匹配，一旦命中某条动作规则就会终止整条规则链的后续遍历。如果 REJECT 规则写在 LOG 前面，数据包命中 REJECT 后直接被丢弃，后续 LOG 规则不会执行，无法生成拦截审计日志；将 LOG 放置在 REJECT 前方，会先完成日志记录，再执行拒绝动作，完整留存拦截行为日志，因此 LOG 必须放在 REJECT 前面。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
>答：（1）underlay 抓包仅能看到 UDP 51820 加密隧道数据包，内网真实 IP、业务端口、应用层明文数据全部被封装加密，链路中间节点无法窃听、篡改业务数据，极大提升了传输链路安全性。（2）该抓包依旧可以暴露 VPN 两端的公网 IP 地址、固定 WireGuard 监听端口 51820，还能根据数据包长度、收发时长、流量总量识别出 VPN 通信流量特征，存在流量特征被识别、VPN 服务端口被扫描探测的风险。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
>答：允许 VPN 用户无差别访问整个 10.0.0.0/8 内网网段，权限过度放开，一旦 VPN 账号泄露，攻击者可以对内网所有主机进行端口扫描、漏洞探测，横向渗透数据库、运维管理核心节点；无法按照不同岗位、外包、在职员工做权限拆分，不能精细化回收离职人员访问权限；缺少端口隔离，单台内网主机被攻陷后攻击会快速扩散至全网；同时无法针对不同业务做访问审计，安全事件溯源难度大幅提升。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
>答：client -> vpn:9090 经过 INPUT 链，client -> server:8080 经过 FORWARD 链。
前者访问目标是 VPN 网关本机监听的端口，数据包需要交付网关本机运行的进程处理，属于本机入站流量，走 INPUT 链；后者访问目标是后端独立内网服务器，VPN 网关仅做路由转发，属于过境转发流量，走 FORWARD 链，两条流量目的地属性不同，经过的 iptables 链不一样。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
>答：NAT 机制会修改数据包的源 IP、目的 IP 或端口实现地址转换，而 WireGuard 隧道只是把完整的内层 IP 数据包整体封装在外层 UDP 报文中传输，内层 IP 报文不会做任何地址改写；conntrack 同时独立跟踪外层 UDP 隧道连接、内层真实 TCP 业务连接两条会话，WireGuard 只是封装传输载体，不存在地址转换逻辑，因此不能将 WireGuard 隧道理解成 conntrack 的 NAT 映射。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
>答：（1）SSH 管理端口仅允许运维固定办公公网 IP、内网专属运维管理网段接入，禁止 VPN 隧道网段、全网公网任意地址访问 SSH 端口。（2）日志配置方式：在 INPUT 链 SSH 放行规则之前添加限流 LOG 规则，自定义专属日志前缀，记录访问源 IP、目标端口、通信协议；先完成日志记录，再放行可信来源 SSH 流量，其余所有来源 SSH 访问全部 REJECT 拒绝，每一次接入、拦截行为都会留存完整审计日志。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
>答：排查顺序：第一步检查后端业务服务是否正常启动、监听地址是否允许隧道网段接入；第二步查看两端主机路由表，确认回程路由是否正确指向 VPN 网关 wg0 接口；第三步查看 iptables 规则计数器，确认流量是否命中放行或拒绝规则；第四步核对防火墙规则完整性、规则先后顺序是否正确；第五步查看拦截审计日志，定位流量被丢弃的具体原因；最后可在 wg0、内网接口分别抓包，精准定位流量中断节点。
