...
| :--- | :--- |
| 只有第一条规则时是否成功 |否|
| 第一条规则放行的是什么方向的包 |内网lan → 外网wan，目的端口 38070的 TCP 请求包|
| 失败的原因 |仅放行客户端发出的请求包，wan服务端返回的响应包被防火墙默认DROP策略拦截，无法双向通信|
| 第二条规则匹配的是什么 |外网wan → 内网lan，源端口 38070的 TCP 响应包|
| 加第二条规则后是否成功 |是|
| 当前规则共几条 | 2条|
...
答：无状态防火墙仅基于端口放行流量，无法区分数据包是响应包还是主动发起的连接包，存在安全漏洞
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：场景示例：内网主机访问外网网页，防火墙需放行外网返回的网页响应包；但拒绝外网主动发起对内网的连接攻击。
（1）无状态规则：只能开放外网服务器端口，会同时放行外网主动对内网的访问，无法区分方向；
（2）有状态规则：可跟踪 TCP 连接状态，仅放行内网主动连接的响应包，拦截外网主动发起的新建连接。
![无状态规则](stateless.png)
...
答：防火墙通过 “连接跟踪（conntrack）” 识别数据包为内网主动连接的响应包，匹配 ESTABLISHED,RELATED 规则自动放行。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：有状态规则仅放行内网主动向外发起的新建连接，拒绝外网主动向内发起的连接；无状态规则仅匹配端口，不区分连接方向与状态，wan 可从 38070 端口主动发起连接。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:51262|
...
答：防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的连接记录，而非单纯数据包头部字段。两者区别（1）数据包头部：仅包含源 IP、目的 IP、端口、TCP 标志位等，无法区分是主动发起的新连接，还是已有连接的响应包；（2）conntrack 连接跟踪表：记录了完整 TCP 连接的建立历史与状态，能识别该数据包是内网主动发起连接的后续响应包，以此精准放行合法回包、拦截外网主动发起的攻击连接。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态防火墙仅根据数据包头部端口、IP独立判断放行与否，不记录连接状态；有状态防火墙依靠连接跟踪（conntrack）记录整条 TCP 连接的状态，可区分新建连接与响应包，安全粒度更精细。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：（1）意义：已建立 / 关联连接的流量占比更高，优先匹配可提升处理效率；同时自动放行内网发起连接的返回响应包。（2）不能调换，若调换会导致所有数据包先匹配新建连接规则，响应包无法被放行，通信失败。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被识别为 NEW（新连接）；防火墙默认策略为 DROP，会直接丢弃该数据包，通信中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：（1）RELATED 是与已有连接相关联的附属连接 / 辅助流量。（2）场景：FTP 主动 / 被动模式下的数据通道、ICMP 差错报文等，会出现 RELATED 状态。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：默认拒绝模式最小化开放面，仅放行必要流量，未知漏洞、恶意流量默认被拦截；事后补堵模式存在大量开放端口，攻击面大，容易被入侵后再补救，安全风险更高。
