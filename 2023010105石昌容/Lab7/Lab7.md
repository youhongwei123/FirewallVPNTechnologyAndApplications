...
| :--- | :--- |
| 只有第一条规则时是否成功 |失败|
| 第一条规则放行的是什么方向的包 |仅放行目标端口为 38070 的入方向（从 WAN 到 LAN）TCP 包，也就是客户端发往服务器的请求包|
| 失败的原因 |只放行了请求包，但服务器的响应包（源端口 38070）被防火墙默认的 DROP 策略丢弃，TCP 三次握手 / 后续通信无法完成，导致连接超时|
| 第二条规则匹配的是什么 |匹配源端口为 38070 的 TCP 包，也就是服务器发回给客户端的响应包|
| 加第二条规则后是否成功 |成功|
| 当前规则共几条 |2 条|
...
答：基于端口的无状态防火墙规则无法区分 “主动连接的请求” 和 “已有连接的响应”，会导致不必要的安全暴露，无法阻止外部主动发起的非法连接。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：假设你在局域网内运行一个对外提供服务的 Web 服务器，端口为 80。
无状态规则：
为了允许客户端访问，你需要放行目标端口 80 的请求包，同时放行源端口 80 的响应包。但这会导致外部攻击者可以主动从任意端口发起连接到你的 80 端口，因为规则无法区分这是合法响应还是非法主动连接。
有状态规则：
可以只放行 ** 已建立 / 相关连接（ESTABLISHED/RELATED）** 的响应包，仅允许内网主动发起的连接的回应包通过，而拒绝所有外部主动发起的新连接请求，既保证了正常业务，又阻止了外部主动扫描和攻击。
简单来说：无状态规则只能看单个包的端口信息，而有状态规则能识别包属于哪个已建立的连接，从而精准放行合法回应，拒绝非法主动连接。
![无状态规则](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2 条|2 条|
| 是否需要手动考虑返回方向 |是，必须额外放行服务器响应的源端口包|否，无需单独配置返回方向的规则|
| 能否区分"新连接"和"响应包" |不能，仅基于端口匹配，无法区分包的状态|能，通过conntrack模块识别NEW/ESTABLISHED状态|
| `wan` 能否主动发起到 `lan` 的连接 |能，只要源端口是 38070 就能被无状态规则放行|不能，仅放行NEW状态的目标端口 38070 请求，拒绝wan主动发起的新连接|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
答：因为有状态防火墙的conntrack模块会记录连接状态。当lan主动向wan发起连接并建立会话后，服务器返回的响应包会被标记为ESTABLISHED/RELATED状态，而第一条规则-m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT会自动放行这些属于已有会话的响应包，无需单独配置返回方向的规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：有状态规则：仅放行NEW状态、目标端口为 38070 的请求包（仅允许lan主动发起的新连接），拒绝wan主动发起的所有NEW状态请求；且响应包仅允许已有会话的ESTABLISHED包通过，不会放行wan主动发起的新连接。
无状态规则的--sport 38070：仅匹配源端口为 38070 的包，无法区分包是响应包还是新连接请求。因此wan侧主机可以从 38070 端口主动发起连接到lan，无状态规则会直接放行该SYN包，无法阻止主动连接。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:39806|
...
答：防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的记录，而不是单纯看 IP/TCP 包头里的字段。
两者的核心区别：
包头字段：只包含 IP、端口、TCP 标志位等静态信息，无法判断包属于哪个连接、连接处于什么阶段。比如一个 SYN 包，无法区分是新连接请求，还是旧连接的异常重放。
conntrack 表记录：内核会为每个连接维护状态（如 NEW/ESTABLISHED/RELATED/TIME_WAIT 等），并记录连接的五元组信息。防火墙的有状态规则是匹配这个表中的状态，而不是单纯的包头字段。这使得它能区分 “正常会话的响应包” 和 “伪造的新连接请求”，从而实现更精准的放行与控制。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤：仅根据单个 IP 包的源 / 目的 IP、端口、协议等静态字段匹配规则，不记录连接上下文，无法识别包的状态。
有状态检测防火墙：通过conntrack模块维护连接状态表，记录会话的完整生命周期，能区分新连接、已建立会话的响应包等状态。
本质区别：是否感知并维护连接的上下文状态。无状态是 “看单个包”，有状态是 “看整个会话”。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：顺序意义：防火墙规则是按从上到下依次匹配的，匹配到就停止。将ESTABLISHED,RELATED放在最前面，能让已有会话的响应包优先被快速放行，避免后续规则的无效匹配，大幅提升处理效率；NEW规则仅处理新发起的连接请求。
能否调换：不能。如果调换顺序，NEW规则会先匹配所有包，包括已有会话的响应包，导致响应包无法被快速放行，还可能因匹配错误导致连接失败，同时也会降低防火墙处理效率。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包到达时，内核找不到对应的conntrack条目，会将其标记为NEW状态。
防火墙会按照规则匹配处理：如果规则允许目标端口的NEW包通过，则会重新建立连接；如果不允许，则会直接丢弃该包，导致连接中断。


4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED状态描述的是和已有会话存在关联、但并非同一连接的辅助流量，典型场景是 FTP 协议的数据通道流量：FTP 的控制连接建立后，数据连接是独立的，但和控制连接存在关联，会被标记为RELATED状态。
本实验只有单 TCP 连接的 HTTP 流量，没有关联的辅助会话，因此不会出现RELATED状态；在 FTP、SIP 等存在多会话关联的协议场景下，会观察到RELATED状态的流量。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：攻击面更小：默认拒绝模式下，所有流量默认被阻断，仅放行明确授权的流量，从根本上减少了可被攻击的服务和端口，攻击者无法直接访问未授权的资源。
风险可控：所有业务流量都必须经过显式授权，任何未被允许的流量（包括未知的攻击、异常访问）都会被直接阻断，避免了 “默认允许” 模式下漏网流量带来的安全风险。
故障影响更小：“事后补堵” 模式下，一旦出现规则配置错误或遗漏，可能会暴露大量服务；而 “默认拒绝” 模式下，配置错误只会导致合法业务中断，不会造成安全暴露。
