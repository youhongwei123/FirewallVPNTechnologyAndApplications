...
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0；10.20.0.0/24 dev wg0|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT|所有协议、所有网口，连接状态为 RELATED、ESTABLISHED|放行已有连接的回应数据包，保障会话双向通信|
| 2 |ACCEPT|TCP 协议，入接口 wg0、出接口 veth-vpn-lan，源 10.10.10.1、目的 10.20.0.2，目的端口 8080，新建连接|仅允许 client（10.10.10.1）新建访问 server 8080 端口的 TCP 流量转发|
| 3 |LOG|所有协议，入接口 wg0，任意源目地址；限流 5 / 分钟，日志前缀VPN-DENY:|对从 VPN 隧道进入、未匹配放行规则的流量记录日志|
| 4 |REJECT|所有协议，入接口 wg0，任意源目地址|拒绝所有未放行的 VPN 入站流量，返回 ICMP 端口不可达|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功|新建连接匹配规则 2；回程应答匹配规则 1|规则 1、规则 2 计数器均增加|
| `client -> server:2222` |失败|未匹配放行规则，依次命中规则 3（LOG）、规则 4（REJECT）|规则 3、规则 4 计数器均增加|
| `client -> server ping` |失败|ICMP 包无放行规则，依次命中规则 3（LOG）、规则 4（REJECT）|规则 3、规则 4 计数器均增加|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |目标 10.20.0.2 是其他主机，数据包需要 vpn 命名空间转发，走转发 FORWARD 链|
| `client -> server:2222` | `FORWARD` |目标是内网另一台 server 主机，不属于 vpn 本机进程，必须经过转发链|
| `client -> vpn:9090` | `INPUT` |目标 IP 10.10.10.2 是 vpn 命名空间本机 wg0 地址，数据包发给本机应用，进入 INPUT 接收链|
...
| 抓包位置 | 看到的地址 | 协议/端口 | 说明 |
| :--- | :--- | :--- | :--- |
| `veth-vpn-ul` |源 192.0.2.2，目的 192.0.2.1| UDP 51820 | 外层加密包 |
| `wg0` |源 10.10.10.1，目的 10.20.0.2| TCP/ICMP | 解密后的内层包 |
| `veth-vpn-lan` |源 10.10.10.1，目的 10.20.0.2| TCP 8080 | 被允许转发的内网包 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |已建立连接、关联连接无法正常回程，客户端新连接发起后收不到服务端回包，curl 超时连接失败；正常已放行的回程报文被 iptables 丢弃，业务访问中断|INPUT/FORWARD 链缺少 state --state ESTABLISHED,RELATED 放行规则，服务端回复的应答数据包匹配不到放行策略，被默认策略拦截|重新添加状态放行规则：
iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |客户端访问被拒绝（curl 连接失败），但是系统日志里看不到预设的自定义拦截日志，日志没有生成|iptables 规则从上到下依次匹配，数据包先匹配到 REJECT 直接丢弃，不再继续向后匹配 LOG 规则，日志不会被记录|调整规则顺序，LOG 规则写到 REJECT 规则前面，先记录日志再拒绝数据包|
| 管理面规则错写到 `FORWARD` |客户端依然能正常 curl 访问 9090 端口，访问没有被拦截；FORWARD 链计数增长、能抓到对应 LOG 日志，但是目标本机端口没有生效拦截|9090 是本机监听端口，入站流量走 INPUT 链，FORWARD 链只处理跨主机转发流量；规则挂载到 FORWARD 无法拦截本机入站访问，规则不生效|删除 FORWARD 链上错误规则，重新把拦截规则添加到 INPUT 链对应位置|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功|iptables FORWARD 规则 2 专门放行源 10.10.10.1、目的 10.20.0.2、TCP 8080 的新建流量；回程应答匹配规则 1（RELATED,ESTABLISHED），双向流量均可转发|
| `client -> server:2222` |失败|无对应放行 ACCEPT 规则，流量先匹配规则 3 记录日志，再匹配规则 4 被 REJECT 拒绝，返回 ICMP 端口不可达|
| `client -> server ping` |失败|ICMP 数据包没有放行规则，同样依次触发 LOG 与 REJECT 规则，数据包被丢弃，返回端口不可达、100% 丢包|
...
| `VPN-DENY` 日志条数 |3 条|
| 日志中被拒绝的目标端口 |2222|
| underlay 抓包看到的协议 |UDP（WireGuard 底层封装基于 UDP）|
| `wg0` 抓包看到的源地址 |10.10.10.1|
| 内网接口是否能看到被拒绝的 2222 连接 |否（数据包在 vpn 命名空间 FORWARD 链就被 REJECT 丢弃，没有转发到内网 veth-vpn-lan 接口）|
...
| :--- | :--- |
| 模拟管理服务端口 |9090|
| 加 INPUT 规则前是否能访问 |能访问（INPUT 链默认策略为 ACCEPT，无拦截限制）|
| 加 INPUT 规则后是否能访问 |不能访问（匹配 LOG+REJECT 规则，连接被拒绝）|
...
>答：AllowedIPs 作用：WireGuard 隧道层路由规则，决定哪些流量封装进隧道、哪些走本地路由；本质是路由分发，只控制流量是否进隧道，不做精细端口 / 协议拦截。
iptables 作用：四层防火墙访问控制，可基于源 IP、目的 IP、端口、协议、连接状态做细粒度放行 / 拒绝 / 日志，是访问权限校验。
不能只靠 AllowedIPs 的原因
AllowedIPs 只能管网段路由，无法限制单端口、TCP/UDP 等四层粒度；
AllowedIPs 无日志、限流、连接状态管控能力；
一旦路由配置失误，会出现越权访问，缺少安全拦截兜底。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
>答：INPUT链：只处理目标是本机 VPN 网关的入站数据包；
FORWARD链：处理跨设备转发流量（VPN 客户端 → 网关 → 内网服务器），数据包只是经过网关转发，并非发给网关自身进程；
访问内网服务器属于转发流量，匹配 FORWARD 链，写 INPUT 完全不会生效。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
>答：客户端能发出 TCP 握手 SYN 包到达 server，server 回复 SYN+ACK 回程包；
回程包经过 VPN 网关 FORWARD 链时，无 ESTABLISHED 放行规则，回程报文被防火墙丢弃；
客户端反复重传 SYN，最终 curl 连接超时失败；
抓包可见单向 SYN 包，无完整三次握手。
4. 为什么 LOG 必须放在 REJECT 前面？
>答：iptables 规则是自上而下顺序匹配：
数据包先匹配靠前规则，一旦命中 REJECT 会直接终止匹配、丢弃报文；
LOG 放在 REJECT 后方时，报文已经被拒绝，不会走到 LOG 规则，拦截行为无日志记录；
顺序正确：先 LOG 记录流量，再 REJECT 拒绝流量。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
>答：安全意义
WireGuard 全程加密内层载荷，底层只暴露固定 UDP 51820 端口，中间人无法窃取内网通信明文、无法篡改内层 TCP / 业务数据。
可暴露的信息
两端公网 IP、固定 WireGuard 端口 51820；
数据包大小、发包频率、流量带宽，可推断业务活跃程度；
握手报文存在可识别特征，能识别这是 WireGuard VPN 流量；
无法解析内层目的 IP、端口、应用协议、传输内容。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
>答：横向渗透风险：VPN 客户端一旦中病毒 / 被劫持，攻击者可扫描渗透全部内网网段，横向遍历服务器；
权限过大：用户本只需访问单一业务，却能触碰数据库、运维管理、核心业务服务器；
泄露面扩大：内网所有端口、服务全部暴露给远程 VPN 接入端，漏洞攻击面翻倍；
审计失控：无法细分权限，难以定位哪个 VPN 账号发起了异常内网扫描 / 攻击；
违规操作风险：员工可随意拷贝内网全部业务数据、财务资料。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
>答：client → vpn:9090：走INPUT链
9090 端口监听在 VPN 网关本机进程，数据包目标是网关自身，属于本机入站流量。
client → server:8080：走FORWARD链
目标是另一台内网 server，流量仅经网关转发，不是发给网关本机程序。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
>答：NAT 是IP / 端口地址转换，修改报文头部 IP / 端口；WireGuard 是封装加密，外层 UDP 只是传输载体，内层完整保留原始 TCP/IP 头，无地址改写；
NAT 是四层转发变换，WireGuard 是二层隧道封装，工作层级不同；
conntrack 识别两条独立流：外层 UDP 隧道流、内层业务 TCP 流；NAT 只会生成一条转换映射流；
NAT 通常用于多主机共享公网 IP，WireGuard 用于端到端加密组网，设计目标完全不同。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
>答：允许访问来源
优先：机房内网运维 IP 段、固定管理员办公公网 IP；
谨慎放开：仅可信 VPN 管理账号 IP，禁止全部 VPN 客户端网段；
绝对拒绝：公网全网、未知匿名地址。
日志规则写法
LOG 规则放在放行 / 拒绝规则之前；
自定义日志前缀（如VPN-SSH-ACCESS），开启日志级别 4；
示例命令：iptables -I INPUT -p tcp --dport 22 -s 可信IP段 -j LOG --log-prefix "VPN-SSH-ALLOW:" --log-level 4
iptables -I INPUT -p tcp --dport 22 -s 可信IP段 -j ACCEPT
iptables -A INPUT -p tcp --dport 22 -j LOG --log-prefix "VPN-SSH-DENY:" --log-level 4
iptables -A INPUT -p tcp --dport 22 -j REJECT
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
>答：第一步：检查服务监听
确认 server 8080/vpn 9090 端口正常监听（ss -tuln）；
第二步：核对路由表
两端 ip route 确认 VPN 网段路由指向 wg 网卡，无路由冲突；
第三步：查看防火墙规则与计数器
iptables -L -n -v --line-numbers 看规则数据包计数，确认流量是否被匹配拦截；
第四步：检索防火墙系统日志
journalctl 过滤自定义 LOG 前缀，查看拒绝 / 放行日志；
补充辅助：两端抓包（tcpdump/wireshark）验证报文收发、回程是否丢包。
