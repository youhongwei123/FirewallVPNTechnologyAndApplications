...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：作用：域名系统（DNS）的核心作用是将人类可读的域名（如 www.yxnu.edu.cn）转换为计算机可识别的 IP 地址（如 202.203.86.93），实现域名与 IP 地址的映射。
   原因：网络通信本质上是基于 IP 地址进行的，计算机无法直接理解域名，必须先通过 DNS 解析获取目标网站对应的 IP 地址，才能建立 TCP 连接并访问网站资源。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致，本次抓包中请求包和响应包的 Transaction ID 均为 0x9a5a。
   作用：Transaction ID 是 DNS 事务的唯一标识符，用于将响应包与对应的请求包进行配对，避免在并发多个 DNS 查询时出现响应混淆，确保客户端能正确识别每个查询的结果。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：是 DNS 中最基础的记录类型，用于将域名映射到 IPv4 地址，即查询域名对应的 IPv4 地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用 UDP 协议，端口为 53。当 DNS 响应数据超过 512 字节时（UDP 报文最大限制），会改用 TCP 传输。
进行区域传输（Zone Transfer）时，DNS 服务器之间同步域名数据必须使用 TCP。
部分安全场景（如 DNSSEC）下，会强制使用 TCP 保证数据完整性。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：不支持 IPv6：gethostbyname() 仅能解析 IPv4 地址（A 记录），无法处理 IPv6 地址（AAAA 记录），不符合现代网络的双栈需求。
线程不安全：该接口返回的 struct hostent 是静态分配的全局变量，多线程并发调用时会导致数据竞争和结果错乱。
已被标记为废弃：POSIX 标准已将其标记为过时，推荐使用更现代的 getaddrinfo() 接口替代。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：说明该域名配置了负载均衡或高可用策略：
网站部署了多台服务器，通过 DNS 轮询（Round Robin）等方式将请求分发到不同 IP 地址的服务器，提升访问速度和服务可靠性。
也可能是 CDN（内容分发网络）的实现，将用户请求导向最近的节点，降低延迟。
