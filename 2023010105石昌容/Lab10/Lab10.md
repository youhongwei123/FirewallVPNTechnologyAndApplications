...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败|curl 提示 Network is unreachable，连接超时|
| `office → 10.0.3.2:8080` |成功|返回 HTTP 目录列表，显示 dmz 目录内容|
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED|放行相关 / 已建立的连接|
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 ctstate NEW|放行目标为 10.0.3.2:8080 的新 TCP 连接|
...
> 答：203.0.113.1（fw 的外网 IP）。conntrack 会自动将 dmz 回复包的源地址从 10.0.3.2 还原为 203.0.113.1，wan 侧完全感知不到内网真实 IP。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条已建立的连接的后续包会正常转发。conntrack 记录了已有的映射关系，只要连接状态为 ESTABLISHED，内核会基于 conntrack 条目继续完成地址还原，无需依赖 DNAT 规则；但新连接会因为没有 DNAT 规则而失败。
...
> 答：LOG 规则在 filter 表的 FORWARD 链触发，此时 nat 表的 PREROUTING 链已经完成 DNAT 改写，因此 DST 是改写后的内网地址。
若要记录原始目标地址，需将 LOG 规则放在 nat 表的 PREROUTING 链（DNAT 改写之前），此时能捕获到改写前的 dst=203.0.113.1。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：能。端口扫描的特征是：短时间内来自同一源 IP 的大量 NEW 状态连接请求，目标端口分散（如 1-65535 随机 / 顺序扫描），且大部分端口无对应 DNAT/ACCEPT 规则，日志中会出现大量不同 DPT（目标端口）的 DNAT-TO-DMZ 前缀日志（或无匹配规则的丢弃日志），且连接成功率极低。安全人员可通过统计「单源 IP 短时间内访问的端口数」「无规则匹配的连接数」识别扫描行为。
...
| `fw` 四个接口地址 |10.0.1.1/24（office 侧）、10.0.2.1/24（guest 侧）、10.0.3.1/24（dmz 侧）、203.0.113.1/24（wan 侧）|
| `wan → 203.0.113.1` ping 结果 |成功（0% 丢包）|
| `wan → 10.0.3.2` ping 结果 |失败（100% 丢包）|
| `office → 10.0.3.2` ping 结果 |成功（0% 丢包）|
| `office → 203.0.113.1` ping 结果 |成功（0% 丢包）|
...
| FORWARD 8080 |filter|FORWARD|-d 10.0.3.2 -p tcp --dport 8080 -m conntrack --ctstate NEW|ACCEPT|
| FORWARD 2222 |filter|FORWARD|-d 10.0.3.2 -p tcp --dport 2222 -m conntrack --ctstate NEW|ACCEPT|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：- DNAT 改写目标地址发生在 nat 表的 PREROUTING 链（路由决策前）；SNAT 改写源地址发生在 nat 表的 POSTROUTING 链（路由决策后、出网卡前）。
    若 DMZ 既对外提供服务又要访问外网：
    入站（外网→DMZ）：PREROUTING 链做 DNAT，将外网请求的目标地址改写为 DMZ 内网 IP；
     出站（DMZ→外网）：POSTROUTING 链做 SNAT，将 DMZ 内网源地址改写为 fw 外网 IP，确保外网回复能正确返回。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：优先级：DNAT 规则先生效。因为 PREROUTING 链在路由决策前改写目标地址，包会被转发到 DMZ，而非进入 fw 本机的 INPUT 链。
    共存方案：
    方案 1：为 fw 本地服务改用其他端口（如 8081）；
    方案 2：在 DNAT 规则中增加源地址匹配（如只对特定外网 IP 做 DNAT），其余 IP 的请求走 fw 本地服务；
    方案 3：调整 sysctl 参数 net.ipv4.ip_local_port_range 或使用 iptables 的 --to-ports 分流，或通过 INPUT 链优先匹配本地服务规则。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 会记录完整的四元组（源 IP、源端口、目的 IP、目的端口） 映射关系：
   外部 203.0.113.2:xxx → 203.0.113.1:8080 对应内网 203.0.113.2:xxx → 10.0.3.2:8080；
    外部 203.0.113.2:yyy → 203.0.113.1:8081 对应内网 203.0.113.2:yyy → 10.0.3.3:8080。
    回复包到达 fw 时，内核会根据 conntrack 中记录的「内网 IP + 端口 → 公网 IP + 端口」映射，精准还原到对应的公网端口（8080/8081）。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能回到 wan。核心机制是 conntrack 的「逆向地址还原」：conntrack 记录了原始请求的四元组（203.0.113.2 → 203.0.113.1:8080）和 DNAT 映射（→10.0.3.2:8080），当 dmz 回复包（10.0.3.2 → 203.0.113.2）到达 fw 时，conntrack 会自动将源地址改写为 203.0.113.1，使包能被 wan 识别并接收。若 conntrack 未生效（如无匹配条目），则需要在 POSTROUTING 链添加 SNAT 规则（-s 10.0.3.2 -j SNAT --to-source 203.0.113.1），强制改写源地址。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：防火墙无法正常工作。
   后果：新的 NAT 连接无法建立（conntrack 无法创建新条目），已建立的连接可能因条目被清理而中断；包转发效率急剧下降，甚至触发内核丢包。
   防护措施：
   1.调大 conntrack 最大条目数（sysctl net.netfilter.nf_conntrack_max）；2. 缩短连接超时时间（如 net.netfilter.nf_conntrack_tcp_timeout_established），释放闲置条目；
   3.配置 conntrack 速率限制（如 iptables -m limit），拦截高频攻击源；4.部署入侵检测 / 防御系统（IDS/IPS），识别并阻断连接耗尽攻击；
   5.分流高并发服务到专用网关，减轻防火墙压力。

6.从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础：Linux 内核的 netfilter 框架（iptables 是其用户态工具），核心是「数据包的钩子点拦截 + 规则匹配 + 动作执行」，且依赖 conntrack 实现有状态连接追踪。
   安保系统类比：
1.Lab7（有状态防火墙）：大楼的门禁 + 闸机，只允许已授权的人 / 车辆进出（匹配规则的包放行），且记录「谁进入过、是否是合法访客」（conntrack 追踪连接状态）；
2.Lab8（NAT/SNAT）：大楼的「对外统一身份」，内部人员外出时统一使用大楼的对外地址（改写源地址），外部无法直接看到内部人员的真实身份；
3.Lab9（分区审计）：大楼的分区门禁 + 监控录像，不同区域（办公区 / 访客区 / DMZ）设置不同通行规则，且对违规访问行为记录日志（审计）；
4.Lab10（端口映射 / DNAT）：大楼的「对外接待窗口」，外部人员无法直接进入内部区域，只能通过指定窗口（公网端口）联系内部人员（内网服务），所有交互都在窗口（防火墙）的监督下完成。
