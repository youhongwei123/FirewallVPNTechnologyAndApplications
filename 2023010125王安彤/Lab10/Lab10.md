...
> 答：203.0.113.1（防火墙公网 IP），conntrack 自动将回复包源地址还原为防火墙公网 IP。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：仍能正常通信，conntrack 会继续按原有映射关系处理该连接的数据包，直到连接断开或条目过期。

![alt text](conntrack_dnat.png)
...
| `wan → 203.0.113.1:2222` |成功 |无 |
| `wan → 203.0.113.1:3306` |失败 | 无|

![alt text](multi_port.png)
---
...
> 答：LOG 规则在FORWARD 链触发，此时已完成 DNAT 改写。要记录原始目标地址，应将 LOG 规则放在 nat 表的 PREROUTING 链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：能。可通过日志中同一源 IP 短时间内大量访问不同目标端口、访问大量未映射端口、日志前缀无匹配有效服务等特征识别端口扫描行为。

![alt text](log_audit.png)
...
| `wan → 10.0.3.2` ping 结果 | 成功|
| `office → 10.0.3.2` ping 结果 |失败 |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 **PREROUTING（路由前）改写目标地址；SNAT 在POSTROUTING（路由后 / 发送前）** 改写源地址。入站流量先经 PREROUTING 做 DNAT，再路由转发；出站流量先路由，再经 POSTROUTING 做 SNAT，两者配合实现内外网双向互通。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 先处理，PREROUTING 优先于 INPUT 链。可修改公网端口（如 8081 映射内网 8080），或用 -i 接口限定 DNAT 仅作用于外网接口，避免本地流量被转发。
