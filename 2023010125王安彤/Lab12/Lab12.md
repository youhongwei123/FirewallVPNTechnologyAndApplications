...
| 是否看到 `latest handshake` |是（握手时间正常刷新） |

截图：

![alt text](wg_status.png)
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |--ctstate ESTABLISHED,RELATED |放行应答、关联连接，保证双向通信 |
| 2 |ACCEPT |-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 tcp dpt:8080 |仅允许 VPN 客户端访问内网 8080 服务 |
| 3 |LOG |-i wg0 limit |记录所有被拒绝的 VPN 流量，添加日志前缀 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |1、2 |是（行 1、行 2 计数上涨） |
| `client -> server:2222` |失败 | 3、4|是（行 3、行 4 计数上涨） |
| `client -> server ping` |失败 |3、4 | 是（行 3、行 4 计数上涨）|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量穿过 VPN 网关，转发至其他主机 |
| `client -> server:2222` | `FORWARD` |流量穿过 VPN 网关，转发至其他主机 |
| `client -> vpn:9090` | `INPUT` |流量目的地为 VPN 网关本机进程，不转发 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |client -> server:8080 超时 / 访问失败 |客户端请求包被放行，但 server 应答包无规则匹配，被默认 DROP，双向通信中断 |重新插入 ESTABLISHED,RELATED 规则至 FORWARD 链首行 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但无对应 WRONG-ORDER 日志 |iptables 规则顺序匹配，REJECT 终止数据包处理，后续 LOG 规则无法执行 |将 LOG 规则调整至 REJECT 规则前方 |
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 依旧成功，计数器与日志无变化 |访问网关本机的流量走 INPUT 链，FORWARD 链规则不生效 |删除 FORWARD 错误规则，将拦截 / 日志规则写入 INPUT 链 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |匹配 FORWARD 放行规则 |
| `client -> server:2222` |失败 | 被 REJECT 规则拦截并记录日志|
| `client -> server ping` |失败 |无 ICMP 放行规则，被拦截 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条（随访问次数增加） |
| 日志中被拒绝的目标端口 | 2222|
| underlay 抓包看到的协议 |UDP 51820 |
| `wg0` 抓包看到的源地址 |10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 |否（被网关拦截） |
...
| 外层 UDP 目的地址 |192.0.2.1 |
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |wg show（非 conntrack） |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |8080 访问超时失败 |应答包无放行规则，双向不通 |恢复连接状态放行规则 |
| `LOG` 放在 `REJECT` 后面 |访问拒绝、无日志 |REJECT 终止流程，LOG 不执行 | LOG 前置，REJECT 后置|
...
AllowedIPs：路由层控制，仅定义哪些网段的流量进入 WireGuard 隧道，无端口、协议、细粒度权限管控。
iptables：包过滤层控制，基于五元组、连接状态做端口 / 协议 / IP 限制、日志审计、拒绝拦截。
仅靠 AllowedIPs 会导致 VPN 用户拥有内网全权限，存在安全风险，必须配合防火墙做最小权限管控。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
客户端流量经 VPN 网关转发至内网服务器，数据包目的地非网关本机，属于跨主机转发流量，Linux 内核将此类流量交由 FORWARD 链处理；INPUT 仅处理目的地为本机的流量。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端可发起 TCP 握手请求，但服务器返回的应答包无匹配规则，被默认 DROP，最终表现为连接超时、访问失败，双向通信中断。
4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则按顺序匹配，REJECT/DROP 会终止数据包处理流程；LOG 仅做日志记录、不改变流量走向，若后置则无法记录被拒绝的流量。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
安全价值：内层业务流量全程加密，公网抓包无法窃取业务数据、端口、IP 等敏感信息。
可暴露信息：两端公网 IP、固定 UDP 端口 51820，攻击者可识别 WireGuard 服务并针对性探测。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
VPN 用户一旦接入，可横向渗透内网所有主机、数据库、管理端口；若客户端中毒 / 失陷，攻击者可通过 VPN 跳板对内网发起攻击，违反最小权限原则。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> server:8080：FORWARD，流量跨网关转发至其他主机。
client -> vpn:9090：INPUT，流量目标为网关本机应用进程。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
WireGuard 是加密隧道，负责二层 / 三层数据包封装解密，隧道状态由自身维护；
conntrack 仅跟踪解密后的内层业务连接，不感知隧道加密逻辑；
NAT 是地址转换，WireGuard 无地址改写，二者技术本质不同。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
允许来源：仅企业办公网段、运维固定 IP，拒绝公网及所有 VPN 客户端。
日志规则：在 INPUT 链针对 SSH 端口添加 LOG 规则，记录源 IP、访问时间，便于审计溯源。
