...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |放行 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |A-TO-B-DENY |FORWARD DROP |
| `gwB` |放行 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |VPN-S2S-DENY |FORWARD DROP |
...
| `siteA -> siteB:2222` | 失败 |curl 连接超时 / 拒绝 |存在 A-TO-B-DENY 日志 |
| `siteA -> siteB ping` | 失败 |ICMP 目的不可达 |存在 A-TO-B-DENY 日志 |
...
| underlay 是否连通 |连通，gwA ↔ gwB ping 无丢包 |
...
| `latest handshake` |存在正常握手时间（秒级） |存在正常握手时间（秒级） |
| `transfer` 计数 |rx/tx 均有数据包 |rx/tx 均有数据包 |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功，返回网页目录 |新建 TCP 8080 ACCEPT 规则 + ESTABLISHED 回程允许 |
| `siteA -> siteB:2222` |失败，连接拒绝 |LOG 日志规则 + REJECT 拒绝规则 |
| `siteA -> siteB ping` |失败，端口不可达 |LOG 日志规则 + REJECT 拒绝规则 |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 ↔ 192.0.2.2 |UDP 51820 |WireGuard 外层加密封装包，看不到内网真实 IP |
| `wg0` |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |解密后原始业务流量，完整三层 IP 报文 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |解封装后转发至内网服务器的明文业务包 |
...
使用场景
远程接入：单台远程客户端（手机 / 笔记本）接入企业总部内网；一对一客户端 - 网关。
站点到站点：两个完整私有网段互通（总部↔分支、本地机房↔云 VPC）；网关↔网关。
AllowedIPs 配置
远程接入：网关填客户端隧道 IP，客户端填企业内网网段。
站点到站点：两端网关均填写对端完整内网网段。
路由逻辑
远程接入：仅客户端单设备路由指向隧道。
站点到站点：两端内网所有主机流量经本地网关转发至隧道。
Endpoint 角色
远程接入：客户端主动连接固定网关公网地址。
站点到站点：两端网关互为对等端点，双向持久保活。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
AllowedIPs 在 WireGuard 有两层核心作用：
路由下发：wg-quick 自动生成路由，匹配网段流量全部转发至对应 peer 隧道接口；填写对端内网，才能让内网跨站流量进入隧道加密。
报文校验：隧道收到加密报文后，校验内层源 IP 是否属于该 peer 的 AllowedIPs 网段；防止非法伪造内网 IP 的数据包通过隧道。
若只填写隧道 IP（10.100.0.0/24），仅网关之间互通，内网主机无法跨站访问。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
网关是三层转发设备：
siteA 内网流量到达 gwA 后，需要 IP 转发将报文从 LAN 口转发至 wg0 隧道口；
隧道解密后到达 gwB，需要 IP 转发将报文从 wg0 隧道口转发至 B 站点 LAN；
回程流量同理反向转发。
未开启 IP 转发时，网关只会接收本机流量，不会转发跨接口数据包，内网两端完全不通。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
siteA 主机目标为 10.2.0.0/24，本地路由表无该网段明细路由：
主机匹配默认路由，把跨站数据包发给 gwA；
gwA 依靠 WireGuard 自动下发的 10.2.0.0/24 隧道路由完成加密转发；
若主机网关错误，跨站流量会直接丢弃或发往错误出口，无法进入 VPN 隧道。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
按由底层到上层、由简单到复杂顺序排查：
检查两端网关ip_forward=1是否开启；
核对两端 peer 的AllowedIPs是否填写完整对端内网网段；
查看网关路由表：确认对端内网路由指向 wg0 接口；
检查内网主机默认网关是否指向本地 VPN 网关；
检查 iptables FORWARD 链是否放行隧道 / 内网转发流量；
tcpdump 抓包：underlay 看加密包收发、wg0 看解密内层包，定位丢包位置；
查看内核 nf_log 防火墙拒绝日志，确认是否被策略拦截。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
双向部署好处（纵深防御）
双重防护：单网关配置错误不会泄露对端内网；gwA 出口拦截出站非法流量，gwB 入口拦截隧道入站恶意流量。
流量提前拦截：非法流量在 gwA 直接丢弃，无需加密传输，节省带宽与加密开销。
双向访问管控：如需 B 主动访问 A，两端均可独立配置访问策略，权限隔离清晰。
独立审计日志：两端分别记录拒绝流量，便于区分是出站拦截还是入站拦截。
只在一侧写规则可行但存在缺陷
仅 gwA 配置规则：非法流量仍会加密通过公网传输到 gwB，浪费带宽；若 gwA 防火墙失效，B 站点无任何防护，内网完全暴露。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
隐藏信息（加密内层）
真实内网源 / 目的 IP（10.1.0.2/10.2.0.2）
业务 TCP 端口（8080/2222）、ICMP 报文、HTTP 明文业务数据
内网通信的三层四层原始报文内容
暴露信息（外层 underlay 封装）
两端网关公网 /underlay IP（192.0.2.1、192.0.2.2）
WireGuard 固定 UDP 端口 51820
数据包长度、流量收发时间、传输带宽
隧道持续保活心跳包特征，可识别使用 WireGuard 协议
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
分WireGuard 配置与iptables 防火墙两部分修改：
WireGuard 配置（已完成，本实验双向 AllowedIPs 已互通路由，无需改动）
gwB 出站防火墙（B 内网→wg0 隧道）新增允许规则：
# gwB FORWARD链放行siteB访问siteA指定端口
sudo ip netns exec gwB iptables -A FORWARD -i veth-gwb-lan -o wg0 -s 10.2.0.0/24 -d 10.1.0.2 -p tcp --dport XXXX -m conntrack --ctstate NEW -j ACCEPT
gwA 入站防火墙（wg0 隧道→A 内网）新增允许规则：
# gwA FORWARD链放行隧道进入A内网的指定端口流量
sudo ip netns exec gwA iptables -A FORWARD -i wg0 -o veth-gwa-lan -s 10.2.0.0/24 -d 10.1.0.2 -p tcp --dport XXXX -m conntrack --ctstate NEW -j ACCEPT
---
