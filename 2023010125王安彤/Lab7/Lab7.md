...
| :--- | :--- |
| 只有第一条规则时是否成功 | 否|
| 第一条规则放行的是什么方向的包 |lan → wan 的 TCP 新请求（目的端口 38070 |
| 失败的原因 |服务端 wan 返回的响应包（源端口 38070）被 FORWARD 链默认策略 DROP 丢弃 |
| 第二条规则匹配的是什么 |wan → lan 的 TCP 响应包（源端口 38070） |
| 加第二条规则后是否成功 |是 |
| 当前规则共几条 |2 条 |
...
无状态规则无法区分连接方向，仅基于源 / 目的端口放行，会导致反向主动连接也被允许，存在安全漏洞。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
例如，仅允许 lan 主动访问 wan 的 80 端口，但拒绝 wan 主动发起的所有 TCP 连接。无状态规则需要同时放行目的端口 80 和源端口 80，会导致 wan 主动从 80 端口发起的连接也被放行；而有状态规则仅放行 NEW 状态的出站连接和 ESTABLISHED 状态的响应包，可彻底阻止反向主动连接。
![alt text](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2 条 |2 条 |
| 是否需要手动考虑返回方向 | 是（需额外放行源端口 38070）| 否（由 conntrack 自动识别并放行响应包）|
| 能否区分"新连接"和"响应包" | 否（仅看端口，不区分连接状态）|是（通过 conntrack 状态区分 NEW 和 ESTABLISHED） |
| `wan` 能否主动发起到 `lan` 的连接 | 能（只要使用 38070 端口发起连接，就会被无状态规则放行）| 否（只有 lan 发起的 NEW 连接被允许，wan 主动发起的 NEW 连接被拒绝）|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为有状态防火墙通过 conntrack 记录了 lan → wan 的连接状态，当 wan 返回的响应包匹配 ESTABLISHED 状态时，会被第一条规则直接放行，无需额外配置返回方向规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
无状态规则的 --sport 38070 仅匹配源端口，无法区分连接是响应包还是新连接，因此 wan 主动从 38070 端口发起的 NEW 连接也会被放行；而有状态规则仅允许 lan 侧发起的 NEW 连接（目的端口 38070），wan 侧发起的 NEW 连接无法匹配任何规则，会被默认策略 DROP 丢弃。
![alt text](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） | 10.0.0.2:49444（lan 侧客户端地址 / 端口）|
| 条目中的目的地址和目的端口 |192.0.2.2:38070（wan 侧服务端地址 / 端口） |
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |ESTABLISHED |

简答题：

1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
依据的是 conntrack 表里的记录，而非仅包头字段。
仅看包头字段：无法区分包是新连接还是已有连接的响应，只能通过端口、IP 等静态字段判断，容易被伪造。
看 conntrack 记录：能根据连接的历史状态（如三次握手过程）判断包是否属于合法的已有连接，即使端口相同，非连接相关的伪造包也会被识别并拒绝。

![alt text](conntrack.png)
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态防火墙仅基于数据包的头部字段（IP、端口、协议）独立判断每个包，不维护连接上下文；有状态防火墙会维护连接状态表，结合数据包所属的连接状态（如 NEW/ESTABLISHED）进行判断，能区分请求包和响应包，也能识别非法伪造的包。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：意义：ESTABLISHED,RELATED 规则匹配的包数量更多（每个连接的后续包都匹配此规则），放在前面可以快速命中，减少后续规则的匹配次数，提升防火墙转发效率。
不能调换：如果将 NEW 放在前面，已建立连接的响应包无法匹配 NEW 规则，只能继续向后匹配，若后续规则也无法匹配，就会被默认策略 DROP 丢弃，导致合法连接中断。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被标记为 INVALID 状态（不属于任何已知连接，也不是合法的新连接）。防火墙默认会丢弃 INVALID 状态的包，因此这些包会被 FORWARD 链的默认策略 DROP 丢弃，连接会异常中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已有连接相关、但不属于同一连接的辅助连接流量。常见场景：FTP 协议中，控制通道（21 端口）建立后，数据通道会发起新的连接，该连接属于 RELATED 状态；ICMP 错误报文（如端口不可达、超时）也会被标记为 RELATED，因为它们是对已有连接的响应。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：攻击面最小化：默认拒绝会封闭所有端口和连接，仅放行明确允许的流量，不存在未被规则覆盖的 “死角”，攻击者无法利用未授权的端口 / 连接发起攻击。
漏洞风险更低：默认允许模式下，若未及时封堵漏洞端口或服务，攻击者可直接利用这些未授权通道入侵；而默认拒绝模式下，即使存在漏洞，没有放行规则的流量也无法通过防火墙，无法被利用。
规则更清晰：默认拒绝模式下，规则都是 “白名单”，只允许明确的合法流量，便于审计和维护；默认允许模式下，规则是 “黑名单”，容易遗漏，且攻击者可绕过封堵规则。
