...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 | 路由不可达，无法建立连接|
| `office → 10.0.3.2:8080` |成功 | 正常返回HTTP目录列表|
...
| `filter` | `FORWARD` | 1 | ACCEPT |所有已建立、关联连接 |放行回包流量 |
| `filter` | `FORWARD` | 2 | ACCEPT | dst=10.0.3.2 tcp dpt:8080 新连接|放行8080转发 |
...
| `wan → 203.0.113.1:80` | 失败| 没有80端口的 DNAT 规则 |
...
| 连接状态 | ESTABLISHED,ASSURED|

**简答题：**

1. `wan` 的 curl 收到的 HTTP 响应，源 IP 是什么？（提示：`wan` 无感知 DNAT 存在）

> 答：外网主机收到的响应源 IP 是 203.0.113.1，完全感知不到内网真实服务器地址 10.0.3.2。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：依然可以正常通信，连接跟踪表已记录映射关系，后续流量自动完成地址还原，不再匹配 DNAT 规则。
...
> 答：LOG 规则在 filter 表 FORWARD 链触发，此时已经完成 PREROUTING 的 DNAT 改写，只能看到改写后的内网地址。若要记录改写前的原始公网目标地址，应将 LOG 规则放在 nat 表 PREROUTING 链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以发现端口扫描攻击。短时间内同一源 IP 频繁访问大量不同目标端口、产生大量 NEW 连接日志，无正常业务访问特征，即可判定为端口扫描。
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 | nat|PREROUTING |-p tcp -d 203.0.113.1 --dport 8080 | DNAT --to-destination 10.0.3.2:8080|
| DNAT 2222 |nat | PREROUTING| -p tcp -d 203.0.113.1 --dport 2222|DNAT --to-destination 10.0.3.2:2222 |
| FORWARD 8080 |filter | FORWARD|-p tcp -d 10.0.3.2 --dport 8080 |ACCEPT |
| FORWARD 2222 |filter |FORWARD |-p tcp -d 10.0.3.2 --dport 2222 | ACCEPT|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 nat 表 PREROUTING 链改写目的地址，SNAT 在 nat 表 POSTROUTING 链改写源地址。入站流量通过 PREROUTING 做端口映射，让外网可以访问 DMZ 服务；出站流量通过 POSTROUTING 做源地址伪装，让 DMZ 内网主机可以正常访问外网。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则优先级更高，会优先抢占端口流量。共存方案：更换 DNAT 对外端口为非占用端口，或调整 iptables 规则顺序，让本地 INPUT 链优先匹配。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 通过五元组（源 IP、目的 IP、源端口、目的端口、传输协议）记录映射关系，回程数据包自动匹配对应会话，精准还原到对应的公网端口。


4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：可以正常回到 wan，依靠 conntrack 连接跟踪自动完成源地址还原。若复杂网络路由环境下不通，需要添加 MASQUERADE 源地址伪装规则保障回包路由。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：不能正常工作，新连接无法建立、端口映射失效、网络卡顿断连。防护措施：调大 conntrack 最大连接数、缩短空闲连接超时时间、配置防火墙并发限流、防御端口扫描与 DDOS 攻击。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础是 iptables 四表五链架构与 conntrack 连接跟踪机制。类比：有状态防火墙是大楼身份门禁、NAT 是地址身份翻译、分区隔离是楼层围栏、端口映射是对外专属服务接待窗口。
