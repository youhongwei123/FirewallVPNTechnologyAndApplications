...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |源 10.1.0.0/24 目标 10.2.0.2 tcp8080 |A-TO-B-DENY |DROP |
| `gwB` |源 10.1.0.0/24 目标 10.2.0.2 tcp8080 |VPN-S2S-DENY |DROP |
...
| `siteA -> siteB:2222` | 失败 |失败 |产生 A-TO-B-DENY |
| `siteA -> siteB ping` | 失败 |失败 |产生 A-TO-B-DENY |
...
| `latest handshake` |存在握手时间 |存在握手时间 |
| `transfer` 计数 |收发字节非0 |收发字节非0 |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |gwA/gwB 8080 放行规则 |
| `siteA -> siteB:2222` |失败 |gwA LOG+REJECT |
| `siteA -> siteB ping` |失败 |gwA LOG+REJECT |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1/192.0.2.2 |UDP51820 |加密外层 |
| `wg0` |10.1.0.2/10.2.0.2 |TCP8080 |解密内层 |
| B 站点 LAN |10.1.0.2/10.2.0.2 |TCP8080 |内网放行流量 |
...
1. 远程接入 VPN 和站点到站点 VPN 的区别是什么？
   答：远程接入：单客户端接入企业内网，客户端 AllowedIP 写内网，网关仅放行客户端隧道 IP；站点到站点：两个完整私有网段互通，两端网关 AllowedIP 互写对方完整内网段，实现全网段路由互通。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
   答：wg 自动生成去往对端内网的隧道路由，同时校验入站数据包源 IP 合法性，防止非法网段流量进入隧道。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
   答：流量需要在 wg0 隧道接口与 lan 内网接口之间转发，关闭转发则跨网段数据包直接丢弃。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
   答：内网主机访问对端网段时，数据包转发至本地 VPN 网关，由网关通过 WireGuard 隧道封装发送。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
   答：①检查两端 AllowedIP 配置是否为完整对端网段 ② 网关 ip_forward 是否开启 ③ 内网主机默认路由 ④ iptables FORWARD 默认策略 / 放行规则 ⑤ 抓包查看三层封装转发
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
   答：纵深防御，即使一侧网关配置出错，另一侧仍可保护内网；仅单边防火墙会出现非法流量进入内网后才拦截，存在安全风险。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
   答：WireGuard 完整加密内层 IP、端口、业务数据；仅暴露两端公网 underlay IP、UDP5182 特征端口。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
  答：gwB FORWARD 新增放行 10.2.0.0/24 访问指定端口规则，gwA 同步新增对应入站放行规则，两端 AllowedIP 保持不变。
---
