...
| :--- | :--- |
| 只有第一条规则时是否成功 | 否（超时失败）|
| 第一条规则放行的是什么方向的包 |lan → wan，目的端口为38070的请求包 |
| 失败的原因 | wan → lan 的响应包目的端口是随机临时端口，不匹配规则，被默认DROP丢弃|
| 第二条规则匹配的是什么 |	wan → lan，源端口 38070 的响应包 |
| 加第二条规则后是否成功 |是 |
| 当前规则共几条 | 2 条|
...
答：无状态规则只看端口号，不区分连接方向，存在被反向主动连接绕过的安全漏洞。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
例如：只允许内网主动访问外网的 HTTP 服务，禁止外网主动发起 HTTP 连接。无状态规则需要放行双向端口，会导致外网可主动从80端口发起连接；有状态规则可只放行内网发起的新连接，外网的新连接会被拒绝。
![无状态规则](stateless.png)
...
| 规则条数 | 两条|两条 |
| 是否需要手动考虑返回方向 |是|否 |
| 能否区分"新连接"和"响应包" |不能，只看端口号 |能通过ctstate区分连接状态 |
| `wan` 能否主动发起到 `lan` 的连接 | 能，从 38070 端口发起会被放行| 不能，新连接请求被 NEW 规则拒绝|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
答：因为 ESTABLISHED,RELATED 规则匹配已经由 conntrack 记录在案的连接，响应包属于已建立连接的一部分，会被自动放行，不需要单独写规则。
1. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：有状态规则只允许 lan 侧发起的 NEW 连接，wan 主动发起的连接会被 ctstate NEW 规则拒绝；而无状态的 --sport 38070 只看端口号，不区分发起方，所以无法阻止反向连接。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） | 10.0.0.2，随机临时端口|
...
答：依据的是 conntrack 表里的记录，而不是单纯看包头字段。区别在于：conntrack 会记录整个连接的状态（比如是否已经完成三次握手），而包头字段只包含 IP、端口等静态信息，无法区分包是新连接还是已建立连接的响应包。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤只根据数据包的包头信息（IP、端口、协议）判断是否放行，不记录连接状态；有状态检测防火墙会通过conntrack维护连接状态表，能区分新连接、已建立连接和相关连接，安全性更高。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：不能调换。因为 ESTABLISHED,RELATED 规则的命中率更高，大多数包（响应包、数据传输）都匹配这条，放在前面可以直接放行，减少后续规则匹配的开销；如果调换顺序，所有包都要先匹配 NEW 规则，会增加不必要的匹配次数，降低效率。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被视为 INVALID 状态，因为没有对应的连接记录。如果没有配置 INVALID 规则，会被默认策略 DROP，导致连接中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已有连接相关的、但不是同一连接的流量，比如 FTP 协议的数据通道（主动模式下，数据连接是从服务器发起的，但与控制通道相关）。本实验只有单一 TCP 连接，所以不会出现 RELATED 状态。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答："默认拒绝" 是最小权限原则，只有明确允许的流量才能通过，能有效减少攻击面，即使有配置遗漏也不会被利用；"默认允许" 会放行所有流量，事后补堵很难覆盖所有攻击面，容易被绕过，安全性低。
