...
| 测试项 | 结果（成功/失败） | 原因简述 |
| :----- | :-------------- | :------- |
| `wan → 203.0.113.1`（fw 外网 IP） | | 同一外网直连网段 |
| `wan → 10.0.3.2`（dmz 内网 IP） | | wan 没有到 `10.0.3.0/24` 的路由 |
| `office → 10.0.3.2` | | 都在内网，经 fw 转发 |
| `office → 203.0.113.1` | | 访问 fw 自身的外网接口 |
...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` | 失败| 提示网络不可达，无法建立连接|
| `office → 10.0.3.2:8080` |成功 | 正常返回 HTTP 目录页面|
...
| `filter` | `FORWARD` | 1 | ACCEPT | 所有已建立 / 关联连接|放行 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 新连接 |放行 |
...
> 答：203.0.113.1（防火墙外网 IP，wan 感知不到内网地址）

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：conntrack 会继续沿用已有的映射关系，后续数据包仍会正常转发；新连接因无 DNAT 规则无法映射。
...
> 答：LOG 在filter表FORWARD链触发，此时 DNAT 已完成改写；要记录原始地址需将 LOG 放在nat表PREROUTING链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：短时间内出现大量不同DPT（目标端口）、同一源 IP 的日志，即为端口扫描行为。
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat |PREROUTING |目的 203.0.113.1、TCP、目的端口 8080 | DNAT 10.0.3.2:8080|
| DNAT 2222 | nat|PREROUTING |目的 203.0.113.1、TCP、目的端口 2222 |DNAT 10.0.3.2:2222 |
| FORWARD 8080 |filter | FORWARD| 目的 10.0.3.2、TCP、8080、新连接|ACCEPT|
| FORWARD 2222 |filter | FORWARD|目的 10.0.3.2、TCP、2222、新连接 |ACCEPT |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 nat 表 PREROUTING 链 改写目标地址；SNAT 在 nat 表 POSTROUTING 链 改写源地址。
入站流量（外网访问 DMZ 服务）：PREROUTING 执行 DNAT，将公网地址转为内网地址，实现端口映射；
出站流量（DMZ 主动访问外网）：POSTROUTING 执行 SNAT，将内网私有地址转为防火墙公网地址，实现内网主机上网。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 优先。数据包先经过 PREROUTING 完成地址改写，路由判定为转发流量走 FORWARD，不会交给本机 INPUT 链，本地服务无法接收报文。
共存方案：使用不同外网端口做映射，例如外网 8080 映射防火墙本地服务，外网 8081 映射内网 DMZ 服务；也可结合接口、源地址做精细化规则区分流量。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 依靠五元组（源 IP、源端口、目的 IP、目的端口、协议） 记录完整映射关系。外网不同访问端口对应不同连接条目，回复报文匹配对应条目后，自动将内网主机地址 / 端口逆向还原为防火墙对应的公网 IP 与端口，保证流量原路返回。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：可以正常回到 wan。依靠 conntrack 连接追踪机制，防火墙识别出这是 DNAT 连接的回应报文，自动执行逆向地址转换，将回复包源地址从内网 10.0.3.2 还原为防火墙公网 IP 203.0.113.1，外网主机全程无感知，因此无需额外配置 SNAT。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：防火墙无法正常工作。conntrack 表满后无法新建连接追踪条目，新的 TCP/UDP 连接会直接中断，NAT、端口映射、有状态防火墙规则全部失效，网络大面积断连。
防护措施：① 限制单 IP 最大并发连接数；② 缩短空闲连接超时时间；③ 调大 conntrack 最大容量；④ 配置防端口扫描、DDOS 规则；⑤ 开启连接数监控告警。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础是 Linux Netfilter 框架 + conntrack 连接追踪模块，所有过滤、NAT、日志功能都基于此实现。
类比：有状态防火墙 = 大楼门禁，管控人员进出；NAT = 内部人员外出时统一身份登记隐藏；分区隔离 = 大楼区域划分，不同区域互相隔离；端口映射 = 对外开放指定服务窗口，仅允许外部访问指定业务；日志审计 = 安防监控录像，记录所有进出行为。
