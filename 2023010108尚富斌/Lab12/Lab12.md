...
| 测试项 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `client -> 192.0.2.1` | 成功 |ping 通，收到回复 |
| `server -> 10.20.0.1` | 成功 | ping 通，收到回复|
| `client -> 10.20.0.2` | 失败 |网络不可达 / 无响应 |
...
| 是否看到 `latest handshake` | 是，显示最近握手时间戳|
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功 | 正常返回网页目录|
| `client -> server:2222` | 成功 | 正常返回网页目录|
| `client -> server ping` | 成功 | 收到 ICMP 回复|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT	 |conntrack --ctstate ESTABLISHED,RELATED|放行内网回包、关联连接 |
| 2 | ACCEPT	|-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 tcp dport 8080 NEW | 仅允许 VPN 客户端新建 8080 业务访问|
| 3 | LOG|-i wg0，限流 5/min | 记录所有 VPN 非法访问日志|
| 4 | REJECT| -i wg0| 拒绝所有其他 VPN 转发流量|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 2 行 ACCEPT |是，数据包计数上涨 |
| `client -> server:2222` |失败 | 第 3 行 LOG、第 4 行 REJECT| 两条规则计数均上涨|
| `client -> server ping` |失败 |第 3 行 LOG、第 4 行 REJECT | 两条规则计数均上涨|
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |访问成功 | 连接被拒绝、超时|是，生成 VPN-MGMT-DENY 日志 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` | 数据包途经网关转发，目的不是网关本机进程|
| `client -> server:2222` | `FORWARD` |数据包途经网关转发，目的不是网关本机进程 |
| `client -> vpn:9090` | `INPUT` | 目的 IP 是网关本机，数据包交给本机应用处理|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |8080 请求发出去，但无返回，curl 超时 |客户端请求放行，服务器回程响应无匹配规则被 DROP |FORWARD 链首行重新插入 ESTABLISHED,RELATED 放行规则 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，无对应日志输出 |iptables 顺序匹配，REJECT 直接终止流程，LOG 不再执行 |调整规则顺序，LOG 在前、REJECT 在后 |
| 管理面规则错写到 `FORWARD` | 依然能正常访问 vpn:9090，规则计数器无增长、无日志| 访问本机流量走 INPUT 链，FORWARD 链规则不会匹配生效|删除 FORWARD 里错误规则，在 INPUT 链添加拦截 + 日志规则 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | 	成功|匹配专门放行 8080 端口的 FORWARD 规则 |
| `client -> server:2222` |	失败 | 被默认 DROP，触发 LOG+REJECT|
| `client -> server ping` |	失败 |ICMP 流量无放行规则，被拒绝 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |2 条（2222、ping 各 1 条） |
| 日志中被拒绝的目标端口 |2222 |
| underlay 抓包看到的协议 |UDP |
| `wg0` 抓包看到的源地址 |	10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 |否，网关直接丢弃未转发 |
...
| 外层 UDP 目的地址 | 192.0.2.1|
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |wg show；conntrack 只管内层业务连接，不维护加密隧道握手状态 |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端能发起 TCP 连接请求，但访问 server:8080 超时无响应 |客户端新建连接被放行，内网服务器回程响应包无匹配规则，被 FORWARD 默认策略丢弃 | 在 FORWARD 链首行重新插入放行已建立 / 关联连接的 iptables 规则|
| `LOG` 放在 `REJECT` 后面 |违规流量被拒绝，但无法捕获对应的 WRONG-ORDER 日志 |iptables 规则自上而下顺序匹配，REJECT 动作直接丢弃数据包并终止规则遍历，后续 LOG 规则不会执行 | 调整规则顺序，将 LOG 规则写在 REJECT 规则之前|
| 管理面规则错写到 `FORWARD` | VPN 客户端依旧可以正常访问 vpn:9090，规则计数器无增长、无拒绝日志|访问网关本机服务的流量走 INPUT 链，FORWARD 链规则不会匹配该流量，拦截策略完全失效 | 删除 FORWARD 链内错误规则，在 INPUT 链添加对应的日志 + REJECT 拦截规则|

---

## 思考题

1. `AllowedIPs` 和 iptables 访问控制分别解决什么问题？为什么不能只靠 `AllowedIPs` 做权限控制？AllowedIPs：路由控制，决定哪些流量走 VPN 隧道、哪些不走，仅做路由分流，不限制端口、协议；
iptables：访问权限控制，对进入网关的流量做五元组精细化放行 / 拒绝、日志审计。
局限：AllowedIPs 只能管控网段路由，同一网段内所有端口、ICMP 全放开，无法实现最小权限，内网高危端口、数据库、运维端口会全部暴露，安全边界缺失。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？流量路径：客户端→wg0→VPN 网关转发→内网服务器，数据包目的地不是 VPN 网关本机进程，属于跨设备转发流量；INPUT 链只处理目的地是网关本机的数据包，转发流量只会匹配 FORWARD 链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？客户端 SYN 包匹配放行规则成功发往内网服务器，但服务器回包回程方向没有放行规则，被 FORWARD 默认 DROP；客户端收不到 ACK，TCP 握手超时，curl 卡死最终访问失败。
4. 为什么 LOG 必须放在 REJECT 前面？iptables 规则自上而下顺序匹配，REJECT 会直接丢弃数据包并终止规则遍历；如果 LOG 在后，数据包不会走到日志规则，无法留存攻击 / 违规访问记录。LOG 仅做记录，不中断匹配流程。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？安全意义：外层网络抓包无法解析内层 IP、端口、HTTP 明文内容、访问的业务服务，加密封装保证传输层数据窃听无效；
泄露信息：能看到两端公网 IP、固定 UDP 端口 51820、数据包长度、发包频率，可识别 VPN 流量特征，存在流量指纹识别风险。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？VPN 终端沦陷后，攻击者横向遍历全内网，渗透数据库、核心业务服务器；
员工违规访问运维后台、交换机管理口、监控系统等非授权服务；
无法精细化审计，无法定位哪条 VPN 接入访问了敏感内网资产；
一旦账号泄露，攻击者可遍历整个内网网段。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？client->vpn:9090：INPUT 链，目的 IP 是网关本机，数据包上交给本机监听进程；
client->server:8080：FORWARD 链，网关仅做三层转发，本机不接收处理该数据包。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？NAT 会修改 IP / 端口做地址转换，WireGuard 只是加密封装，源目 IP 全程不变，无地址转换行为；
WireGuard 隧道握手、保活状态由 wg 内核模块独立维护，conntrack 不感知加密隧道生命周期；
conntrack 跟踪的是解密后的内层业务连接，外层 UDP 只是加密传输载体，二者层级完全独立。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？放行来源：仅企业固定运维公网 IP、内网运维管理段，绝对不允许 VPN 隧道 (wg0) 接入地址访问 SSH；
规则写法：INPUT 链针对 SSH 端口，先 LOG（自定义日志前缀、限流防日志风暴），再拒绝 wg0 入站流量，仅信任管理 IP 放行；所有非法 SSH 访问留存日志便于溯源。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？确认 wg show latest handshake 正常，隧道加密层连通；
客户端检查路由表：目标内网网段是否路由指向 wg0；
内网服务器检查网关、回包路由是否指向 VPN 网关；
VPN 网关查看 iptables FORWARD/INPUT 链计数器，看数据包是否命中拒绝规则；
查看 dmesg/journalctl 防火墙拒绝日志，定位拦截原因；
内网服务器检查业务端口正常监听、无本地防火墙拦截。
