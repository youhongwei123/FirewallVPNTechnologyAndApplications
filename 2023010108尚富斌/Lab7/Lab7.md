...
| 第一条规则放行的是什么方向的包 |lan → wan 方向，目的端口为 38070 的新连接请求包 |
| 失败的原因 | wan → lan 方向的响应包目的端口是 lan 的随机临时端口，不匹配任何规则，被默认 DROP 丢弃，导致客户端无法收到响应，超时失败|
| 第二条规则匹配的是什么 |wan → lan 方向，源端口为 38070 的 TCP 包（即服务端的响应包） |
| 加第二条规则后是否成功 |成功（request succeeded） |
| 当前规则共几条 |2 条 |
...
无状态规则仅通过端口号判断流量，无法区分 “合法响应包” 和 “伪造的主动连接请求”，导致内网端口暴露，存在安全漏洞。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
例如：FTP 主动模式中，控制连接的端口是 21，但数据连接的端口是服务端随机分配的。无状态规则需要放行所有大于 1024 的端口，无法精准控制；而有状态规则通过 conntrack 的 RELATED 状态，可以自动放行与控制连接关联的数据连接，无需开放大量端口。
...
| :--- | :--- | :--- |
| 规则条数 |2 条 |2 条 |
| 是否需要手动考虑返回方向 |需要（必须写 --sport 38070）	 |不需要（ESTABLISHED,RELATED 自动放行响应包） |
| 能否区分"新连接"和"响应包" |不能（仅看端口号） | 能（基于 conntrack 连接状态判断）|
| `wan` 能否主动发起到 `lan` 的连接 |能（绑定 38070 端口即可绕过） | 不能（仅允许 lan 发起的 NEW 连接）|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为 lan 发起的连接被允许建立后，conntrack 会记录这条连接的状态为 ESTABLISHED。服务端返回的响应包属于这条已建立的连接，匹配 --ctstate ESTABLISHED,RELATED 规则，因此被自动放行，无需单独为返回方向写规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态规则仅允许 lan → wan 方向、目的端口为 38070 的 NEW 连接请求。wan 主动发起的连接，其 NEW 包方向是 wan → lan，不匹配 --dport 38070 规则，因此被丢弃。而无状态规则的 --sport 38070 不区分方向和连接状态，wan 只需将源端口设为 38070 即可绕过，无法阻止主动连接。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） | src=10.0.0.2 sport=XXXXX（随机端口）|
...
依据的是 conntrack 表里的记录。
包头字段（如源 / 目的 IP、端口）只能说明包的地址信息，无法判断它是新连接还是已有连接的响应。
conntrack 表记录了连接的完整生命周期状态，防火墙可以据此判断包是否属于一条已被放行的连接，从而精准放行合法响应、阻止伪造的主动连接。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤对每个数据包独立判断，仅依据包头字段（IP、端口、协议）做决策，不记录连接上下文；有状态检测则通过连接跟踪（conntrack）维护连接状态表，将每个包放到所属连接的上下文里判断，能区分新连接请求和已有连接的响应，安全性更高。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：不能调换。
意义：大多数数据包（响应包、数据传输包）都属于已建立连接，将 ESTABLISHED,RELATED 规则放在最前面，可以让这些包快速命中规则并放行，提高防火墙处理效率。
如果调换顺序，每个包都需要先匹配 NEW 规则，大量非新连接的包会多走一条无效匹配路径，降低性能；同时逻辑上也不清晰，容易导致规则冲突。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被标记为 INVALID 状态，不匹配 ESTABLISHED 规则，被默认策略 DROP 丢弃，导致连接中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 描述的是与已有连接相关、但本身属于独立连接的流量。典型场景：FTP 主动模式下，数据通道连接由服务端主动发起，与控制通道连接相关；ICMP 错误回应（如端口不可达）也属于 RELATED 状态。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：默认拒绝：所有流量默认被阻止，仅明确允许的流量才能通过，攻击面最小，即使有规则配置遗漏，也不会暴露额外服务。
默认允许：所有流量默认通过，需要手动禁止风险流量，一旦有规则遗漏或配置错误，就会留下安全漏洞，攻击者可直接访问内网服务。
