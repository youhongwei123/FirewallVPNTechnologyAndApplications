...
| 项目 | 你的填写 |
| :--- | :--- |
| client `wg0` 地址 |192.0.2.2/24 |
| vpn `wg0` 地址 |192.0.2.1/24 |
| client 端 `AllowedIPs` |10.20.0.2/24 |
| client 路由表中的 `wg0` 路由 |10.10.10.1/24 |
| 是否看到 `latest handshake` |是，wg show 输出存在握手时间与收发流量 |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT	 |-m conntrack --ctstate ESTABLISHED,RELATED	 |放行所有已建立连接的回程响应流量 |
| 2 |ACCEPT	 |-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 -p tcp --dport 8080 --ctstate NEW	 |仅允许 VPN 客户端新建 8080 端口 Web 访问 |
| 3 |LOG |-i wg0，限流 5/min，日志前缀 VPN-DENY	 |记录非法 VPN 转发流量，防止日志刷屏 |
| 4 |REJECT	 |-i wg0	 |拒绝所有未匹配放行规则的 VPN 流量|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 1 行 ESTABLISHED、第 2 行 8080 新建 ACCEPT 规则	 |是，两条规则数据包计数均上涨 |
| `client -> server:2222` |失败	 |第 3 行 VPN-DENY 日志规则、第 4 行 REJECT 拒绝规则	 |是，LOG、REJECT 计数同步上涨 |
| `client -> server ping` |失败	 |第 3 行 VPN-DENY 日志规则、第 4 行 REJECT 拒绝规则	 |是，LOG、REJECT 计数同步上涨 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |可正常访问	 |访问失败	 |是，生成 VPN-MGMT-DENY 拦截日志 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量经 VPN 网关中转至另一台独立内网服务器，属于跨设备转发流量 |
| `client -> server:2222` | `FORWARD` |流量经 VPN 网关中转至另一台独立内网服务器，属于跨设备转发流量 |
| `client -> vpn:9090` | `INPUT` |流量最终目的地是 VPN 网关本机运行的程序，属于本机入站流量 |
...
| 抓包位置 | 看到的地址 | 协议/端口 | 说明 |
| :--- | :--- | :--- | :--- |
| `veth-vpn-ul` | | UDP 51820 | 外层加密包 |
| `wg0` | | TCP/ICMP | 解密后的内层包 |
| `veth-vpn-lan` | | TCP 8080 | 被允许转发的内网包 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端 8080 请求 SYN 包可抵达内网，但服务器回程 ACK 全部丢失，curl 持续超时	 |服务器应答报文状态为 ESTABLISHED，无放行规则，被 FORWARD 默认 DROP 丢弃	 |将 ESTABLISHED,RELATED 规则重新插入 FORWARD 链第 1 行
|
| `LOG` 放在 `REJECT` 后面 |访问 3333 端口被拒绝，无 WRONG-ORDER 审计日志输出	 | iptables 自上而下匹配，命中 REJECT 直接终止匹配流程，后置 LOG 无法执行	|调整规则顺序，LOG 日志规则写在 REJECT 拒绝动作之前 |
| 管理面规则错写到 `FORWARD` |client 依旧正常访问 vpn:9090，FORWARD 计数器无增长、无拦截日志	 |访问本机进程流量仅走 INPUT 链，FORWARD 链规则不会匹配该流量	 |删除 FORWARD 错误规则，在 INPUT 链添加配套 LOG+REJECT 拦截规则 |
...
| vpn underlay 地址 | 192.0.2.3/24 |
| server 地址 | 192.0.2.1/24 |
| client wg0 地址 | 10.0.0.2/24 |
| vpn wg0 地址 | 10.0.0.1/24 |
| 是否看到 WireGuard 握手 | 是 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |匹配 FORWARD 第 2 行 8080 新建连接放行规则，回程匹配第 1 行 ESTABLISHED 规则 |
| `client -> server:2222` |失败 |无对应放行规则，先触发 LOG 记录流量，再被 REJECT 拒绝 |
| `client -> server ping` |失败	 |未开放 ICMP 报文放行规则，触发 LOG 与 REJECT 拦截 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条，访问 2222、ping 服务器各生成 1 条独立日志 |
| 日志中被拒绝的目标端口 |2222 |
| underlay 抓包看到的协议 |UDP 51820（WireGuard 外层加密报文） |
| `wg0` 抓包看到的源地址 |10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 |否，流量在 FORWARD 转发阶段直接丢弃，内网接口无数据包 |
...
| :--- | :--- |
| 模拟管理服务端口 |9090 |
| 加 INPUT 规则前是否能访问 |可以正常访问 |
| 加 INPUT 规则后是否能访问 |访问失败 |
| 管理面拒绝日志前缀 |VPN-MGMT-DENY: |
| `client -> vpn:9090` 经过的链 |INPUT 链 |
### F. conntrack 观察
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |8080 客户端 SYN 请求可达内网，服务器回程 ACK 全部丢失，curl 超时失败	 |服务器响应报文属于 ESTABLISHED 状态，无放行规则，被 FORWARD 默认 DROP 丢弃	 |将 ESTABLISHED,RELATED 规则插入 FORWARD 链第 1 行
 |
| `LOG` 放在 `REJECT` 后面 |访问 3333 端口被拒绝，无 WRONG-ORDER 审计日志输出	 |iptables 自上而下匹配，命中 REJECT 直接终止匹配流程，后置 LOG 无法执行	 |调整规则顺序，LOG 日志规则写在 REJECT 拒绝动作之前
 |
| 管理面规则错写到 `FORWARD` |client 依旧正常访问 vpn:9090，FORWARD 计数器无增长、无拦截日志	 |访问本机进程流量仅走 INPUT 链，FORWARD 链规则不会匹配该流量	 |删除 FORWARD 错误规则，在 INPUT 链添加配套 LOG+REJECT 拦截规则
...
1. `AllowedIPs` 和 iptables 访问控制分别解决什么问题？为什么不能只靠 `AllowedIPs` 做权限控制？
   答：AllowedIPs 属于 WireGuard 路由层配置，它仅规定哪些目标网段的流量会被送入加密 VPN 隧道，只能实现粗粒度的网段放行，无法限制隧道内部的端口、传输协议；而 iptables 是 Linux 内核防火墙，会在 WireGuard 解密出内层流量后做精细化访问管控，可以限制特定端口、记录非法访问审计日志、主动拦截违规流量。
如果仅依靠 AllowedIPs，VPN 接入用户会获得内网所有端口的完整访问权限，完全违背网络安全最小权限原则，同时没有日志留存，发生入侵后无法溯源定位，因此必须搭配 iptables 防火墙使用。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
   答：客户端访问内网 server 的流量，源 IP 是 10.10.10.1、目的 IP 是 10.20.0.2，两个 IP 地址都不属于 VPN 网关本机，流量只是借助网关中转转发，属于跨主机转发流量，由 FORWARD 链统一处理；INPUT 链只处理目的地为网关本机运行进程的流量，所以访问内网服务器的放行、拦截规则必须配置在 FORWARD 链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
   答：客户端发送的 TCP SYN 新建连接报文，可以匹配 8080 端口放行规则，正常送达内网 server；但服务器返回的 ACK 应答报文连接状态为 ESTABLISHED，没有对应的放行规则，会被 FORWARD 默认策略 DROP 丢弃，TCP 三次握手无法完成，curl 会持续连接超时，最终访问失败。
4. 为什么 LOG 必须放在 REJECT 前面？
   答：iptables 规则按照从上到下的顺序逐条匹配，一旦报文匹配到 REJECT、DROP、ACCEPT 这类会终止处理流程的动作规则，系统会立刻停止执行后续所有规则。如果 LOG 写在 REJECT 后方，报文被直接拒绝后不会执行 LOG 日志记录操作，无法留存非法访问记录；只有 LOG 在前、REJECT 在后，才能先记录流量行为，再拒绝报文。
5. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
   答：安全层面意义：内网所有业务报文都会被完整加密封装进 UDP 51820 外层报文中传输，底层网络抓包无法解析内网 IP、业务端口、HTTP 明文内容，避免中间人嗅探内网资产、用户访问行为。可暴露的信息：两端 underlay 底层 IP 地址、固定 WireGuard 51820 端口（攻击者可识别出 VPN 流量）、报文长度与收发时间间隔，能通过流量特征大致分析用户操作行为，但无法获取内网核心业务数据。


6. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
   答：1、权限无边界开放，远程 VPN 用户可随意访问数据库、运维 SSH、后台管理等高风险端口，账号泄露或客户端中毒后，攻击者能横向渗透整个内网；
      2、无法按业务划分安全隔离边界，生产核心业务与普通办公服务无防护区分；
      3、无法精细化审计用户每一条访问行为，入侵事件发生后难以定位攻击路径；
      4、扩大内网攻击暴露面，内网漏洞、弱口令全部对外网远程用户开放；
      5、不符合网络安全等级保护、数据安全法的最小权限访问合规要求。
7. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
   答：client -> server:8080 走 FORWARD 链：流量目的地是另一台独立内网服务器，仅经过网关中转，属于跨设备转发流量；
client -> vpn:9090 走 INPUT 链：流量目的地是 VPN 网关本机运行的 HTTP 服务进程，报文解密后直接交付本机程序，属于本机入站流量。
二者流量目标主体不同，Linux 内核划分的流量类型不同，因此匹配不同过滤链。
8.  如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
   答：1、维护主体不同：WireGuard 加密隧道会话由 WireGuard 内核模块独立维护，依靠 wg show 查看隧道握手、流量；conntrack 仅跟踪解密完成后的内层 TCP/ICMP 业务连接，不管理加密隧道握手会话；
2、实现原理不同：NAT 仅单纯修改报文 IP、端口地址，不存在加密封装；WireGuard 是完整 IP 报文加密封装，将整个内层数据包装进 UDP 外层；
3、生命周期独立：内层 TCP 业务连接断开不会销毁 VPN 加密隧道，隧道保活、握手机制完全独立于业务 TCP 连接，二者不能等同为 NAT 映射。
9.  当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
  答：允许访问来源：仅允许企业内网固定运维工作站 IP 访问 SSH，完全拒绝所有 VPN 隧道网段（10.10.10.0/24）、公网任意地址访问 SSH。
日志配置方法：在 INPUT 链最前端添加 LOG 规则，设置专属日志前缀区分 SSH 非法访问，配置流量限流防止日志刷屏，LOG 规则后搭配 REJECT 规则拦截 VPN 用户访问 SSH。
---
