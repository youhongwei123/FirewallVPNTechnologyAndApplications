...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |是，显示最近握手时间          |
| 是否看到 `transfer` 计数               |是，显示收发数据包字数         |
...
| 是否能访问 `10.20.0.2:8080`  |是，访问成功，返回文件目录列表          |
...
| `latest handshake` 是否还在      | 是，隧道握手状态正常         |
| 去 `10.20.0.0/24` 的路由是否消失 |是，路由表中无该网段路由          |
| 是否还能访问 `10.20.0.2:8080`    |否，连接超时，访问失败          |
| 失败的直接原因                   | 修改 AllowedIPs 后，系统路由表不再生成指向 wg0 的 10.20.0.0/24 路由，访问内网服务器的数据包无法进入 WireGuard 隧道，直接被丢弃或走默认路由，因此业务不通。         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：隧道握手与业务数据传输使用两套独立路径。隧道握手依靠底层 underlay 地址（192.0.2.0/24）通过 UDP 51820 端口通信，只校验密钥与网络连通性，和路由规则无关，因此握手状态始终正常。而业务数据传输依赖 AllowedIPs 生成的系统路由，只有目标网段匹配路由规则，数据包才会进入 wg0 隧道接口。本次实验路由规则失效，数据包无法进入隧道，最终导致业务访问失败。
截图：
...
| 协议                     |UDP（端口 51820）	                                    |ICMP / HTTP          |
| 是否能看到 ICMP/HTTP 内容 |否，仅加密密文，无明文业务数据	                                    |  是，可看到完整明文协议内容
        |
| 为什么内外两层看到的不同 | WireGuard 采用封装加密机制，原始业务数据包会被整体加密，再封装为外层 UDP 报文在底层网络传输。外层网络只能解析 UDP 头部，无法解密内层数据，因此两层抓包内容完全不同。	                                   |          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：outer 底层抓包对应信封，只能看到外层收发地址与 UDP 封装信息；inner 隧道接口抓包对应信纸，也就是真实的内网地址和业务明文数据。
该设计实现了地址隐藏 + 数据加密双重防护，公网环境中的抓包者无法获取真实内网通信地址和业务内容，能有效防止数据窃听、流量溯源与网络攻击，保障传输安全。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：AllowedIPs 有两大核心作用：
控制出口路由：wg-quick 会自动将配置中的网段写入系统路由表，指定对应网段的流量转发至 wg0 隧道接口；
入口访问控制：隧道收到解密后的数据包时，会校验数据包源 IP 是否在对端 AllowedIPs 网段内，不在则直接丢弃。
流量转发顺序为先路由、后访问控制。如果 AllowedIPs 配置错误，数据包根本不会进入隧道接口，访问控制规则也就无法生效。因此它最先影响路由转发，其次才起到访问控制的作用。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：二者依赖的配置、地址和转发逻辑完全独立。隧道握手使用底层 underlay 地址和 UDP 协议，仅完成密钥验证与链路保活，不依赖系统路由；业务数据传输必须依靠 AllowedIPs 生成的路由规则，数据包才能进入隧道。握手链路正常不代表业务路由正常，因此会出现隧道在线但业务无法访问的现象。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0 代表全网所有流量全部转发至 VPN 隧道，风险如下：
客户端所有上网流量都会经过 VPN 网关，网关一旦被控制，所有数据都会被窃听、篡改；
内网网段完全暴露，攻击者可借助隧道渗透内网设备，扩大攻击范围；
大量流量集中转发会造成 VPN 网关负载过高，引发网络卡顿、断网；
系统默认路由被覆盖，可能导致客户端本地网络访问异常。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 采用双向身份认证机制：通信双方都需要使用自身私钥签名数据包，对方使用对应的公钥验证身份。攻击者仅拥有 VPN 网关私钥，只能冒充网关发送数据，无法伪造客户端的签名报文，VPN 网关会直接丢弃非法数据包，因此无法冒充客户端建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：底层抓包外层源地址为 192.0.2.2。WireGuard 会将内层原始 IP 报文、端口、业务数据全程加密，再封装为标准 UDP 报文在公网传输。普通抓包工具只能解析外层 UDP 协议头部，没有对应解密密钥，无法还原内层明文数据，因此看不到真实通信内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：地址隐藏：对外隐藏内网真实 IP 地址，避免内网网段被探测、溯源；
数据加密：对业务数据进行加密传输，防止传输过程中被窃听、篡改，保证数据机密性与完整性。
