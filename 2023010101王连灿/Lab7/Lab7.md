...
| :--- | :--- |
| 只有第一条规则时是否成功 |访问失败 |
| 第一条规则放行的是什么方向的包 |放行内网 lan 向外网 wan 访问、目的端口为 38070 的正向请求数据包 |
| 失败的原因 |防火墙 FORWARD 默认策略为丢弃，仅放行正向请求包，外网返回的响应包目的端口为客户端随机临时端口，不匹配已设置的规则，被防火墙直接丢弃，客户端收不到响应而超时
 |
| 第二条规则匹配的是什么 |匹配源端口为 38070、由外网 wan 发回内网 lan 的响应回程数据包 |
| 加第二条规则后是否成功 |访问成功 |
| 当前规则共几条 |2 条 |
...
无状态包过滤仅依靠 IP 地址、端口号等包头固定字段逐条匹配放行数据包，不会记忆整条连接的建立过程与通信状态。只要外网主机将自身程序端口绑定为 38070，就能满足规则匹配条件，绕过防护主动向内网发起连接请求，无法区分合法响应包和恶意新建连接请求，存在明显安全漏洞。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
无状态规则只能基于固定端口、IP 等静态字段配置放行策略，面对采用随机临时端口传输的业务、FTP 等存在关联子连接的应用场景无法有效防护；同时无法识别连接状态，外网容易通过伪造端口方式主动渗透内网，而有状态防火墙可依靠连接跟踪机制识别合法会话，从根源上阻断外网主动入侵行为。
...
| :--- | :--- | :--- |
| 规则条数 |2 条	 |2 条 |
| 是否需要手动考虑返回方向 |需要手动配置回程源端口规则才能双向通信	 |无需手动配置返回方向，系统自动识别回程流量 |
| 能否区分"新连接"和"响应包" |不能区分，仅依据端口和 IP 字段匹配	 |可以依靠连接跟踪状态严格区分新建连接与已有连接的响应数据包 |
| `wan` 能否主动发起到 `lan` 的连接 |可以通过伪造端口绕过规则，主动访问内网	 |不能主动发起，仅允许内网主动发起的合法新连接接入
 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
有状态防火墙依靠内核 conntrack 连接跟踪模块，已经记录了内网 lan 主动向外网 wan 发起的合法连接会话。当外网返回响应数据包时，系统会自动匹配连接跟踪表中已建立的会话状态，识别为合法会话的后续流量并自动放行，因此不需要单独为返回方向编写防火墙规则。

2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态防火墙通过连接跟踪表记录每条 TCP 连接的完整生命周期，判断数据包是否合法依据会话状态，不单纯依赖端口号匹配；而无状态规则仅匹配源端口字段，无法判断数据包是否属于已有合法会话。外网可以随意绑定 38070 端口发起新连接绕过无状态规则，而有状态规则只允许内网发起的 NEW 新连接，外网主动发起的新建连接会被直接拦截。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |源地址：10.0.0.2，源端口为系统随机分配端口 |
| 条目中的目的地址和目的端口 | 目的地址：192.0.2.2，目的端口：38070|
| 连接状态（`-E` 中看到的 `[UPDATE]` 行） |ESTABLISHED（连接已建立） |

简答题：

1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
防火墙放行 ESTABLISHED 状态的数据包，依据的是内核 conntrack 连接跟踪表里记录的会话状态，而不是单纯依靠数据包头部的 IP 和端口字段。区别在于：仅看包头只能做静态字段匹配，无法判断数据包所属会话；连接跟踪表能记录整条连接的建立、传输、断开全过程，能精准区分合法会话响应包和恶意伪造的陌生数据包，安全性更高。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤防火墙对每一个到达的数据包进行独立检测，仅根据 IP 地址、协议、端口等包头静态字段决定放行或丢弃，不记忆过往的连接通信记录；有状态检测防火墙会维护内核连接跟踪表，记录每一条 TCP 连接的建立、交互、断开完整生命周期，把每个数据包纳入所属会话上下文进行状态匹配判断，不再孤立处理单个数据包，这是两者最本质的区别。


2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：两条有状态规则顺序不能调换。将 ESTABLISHED、RELATED 规则放在第一条，是因为通信过程中绝大多数都是已建立连接的交互数据包，放在前面可优先快速匹配放行，提升防火墙转发效率；同时先放行合法会话流量，再限制仅允许内网发起 38070 端口新连接，逻辑层次清晰。如果调换顺序，会先逐条匹配所有新连接规则，增加不必要的匹配开销，还可能造成流量匹配逻辑混乱，存在安全管控漏洞。


3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：若 conntrack 连接跟踪条目因超时被系统清除，后续到达的属于该连接的数据包会被标记为 INVALID 无效状态。防火墙默认策略为 DROP 丢弃时，会直接拦截这类无效数据包，导致原有通信会话中断，业务无法继续正常传输。


4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态代表与已有合法主连接存在关联、衍生出来的子连接或配套控制流量。本实验仅为简单 TCP 单点通信，没有出现该状态；常见应用场景有 FTP 协议的数据传输子通道、ICMP 网络超时、路由不可达等网络差错应答报文，这类流量依附于已有主连接，会被标记为 RELATED 状态放行。


5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：“默认拒绝，按需放行” 遵循最小权限安全原则，所有端口和流量默认全部拦截，只手动开放业务必需的端口和合法会话，最大限度缩小攻击面，杜绝未授权端口和陌生流量入侵；而 “默认允许，事后补堵” 开局开放所有流量权限，后期再手动封堵漏洞，容易遗漏隐蔽端口和未知安全隐患，恶意流量可利用开放端口提前渗透，安全防护可靠性远低于默认拒绝策略。
