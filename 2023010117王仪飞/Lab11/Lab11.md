...
| `ip route` 中 wg0 相关的条目 | `10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1`<br>`10.20.0.0/24 dev wg0 scope link` |
...
| 失败的直接原因                   | client不再有到10.20.0.0/24的wg0路由，发往10.20.0.2的流量不会进入WireGuard隧道 |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：`wg show`显示隧道仍然存在，是因为WireGuard的Peer之间仍能通过Endpoint正常发送握手和Keepalive报文；但将`AllowedIPs`改为`10.30.0.0/24`后，客户端不再拥有到`10.20.0.0/24`的wg0路由，因此访问`10.20.0.2:8080`的业务流量不会进入WireGuard隧道而导致失败。也就是说，握手和业务数据都依赖同一个隧道，但握手用于维持Peer连接，而业务数据还必须先匹配`AllowedIPs`对应的路由才能被封装并传输。
...
| 协议                     | UDP | ICMP |
| 是否能看到 ICMP/HTTP 内容 | 否，只能看到加密后的WireGuard数据 | 是，可以直接看到ICMP/HTTP等业务数据 |
| 为什么内外两层看到的不同 | underlay上看到的是WireGuard封装后的外层UDP数据包，业务内容已被加密 | wg0上看到的是解封装后的内层原始IP数据包，因此能看到真实业务流量 |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：如果把VPN隧道比喻成“信封+信纸”，那么underlay抓到的信息相当于信封上的内容，只能看到寄件人地址、收件人地址以及邮寄方式，但看不到信里写了什么；而wg0抓到的信息相当于打开信封后看到的信纸内容，可以看到真实的通信双方地址以及ICMP、HTTP等业务数据。这样设计意味着业务数据在公网或底层网络上传输时是加密的，即使攻击者截获了数据包，也只能看到外层通信信息而无法读取实际内容，从而保证了通信的机密性和安全性。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：`AllowedIPs`同时承担两项功能：一是告诉WireGuard哪些目标网段的流量应该发送给某个Peer，相当于路由配置；二是限制该Peer可以声明和接收哪些地址范围，具有一定的访问控制作用。在实际工作中，系统会先根据`AllowedIPs`自动生成路由，因此它首先决定数据包是否进入隧道；如果目标地址根本没有匹配到`AllowedIPs`，流量连隧道都不会进入，所以看起来像访问控制，实际上最直接影响的是路由选择。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：WireGuard的握手和Keepalive报文直接通过Peer的Endpoint地址进行通信，用于维持隧道状态；而业务数据是否能够传输，还取决于路由是否将目标流量送入wg0接口。在实验中虽然握手仍然正常，因此`wg show`显示在线，但由于修改了`AllowedIPs`导致访问目标网段的路由消失，业务流量无法进入隧道，所以`curl`访问失败。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：将`AllowedIPs`配置为0.0.0.0/0意味着所有IPv4流量都会被路由到该Peer，通过VPN隧道传输。这样虽然实现了全流量代理，但如果配置错误或对端不可信，可能导致所有网络访问都被转发到VPN节点，从而增加数据泄露、流量劫持、误路由甚至网络中断的风险，因此应根据实际需求尽量限制允许通过隧道的网段范围。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard使用基于公私钥的双向身份认证机制，client和vpn都必须证明自己持有与已配置公钥对应的私钥。即使攻击者获得了vpn节点的私钥，也只能伪装成vpn，而无法生成client的合法身份认证信息，因此无法以client的身份成功完成握手并建立受信任的隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：本实验中底层抓包看到的外层源地址是192.0.2.2，目的地址是192.0.2.1，对应client与vpn之间的underlay网络地址。由于WireGuard会先将内层业务数据加密，再封装进UDP数据包中进行传输，因此抓包者只能看到外层IP和UDP信息，而无法解密得到其中真实的ICMP、HTTP等业务内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：这说明VPN的封装机制同时提供了机密性和隐藏性两方面的安全保障。一方面，通过加密保护业务数据内容，使第三方无法读取实际通信信息；另一方面，通过封装隐藏真实的通信地址和协议细节，外部观察者只能看到VPN节点之间的通信，而无法直接获知内网主机之间的真实业务关系和通信内容。
