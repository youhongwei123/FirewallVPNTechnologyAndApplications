...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |curl 命令执行后提示无法连接到服务器，连接超时，无法访问目标 8080 端口的服务 |
| `office → 10.0.3.2:8080` |成功 |curl 命令正常执行，成功获取到目标服务返回的 HTML 目录列表页面，页面中显示了 Lab10.md 文件的访问链接 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |所有源 / 目的地址，ctstate RELATED,ESTABLISHED |放行已建立连接的响应数据包 |
| `filter` | `FORWARD` | 2 | ACCEPT |目的地址10.0.3.2，TCP 协议，目的端口8080，ctstate NEW |放行新发起的到 DMZ 区域 8080 端口的 TCP 连接 |
...
> 答：wan 端 curl 收到的 HTTP 响应源 IP 是：203.0.113.1。原因：DNAT 场景下，防火墙会把内网服务器（10.0.3.2）的回包做反向 NAT，把源 IP 改回防火墙的公网 IP 203.0.113.1；wan 侧完全感知不到内网真实地址，只看到公网 IP 作为响应源。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条连接的后续包仍然可以正常转发，不会中断。原因：已建立的连接由 conntrack 跟踪表全权处理，不再需要重新匹配 DNAT 规则。即使删除 DNAT 规则，只要连接跟踪条目还在，防火墙会继续按照原有转换记录处理该连接的所有后续数据包，直到连接超时 / 断开。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |	成功 |是 |
| `wan → 203.0.113.1:2222` |	成功 | 是|
| `wan → 203.0.113.1:3306` |失败 | 否|
...
| 日志中 `DST=` 字段是哪个地址 |10.0.3.2（DMZ 区域服务器的内网地址） |
...
> 答：LOG 规则在 filter 表 FORWARD 链触发（DNAT 之后），所以看到的是改写后的内网地址 10.0.3.2。要记录原始目标地址（改写前的 203.0.113.1），LOG 应放在 nat 表 PREROUTING 链（DNAT 之前）。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：能，LOG 日志可以帮助发现端口扫描攻击，因为端口扫描的典型特征是同一源 IP 在短时间内向目标主机的大量不同端口发送探测包，而 LOG 日志会记录这些数据包的源地址、目的端口等信息，安全人员可以通过分析日志中是否存在同一源 IP 在短时间内高频访问大量不同端口、目的端口呈无业务关联的连续 / 跳跃分布，且伴随大量被拒绝或丢弃的连接请求，来识别出端口扫描行为。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `wan` 地址 |203.0.113.2/24（默认网关：203.0.113.1） |
| `fw` 四个接口地址 |（veth-fw-wan）：203.0.113.1/24 ;（veth-fw-dmz）：10.0.3.1/24;（veth-fw-office）：10.0.1.1/24;（veth-fw-guest）：10.0.2.1/24
 |
| `wan → 203.0.113.1` ping 结果 | 成功（收到响应包）|
| `wan → 10.0.3.2` ping 结果 |失败（网络不可达） |
| `office → 10.0.3.2` ping 结果 | 成功（收到响应包）|
| `office → 203.0.113.1` ping 结果 |成功（收到响应包） |
...
| 测试项 | 结果 |
| :----- | :--- |
| `wan → 203.0.113.1:8080` |成功（返回 HTML 目录页面） |
| `wan → 203.0.113.1:2222` |成功（返回 HTML 目录页面） |
| `wan → 203.0.113.1:3306` |失败（无法连接服务器） |
| `wan → 10.0.3.2:8080` | 失败（无法连接服务器）|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 改写目标地址发生在 PREROUTING 阶段（数据包刚进入防火墙，路由决策之前），SNAT 改写源地址发生在 POSTROUTING 阶段（数据包即将离开防火墙，路由决策之后）。对于 DMZ 既对外提供服务、又访问外网的场景：PREROUTING 链：负责处理入站流量，将公网用户访问防火墙公网 IP 的请求，通过 DNAT 改写目标地址，转发到 DMZ 内网服务器。POSTROUTING 链：负责处理出站流量，将 DMZ 服务器访问外网的请求，通过 SNAT 改写源地址为防火墙公网 IP，让外网能正常回包。简单说，PREROUTING 解决 “别人怎么找到你”（入站 NAT），POSTROUTING 解决 “你怎么出去”（出站 NAT），两者配合才能让 DMZ 同时具备对外服务和访问外网的能力。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先在 PREROUTING 阶段改写数据包目标地址，把请求转发到内网服务器，因此会先抢到包，导致本地 Web 服务收不到请求；要让两者共存，可将 DNAT 规则的匹配条件改为仅对非本地回环 / 非本机发出的流量生效，或改用 --destination 203.0.113.1 --not -i lo 限定仅外部接口流量触发 DNAT，也可调整本地服务监听端口（如改为 8081），避免端口冲突。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：防火墙依靠 连接跟踪（conntrack）表 来完成逆向还原，确保回复包能回到正确的公网端口。当外部请求到达 203.0.113.1:8080 时，DNAT 规则会将其目标地址改写为 10.0.3.2:8080，同时 conntrack 会记录下这个连接的映射关系：外部地址+端口 <-> 内网地址+端口；同理，203.0.113.1:8081 的请求会被映射到 10.0.3.3:8080，并记录下另一条独立的连接跟踪条目。当服务器的回复包经过防火墙时，conntrack 会根据包的源 / 目的地址和端口匹配到对应的连接记录，然后将回复包的源地址和端口逆向还原为 203.0.113.1 上的对应端口，从而确保不同服务器的回复能正确回到各自的外部连接上。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：在没有 SNAT/MASQUERADE 规则的情况下，dmz服务器回复包的源地址为10.0.3.2，无法直接回到wan侧客户端，因为wan客户端的路由表中不存在10.0.3.0/24网段的路由，无法识别并回应内网私网地址；要让回复包正常返回，需要在fw的POSTROUTING链配置 SNAT/MASQUERADE 规则，将内网服务器回复包的源地址改写为fw的公网 IP（203.0.113.1），这样客户端收到的回复包源地址为防火墙公网 IP，就能匹配上原始连接并正常回应，完成整个通信流程。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：如果 conntrack 表满了，防火墙无法正常处理新连接，因为内核无法为新连接创建跟踪条目，导致所有新的 TCP/UDP 连接请求直接被丢弃，用户会出现 “无法建立连接”“端口映射失效”“内网无法访问外网” 等故障，已建立的连接通常不受影响，仍能继续通信。后果主要是：新连接建立失败、NAT 映射失效、防火墙服务拒绝，严重时会造成业务中断；防护措施包括：调整内核参数 net.netfilter.nf_conntrack_max 增大表容量、设置 nf_conntrack_tcp_timeout_established 缩短空闲连接超时、启用 SYN Cookies 缓解 SYN 洪水攻击、使用 --limit 限制单 IP 连接数，以及部署 DDoS 防护设备分流攻击流量。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：这四次实验的共同基础是 连接跟踪（conntrack）：它既支撑了有状态防火墙的RELATED/ESTABLISHED规则、NAT 的双向地址 / 端口映射、分区流量的日志审计，也让端口映射能通过连接记录自动完成反向转换，所有功能都建立在它维护的连接状态表之上。如果把防火墙比作大楼安保系统：Lab7 的有状态防火墙对应身份核验与访客登记系统，只允许登记过的合法访客（已建立连接）进出，拒绝未授权人员；Lab8 的 NAT 对应内部工位隐藏与对外窗口，让大楼内部的私网地址对外只呈现统一的公网 IP，保护内部结构；Lab9 的分区审计对应楼层门禁与监控摄像头，不同区域（办公 / 访客 / DMZ）设置独立权限，并对进出行为全程留痕；Lab10 的端口映射对应对外服务窗口指引，把外部访客对 “大楼统一地址 + 特定窗口号” 的请求，定向转发到内部对应服务的工位，既提供服务又不暴露内部细节。
