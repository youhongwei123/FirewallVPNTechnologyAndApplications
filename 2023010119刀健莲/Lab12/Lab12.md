...
| client 路由表中的 `wg0` 路由 | 1. 10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1
2. 10.20.0.0/24 dev wg0 scope link|
| 是否看到 `latest handshake` |是，可查看到最新握手时间记录 |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 | ACCEPT|所有源目 IP，连接状态为 RELATED、ESTABLISHED |放行已建立连接及关联回复报文，实现有状态放行 |
| 2 | ACCEPT|入接口 wg0、源 10.10.10.1、目的 10.20.0.2，TCP 目的端口 8080，新建连接 |仅允许客户端新建访问内网 8080Web 服务的 TCP 连接 |
| 3 |LOG |入接口 wg0，其余任意；限速每分钟 5 条、突发 10 条，日志前缀VPN-DENY: |对 wg0 入方向被拒绝的流量做限速日志记录，不拦截流量 |
| 4 |REJECT | 入接口 wg0，任意源目|拒绝 wg0 接口所有未匹配前面规则的入站流量，返回 ICMP 端口不可达 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` | 	成功| 第 2 条 ACCEPT 规则|是，计数由 0 变为 1 |
| `client -> server:2222` |失败 |第 3 条 LOG、第 4 条 REJECT 规则 |是，两条规则计数同步上涨 |
| `client -> server ping` |失败 |第 3 条 LOG、第 4 条 REJECT 规则 |是，两条规则计数同步上涨 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |可正常连通 |访问失败 |是，生成VPN-MGMT-DENY日志 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |客户端访问内网服务器的 8080 端口，流量需经 VPN 网关的 FORWARD 链转发，该端口匹配 FORWARD 链的 ACCEPT 规则，可正常连通 |
| `client -> server:2222` | `FORWARD` |客户端访问内网服务器的 2222 端口，流量经 VPN 网关的 FORWARD 链，该端口无匹配的 ACCEPT 规则，命中 LOG+REJECT 规则，访问被拒绝并生成VPN-DENY日志 |
| `client -> vpn:9090` | `INPUT` |客户端直接访问 VPN 网关自身的 9090 端口，流量目标为网关本机，无需经过 FORWARD 转发链，直接进入 INPUT 链，命中 INPUT 链的 LOG+REJECT 规则，访问被拒绝并生成VPN-MGMT-DENY日志 |
...
| 抓包位置 | 看到的地址 | 协议/端口 | 说明 |
| :--- | :--- | :--- | :--- |
| `veth-vpn-ul` |源192.0.2.2，目的192.0.2.1 | UDP 51820 | 外层加密包 |
| `wg0` | 源10.10.10.1，目的10.20.0.2| TCP/ICMP | 解密后的内层包 |
| `veth-vpn-lan` |源10.10.10.1，目的10.20.0.2 | TCP 8080 | 被允许转发的内网包 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端访问 8080 等已放行业务时，TCP 连接无法正常维持，出现访问卡顿、断连、超时或连接重置；ping 等 ICMP 通信出现丢包，仅首次新建连接可能短暂正常，后续数据报文全部被拦截 |ESTABLISHED,RELATED是有状态防火墙的核心规则，用于放行已完成三次握手的 TCP 连接后续数据报文、以及 ICMP 等关联回复报文；删除该规则后，仅 NEW 状态的新建连接能被放行，后续 ACK、数据报文都会被默认 DROP 策略拦截，TCP 连接无法正常维持，业务通信中断 | 重新将该规则添加到 FORWARD 链最前端，执行命令：
sudo ip netns exec vpn iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |访问 2222、9090 等被拒绝的端口时，仅能实现访问拦截，无法生成VPN-DENY/VPN-MGMT-DENY前缀的拦截日志，无法排查访问被拒绝的原因 |iptables 规则按从上到下的顺序匹配执行，REJECT 规则会直接拒绝数据包并终止后续规则匹配；LOG 规则放在 REJECT 后面时，被拦截的数据包已被 REJECT 处理，不会再走到 LOG 规则，因此无法生成日志 |调整规则顺序，先删除原有规则，再按「LOG 在前、REJECT 在后」的顺序重新添加，确保数据包先被记录日志，再执行拒绝操作 |
| 管理面规则错写到 `FORWARD` | 客户端访问 VPN 网关自身 9090 管理端口时，访问控制完全不生效，无法被拦截，也无法生成VPN-MGMT-DENY日志，管理面安全策略失效|管理面访问的目标是 VPN 网关本机，流量会进入 INPUT 链处理；FORWARD 链仅处理经过网关转发的跨网段流量，将管理面规则错写到 FORWARD 链，目标为网关本机的流量完全不会匹配到这些规则，导致安全策略失效 | 1. 从 FORWARD 链删除错误的管理面规则；
2. 将管理面的 LOG、REJECT 规则重新添加到 INPUT 链中，确保目标为网关本机的流量能匹配到对应规则|
...
| server 地址 |	10.20.0.2|
| client wg0 地址 |	10.20.0.1 |
| vpn wg0 地址 |	10.20.0.2 |
| 是否看到 WireGuard 握手 |是，外层 UDP51820 存在持续握手报文 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |FORWARD 链单独放行源 10.10.10.1 访问 10.20.0.2:8080 新建 TCP 流量，已放行 |
| `client -> server:2222` |失败 |无对应 ACCEPT 规则，命中 LOG+REJECT 规则被拒绝 |
| `client -> server ping` |失败 |	仅放行 TCP 8080，ICMP 报文无放行规则，被 REJECT 拦截 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条（限流内正常生成） |
| 日志中被拒绝的目标端口 | 2222|
| underlay 抓包看到的协议 |UDP（WireGuard 外层封装 |
| `wg0` 抓包看到的源地址 |10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 | 能|
...
| :--- | :--- |
| 模拟管理服务端口 |9090 |
| 加 INPUT 规则前是否能访问 |可以正常连通访问 |
| 加 INPUT 规则后是否能访问 |访问失败，连接被拒绝 |
...
| 外层 UDP 目的地址 | 192.0.2.1|
| 外层 UDP 目标端口 |51820 |
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` | wg show；conntrack 仅跟踪四层流量连接，无法查看 WG 隧道握手、对等点、收发字节等隧道专属状态|

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |新建 TCP 连接握手成功，但后续报文无法传输，业务中断、网页只发请求不收响应 |该规则放行已建立连接的回程 ACK / 数据报文，删除后回程报文被 FORWARD 默认 DROP 拦截 |在 FORWARD 链首行重新添加规则：sudo ip netns exec vpn iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |流量被正常拒绝，但无VPN-DENY拦截日志生成 |iptables 自上而下匹配，REJECT 直接丢弃报文，不再继续匹配后续 LOG 规则 |先删除旧两条规则，调整顺序：先 LOG、后 REJECT 重新插入 |
| 管理面规则错写到 `FORWARD` |客户端仍能正常访问网关 9090 端口，访问控制策略完全失效 |访问网关本机流量进入 INPUT 链，不会经过转发链 FORWARD，规则永远无法命中 |删除 FORWARD 链内错误规则，将 LOG+REJECT 规则添加至 INPUT 链 |
...
AllowedIPs是 WireGuard 隧道层面的路由与对等节点入网范围管控，用来限定哪些网段流量能进入隧道转发、指定对端可接收的流量网段，仅做路由可达性限制；iptables 是内核防火墙四层精细化访问控制，能基于源目 IP、端口、协议、连接状态做细粒度放行或拦截，还可日志记录拒绝流量。只依靠 AllowedIPs 只能控制网段能否走隧道转发，无法针对具体端口、TCP 新建连接、本机管理端口做精细权限管控，也不能拦截已在 AllowedIPs 网段内的非法业务访问，缺少端口级安全策略和访问审计能力，因此必须配合 iptables 协同做完整权限控制。


2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
VPN 用户访问内网服务器，数据包只是经由 VPN 网关中转转发，目标并非网关本机，流量会流经 FORWARD 转发链处理；INPUT 链仅处理目的地是网关自身 IP 的入站数据包，该访问场景不会进入 INPUT 链，因此放行或拦截内网跨机访问的规则必须配置在 FORWARD 链中。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端能完成 TCP 三次握手发起 HTTP 请求，但服务器回复的 ACK 和响应数据包回程时无法被放行，请求会卡死超时、页面无法正常返回数据，连接无法完整建立通信。

4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则从上至下依次匹配执行，数据包匹配到 REJECT 规则后会直接被拒绝丢弃，不再继续向下遍历后续规则；若 LOG 写在 REJECT 之后，拦截流量无法触发日志记录，只有将 LOG 前置，数据包先完成日志打印，再执行拒绝动作，才能完整留存拦截审计信息。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
外层underlay链路仅能看到UDP 51820加密报文，无法解析内层真实IP、端口与应用数据，第三方抓包者无法窃取业务明文内容、无法篡改内层流量，保障了传输加密安全；但依然会暴露VPN两端公网底层IP、固定VPN端口号、数据包收发时长与报文长度特征，攻击者可据此识别WireGuard VPN通信行为，还能通过流量时序、包长规律推断业务访问频次。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
VPN用户可直达内网全部网段，一旦用户终端中毒、账号泄露，攻击者就能横向遍历内网扫描所有业务主机、暴力破解服务端口，窃取数据库、核心业务数据；内网未做细分权限隔离，VPN接入后可随意访问运维管理端口、服务器远程登录端口、核心中间件高危端口，极易触发越权操作、篡改系统配置；同时无法针对不同员工做最小权限划分，离职或权限回收不及时会持续留存内网接入通道，也难以定位内网攻击流量具体来自哪一位VPN接入用户，安全审计与故障溯源难度大幅上升。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
`client -> vpn:9090` 的访问目标是VPN网关本机，数据包进入网关后交由`INPUT`链处理；`client -> server:8080`只是借网关中转、最终目标是内网另一台服务器，无需交付本机上层应用，会走`FORWARD`转发链，二者经过的链不同是因为数据包最终投递目的地不一样，INPUT链只处理目的地为本机的入站流量，FORWARD链专门处理跨主机转发流量。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
WireGuard是IP层完整封装加密隧道，只是对原始IP报文整体封装传输，不修改内层源目IP，不存在NAT的地址替换动作，而conntrack里的UDP条目仅记录两端外层公网UDP套接字连通状态，并非地址映射关系，同时wg show才能查看隧道握手、对等节点等专属隧道状态，因此不能将WG隧道视作NAT映射。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
仅允许企业内网运维固定IP、运维跳板机IP访问VPN网关SSH端口，拒绝包括VPN客户端网段在内的所有外部来源；在INPUT链SSH放行规则前添加LOG规则，限定仅拦截流量生成日志，打上专属日志前缀，记录源地址、访问端口，方便审计溯源，示例规则为先插入LOG规则记录拒绝的SSH访问，再配置IP白名单ACCEPT放行SSH，其余全部拒绝。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
先查看防火墙规则计数器，确认业务流量有没有命中放行/拒绝规则；接着查看内核拒绝日志，定位是否被拦截；然后检查两端路由表，确认回程路由可达；再核验目标服务器业务端口正常监听；最后排查应用服务自身运行状态。
