...
| 项目 | 你的填写内容 |
| :--- | :--- |
| ping 是否有回应 |无回应，100% 丢包，请求超时 |
| `wan` 收到的请求源地址 |10.0.0.2 |
| `wan` 无法回复的原因 | wan 的路由表中没有到 10.0.0.0/24 的回程路由，无法将 ICMP echo reply 包转发给 lan 主机|
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 加 NAT 后 ping 是否成功 |成功，0% 丢包，收到全部 4 个 ICMP echo reply |
...
| NAT 把源地址改成了什么 |192.0.2.1（fw 的外网接口 IP） |
...
额外记录的信息：NAT 额外记录了内网真实地址（10.0.0.2）与转换后公网地址（192.0.2.1）的映射关系，包括端口信息（多连接场景）。这说明状态防火墙和 NAT 都依赖 conntrack 提供的连接跟踪能力，它们是基于同一套底层连接表实现的不同功能，共用状态信息，因此可以同时工作且互不冲突。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会通过源端口号区分不同内网主机的连接。两台主机访问同一外网服务器的同一端口时，NAT 会为它们分配不同的临时端口，在 conntrack 中记录「内网地址 + 端口」与「NAT 后公网地址 + 端口」的映射关系，从而将回复包转发给正确的内网主机。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| TCP 38070 访问是否成功 |成功，收到服务端返回的 Hello from WAN Server |
| ping 是否成功 |失败，100% 丢包，请求超时 |
| ping 失败的原因 |fw 的 FORWARD 链规则仅允许 TCP 38070 端口的新连接，未放行 ICMP 协议，因此 ping 包被防火墙丢弃 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址段（如 10.0.0.0/8）是内网专用地址，全球无数内网都在重复使用，公网路由器没有也无法维护到这些地址的路由，因此无法直接访问。NAT 通过在内网出口将私有地址转换为出口的公网 IP，让内网主机以公网 IP 的身份访问外网；外网回复包以公网 IP 为目标，再由 NAT 根据 conntrack 记录还原为私有地址，转发给对应内网主机，从而实现内网主机访问外网。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：当出口 IP 固定（如静态公网 IP），推荐用 SNAT --to-source，性能略高，配置稳定。
当出口 IP 动态变化（如家用拨号宽带、DHCP 获取 IP），必须用 MASQUERADE，它会自动跟随接口 IP 变化更新地址转换规则，无需手动维护。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：会对协议载荷中携带 IP 地址 / 端口信息的协议造成麻烦，典型如 FTP、SIP、H.323、P2P 协议等。因为 NAT 只修改 IP / 传输层头部的地址信息，无法解析和修改应用层载荷中的地址，导致外网回复包的目标地址仍是内网私有地址，无法正常建立连接。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：两者都依赖 conntrack 连接跟踪机制，共用同一套状态表：
NAT 需要 conntrack 记录地址映射关系，实现双向通信；
状态防火墙需要 conntrack 判断包是否属于合法连接。
集成在一起可以共用底层资源，减少系统开销，同时实现「多设备共享公网 IP + 内网安全防护」的核心需求，是家用路由器的标准设计。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在保留现有规则的基础上，添加一条放行 ICMP 协议的规则即可：
   sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
   该规则会放行所有 ICMP 包，让 ping 正常通过防火墙。
