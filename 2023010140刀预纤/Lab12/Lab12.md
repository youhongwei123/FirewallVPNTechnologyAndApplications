...
| vpn `wg0` 地址 |10.10.10.2/32 |
| client 端 `AllowedIPs` |10.20.0.0/24, 10.10.10.2/32 |
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1
10.20.0.0/24 dev wg0 scope link |
| 是否看到 `latest handshake` | 是，两端均显示 30 seconds ago|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT|tcp、入接口 wg0、出接口 veth-vpn-lan、源 10.10.10.1、目的 10.20.0.0/24、目的端口 8080、连接状态 NEW | 放行客户端首次新建访问 8080 端口的 TCP 连接|
| 2 |LOG |所有协议、入接口 wg0、不限源目地址，限流 5 / 分钟、突发 10 | 对 VPN 进来的所有未匹配流量打印内核日志，前缀 VPN-DENY|
| 3 |REJECT | 所有协议、入接口 wg0、不限源目地址，拒绝类型 icmp-port-unreachable|拦截所有 VPN 侧未放行的流量，回复端口不可达 |
| 4 |无 |无 |无
 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 | 行 2 ACCEPT（tcp dport:8080 ctstate NEW）、行 1 ACCEPT（RELATED,ESTABLISHED 回程）| 是，pkts 数值从 0 涨到 60|
| `client -> server:2222` |失败 |行 3 LOG、行 4 REJECT |是，LOG 与 REJECT 计数器同步上涨 |
| `client -> server ping` |失败 |行 3 LOG、行 4 REJECT |是，ICMP 流量匹配拦截规则，计数器增加 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |访问成功，可以打开页面 |访问失败，连接被拒绝 | 是，生成 VPN-MGMT-DENY 内核拦截日志|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` | 数据包是跨设备转发流量，源 client、目的内网 server，不属于 vpn 本机进程，走转发 FORWARD 链|
| `client -> server:2222` | `FORWARD` | 同样是跨设备转发流量，目标不是 vpn 本机程序，由 FORWARD 链匹配拦截|
| `client -> vpn:9090` | `INPUT` |数据包目标 IP 是 vpn 本机隧道地址 10.10.10.2，访问 vpn 本机运行的 web 服务，发给本机进程，进入 INPUT 入站链 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |客户端 8080 请求握手能发出去，但服务器回包无法回程转发，curl 访问超时失败 | 缺少已建立连接回程放行规则，服务器回复数据包被 FORWARD 默认 DROP 丢弃| sudo ip netns exec vpn iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |流量被直接拒绝，内核完全打印不到对应拦截日志 |iptables 自上而下匹配，REJECT 提前终止数据包，后面 LOG 规则永远不会命中 |调换顺序，LOG 规则插入在 REJECT 上方，先记录日志再拒绝包 |
| 管理面规则错写到 `FORWARD` |访问 vpn 本机 9090 服务完全不受拦截限制，依旧可以成功打开页面 |访问本机进程流量走 INPUT 链，FORWARD 转发链规则对本机入站流量无任何匹配效果 | 删除 FORWARD 里错误拦截规则，在 INPUT 链添加 9090 端口 LOG+REJECT 规则|
...
| client wg0 地址 |10.10.10.1/24 |
| vpn wg0 地址 |10.10.10.2/32 |
| 是否看到 WireGuard 握手 |是，两端均存在 latest handshake 记录 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功| FORWARD 链专门放行源 10.10.10.1 访问 10.20.0.2 的 8080 新建 TCP 连接，且存在 ESTABLISHED 回程放行|
| `client -> server:2222` | 失败| 不属于放行端口，匹配 LOG 后被 REJECT 拒绝|
| `client -> server ping` | 失败| ICMP 无放行规则，匹配拦截规则被拒绝|
...
| 内网接口是否能看到被拒绝的 2222 连接 |否，数据包在 wg0 解密后直接被 FORWARD 链拦截，无法转发到内网接口 |
...
| 外层 UDP 目的地址 | 192.0.2.1|
| 外层 UDP 目标端口 | 51820|
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` | wg show（conntrack 仅跟踪业务流量，wg show 才展示隧道握手、收发字节等隧道本身状态）|

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |client 发起 8080 请求握手可达，服务器回包无法回程转发，curl 超时失败 |缺少已建立连接回程放行策略，服务器回复数据包被 FORWARD 默认 DROP 丢弃 |sudo ip netns exec vpn iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 | 非法流量直接被拒绝，内核无对应拦截日志输出|iptables 自上而下匹配，REJECT 提前终止数据包，后方 LOG 规则永远不会匹配命中 | 调整顺序，将 LOG 规则插入至 REJECT 规则上方，先记录日志再拒绝数据包|
| 管理面规则错写到 `FORWARD` |client 访问 vpn 本机 9090 服务完全不受拦截，依旧能正常打开页面 |访问本机进程的流量走 INPUT 入站链，FORWARD 转发链规则对本机入站流量无匹配作用 | 删除 FORWARD 内错误拦截规则，在 INPUT 链添加 9090 端口 LOG+REJECT 防护规则|
...
答：AllowedIPs 作用：WireGuard 隧道封装层面，限定哪些内层 IP 流量可以通过隧道收发，不在范围内的流量不会被封装转发，属于隧道基础路由隔离；
iptables 作用：防火墙细粒度访问控制，可按端口、协议、连接状态、源目的地址精确放行 / 拒绝流量；
不能只靠 AllowedIPs：AllowedIPs 仅管控路由封装，无法区分同一网段下不同端口、不同服务；比如允许10.20.0.0/24后，客户端天然能访问该网段所有端口，无最小权限隔离，必须搭配 iptables 限制仅开放业务端口。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？

答：client -> server:8080流量路径：client→wg0 解密→vpn 网关转发→内网 server；
INPUT 链只匹配目标为本机 vpn 网关的数据包；
FORWARD 链专门处理跨设备转发流量（源 client、目的内网 server，不是网关本机进程），因此转发过滤必须配置在 FORWARD 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？

答：客户端 TCP SYN 握手包可以发送到内网 server，server 回复的 ACK 回程包没有放行规则，被 FORWARD 默认策略 DROP；三次握手无法完整建立，curl超时访问失败。

4. 为什么 LOG 必须放在 REJECT 前面？

答：iptables 规则自上而下顺序匹配，一旦匹配 REJECT 就会直接丢弃数据包、终止匹配流程；如果 LOG 在 REJECT 后方，数据包永远不会走到 LOG 规则，拦截行为无日志记录，无法审计排查。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？

答：安全意义：内层所有 TCP/ICMP 业务流量全程加密，底层抓包无法解析真实业务地址、端口、传输内容，中间人无法窃取业务数据；
可暴露信息：能识别 WireGuard 隧道存在、两端底层公网 IP、固定隧道端口 51820、数据包收发时长与流量大小（可分析传输行为）。


6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？

答：权限过大，一旦用户设备中毒、账号泄露，攻击者可横向渗透全内网所有业务、数据库、办公主机；
无端口隔离，任意漏洞、弱口令都能被利用横向扩散；
审计范围巨大，无法精准定位异常访问行为；
违背最小安全权限原则。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？

答：client -> vpn:9090：走INPUT 链；目标 IP 是 vpn 网关本机，访问网关本地运行的 web 管理服务，数据包交付本机进程；
client -> server:8080：走FORWARD 链；目标是另一台内网服务器，网关仅做流量转发，不属于本机入站业务。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？

答：NAT 是修改 IP / 端口地址转发，WireGuard 是完整 IP 数据包加密封装，不是地址改写；
conntrack 跟踪四层业务连接（TCP/ICMP），无法识别 WireGuard 加密隧道握手状态；
wg show才是隧道专属状态工具，能展示密钥、握手、隧道收发流量，conntrack 只能看到外层 UDP 封装包，无法管控隧道本身。

9.  如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？

答：来源限制：仅允许运维固定办公公网 IP、内网运维网段，禁止 VPN 隧道网段、公网全量开放；
日志配置：INPUT 链 SSH 规则前添加 LOG 规则，自定义日志前缀（如VPN-SSH-DENY），限流避免日志风暴，拒绝非法 SSH 访问并留存审计日志。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？

答：第一步：检查内网 server、网关服务监听状态（确认 8080 等服务正常监听对应地址端口）；
第二步：核对两端路由表，确认内层网段路由指向 wg0 接口；
第三步：查看 iptables FORWARD/INPUT 规则与计数器，验证放行规则是否命中；
第四步：抓取内核日志journalctl -k，查看 VPN-DENY 拦截日志定位拒绝原因；
第五步：核对规则 pkts/bytes 计数器，确认流量是否匹配预期放行 / 拦截规则。
