...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |curl 提示 Network is unreachable，无法建立连接 |
| `office → 10.0.3.2:8080` |成功 | 成功返回 HTTP 目录列表，显示当前目录下的文件|
...
| `filter` | `FORWARD` | 1 | ACCEPT | ctstate RELATED,ESTABLISHED| 放行所有已建立 / 相关的连接（回包）|
| `filter` | `FORWARD` | 2 | ACCEPT | dst=10.0.3.2 tcp dpt:8080 ctstate NEW| 放行目标为 dmz:8080 的新连接请求|
...
> 答：wan 收到的 HTTP 响应的源 IP 是防火墙的外网 IP 203.0.113.1。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条已建立连接的后续包，仍会按照原有的 DNAT 映射关系正常转发，不会因为 DNAT 规则的删除而中断。
...
> 答：LOG 规则在 DNAT 地址改写后执行，故记录改写后的内网地址。若要记录原始地址，需将 LOG 规则添加到 nat 表 PREROUTING 链（DNAT 规则之前）。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以。同一源 IP 短时间内连续访问大量不同目标端口，即为端口扫描特征。
...
| :--- | :------- |
| `wan` 地址 |203.0.113.2 |
| `fw` 四个接口地址 |203.0.113.1、10.0.1.1、10.0.2.1、10.0.3.1 |
| `wan → 203.0.113.1` ping 结果 | 成功，收到 2 个包，无丢包|
| `wan → 10.0.3.2` ping 结果 | 失败，提示 Network is unreachable|
| `office → 10.0.3.2` ping 结果 | 成功，收到 2 个包，无丢包|
| `office → 203.0.113.1` ping 结果 | 成功，收到 2 个包，无丢包|
...
| `wan → 203.0.113.1:3306` | 失败，curl 提示无法连接|
| `wan → 10.0.3.2:8080` | 失败，无法建立连接|
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 改写目标地址，发生在 nat 表的 PREROUTING 链，在路由决策之前执行。
SNAT 改写源地址，发生在 nat 表的 POSTROUTING 链，在路由决策之后执行。
当入站和出站都需要 NAT 时：
PREROUTING：负责入站方向的 DNAT，将公网访问请求的目标地址改写为 DMZ 服务器的内网地址。
POSTROUTING：负责出站方向的 SNAT，将 DMZ 服务器访问外网的请求源地址改写为防火墙的公网地址。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 规则会先 “抢到” 这个包。因为 PREROUTING 链的执行顺序在路由决策之前，请求会被改写为内网地址并转发，无法到达防火墙本地服务。
共存方法：
给本地服务绑定单独的 IP 地址，不使用 203.0.113.1。
给本地服务和 DNAT 映射使用不同端口，例如本地服务监听 203.0.113.1:8081，DNAT 映射 203.0.113.1:8080。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：依赖 conntrack 连接跟踪表。当外部请求建立连接时，conntrack 会完整记录连接的五元组信息，包括外部客户端地址、防火墙公网端口、对应的内网服务器地址和端口。
回复包返回时，防火墙会匹配 conntrack 中的记录，将服务器回复包的源地址，逆向还原为对应的公网 IP 和端口，确保外部客户端收到的回复，来自它最初请求的公网端口。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：这个包能回到 wan。
机制：DMZ 服务器的默认网关是防火墙，回复包会先发送给防火墙。防火墙的 conntrack 会根据 DNAT 记录，自动将回复包的源地址（10.0.3.2）逆向还原为防火墙的公网 IP（203.0.113.1），再转发给 wan，因此不需要额外配置 SNAT 规则。


5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：防火墙无法正常工作。
后果：无法新建任何连接，包括正常的业务请求，导致服务不可用。
防护措施：
调大 net.netfilter.nf_conntrack_max 参数，增加连接跟踪表的容量。
缩短连接超时时间，加快无效连接的回收。
配置单 IP 并发连接限制，防止单个 IP 占满连接表。
配置防攻击规则，过滤异常扫描和攻击包。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：四次实验的共同基础是 conntrack 连接跟踪机制。
对应关系：
Lab7（有状态防火墙）：大楼的访客身份核验与通行权限控制。
Lab8（SNAT）：内网人员外出时，统一伪装成大楼对外身份。
Lab9（分区审计）：大楼各区域的进出日志与行为审计。
Lab10（端口映射 / DNAT）：外部访客通过大楼前台，定向访问内部特定服务区域。
