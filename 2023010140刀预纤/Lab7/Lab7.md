...
| :--- | :--- |
| 只有第一条规则时是否成功 |失败 |
| 第一条规则放行的是什么方向的包 | lan → wan 方向、目的端口为 38070 的 TCP 包（客户端到服务端的请求包）|
| 失败的原因 |防火墙默认策略为 DROP，服务端返回的响应包（源端口 38070）没有被规则放行，无法回到客户端，导致连接超时 |
| 第二条规则匹配的是什么 |wan → lan 方向、源端口为 38070 的 TCP 包（服务端到客户端的响应包） |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 | 2条|
...
无状态的端口放行规则不区分连接方向，只能按源 / 目的端口放行所有匹配的包，会被外部主机利用源端口主动发起连接，存在安全隐患。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
   只允许内网主动访问外网，不允许外网主动访问内网。
无状态规则：必须同时放行请求和响应方向，无法区分 “主动请求的响应” 和 “外部主动发起的连接”，会被绕过。
有状态规则：通过 RELATED,ESTABLISHED 状态只放行内网请求的响应包，外部主动发起的新连接会被拒绝，能实现真正的访问控制。
...
1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
   因为 conntrack 会记录连接状态，当 lan 主动向 wan 发起连接并建立会话后，响应包会被标记为 RELATED 或 ESTABLISHED 状态，被第一条规则 --ctstate ESTABLISHED,RELATED -j ACCEPT 放行，无需单独写返回方向的规则。

2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
   无状态的 --sport 38070 只检查源端口，不区分包的方向和状态，因此 wan 可以用端口 38070 主动发起新连接。而有状态规则通过 --ctstate NEW 只允许 lan 侧发起的新连接，外部主动发起的 NEW 包会被默认策略 DROP，因此无法建立连接。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |src=10.0.0.2，sport=42400 |
...
1. 防火墙放行一个 `ESTABLISHED` 包时，依据的是包头里的字段，还是 conntrack 表里的记录？这两者有什么区别？
  ...
  答： 依据的是 conntrack 表里的连接记录，而不是单纯的包头字段。
区别：
包头字段只能看到当前包的源 / 目的 IP、端口，无法判断它是新连接还是已有连接的响应包。
conntrack 表会记录连接的完整状态（NEW/ESTABLISHED/RELATED等），防火墙根据表中的记录来放行响应包，而不是只看端口号。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：最本质的区别是是否维护连接状态信息。
无状态包过滤：只检查单个数据包的 IP、端口等五元组信息，不记录连接状态，无法区分新连接和已有连接的响应包。
有状态检测防火墙：通过连接跟踪（conntrack）模块维护连接状态表，能识别连接的建立、传输和关闭过程，区分 “新连接请求” 和 “已有连接的响应包”，因此安全性更高，规则更简洁。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：这个顺序是为了优先放行已有连接的流量，提升效率和安全性。
意义：ESTABLISHED,RELATED 匹配的是已建立连接的响应包，这类包数量多、出现频率高，放在第一条可以被快速匹配放行，减少后续规则的匹配次数；而 NEW 规则只匹配新连接请求，处理频率低，放在第二条即可。
不能调换：如果把 NEW 放在第一条，会导致所有新连接请求都先被匹配，再去匹配 ESTABLISHED,RELATED，效率更低；更关键的是，调换后规则逻辑没有错误，但会影响防火墙性能，不符合安全规则的最佳实践。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：
后续包会被标记为 INVALID 状态，因为防火墙的连接跟踪表中已经没有对应的会话记录了。
处理方式：默认情况下，INVALID 状态的包会被防火墙的默认策略 DROP 丢弃，连接会被强制中断，无法继续传输数据

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是和已有连接相关联的、独立的辅助连接流量，这类流量不是主连接本身，但依赖主连接而存在。
典型场景：FTP 协议的数据连接。FTP 的控制连接是一条主连接，而数据连接是由服务器主动发起的辅助连接，它和控制连接是关联的，因此会被标记为 RELATED 状态，被有状态防火墙放行。本实验用的是 TCP 直连，没有这类关联连接，所以观察不到

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：本质优势是从 “黑名单防御” 升级为 “白名单防御”，攻击面更小，安全性更可靠。
“默认拒绝，按需放行”：所有流量默认被阻断，只有明确被规则允许的流量才能通过，相当于建立了一个 “白名单”，未授权的流量（包括未知攻击）都会被直接阻断，即使规则有遗漏，也不会直接暴露风险。
“默认允许，事后补堵”：所有流量默认放行，再通过规则禁止危险流量，相当于建立了一个 “黑名单”，一旦有新的攻击方式未被规则覆盖，就会直接进入内网，存在很大的安全隐患。
