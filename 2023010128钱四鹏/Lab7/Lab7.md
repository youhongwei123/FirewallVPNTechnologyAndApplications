...
| 失败的原因 |wan 侧的响应包目的端口是客户端随机临时端口，不匹配 --dport 38070，被 FORWARD 链默认策略 DROP 丢弃 |
| 第二条规则匹配的是什么 |所有源端口为 38070 的 TCP 包，包括 wan → lan 的响应包 |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 |2条 |
...
无状态规则仅依据端口号放行流量，无法区分数据包是合法连接的响应还是外网主动发起的新连接，存在安全绕过漏洞。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
示例：内网客户端随机高端口访问外网网页服务，外网使用随机端口回传数据；无状态规则无法预判随机端口，只能大范围放行，而有状态规则可以基于连接跟踪自动放行响应流量。
![无状态规则](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2条 | 2条|
| 是否需要手动考虑返回方向 |需要手动配置双向端口规则 |不需要，由 conntrack 自动放行已建立连接的双向流量 |
| 能否区分"新连接"和"响应包" |不能，仅根据包头 IP 和端口判断 | 能，依靠 conntrack 连接状态区分|
| `wan` 能否主动发起到 `lan` 的连接 |可以伪造源端口绕过规则 | 不能，外网发起的 NEW 状态连接会被拒绝|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
响应包属于 conntrack 中 ESTABLISHED 已建立连接的状态，匹配 --ctstate ESTABLISHED,RELATED 规则自动放行，因此不需要单独配置返回方向的规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态规则依靠连接跟踪表记录连接发起方，仅放行内网主动发起连接的后续流量；外网主动发起的连接是全新的 NEW 状态，无跟踪记录，会被直接丢弃。而无状态规则的 --sport 38070 仅校验源端口，无法识别连接发起方向，容易被伪造端口绕过。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:59918 |
...
防火墙放行 ESTABLISHED 包时，依据的是 conntrack 连接跟踪表里的记录，而不是单纯的包头字段。
区别：仅看包头只能匹配固定的 IP、端口和协议；而 conntrack 能记录整条连接的生命周期、发起方向和状态，从而区分新连接和合法的响应流量。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤对每个数据包独立匹配规则，只依据 IP、端口、协议等包头信息，不记忆连接上下文；有状态检测防火墙维护内核连接跟踪表，记录每条连接的状态与生命周期，基于连接状态而非单个包头做访问控制。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：ESTABLISHED,RELATED 规则处理的是已建立连接的绝大多数流量，放在第一条可以优先匹配、提升转发性能；同时逻辑上先放行已有合法连接，再限制新连接入口。不可以调换，否则会先匹配所有新建请求，浪费系统性能且逻辑混乱。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续数据包会被标记为 INVALID 无效状态；在默认策略为 DROP 的情况下，防火墙会直接丢弃该报文，原有通信中断，客户端需要重新发起新连接。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已有主连接关联衍生的子连接或关联报文。典型场景：FTP 主动 / 被动模式的数据传输子通道、ICMP 网络不可达等差错报文、P2P 衍生连接等。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答："默认拒绝，按需放行" 遵循最小权限原则，仅开放业务必需的端口与合法连接，恶意扫描、漏洞攻击、未知服务默认无法穿透；而 "默认允许，事后补堵" 会开放全部流量，只能事后逐条封禁风险，容易遗漏安全规则，攻击面大，极易被入侵。
