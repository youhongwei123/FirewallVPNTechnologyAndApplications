...
| 是否能访问 `10.20.0.2:8080`  |    是，可以正常获取网页内容      |
...
| `latest handshake` 是否还在      |    是，握手记录正常存在      |
| 去 `10.20.0.0/24` 的路由是否消失 |    是，路由表无该网段指向 wg0 的条目      |
| 是否还能访问 `10.20.0.2:8080`    |    否，curl 连接超时失败      |
| 失败的直接原因                   |   路由表无去往 10.20.0.0/24 路由，数据包不会送入 wg0 隧道，无法进入 VPN 转发       |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
隧道握手依托底层 underlay 的 192.0.2.x UDP 链路，和 AllowedIPs 无关；业务数据转发依赖 AllowedIPs 生成路由 + 白名单校验，二者是两套独立路径，因此握手正常但业务不通。
截图：
...
| 协议                     |         UDP（51820 端口）                           |     ICMP/HTTP     |
| 是否能看到 ICMP/HTTP 内容 |                       否，全部加密为二进制密文             |     是，明文原始报文     |
| 为什么内外两层看到的不同 |         underlay 是 WireGuard 加密封装后的外层 UDP 报文；wg0 是解密剥离封装后的原始内层 IP 报文                           |          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
外层 underlay=信封（仅封装收发主机底层 IP，信封内部内容被加密）；内层 wg0=信纸（原始通信 IP 与业务明文数据）。安全性：①隐藏真实内网通信地址；②业务传输数据全程加密，链路窃听者无法窃取报文内容。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：AllowedIPs 两件作用：①出口路由控制：wg-quick 自动把网段写入本机路由表，目标匹配网段的流量全部转发至 wg0 隧道接口；②入站源 IP 白名单：对端解密后的内层数据包，源 IP 不在 AllowedIPs 网段则直接丢弃。
因为 wg-quick 启动时会优先根据 AllowedIPs 自动生成操作系统路由条目，路由决定数据包是否进入隧道，因此它优先作用路由，表现看似访问控制实则底层修改路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手使用 Endpoint 配置的底层 underlay 公网 IP（192.0.2.x），仅完成密钥协商，不受 AllowedIPs 影响；业务数据转发依赖 AllowedIPs 生成路由 + 入站白名单校验，受 AllowedIPs 配置控制。二者传输寻址地址、校验规则完全独立，所以握手正常连通但业务访问失败。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：AllowedIPs=0.0.0.0/0代表本机所有 IP 流量全部进入 VPN 隧道。风险：①本机访问互联网、局域网全部强制走 VPN，VPN 带宽被占满；②本地内网资源无法直连；③一旦 VPN 网关故障，本机全网断网；④所有本机流量被 VPN 网关接管，存在隐私泄露风险。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能冒充客户端建立隧道。WireGuard 是双向非对称密钥验签：客户端必须使用自身私钥对报文签名，VPN 网关使用预先配置的客户端公钥校验签名。攻击者仅有 VPN 私钥，无法伪造客户端私钥生成合法签名，网关验签失败直接丢弃报文。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层抓包源地址：192.0.2.2（client 底层物理网卡 IP）；内层内容被 WireGuard 采用加密算法整体封装加密，链路抓包只能获取加密后的密文，没有对应私钥无法解密，因此看不到真实内层 IP 与业务数据。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①身份隐藏安全：外层仅暴露底层公网 IP，内网真实通信 IP 被封装隐藏，中间人无法获知两端内网地址；
②数据传输加密安全：原始报文整体加密封装，链路抓包无法解析明文业务数据，防止传输窃听。
