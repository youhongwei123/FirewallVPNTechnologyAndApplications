...
| client 路由表中的 `wg0` 路由 |10.20.0.0/24 dev wg0 |
| 是否看到 `latest handshake` |是，显示 1 minute, 14 seconds ago |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |-m conntrack --ctstate ESTABLISHED,RELATED |放行服务器返回的应答流量 |
| 2 |ACCEPT |-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 -p tcp --dport 8080 --ctstate NEW |仅允许 VPN 客户端访问 server:8080 |
| 3 |LOG |-i wg0 -m limit --limit 5/min --log-prefix "VPN-DENY: " |记录所有被拒绝的 VPN 转发流量 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |第 2 条 ACCEPT |是，pkts 计数增加 |
| `client -> server:2222` |失败 |第 3 条 LOG + 第 4 条 REJECT |是，LOG 和 REJECT 计数均增加 |
| `client -> server ping` |失败 |第 3 条 LOG + 第 4 条 REJECT |是，LOG 和 REJECT 计数均增加 |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |成功 |失败 |是，产生 VPN-MGMT-DENY 日志 |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |流量穿过 VPN 网关，转发到内网服务器 |
| `client -> server:2222` | `FORWARD` |流量穿过 VPN 网关，转发到内网服务器 |
| `client -> vpn:9090` | `INPUT` |流量目的地是 VPN 网关本机进程，不转发 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |client -> server:8080 超时失败 |服务器返回的应答流量无规则匹配，被默认 DROP |重新添加 ESTABLISHED,RELATED 放行规则 |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但看不到对应日志 |REJECT 会终止规则匹配，后续 LOG 规则不会执行 |调整规则顺序：LOG 在前，REJECT 在后 |
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 仍然可以访问 |访问 VPN 网关本机的流量走 INPUT 链，FORWARD 规则不生效 |将管理面拦截规则迁移到 INPUT 链 |
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |iptables 规则放行该端口 |
| `client -> server:2222` |失败 |被 REJECT 规则拒绝 |
| `client -> server ping` |失败 |被 REJECT 规则拒绝 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条（每次非法访问生成一条） |
| 日志中被拒绝的目标端口 |2222 |
| underlay 抓包看到的协议 |UDP 51820 |
| `wg0` 抓包看到的源地址 |10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 |否，流量在 VPN 网关被拒绝，未转发到内网 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |合法端口也无法访问、超时 |服务器应答流量被默认 DROP |重新添加状态放行规则 |
| `LOG` 放在 `REJECT` 后面 |流量被拒绝，但无日志 |REJECT 终止匹配流程，LOG 规则不执行 |调整规则顺序，LOG 在前 |
| 管理面规则错写到 `FORWARD` |仍可访问网关管理端口 |访问本机走 INPUT 链，FORWARD 规则无效 |将规则迁移到 INPUT 链 |
...
AllowedIPs：是 WireGuard 层面的路由与隧道准入控制，仅决定哪些网段的流量被加密并送入隧道，只能做粗粒度的网段级放行。
iptables：是 Linux 内核层面的四层精细化访问控制，可以基于源 / 目的 IP、端口、协议、连接状态做细粒度的放行、拒绝和日志审计。
原因：AllowedIPs 无法区分同一网段内的不同端口和服务，一旦放行整个内网网段，VPN 用户就能访问所有服务，权限完全失控，无法实现最小权限原则。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
因为 client -> server 的流量路径是 client → vpn 网关 → server，流量的最终目的地是内网服务器，不是 VPN 网关本机，因此会经过 FORWARD 链（转发链）处理；而 INPUT 链仅处理目的地是网关本机的流量，无法拦截转发流量。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
客户端发起的 SYN 请求能命中 8080 端口的 ACCEPT 规则，被正常转发到服务器；但服务器返回的 SYN-ACK 应答流量方向是 server → vpn → client，不匹配 8080 端口的规则，会被默认 DROP，最终导致 TCP 连接超时失败。
4. 为什么 LOG 必须放在 REJECT 前面？
iptables 规则是按顺序逐条匹配的，一旦命中 REJECT 规则，内核会直接拒绝流量并终止当前链的匹配流程，后面的所有规则都不会执行。如果 LOG 放在 REJECT 后面，永远没有机会记录被拒绝的流量日志。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
安全意义：内层的 IP 地址、端口、应用数据全部被加密，中间人无法窃取或篡改业务流量，保证了传输层的机密性和完整性。
暴露的信息：能看到两端的公网 IP 地址、UDP 51820 端口的通信行为，攻击者可据此识别出部署了 WireGuard VPN 服务，进而针对性发起扫描或攻击。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
远程用户获得了内网的全量访问权限，一旦用户终端被入侵，攻击者可直接横向渗透整个内网，访问数据库、运维管理系统、敏感业务服务器，造成大规模数据泄露、服务器被控、业务中断等严重安全事件。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
client -> server:8080：经过 FORWARD 链，因为流量的最终目的地是内网服务器，需要 VPN 网关转发。
client -> vpn:9090：经过 INPUT 链，因为流量的目的地是 VPN 网关本机的 9090 端口进程，不需要转发。
区分的核心是：流量的最终目的地是否为网关本机。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
WireGuard 是独立的加密隧道协议，负责报文的加解密、握手和隧道维护，状态由 wg 组件管理，和内核的 conntrack 模块无关。
conntrack 仅跟踪解密后的内层业务连接（如 TCP 8080 连接），不管理外层的 WireGuard UDP 隧道；而 NAT 映射是 conntrack 模块的地址转换功能，两者的工作逻辑、管理对象完全不同。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
访问控制：仅允许企业内部运维网段、堡垒机 IP 访问，拒绝所有来自 VPN 隧道（wg0 接口）的访问。
日志规则：在 INPUT 链中，先添加 LOG 规则记录所有访问 SSH 端口的流量，再添加 REJECT 规则拒绝 VPN 来源的访问，日志前缀可设为 VPN-SSH-DENY:，便于后续审计。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
检查内网服务器服务是否正常监听端口（如 ss -lntp）；
检查客户端和服务器的路由表，确认 VPN 隧道路由和默认网关配置正确；
检查 VPN 网关的 FORWARD 链规则，确认是否放行目标端口；
查看 iptables 规则计数器，判断流量是否命中了拒绝规则；
查看内核日志，确认是否产生了拒绝日志，定位被拦截的流量类型。
