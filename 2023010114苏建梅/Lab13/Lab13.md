...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |-i veth-gwa-lan -o wg0 -s 10.1.0.0/24 -d 10.2.0.2 -p tcp --dport 8080 -m conntrack --ctstate NEW -j ACCEPT | 	A-TO-B-DENY:|DROP |
| `gwB` |-i wg0 -o veth-gwb-lan -s 10.1.0.0/24 -d 10.2.0.2 -p tcp --dport 8080 -m conntrack --ctstate NEW -j ACCEPT |VPN-S2S-DENY: |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 | 成功| 计数器显示1包通过|
| `siteA -> siteB:2222` | 失败 |失败 | 计数器显示1包被LOG+REJECT|
| `siteA -> siteB ping` | 失败 |失败 | 计数器显示3包被LOG+REJECT
|
...
| `latest handshake` |	11 seconds ago |	11 seconds ago |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功|gwA规则2 / gwB规则2（ACCEPT） |
| `siteA -> siteB:2222` | 失败| 	gwA规则3（LOG）/ 规则4（REJECT）|
| `siteA -> siteB ping` | 失败| gwA规则3（LOG）/ 规则4（REJECT）|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay | 	192.0.2.1 ↔ 192.0.2.2|UDP 51820 |	外层加密包（WireGuard封装） |
| `wg0` | 	10.1.0.2 ↔ 10.2.0.2| TCP 8080| 解密后的跨站点内层包（真实业务流量）|
| B 站点 LAN | 	10.1.0.2 ↔ 10.2.0.2|TCP 8080 |转发到 B 站点内网的包 |
...
答：远程接入VPN是单个用户设备接入内网，站点到站点VPN是两个完整网络之间的互联互通。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：AllowedIPs告诉WireGuard哪些目标IP应该通过隧道发送，站点到站点中写对端内网网段，这样发往对端内网的流量才会被加密并送到对端网关。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：网关需要把从一个接口收到的包转发到另一个接口，没有IP转发功能包就无法在网段间传递。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：普通主机必须把默认路由指向本地网关，否则跨网段流量会走默认路由发往错误方向，根本到达不了VPN隧道。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：先看wg show有没有握手，再看路由表有没有指向wg0的对端网段路由，然后检查iptables FORWARD链是否允许转发，最后抓包确认流量是否真的进入了隧道。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：两边都写规则是纵深防御，即使一边配置错误另一边仍能保护；只在一边写也可以，但单点故障风险更高，且无法防止内部误配置。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答：VPN隐藏了内网IP、应用层数据等内部信息，但仍暴露了网关的公网IP、UDP端口51820和流量大小及时序特征。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答：需要添加允许siteB访问siteA服务端口的规则，并确保反向路径的ESTABLISHED连接被允许，同时在两个网关上都配置对应的ACCEPT规则。
