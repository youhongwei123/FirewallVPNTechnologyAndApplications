...
| :--- | :--- |
| 只有第一条规则时是否成功 | 失败|
| 第一条规则放行的是什么方向的包 | 	前往（目标端口）方向的包（出站/请求）|
| 失败的原因 | 	只放行了请求包，没有放行回应的包（源端口为38070的返回包被丢弃）|
| 第二条规则匹配的是什么 |	源端口为38070的TCP包（回应包/入站包） |
| 加第二条规则后是否成功 |	成功 |
| 当前规则共几条 |2条 |

简答题：

1. 第二条规则 `--sport 38070 -j ACCEPT` 放行的是"所有源端口为 38070 的 TCP 包"，不区分这个包是响应还是新发起的连接。如果 `wan` 侧有主机想主动连接 `lan`，只需要让程序从端口 38070 发起连接，这条规则就会放行它的 SYN 包。用一句话概括这个问题的本质：这个问题的本质是：基于端口匹配的过滤规则无法区分TCP连接的方向（主动发起还是被动响应），从而导致规则可能放行非预期的反向连接。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
一个典型的场景是：只允许内网用户访问外网的网页服务（TCP 80端口），但不允许外网主动发起连接进入内网。**

- 无状态规则：如果只放行 `--dport 80`，外网返回的响应包（源端口 80）会被拦截，导致网页无法打开。如果同时放行 `--sport 80`，就无法阻止外网主机主动从 80 端口向内网发起连接（例如恶意程序伪装成网页响应）。
- 有状态规则：防火墙可以记录“内网主动发起的连接”状态，只放行与已有连接相关的响应包，同时阻止任何外网主动发起的 SYN 包，无论其源端口是多少。
![无状态规则](stateless.png)
...
| 是否需要手动考虑返回方向 | 是|否 |
| 能否区分"新连接"和"响应包" |否 | 能|
| `wan` 能否主动发起到 `lan` 的连接 | 能|不能 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为 conntrack 记住了 lan → wan 发起的连接，当 wan 返回的包匹配这条已有连接时，会被标记为 ESTABLISHED 状态，从而匹配第一条放行规则，所以不需要单独写“返回方向”规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
因为有状态规则能区分“连接的方向”（是内网主动发起还是外网主动发起），而无状态规则只看端口，无法区分。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |	10.0.0.2:43726 |
...
有状态防火墙放行一个 `ESTABLISHED` 包时依据的是 conntrack 表里的记录，而不是包头里的字段。这两者的核心区别在于，包头字段只能描述单个包本身的属性，比如源地址、目标地址、端口和标志位，但它无法告诉你这个包是否属于一个已经完成握手的合法连接；而 conntrack 表记录了整个连接的历史状态，防火墙通过查表才知道这个包是某个已有对话的后续部分。换句话说，一个攻击者可以轻松伪造一个带 ACK 标志的包头字段，但只要 conntrack 表里没有对应的连接记录，有状态防火墙就不会把它当作 ESTABLISHED 包放行。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤和有状态检测防火墙最本质的区别在于：无状态过滤只看单个数据包的头部信息（如源IP、目标IP、端口号），独立判断每个包是否通过，完全不关心这个包与前后包之间的关系；而有状态检测会维护一个连接跟踪表，记录所有连接的状态（如NEW、ESTABLISHED、RELATED），根据这个包是否属于一个已有连接来决定是否放行。简单来说，无状态防火墙像是一个只看证件照片的保安，每个来的人都单独检查；而有状态防火墙像是一个记住了谁已经登记入住的酒店前台，能区分第一次来的客人和已经办过入住的回头客。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：这个顺序是有意义的，并且不能随意调换。
如果调换顺序，让 `NEW` 规则在 `ESTABLISHED,RELATED` 之前，本身不会导致功能错误，因为在 iptables 中只要匹配规则就会执行。但将 `ESTABLISHED,RELATED` 放在第一条是最佳实践，其意义在于：绝大多数经过防火墙的包都属于已建立的连接，把它们放在第一条可以让这些包快速匹配并放行，从而提升防火墙的处理效率，减少不必要的规则遍历。如果反过来把 `NEW` 放在第一条，那么每个 `ESTABLISHED` 的包都要先检查是否匹配 `NEW` 规则（显然不匹配），然后再去匹配第二条规则，这就多了一次无意义的检查。虽然对单个包来说影响微乎其微，但在高流量场景下，这种顺序的优化会显著影响防火墙的性能表现。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：如果 conntrack 条目因超时被删除，属于这条连接的后续包到达防火墙时，由于在连接跟踪表中找不到匹配的记录，它会被视为一个 `NEW` 状态的包。防火墙会如何处理这个包，完全取决于规则配置。如果在有状态规则中，`NEW` 只放行了特定方向（如目标端口 38070）的包，而后续包很可能不满足这个条件（比如它是一个源端口 38070 的响应包，或者是连接内的普通数据包），那么它就不会匹配任何 `NEW` 规则，最终被默认策略（通常是 `DROP`）拒绝。这种情况下，原本正常的连接就会中断。这解释了为什么需要合理设置 conntrack 的超时参数：超时太短会导致正常的长空闲连接被误断；超时太长则可能耗尽 conntrack 表空间。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：`RELATED` 状态主要描述的流量是：与一个已有连接相关，但本身属于一个新的独立连接。典型的例子是 FTP 或 ICMP 错误报文这类需要“额外通道”或“辅助连接”的协议。在常见的网络过滤实验中没有观察到它，是因为实验用的是简单的 HTTP 请求，只有一个独立的 TCP 连接，不存在相关联的子连接或辅助通道。`RELATED` 出现的典型场景包括：FTP 主动模式的数据连接（控制连接用 21 端口，服务器会主动从 20 端口向客户端发起一个新的数据连接，这个新连接的状态就是 `RELATED`）；FTP 被动模式的数据连接**（客户端发起新的数据连接，同样关联到已有的控制连接）；ICMP 差错报文（比如“端口不可达”消息，它是针对某个已有 UDP 或 TCP 连接返回的错误，状态也被标记为 `RELATED`）；以及 SIP、H.323 等多媒体协议中动态协商出来的 RTP 媒体流端口。在这些场景下，管理员只需要放行 `ESTABLISHED,RELATED`，无需手动开放大量随机端口，就能让辅助连接正常工作。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：“默认拒绝，按需放行”相比“默认允许，事后补堵”在安全上的本质优势在于：前者假设所有流量都是恶意的，只有明确被授权的才能通过，因此任何未知或新出现的攻击流量都会被自动阻挡；而后者假设所有流量都是善意的，只有当攻击发生并被发现后才会去封堵，这中间存在一个暴露窗口。换句话说，默认拒绝是“先验证后放行”，从未信任开始；默认允许是“先放行后追责”，从完全信任开始，而信任一旦被滥用，往往已经造成了损失。这就是安全设计中的“白名单优于黑名单”原则：白名单虽然需要提前规划，但它能防御未知威胁；黑名单永远滞后于攻击，只能应对已知问题。
