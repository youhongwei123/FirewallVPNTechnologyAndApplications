...
| ping 是否有回应 |无回应 |
| `wan` 收到的请求源地址 |无捕获 |
| `wan` 无法回复的原因 |防火墙阻止了流量。从 fw 命名空间的 tcpdump 输出为空（0 packets captured）可以看出，ICMP 请求根本没能从 lan 通过网络命名空间 fw 转发到 wan，说明数据包在防火墙（fw）上被丢弃了。 |
...
在 Lab8 中，conntrack 除了记录连接状态外，还额外记录了传输层协议的序列号（Sequence Number）和确认号（Acknowledgment Number）。这两个功能都依赖 conntrack，说明 conntrack 不仅能用于 NAT 和状态防火墙，还能用于更精细的 TCP 流量追踪，从而支持更安全、更智能的过滤和转发策略。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
当两台内网主机同时访问同一个外网服务器的同一个端口时，NAT 通过修改源端口号来区分不同的连接。具体过程是：NAT 将内网主机 A 的源 IP 10.0.0.2:12345 和外网请求映射为 192.0.2.1:34567，将内网主机 B 的源 IP 10.0.0.3:12345 映射为 192.0.2.1:34568，这样即使目标服务器回复时目的地址都是 192.0.2.1，NAT 也可以根据回复包中的目的端口号（34567 或 34568）反向查找连接表，将包还原给对应的内网主机。也就是说，NAT 使用 IP 地址加端口号的四元组（源 IP、源端口、目标 IP、目标端口）来唯一标识每一条连接。
...
| TCP 38070 访问是否成功 | 成功|
| ping 是否成功 | 失败|
| ping 失败的原因 | 防火墙 FORWARD 链默认策略为 DROP，且只放行了 TCP 端口 38070 的新连接以及状态为 ESTABLISHED,RELATED 的包，没有放行 ICMP 协议。|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址（如 10.0.0.0/8）的主机通常无法被互联网直接访问，是因为互联网上的路由器不会转发目标地址为私有 IP 地址的数据包，这些地址在公网中是不可路由的。NAT 通过在网关设备上将私有源地址转换为公网地址，使得内网主机能够访问互联网，同时当公网服务器回复时，NAT 根据记录的状态表再将回复包的目的地址转换回对应的私有地址，从而实现了内网主机对互联网的访问。但由于 NAT 转换表通常是由内网主动发起连接时才建立，外网主机无法主动向内网私有地址发起连接，这就天然阻挡了外部直接访问内网主机的请求。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：在实际场景中，选择 MASQUERADE 还是 SNAT 主要取决于网关的出接口 IP 地址是否固定。SNAT --to-source 192.0.2.1 适合网关拥有固定公网 IP 的场景，比如数据中心服务器或企业专线。因为源地址是固定的，转换效率更高，且便于日志审计和外部服务做 IP 白名单。MASQUERADE 适合网关 IP 不固定的场景，比如家庭宽带通过 PPPoE 拨号获取动态 IP。MASQUERADE 会自动检测当前出口网卡的 IP 并进行转换，不需要像 SNAT 那样在 IP 变化时手动修改规则。其代价是性能比 SNAT 稍低，因为每次都要自动获取 IP。简单总结：IP 固定用 SNAT，IP 会变用 MASQUERADE。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT 给依赖网络层信息嵌入应用层数据的协议带来了麻烦，比如 FTP、SIP、IPsec。原因是这些协议在应用层载荷中携带了 IP 地址和端口号信息，而 NAT 只修改数据包的 IP 头和 TCP/UDP 头，不修改应用层内容。当内网主机主动通信时，对端收到的应用层数据中仍然包含内网私有地址，导致对端无法正确建立后续连接或会话。解决这类问题通常需要配合应用层网关或 ALG 来同步修改应用层中的地址信息。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：家用路由器通常把状态检测和 NAT 集成在一起，是因为两者都依赖 conntrack 来追踪连接状态。conntrack 能够记录每个连接的源地址、目的地址、端口等信息，NAT 需要利用这些信息来修改地址并在回复时还原，而状态检测防火墙需要知道连接处于 NEW、ESTABLISHED 还是 RELATED 状态来决定是否放行数据包。既然两者都需要同一个连接追踪表，集成在一起可以避免重复维护两套状态表，节省家用路由器有限的硬件资源，同时提高处理效率。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在保留现有规则的基础上，添加一条允许 ICMP 的规则即可：

```bash
sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
```

这条规则会放行所有 ICMP 流量。如果你只想允许特定的 ICMP 类型（比如只允许 echo request 和 echo reply），可以写成：
```bash
sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -j ACCEPT
sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-reply -j ACCEPT
```

由于现有的 FORWARD 链默认策略是 DROP，并且已经有一条放行 ESTABLISHED,RELATED 的规则，新增的 ICMP 规则放在后面就能生效。建议使用 `-I` 插入到合适位置，或者按顺序添加即可。
