...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 | 	ACCEPT| 	ctstate RELATED,ESTABLISHED|	放行已建立的连接和相关连接（响应流量） |
| 2 |	ACCEPT |	-i wg0 -o veth-vpn-lan -s 10.10.10.1 -d 10.20.0.2 -p tcp --dport 8080 --ctstate NEW |只允许 VPN 客户端（10.10.10.1）访问 server（10.20.0.2）的 8080 端口 |
| 3 | 	LOG| 	-i wg0 -m limit --limit 5/min|记录被拒绝的 VPN 访问，限制速率避免日志泛滥，前缀 "VPN-DENY: " |
| 4 |	REJECT | -i wg0|	拒绝来自 VPN 的其他所有转发流量 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |规则1 + 规则2 | 是|
| `client -> server:2222` |失败 |规则3 + 规则4 | 是|
| `client -> server ping` |失败 |规则3 + 规则4| 是|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` | 流量穿过 VPN 网关，转发到内网 server，目的地不是网关自身|
| `client -> server:2222` | `FORWARD` |流量穿过 VPN 网关，转发到内网 server，目的地不是网关自身 |
| `client -> vpn:9090` | `INPUT` |	流量目的地是 VPN 网关自身的 IP 地址，需要本机进程处理 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |curl 8080 失败，ping 正常 | client 发出的 SYN 包能通过规则2，但 server 返回的 SYN+ACK 响应包不匹配任何允许规则（不是 NEW，也不是 ESTABLISHED/RELATED），被默认 DROP| 重新插入 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 作为规则1|
| `LOG` 放在 `REJECT` 后面 |curl 3333 失败，但 journalctl 无 WRONG-ORDER 日志 | iptables 规则顺序匹配：REJECT 在前，包命中后立即终止，后面的 LOG 规则没有机会执行| 删除错误顺序的规则，确保 LOG 在 REJECT 之前|
| 管理面规则错写到 `FORWARD` |curl 9090 仍然成功，WRONG-CHAIN 规则计数器为 0 |目的地是 vpn 本机（10.10.10.2）的包走 INPUT 链，FORWARD 链规则无法拦截 | 删除 FORWARD 链中的错误规则，恢复 INPUT 链的正确规则|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功| FORWARD链规则2允许源10.10.10.1目的10.20.0.2端口8080的新连接，规则1允许响应流量|
| `client -> server:2222` |失败 |端口2222没有允许规则，被规则4 REJECT，规则3记录日志 |
| `client -> server ping` | 失败|ICMP协议没有允许规则，被规则4 REJECT，规则3记录日志 |

### D. 日志与抓包

| 项目 | 你的填写 |
| :--- | :--- |
| `VPN-DENY` 日志条数 |多条（ping和2222访问均有记录） |
| 日志中被拒绝的目标端口 |2222 |
| underlay 抓包看到的协议 |UDP 51820 |
| `wg0` 抓包看到的源地址 |	10.10.10.1 |
| 内网接口是否能看到被拒绝的 2222 连接 | 否，被FORWARD链拒绝后不会到达内网接口|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` | client访问server:8080超时失败，ping成功|client发出的SYN包匹配允许规则，但server返回的SYN+ACK响应包不匹配任何规则被默认DROP | 重新插入-m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT作为FORWARD链第一条规则|
| `LOG` 放在 `REJECT` 后面 |client访问端口3333被拒绝，但dmesg/journalctl中看不到WRONG-ORDER日志 | iptables规则顺序匹配，包先命中REJECT后处理立即终止，后面的LOG规则没有机会执行|删除错误顺序的规则，确保LOG规则放在REJECT规则之前 |
| 管理面规则错写到 `FORWARD` |client访问vpn:9090仍然成功，FORWARD链中WRONG-CHAIN规则计数器为0 | 目的地为vpn本机地址的流量走INPUT链而非FORWARD链，FORWARD链规则无法拦截|删除FORWARD链中的错误规则，在INPUT链正确添加拦截规则 |
...
答：AllowedIPs控制哪些流量进入WireGuard隧道，解决的是路由选择问题；iptables控制进入隧道后的流量能否被转发或访问，解决的是权限控制问题。只靠AllowedIPs无法限制端口或协议，VPN用户拿到隧道IP后可以访问内网任意服务的任意端口。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：client访问server的流量是穿过VPN网关转发到另一台主机，这种“经过但不终结于本机”的流量由FORWARD链处理；INPUT链只处理目的地为网关自身的流量。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：访问server:8080会出现连接超时或失败。因为client发出的SYN包能匹配允许规则，但server返回的SYN+ACK响应包不匹配任何允许规则，被默认DROP。
4. 为什么 LOG 必须放在 REJECT 前面？
答：iptables规则按顺序匹配，包命中REJECT后处理立即结束，后面的LOG规则没有机会执行。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：外层加密使攻击者无法看到内层IP、端口、协议和HTTP内容，通信内容得到保护。但攻击者仍能观察到通信双方的存在、通信时间、加密包的大小和频率，从而推断出通信模式。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：可能的风险包括：内网中的SSH、数据库、Redis、RDP等服务直接暴露给VPN用户，任意一台内网服务器被入侵后可以横向移动到整个内网，敏感数据可能被大量窃取，内网漏洞更容易被利用。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client到vpn:9090走INPUT链，因为目的地是VPN网关自身的IP地址；client到server:8080走FORWARD链，因为流量穿过VPN网关转发到内网服务器，不终结于网关自身。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：NAT是地址转换，改变包的源或目的地址；WireGuard是加密隧道，内层包被完整封装在外层UDP中，地址保持不变。conntrack分别跟踪外层UDP和内层TCP两种独立连接，WireGuard隧道状态应用wg show查看。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：应该只允许特定管理员IP或跳板机IP访问SSH端口，在INPUT链写规则：先LOG记录所有SSH访问尝试，再用-j ACCEPT放行允许的来源，最后REJECT其他来源。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答：先确认server端服务是否监听在正确的IP和端口，然后检查client和vpn的路由表确保往返路径正确，查看防火墙规则计数确定哪条规则匹配了流量，检查dmesg或journalctl中的LOG日志，最后观察iptables规则计数器增加情况判断包是否到达预期链。
---
