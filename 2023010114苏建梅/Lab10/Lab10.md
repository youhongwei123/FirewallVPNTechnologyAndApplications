...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 | curl: (7) Failed to connect to 10.0.3.2 port 8080 after 0 ms: Could not connect to server|
| `office → 10.0.3.2:8080` |成功 |返回 HTML 目录列表，包含 .bashrc、.config、.mozilla 等文件/文件夹 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED | 放行已建立连接的回包|
| `filter` | `FORWARD` | 2 | ACCEPT | dst=10.0.3.2 tcp dpt:8080 ctstate NEW|放行 DNAT 后的新连接 |
...
| :--- | :--- | :--- |
| 请求包的源地址 |203.0.113.2:34920 | 203.0.113.2:34920|
| 请求包的目的地址 |	203.0.113.1:8080 | 	10.0.3.2:8080|
| 回复包的源地址 |	203.0.113.1:8080 | 	10.0.3.2:8080|
| 回复包的目的地址 |203.0.113.2:34920 | 203.0.113.2:34920|
...
> 答：wan 的 curl 收到的 HTTP 响应，源 IP 是 203.0.113.1。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包仍然可以正常通过，因为 conntrack 模块会接管包的转换，不再需要重新查找 NAT 规则。
...
> 答：说明 LOG 规则是在 DNAT 完成地址转换之后触发的，即位于 FORWARD 链，如果要记录原始目标地址，应该把 LOG 规则放在 `nat` 表的 `PREROUTING` 链中，并且要在 DNAT 规则之前执行，这样才能在地址转换前捕获原始的 `203.0.113.1` 目标地址。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：是的，LOG日志能帮助安全人员发现端口扫描行为。从日志中识别端口扫描行为的方法包括：短时间内出现大量来自同一源IP地址、发往同一目标但不同端口的日志记录，或者日志中出现连续递增的目标端口号，同时每条日志的SYN标志位为1但没有后续的ESTABLISHED或RELATED连接记录，这些模式表明攻击者正在对防火墙进行系统性的端口探测。此外，如果日志中出现针对多个连续端口但都没有命中DNAT规则（即没有对应的ACCEPT规则）的连接尝试，也典型地指示端口扫描行为，安全人员可以通过统计单位时间内同一源IP触发LOG规则的次数并设置阈值来触发告警。
...
| `fw` 四个接口地址 |	10.0.1.1/24, 10.0.2.1/24, 10.0.3.1/24, 203.0.113.1/24 |
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat | 	PREROUTING| dst=203.0.113.1 tcp dpt:8080| DNAT --to-destination 10.0.3.2:8080|
| DNAT 2222 | nat|	PREROUTING | dst=203.0.113.1 tcp dpt:2222| 	DNAT --to-destination 10.0.3.2:2222|
| FORWARD 8080 |filter |FORWARD |dst=10.0.3.2 tcp dpt:8080 ctstate NEW	 | ACCEPT|
| FORWARD 2222 |filter |FORWARD |dst=10.0.3.2 tcp dpt:2222 ctstate NEW |ACCEPT |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 PREROUTING 链（路由决策前）改写目标地址，SNAT 在 POSTROUTING 链（路由决策后）改写源地址。对于 DMZ 既要对外提供服务又要访问外网的场景，PREROUTING 负责将外部请求的目标地址改为 DMZ 内网 IP，使流量能进入 DMZ；POSTROUTING 负责将 DMZ 主动访问外网的源地址改为防火墙外网 IP，使回包能正确返回。两者配合 conntrack 模块，保证双向流量正常传输。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：按照标准的 Netfilter 处理流程，DNAT 规则会先于本地服务“抢到”这个包。因为 DNAT 在 PREROUTING 链完成地址改写，而此时路由决策尚未发生；包的目标地址被改为内网 IP（如 `10.0.3.2:8080`）后，路由模块会判断这是一个需要转发的包，从而送入 FORWARD 链，永远不会进入 INPUT 链交给本地服务。要让防火墙自身的 Web 服务和端口映射共存，必须让两者监听不同的端口。常见做法是：保持 DNAT 规则映射 `203.0.113.1:8080` 到内网服务器，而让防火墙自身的 Web 服务改到其他端口（如 `8081`）监听，或者反过来将映射端口改为其他值（如 `80` 映射到内网 `8080`）。因为 Netfilter 在同一端口上无法区分“发给本机”还是“需要转发”——DNAT 先改写目标地址，包就再也不属于本机了。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：conntrack 在建立连接时已经记录了完整的五元组映射关系，包括协议、源地址、源端口、目标地址、目标端口。当外部客户端访问 `203.0.113.1:8080` 时，conntrack 会记录原始方向是 `客户端IP:随机端口 → 203.0.113.1:8080`，改写后是 `客户端IP:随机端口 → 10.0.3.2:8080`，并将这个映射保存。当 `10.0.3.2:8080` 返回响应包时，conntrack 看到响应包的源地址是 `10.0.3.2:8080`、目的地址是 `客户端IP:随机端口`，立刻查找之前保存的映射表，找到对应的原始五元组，然后将响应包的源地址还原为 `203.0.113.1:8080`、目的地址保持不变。由于映射表中记录了完整的端口信息，即使有多条不同端口映射，conntrack 也能根据响应包的五元组精确匹配到对应的映射条目，从而正确还原到正确的公网端口。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：能回到 `wan`。这个包能正确返回依靠的是 **conntrack 的自动逆向还原**，而不是额外的 SNAT 规则。具体过程如下：当外部客户端的请求经过 `fw` 时，DNAT 规则将目标地址 `203.0.113.1` 改写为 `10.0.3.2`，同时 conntrack 模块记录了这个映射关系（原始方向：`wan客户端 → 203.0.113.1`，改写后：`wan客户端 → 10.0.3.2`）。当 `dmz` 服务器返回响应包时，包的源地址是 `10.0.3.2`，目的地址是 `wan` 客户端。这个响应包到达 `fw` 后，conntrack 根据之前保存的映射表识别出它是已建立连接的一部分，自动将源地址逆向还原为 `203.0.113.1`，然后才转发出去。因此 `wan` 客户端收到的响应源地址是 `203.0.113.1`，完全感知不到内网 IP 的存在。如果 `fw` 上没有 conntrack 自动还原机制（例如在 raw 表绕过了连接跟踪），那么 `dmz` 直接回复 `10.0.3.2` 的响应包会因为源地址是私有 IP 而被 `wan` 客户端丢弃或无法正确路由返回，此时才需要额外配置 SNAT 将源地址强制改为 `203.0.113.1`。但本实验开启了 `ip_forward` 且 iptables 默认使用 conntrack，所以无需 SNAT 也能正常工作。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：如果 conntrack 表满了，防火墙将无法正常工作。所有新连接的包（包括需要 DNAT 的外部请求和需要 SNAT 的内网访问）都无法创建映射条目，导致这些包被静默丢弃，表现为外部用户无法访问映射的服务、内网客户端也无法上网，而已建立的连接在超时前仍能继续通信。防护措施包括增大 conntrack 表容量（修改 `nf_conntrack_max` 参数）、缩短 TCP 超时时间以快速回收条目、对信任的大流量区域在 raw 表设置 `NOTRACK` 跳过连接跟踪、以及使用 `connlimit` 模块限制单个 IP 的并发连接数来防止攻击填满表项。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：这四次实验的共同基础是 **netfilter 框架和 conntrack 连接跟踪机制**。无论是判断包是否属于已建立连接（Lab7）、改写源地址做 SNAT（Lab8）、记录审计日志（Lab9）、还是改写目标地址做 DNAT（Lab10），底层都依赖 netfilter 的钩子点和 conntrack 维护的状态表来追踪每个连接的生命周期。如果把防火墙比作大楼的安保系统：**Lab7 有状态防火墙**对应的是门禁系统——只放行持有有效证件的访客（已建立连接的包），陌生人不经登记一律拦在外面；**Lab8 SNAT** 对应的是收发室统一换发对外信封——内部员工寄出的信件都盖上大楼的公章地址（将内网私有 IP 改为外网 IP），外部回信先送到大楼收发室再分发给内部人员；**Lab9 分区审计**对应的是监控摄像头和访客登记簿——记录每个区域的人员进出情况（网络命名空间隔离和 iptables 日志），形成可追溯的访问记录；**Lab10 端口映射**对应的是前台总机转接——外部访客拨打大楼总机号（公网 IP），前台根据分机号（端口）将来电转接到内部对应办公室（DNAT 到内网服务器），同时外部访客并不知道办公室的实际位置（内网 IP 不暴露）。
