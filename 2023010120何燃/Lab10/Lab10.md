...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` | 失败 | 因为`wan`没有默认路由，也没有到`10.0.3.0/24`的路由，所以这个请求不会离开`wan`命名空间。 |
| `office → 10.0.3.2:8080` | 成功 | `office`的默认网关是10.0.1.1，`fw`已开启IP转发，因此数据包能够经`fw`转发到`dmz`网段，并成功访问10.0.3.2:8080服务。 |
...
| `filter` | `FORWARD` | 1 | ACCEPT | `ctstate RELATED,ESTABLISHED` | 允许已建立和相关连接通过 |
| `filter` | `FORWARD` | 2 | ACCEPT | `dst=10.0.3.2 tcp dpt:8080 ctstate NEW` | 允许转发到10.0.3.2:8080的新TCP连接 |
...
> 答：`wan`收到的HTTP响应源IP仍然是203.0.113.1。因为对于`wan`来说，它始终认为自己是在和203.0.113.1:8080通信，DNAT只是在`fw`内部把目标地址改写成了10.0.3.2:8080。conntrack会在返回路径上自动执行反向NAT，把响应包的源地址从10.0.3.2再改回203.0.113.1，因此客户端不会感知到后端真实服务器的存在。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：同一条已经建立的连接仍然可以继续正常通信。因为conntrack已经记录了该连接对应的NAT映射关系，后续属于这条连接的数据包会直接命中已有的conntrack条目，并继续按照原来的DNAT映射进行地址转换，而不会重新查找iptables NAT规则。只有当连接断开、conntrack条目超时删除后，新建连接才会因为缺少DNAT规则而失败。
...
> 答：日志中的`DST=10.0.3.2`说明LOG规则是在DNAT已经执行完成之后才触发的，也就是说此时数据包的目标地址已经被改写成了真实后端服务器地址。通常这表示LOG规则位于`filter`表的FORWARD链，或者位于DNAT规则之后的处理阶段。如果希望记录改写前的原始目标地址203.0.113.1，则应该把LOG规则放在NAT转换发生之前，例如放在`nat`表的`PREROUTING`链中，并且要位于DNAT规则之前，这样记录到的就是客户端最初访问的公网地址。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以。LOG日志能够帮助安全人员发现端口扫描行为，因为扫描器通常会在短时间内向同一个目标IP的大量不同端口发送连接请求。从日志中可以观察到同一个源IP（连续访问多个不同的目标端口，并且大量请求都带有`SYN`标志但没有后续正常连接建立。此外，这些请求往往出现频率很高、时间间隔很短。安全人员可以根据“同一来源IP+高频访问多个端口”的特征判断存在端口扫描行为，并进一步使用防火墙规则、限速或IDS/IPS系统进行拦截与告警。
...
| :--- | :------- |
| `wan` 地址 | 203.0.113.2 |
| `fw` 四个接口地址 | 10.0.1.1, 10.0.2.1, 10.0.3.1, 203.0.113.1 |
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 | nat | PREROUTING | `dst=203.0.113.1 tcp dpt:8080` | `DNAT to:10.0.3.2:8080` |
| DNAT 2222 | nat | PREROUTING | `dst=203.0.113.1 tcp dpt:2222` | `DNAT to:10.0.3.2:2222` |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT通常发生在数据包刚进入防火墙时，也就是`nat`表的`PREROUTING`阶段，此时系统会先修改目标地址，再决定数据包该被转发到哪里，因此适合做“公网地址->内网服务器”的端口映射。SNAT则通常发生在数据包即将离开防火墙时，也就是`nat`表的`POSTROUTING`阶段，因为此时已经确定了数据包要从哪个出口接口发出，可以把源地址改写成公网地址。因此，在“外部访问DMZ服务”的场景中，`PREROUTING`负责把公网目标地址改写为DMZ内网地址；而在“DMZ主机访问外网”的场景中，`POSTROUTING`负责把内网源地址改写为公网地址，使返回流量能够正确返回到防火墙。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT会先“抢到”这个包。因为数据包进入系统后，会先经过`PREROUTING`链执行DNAT，再进行路由判断。如果DNAT已经把目标地址改写成10.0.3.2:8080，那么该包就会被转发到DMZ，而不会再交给`fw`本机的Web服务。因此本地监听在203.0.113.1:8080的服务实际上收不到这个请求。若要让两者共存，通常需要改用不同端口（例如`fw`使用8080，DMZ使用8081），或者根据来源IP、不同公网IP、不同域名等条件进行更细粒度的DNAT匹配，也可以使用反向代理统一接收请求后再转发给不同后端。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：因为conntrack会为每一条NAT连接保存完整的“五元组”映射关系，包括源IP、目标IP、源端口、目标端口以及协议类型。当外部访问203.0.113.1:8080时，conntrack会记录该连接被DNAT到10.0.3.2:8080；而访问203.0.113.1:8081时，则记录映射到10.0.3.3:8080。因此当回复包返回时，conntrack会根据已有连接状态自动查表，把回复包的源地址和源端口逆向改写回正确的公网IP与公网端口。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：在本实验中，这个包是能够回到`wan`的。因为连接是通过DNAT建立的，conntrack已经记录了完整NAT映射关系。回复包从10.0.3.2返回到`fw`后，会被conntrack自动执行“反向NAT”，把源地址从10.0.3.2改回203.0.113.1，然后再发给`wan`。因此即使没有显式配置SNAT/MASQUERADE，也能够正常完成端口映射通信。不过前提是DMZ主机必须把返回流量发送回`fw`，也就是说DMZ的默认网关必须正确配置为10.0.3.1。如果没有正确网关，回复包就无法返回到`fw`，自然也无法回到`wan`。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：如果conntrack表被占满，防火墙的有状态连接跟踪功能会受到严重影响。新的连接无法再建立对应的conntrack条目，因此NAT映射、状态防火墙规则以及RELATED/ESTABLISHED匹配都可能失效。实际现象可能包括：新连接超时、NAT转换失败、合法用户无法访问服务，甚至整个防火墙性能明显下降。这类情况常见于SYN Flood或大规模并发连接攻击。常见防护措施包括：增大conntrack表容量、缩短超时时间、启用SYN Cookie、使用连接速率限制、部署IDS/IPS，以及在边界设备提前过滤恶意流量。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：这四次实验的共同基础是Linux内核中的Netfilter/iptables框架以及conntrack连接跟踪机制。conntrack负责记录每条连接的状态，而iptables则根据不同表和链对数据包执行过滤、转发、地址转换和日志审计等操作。如果把防火墙比作大楼安保系统，那么：Lab7的有状态防火墙相当于“门禁系统”，只允许合法访客进入；Lab8的NAT像“大楼前台总机”，对外统一使用一个公共号码；Lab9的分区审计像“监控摄像头和访客登记系统”，记录谁从哪里进入哪个区域；Lab10的端口映射则像“前台转接服务”，把外部访客引导到内部不同办公室。四者共同构成了完整的网络访问控制与安全管理体系。
