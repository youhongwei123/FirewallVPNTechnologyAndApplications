...
| :--- | :--- |
| 只有第一条规则时是否成功 | 否 |
| 第一条规则放行的是什么方向的包 | 入站方向的请求包 |
| 失败的原因 | 防火墙只放行目的端口为38070的包，未放行回传响应包 |
| 第二条规则匹配的是什么 | 出站方向且源端口为38070的响应包 |
| 加第二条规则后是否成功 | 是 |
| 当前规则共几条 | 2 |
...
1. 第二条规则 `--sport 38070 -j ACCEPT` 放行的是"所有源端口为 38070 的 TCP 包"，不区分这个包是响应还是新发起的连接。如果 `wan` 侧有主机想主动连接 `lan`，只需要让程序从端口 38070 发起连接，这条规则就会放行它的 SYN 包。用一句话概括这个问题的本质：
   > 这个问题的本质在于无状态防火墙无法识别数据包的上下文上下文，导致其无法区分一个包是服务器对内网请求的被动响应，还是外部主机发起的主动攻击请求。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
   > 有状态规则能解决的典型场景是FTP等多端口协议的动态通信：在FTP主动模式下，服务器需要主动连接客户端的一个随机高位端口来传输数据，无状态规则因为无法预知该随机端口，只能选择风险极大地开放所有高位端口，或者直接导致传输失败；而有状态防火墙可以通过连接跟踪机制识别出该连接是与已知控制连接相关的“关联包”，从而实现只在传输瞬间精准、动态地放行流量，在保证业务可用的同时避免了不必要的安全风险。
...
| :--- | :--- | :--- |
| 规则条数 | 2 | 2 |
| 是否需要手动考虑返回方向 | 是，必须显式定义响应包的放行规则 | 否，状态跟踪机制会自动识别并放行属于已有连接的返回包 |
| 能否区分"新连接"和"响应包" | 不能，仅通过端口匹配，无法识别包的连接状态 | 能，通过检测TCP标志位和连接表区分 `RELATED`、`ESTABLISHED`等状态 |
| `wan` 能否主动发起到 `lan` 的连接 | 能，只要攻击者伪造源端口为38070即可绕过检查 | 不能，只有从`lan`发起的连接才会被记录，外网主动发起的包会被拦截 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
   > 有状态规则之所以不需要显式编写“返回方向”的规则，是因为防火墙内部维护着一张连接跟踪表，当它检测到`lan`主动发起一个符合规则的连接时，会自动将该连接的五元组信息记录在案；当`wan`返回响应包时，防火墙会比对包的特征与记录，发现其属于一个已建立的、合法的连接，从而利用`-m state --state ESTABLISHED`规则自动放行，无需管理员手动干预返回路径。

2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
   > 有状态规则能够阻止`wan`主动连接，是因为它强制要求只有首个包状态为`NEW`且目标符合特定放行策略时才允许建立连接，而外部主动发起的请求包无法匹配到已有的连接记录，会被默认拒绝；相比之下，无状态规则的`--sport 38070`仅仅机械地检查数据包的源端口号，无法判断这个包是针对内部请求的被动响应还是外部恶意的主动发起，因此攻击者只需伪造源端口即可轻松穿透防火墙。
...
> 防火墙放行一个`ESTABLISHED`状态的数据包时，核心依据是`conntrack`表里的记录而非单纯的包头字段。两者的本质区别在于：包头字段仅提供了数据包的瞬时静态信息，容易被攻击者伪造；而`conntrack`表记录了连接的动态历史状态，它通过对比包的五元组和序列号，确认该包是否属于之前已通过安全校验并成功建立的合法会话，只有在表中找到匹配项且状态契合时才会判定为“已建立”并予以放行。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态防火墙仅根据单个数据包的头部字段进行孤立的“静态匹配”，不具备记忆能力；而有状态检测防火墙则具备“上下文意识”，它通过`conntrack`表维护一个动态的会话数据库，能够根据数据包在连接生命周期中所处的阶段（如建立中、已建立、关联中）来做出更智能、更安全的转发决策。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：将`ESTABLISHED,RELATED`放在第一条是为了优化性能，因为在一个活跃的连接中，绝大部分数据包都属于已建立状态，优先匹配这些包可以显著降低CPU消耗并提高处理速度。虽然逻辑上可以与`NEW`调换顺序，但会导致每个后续数据包都要先经过复杂的“新连接”规则匹配，造成严重的性能浪费；且在涉及更复杂的协议干扰时，错误的顺序可能导致某些受限的连接被意外拦截。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：如果`conntrack`条目因超时被删除，属于该连接的后续数据包将被防火墙标记为`INVALID`状态，因为它在连接跟踪表中找不到对应的上下文信息。此时，防火墙通常会遵循“安全优先”原则，由于这些包既不是合法的`NEW`请求，也不属于已知的`ESTABLISHED`会话，它们会被默认规则直接丢弃，导致原有的通信中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：`RELATED`状态专门描述那些由现有连接派生出的新连接流量，它们虽然是新发起的（`NEW`），但在逻辑上与已存在的会话相关。这种状态最常出现在FTP或ICMP错误报文中，防火墙必须识别这种衍生关系，否则会因为“外网主动发起”而将其错误拦截。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：“默认拒绝”构建了一个白名单安全模型，其本质优势在于消除了“未知风险”：它要求管理员必须明确了解并定义每一项允许的业务流量，任何未被预见或未被授权的连接都会被自动拦截。相比之下，“事后补堵”属于黑名单模型，防火墙总是处于被动追赶状态，无法防御尚未被识别的恶意行为，安全性极低。
