...
| `wan` 无法回复的原因 |wan 命名空间中删除了去往 10.0.0.0/24 网段的回程路由，不知道如何将 ICMP 回复包转发给源地址 10.0.0.2，回复包无法发出，因此 lan 端收不到响应|
...
额外记录了NAT 地址映射关系：内网真实地址（10.0.0.2）与防火墙外网口转换后的地址（192.0.2.1）的对应关系，用于回程报文的地址还原。
说明：conntrack（连接跟踪）是防火墙实现状态检测和NAT 地址转换的底层核心基础，防火墙所有需要识别连接上下文的功能都依赖它维护的连接状态表。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会利用 ICMP 的 ID（标识符）/ 传输层端口号 区分不同内网主机的连接：内网不同主机发起访问时，防火墙为每条连接分配唯一的 ID / 端口标识；外网回复报文携带该标识，防火墙查询 conntrack 表，就能将回复准确转发给对应的内网主机。
...
| TCP 38070 访问是否成功 |成功|
| ping 是否成功 |失败|
| ping 失败的原因 |防火墙 FORWARD 链仅放行TCP 38070 端口的新建连接与已有相关连接，未放行 ICMP 协议，ping 的 ICMP 数据包被防火墙 DROP 丢弃|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址是互联网统一规划的保留地址，不会在公网路由器上进行路由转发，公网设备没有去往私有网段的路由，也不会处理私有地址的报文；同时私有地址可在多个内网重复使用，公网无法唯一识别。NAT 解决方案：将内网私有源地址替换为公网可路由的公有地址，让报文能在公网正常传输；回程报文到达后，NAT 再将目的地址还原为内网私有地址，完成通信。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：固定公网 IP（服务器、专线）：使用 SNAT --to-source 公网 IP，性能更高、更稳定；
动态公网 IP（家用宽带、4G/5G 上网）：使用 MASQUERADE，它会自动获取接口当前的公网 IP，无需手动配置，适配 IP 变化。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：FTP、TFTP、SIP（ VoIP ）、P2P 下载 / 远程桌面等协议会出现异常。原因：这些协议会在报文数据部分携带内网私有 IP 地址 / 端口，NAT 仅修改 IP 头部地址，不会自动修改数据段内容，导致对端收到错误的内网地址，无法建立连接。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：conntrack 是状态检测和 NAT 的共同核心底层机制：状态检测依靠它识别连接合法性，NAT 依靠它记录地址映射关系；家用路由器同时需要内网访问公网（NAT）+ 防护公网攻击（状态防火墙），集成两者可以共用一份连接跟踪表，节省资源、简化实现，是最高效的方案。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在防火墙 fw 中添加放行ICMP 新连接的规则，命令如下：
   sudo ip netns exec fw iptables -A FORWARD -p icmp -m conntrack --ctstate NEW -j ACCEPT
