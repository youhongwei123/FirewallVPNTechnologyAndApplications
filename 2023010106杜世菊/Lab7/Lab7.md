...
| :--- | :--- |
| 只有第一条规则时是否成功 |失败|
| 第一条规则放行的是什么方向的包 |lan → wan 方向，目的端口为 38070 的 TCP 包（客户端请求包）|
| 失败的原因 |服务端 wan → lan 的响应包源端口是 38070、目的端口是客户端临时端口，不匹配第一条规则，被默认 DROP 策略丢弃，客户端收不到回应，连接超时|
| 第二条规则匹配的是什么 |wan → lan 方向，源端口为 38070 的 TCP 包（服务端响应包）|
| 加第二条规则后是否成功 |成功|
| 当前规则共几条 |2条|

简答题：

1. 第二条规则 `--sport 38070 -j ACCEPT` 放行的是"所有源端口为 38070 的 TCP 包"，不区分这个包是响应还是新发起的连接。如果 `wan` 侧有主机想主动连接 `lan`，只需要让程序从端口 38070 发起连接，这条规则就会放行它的 SYN 包。用一句话概括这个问题的本质：无状态防火墙规则只检查数据包的源 / 目的端口，不区分包是主动发起的连接还是已有连接的响应，会导致安全策略被绕过。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
场景示例： 允许内网用户主动访问外网的 Web 服务，但禁止外网主动发起任何连接到内网的请求。
无状态规则：需要同时放行lan→wan目的端口 80/443 和wan→lan源端口 80/443 的包，但这样会让外网主机可以直接从 80/443 端口主动发起连接到内网，造成安全漏洞。
有状态规则：只需要放行lan→wan的新连接请求，然后用ESTABLISHED,RELATED状态放行所有已有连接的响应包，既能保证正常访问，又能彻底阻止外网主动发起的非法连接。
![无状态规则](stateless.png)
...
因为有状态防火墙通过conntrack（连接跟踪）模块，会记录下lan → wan建立的 TCP 连接。当wan返回响应包时，conntrack会识别出这个包属于  已建立连接（ESTABLISHED）的后续流量，命中第一条规则--ctstate ESTABLISHED,RELATED -j ACCEPT，因此直接放行，无需手动配置反向规则。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态规则的第二条仅允许目的端口 38070 的新连接请求（NEW 状态）通过，而wan主动发起的连接请求，目的端口是lan的临时端口，无法命中这条规则，会被默认策略 DROP。而无状态的--sport 38070规则只检查数据包的源端口，不区分它是响应包还是主动发起的连接请求，因此wan可以直接从 38070 端口主动发起连接，绕过防火墙策略。
![有状态规则](stateful.png)
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |src=10.0.0.2 sport=59156|
...
防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的记录，而不是仅依赖 IP/TCP 包头里的字段。
包头字段：包含 IP 地址、端口号、TCP 标志位（SYN/ACK/FIN 等），但这些信息只描述了单个数据包本身，无法体现数据包所属的连接上下文。攻击者可以伪造包头字段（比如伪造 ACK 包）来绕过无状态规则。
conntrack 表记录：内核会为每个 TCP/UDP 连接维护一条状态记录，记录连接的双方地址 / 端口、当前状态（NEW/ESTABLISHED/TIME_WAIT 等）、超时时间等信息。当数据包到达时，防火墙会先查询 conntrack 表，判断该包是否属于一条已存在的有效连接。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤：只检查单个数据包的包头信息（源 / 目的 IP、端口、协议、标志位），不维护连接的上下文状态。每个包都是独立判断，无法识别伪造的 “响应包”，容易被绕过。
有状态检测防火墙：通过 conntrack 维护连接的完整状态上下文，能识别数据包属于连接的哪个阶段（新建、已建立、关闭），只放行合法的响应流量，安全性更高。

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：不能调换顺序，原因如下：
iptables 规则是按从上到下的顺序匹配的，匹配到第一条符合的规则就会执行动作，不再继续往下匹配。
如果先匹配 NEW 规则，那么属于已建立连接的响应包（ESTABLISHED 状态）会先被 NEW 规则匹配到，若 NEW 规则有更严格的限制（比如只允许特定端口的新建连接），响应包就会被错误地拒绝。
正确顺序是优先放行 ESTABLISHED,RELATED 状态的流量，保证合法连接的响应包能快速通过；再用 NEW 规则控制新建连接，这样逻辑清晰且高效。

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：后续包会被内核识别为 INVALID 状态（或被当作 NEW 状态的无效连接尝试）。
防火墙的默认策略通常会拒绝 INVALID 状态的包，或者如果规则只允许 ESTABLISHED,RELATED 和合法的 NEW 包，这些后续包会因不匹配任何规则而被丢弃，导致连接异常中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED 状态描述的是与已有连接相关、但不是该连接直接收发的流量，比如：
FTP 协议的数据连接（控制连接是一条 ESTABLISHED 连接，数据连接是和它相关的另一条连接）；
ICMP 错误消息（比如 ping 超时、端口不可达的 ICMP 响应，对应一条已存在的 UDP/TCP 连接）。
本实验只有简单的 TCP 客户端 - 服务器连接，没有触发这种 “相关连接” 的场景，所以观察不到。

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：攻击面最小化：默认拒绝意味着所有流量默认被禁止，只有明确允许的服务 / 连接才能通过，大大减少了可被攻击的暴露点。
错误配置影响更小：如果规则配置错误，只会导致合法流量被拒绝，不会直接暴露安全漏洞；而 “默认允许” 的模式下，配置错误会导致非法流量直接通行，事后再堵漏洞很容易被攻击者利用。
安全策略更清晰：按需放行的规则明确列出了所有允许的流量，便于审计和维护，不容易遗漏隐藏的安全风险。
