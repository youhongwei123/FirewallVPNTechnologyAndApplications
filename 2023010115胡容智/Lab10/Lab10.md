...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |网络不可达，无法路由到内网 |
| `office → 10.0.3.2:8080` |成功 |正常返回网页目录列表 |
...
| `filter` | `FORWARD` | 1 | ACCEPT |ctstate RELATED,ESTABLISHED |放行回应包 |
...
> 答：
源 IP 是 203.0.113.1。
conntrack 会自动对回复包做反向地址转换，把内网 10.0.3.2 的源地址还原成公网 203.0.113.1，因此 wan 侧看不到真实的内网地址。
...
> 答：
后续包仍能正常通信。
因为 conntrack 表中已经记录了完整的双向映射关系，会自动完成地址改写，不再需要重新匹配 PREROUTING 链的 DNAT 规则。只有新的连接才会受 DNAT 规则删除的影响。
...
| :----- | :--- | :----------------- |
| `wan → 203.0.113.1:8080` |成功 |有（映射到 10.0.3.2:8080） |
| `wan → 203.0.113.1:2222` |成功 |有（映射到 10.0.3.2:2222） |
...
> 答：
日志中显示的是改写后的内网地址，说明 LOG 规则在 filter 表的 FORWARD 链触发，此时 nat 表的 PREROUTING 链已经完成了 DNAT 地址改写。如果要记录改写前的原始目标地址，应该把 LOG 规则放在 nat 表的 PREROUTING 链，在 DNAT 改写之前触发。


2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：
可以帮助发现端口扫描。识别方法是：短时间内同一源 IP（如 203.0.113.2）连续出现大量不同 DPT（目标端口）的日志记录，说明该源 IP 正在尝试访问不同端口，属于典型的端口扫描行为。
...
| :--- | :------- |
| `wan` 地址 |203.0.113.2 |
| `fw` 四个接口地址 |203.0.113.1（外网）、10.0.1.1（office）、10.0.2.1（guest）、10.0.3.1（dmz） |
| `wan → 203.0.113.1` ping 结果 |成功（2/2 收到，0% 丢包） |
| `wan → 10.0.3.2` ping 结果 |失败（Network is unreachable） |
| `office → 10.0.3.2` ping 结果 |成功（2/2 收到，0% 丢包） |
| `office → 203.0.113.1` ping 结果 |成功（2/2 收到，0% 丢包） |
...
| 测试项 | 结果 |
| :----- | :--- |
| `wan → 203.0.113.1:8080` |成功（返回 HTTP 响应） |
| `wan → 203.0.113.1:2222` |成功（返回 HTTP 响应） |
| `wan → 203.0.113.1:3306` |失败（无对应 DNAT 规则） |
| `wan → 10.0.3.2:8080` |失败（wan 无内网路由） |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：
   (1)DNAT 改写目标地址发生在 nat 表的 PREROUTING 链（数据包刚进入防火墙时），作用是把公网地址映射为内网地址，使流量能转发到 DMZ 服务器。
(2)SNAT 改写源地址发生在 nat 表的 POSTROUTING 链（数据包离开防火墙前），作用是把内网地址伪装为防火墙公网地址，使内网服务器的出站流量能正常回包。
(3)在 DMZ 双向 NAT 场景中，PREROUTING 负责处理入站连接的 DNAT，POSTROUTING 负责处理出站连接的 SNAT，两者配合实现双向地址转换。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：
   (1)DNAT 规则会先 “抢到” 包。因为 PREROUTING 链在本地路由决策之前执行，DNAT 会先把目标地址改写为内网服务器地址，包会被转发到 DMZ，不会到达本地服务。
(2)要让两者共存，需要调整规则顺序：将本地服务的规则放在 DNAT 规则之前，或者使用 --to-ports 做端口区分，或通过 iptables -A OUTPUT 处理本地流量。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：
   conntrack 会为每个连接记录完整的 NAT 映射关系，包括：
(1)原始请求的源 IP、源端口、目的 IP、目的端口（公网侧）
(2)改写后的源 IP、源端口、目的 IP、目的端口（内网侧）
当服务器回复包到达时，conntrack 会根据回复包的源 / 目的 IP 和端口，匹配到原始连接的记录，自动将目的端口还原为请求时使用的公网端口（如 8080/8081），确保流量能正确返回客户端。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：
   (1)能回到 wan。因为 conntrack 会自动为回复包做反向地址转换：将源地址 10.0.3.2 还原为防火墙公网地址 203.0.113.1，使 wan 侧客户端能正常接收。
(2)本实验中 wan 和 fw 在同一网段，所以无需额外 SNAT 配置。但如果 dmz 服务器主动访问外网，仍需配置 SNAT 规则，否则回复包无法通过路由返回。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：
   (1)不能正常工作。conntrack 表满后，新连接无法建立，NAT 规则无法匹配，会导致端口映射、状态检测功能失效，合法请求也会被丢弃。
(2)后果：业务中断、拒绝服务攻击成功、防火墙失去状态感知能力。
(3)防护措施：
①调整内核参数 net.netfilter.nf_conntrack_max 增大表容量
②缩短超时时间（如 nf_conntrack_tcp_timeout_time_wait）
③配置 limit 模块限制单 IP 连接数
④启用 SYN Cookies 防护 SYN Flood 攻击

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：
   (1)共同基础：conntrack 连接跟踪机制，所有功能都依赖它来维护连接状态和映射关系。
(2)安保系统类比：
①Lab7（有状态防火墙）：大楼的门禁系统，只允许合法的进出方向和状态。
②Lab8（SNAT）：内部员工的 “匿名通行证”，对外隐藏真实身份。
③Lab9（分区审计）：大楼的监控摄像头，记录进出人员的行为日志。
④Lab10（端口映射）：大楼的快递代收点，外部访客通过代收点访问内部房间，保护内部地址不暴露。
