...
| `wan` 无法回复的原因 |wan 路由表中已删除到 10.0.0.0/24 的回程路由，收到请求后不知道把回复包发往何处，因此无法生成 echo reply 响应|
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 加 NAT 后 ping 是否成功 |成功（4 packets transmitted, 4 received, 0% packet loss）|
...
答：在 Lab7 中，conntrack 仅记录连接的双向地址，用于判断数据包是否属于合法连接；在 Lab8 中，conntrack 还额外记录了 NAT 的地址映射关系（原始内网地址与转换后的外网地址的对应关系）。这两个功能都依赖 conntrack，说明了连接跟踪表是状态检测防火墙和 NAT 技术的核心基础，它不仅用于判断连接合法性，还支撑了地址转换功能，让防火墙能同时实现安全控制和内网访问外网的能力。


2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
答：当两台内网主机同时访问同一个外网服务器的同一个端口时，NAT 会通过 “外网 IP + 外网端口号” 的映射关系来区分不同连接。NAT 会为每个内网连接分配一个唯一的外网端口号，当外网回复包返回时，NAT 根据 “目的 IP + 目的端口号” 查找 conntrack 表，找到对应的内网主机地址，将目的地址还原后转发给正确的内网主机。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| TCP 38070 访问是否成功 |成功（请求状态为 HTTP/1.1 200 OK）|
| ping 是否成功 |失败（100% 丢包）|
| ping 失败的原因 |FORWARD 链默认策略为 DROP，且仅配置了允许 TCP 38070 的新连接规则，未放行 ICMP 协议，因此 ping 包被防火墙丢弃|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址（如 10.0.0.0/8）属于内网保留地址，未在互联网路由表中注册，公网设备无法识别和路由这类地址，因此内网主机无法被互联网直接访问；NAT 通过在网关设备上将内网私有 IP 转换为网关的公网 IP，同时记录地址映射关系，收到外网回复后再还原为内网地址转发给对应主机，从而实现内网主机对外网的访问。


2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：MASQUERADE 和 SNAT --to-source 在本实验中因外网 IP 固定而效果相同，实际场景中，外网 IP 固定时使用 SNAT --to-source 更高效，外网 IP 动态变化时则使用 MASQUERADE，它能自动适配当前外网接口 IP，无需手动指定。


3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT 给 FTP（主动模式）、SIP、H.323 等在载荷中携带 IP 地址和端口信息的应用层协议带来了麻烦，因为 NAT 仅修改 IP 头中的地址，不会修改载荷内的私有地址，导致外网收到的地址无效，无法建立连接，这类协议通常需要 NAT 应用层网关辅助才能正常工作。


4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：状态检测和 NAT 都依赖 conntrack 作为底层机制，状态检测需通过它判断连接状态，NAT 需通过它记录地址映射关系，家用路由器集成这两个功能可共享同一连接跟踪表，减少资源开销，同时两者配合能更安全地控制进出流量，实现 “防火墙 + NAT” 一体化。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：要在保留现有规则的基础上让 ICMP 通过，可在 FORWARD 链中添加规则：sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -j ACCEPT; sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-reply -j ACCEPT，或结合状态检测使用 sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT; sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-reply -m conntrack --ctstate ESTABLISHED -j ACCEPT。
