...
| 是否看到 `latest handshake` |是（client 端：48 seconds ago；vpn 端：1 minute, 1 second ago）|
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功 |成功（HTTP 200）|
| `client -> server:2222` | 成功 |成功（HTTP 200）|
| `client -> server ping` | 成功 |成功（之前 ping 10.20.0.2 100% 通）|
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT|所有协议，所有接口，状态为 RELATED,ESTABLISHED|允许已建立或相关的连接通过，保障回应包正常通行|
| 2 |ACCEPT|TCP 协议，入接口 wg0，源地址 10.10.10.1，目的端口 8080，状态 NEW|仅允许 client（10.10.10.1）通过 VPN 访问 server 的 8080 端口|
| 3 |LOG|所有协议，入接口 wg0，所有地址，限制日志频率 5/min|记录被拒绝的 VPN 流量，日志前缀为 VPN-DENY:|
| 4 |REJECT|所有协议，入接口 wg0，所有地址|拒绝所有不匹配前面规则的 VPN 流量，并返回 ICMP 端口不可达|
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功|第 2 条（ACCEPT）|是|
...
| `SRC=` |10.10.10.1|
| `DST=` |10.20.0.2|
| `PROTO=` |TCP（或 ICMP，根据日志类型填写，这里以访问 2222 的 TCP 日志为例）|
| `DPT=` |2222|
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |成功（无规则限制，可正常访问）|失败（被 REJECT 规则阻断）|	是（产生了 VPN-MGMT-DENY 日志）|
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |访问的是不同主机（client → server），数据包需要经过 VPN 网关转发，因此命中 FORWARD 链|
| `client -> server:2222` | `FORWARD` |同上，跨主机访问，需网关转发，命中 FORWARD 链|
| `client -> vpn:9090` | `INPUT` |访问的是 VPN 网关自身的端口，数据包直接进入本机，因此命中 INPUT 链|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有已建立 / 相关连接被断开，已正常访问的服务也无法通信|系统依赖这条规则来允许响应包和关联连接通过，删除后会导致合法流量被误拦截|恢复规则：iptables -I FORWARD 1 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |被拒绝的流量不会生成日志|iptables 规则按顺序匹配，流量先被 REJECT 规则丢弃，不会再匹配后续的 LOG 规则|将 LOG 规则移动到 REJECT 规则前面：iptables -I FORWARD <位置> -i wg0 -p tcp --dport 9090 -m limit --limit 5/min --limit-burst 10 -j LOG --log-prefix "VPN-MGMT-DENY: "|
| 管理面规则错写到 `FORWARD` |访问 vpn:9090 依然成功，规则不生效|FORWARD 链仅处理跨主机转发的流量，访问本机的流量只会经过 INPUT 链，因此 FORWARD 链的规则无法拦截本机端口访问|将规则从 FORWARD 链移动到 INPUT 链：iptables -I INPUT 1 -i wg0 -p tcp --dport 9090 -j REJECT|
...
| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功|FORWARD 链规则放行该端口流量|
| `client -> server:2222` |失败|FORWARD 链规则拒绝该端口流量|
| `client -> server ping` |失败|FORWARD 链规则拒绝 ICMP 报文|
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |所有已建立的正常业务连接（如访问 8080）全部中断|服务器回包属于 RELATED/ESTABLISHED 状态，会被默认策略拦截，无法完成 TCP 会话|恢复规则：iptables -I FORWARD 1 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT|
| `LOG` 放在 `REJECT` 后面 |被拒绝的流量不会产生日志|iptables 按顺序匹配，流量命中 REJECT 后就会被丢弃，不会继续匹配后面的 LOG 规则|将 LOG 规则调整到 REJECT 规则前面|
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 依然可以访问，规则完全不生效|FORWARD 链只处理跨主机转发流量，访问本机端口的流量只会经过 INPUT 链|将拦截规则从 FORWARD 链移动到 INPUT 链|
...
答：AllowedIPs 控制的是 WireGuard 路由层面的流量，决定哪些包会被送入隧道；iptables 是防火墙访问控制，决定隧道内流量的协议、端口、状态，还能生成审计日志。二者作用层级不同，仅靠 AllowedIPs 无法实现精细化的端口 / 状态控制，也无法做安全审计，所以不能只靠它做权限控制。

2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答：VPN 用户访问内网服务器的流量，是从 client → vpn → server 的跨主机转发流量，会经过 VPN 网关的 FORWARD 链，所以转发类规则要写在 FORWARD 链；而访问 VPN 网关自身管理端口的流量，目的地址是网关本机，只会经过 INPUT 链，因此管理面防护规则要写在 INPUT 链。

3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答：删除 ESTABLISHED,RELATED 规则后，访问 server:8080 会直接失败，因为服务器的回包属于 RELATED/ESTABLISHED 状态，会被防火墙拦截，TCP 三次握手无法完成。

4. 为什么 LOG 必须放在 REJECT 前面？
答：LOG 必须放在 REJECT 前面，因为 iptables 规则是从上到下匹配的，一旦命中 REJECT 就会终止匹配，后面的 LOG 规则永远不会被触发，也就无法生成拒绝流量的日志。

5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答：underlay 抓包只能看到 UDP 51820，说明 WireGuard 对业务流量做了加密封装，外部无法看到明文的业务内容，这对通信安全是好事；但它还是会暴露双方的公网地址和 WireGuard 使用的固定端口，可能被识别为 VPN 服务并发起定向攻击。

6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答：企业允许 VPN 用户访问整个 10.0.0.0/8 内网，会带来很多风险：用户终端被攻陷后，攻击者可以遍历整个内网；无法对不同用户做精细化权限控制；无法单独保护数据库、管理后台等敏感服务；流量被泄露或滥用时，影响范围无法限制。

7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答：client -> vpn:9090 经过 INPUT 链，因为访问的是 VPN 网关本机的端口；client -> server:8080 经过 FORWARD 链，因为访问的是跨主机的内网服务器，需要 VPN 网关转发流量，两者的不同源于流量的目的地址是否为本机。

8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答：不能把 WireGuard 隧道理解成 conntrack 的 NAT 映射，因为 NAT 映射只是修改 IP / 端口，而 WireGuard 是对整个 IP 包的加密封装；conntrack 只能分别看到加密后的外层 UDP 连接和解密后的内层业务连接，无法将两者关联为同一个会话，也无法直接反映隧道的握手状态。

9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答：VPN 网关上的 SSH 管理端口，应仅允许受信任的固定 IP 或内网地址访问，禁止公网直接访问；日志应记录访问源 IP、时间、是否允许 / 拒绝，可以用 iptables -I INPUT -s <信任IP> -p tcp --dport 22 -j ACCEPT 允许信任地址，同时添加 LOG 规则记录所有访问尝试，便于审计和告警。

10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答：当 wg show 有握手但业务访问失败时，应按以下顺序排查：① 先检查服务是否正常监听；② 再检查路由是否可达；③ 接着检查 iptables 规则是否放行；④ 然后查看拒绝日志和计数器，定位是被哪条规则拦截；⑤ 最后排查是否存在状态规则（如 RELATED/ESTABLISHED）被删除等问题。
---
