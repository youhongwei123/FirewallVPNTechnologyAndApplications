...
| `lan` 的默认网关 |10.0.0.1|
| 初始访问是否成功 |成功（request succeeded / HTTP/1.1 200 OK）|
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 只有第一条规则时是否成功 |失败（request failed: timeout）|
| 第一条规则放行的是什么方向的包 |lan 发往 wan、目的端口为 38070 的请求包|
| 失败的原因 |wan 返回的响应包目的端口是 lan 的临时随机端口，不匹配第一条规则，被 FORWARD 链默认策略 DROP 丢弃，通信无法完成|
| 第二条规则匹配的是什么 |源端口为 38070 的 TCP 包（包括 wan 发回的响应包）|
| 加第二条规则后是否成功 |成功（request succeeded，收到 HTTP/1.1 200 OK 响应）|
| 当前规则共几条 |2条|
...
答：第二条规则的本质问题是：无状态包过滤仅基于包头字段放行流量，无法区分数据包是合法连接的响应还是主动发起的新连接请求，导致外网主机可以通过伪造源端口绕过防火墙，主动向内网发起连接，造成安全漏洞。

2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
答：一个无状态规则无法解决、但有状态规则能解决的场景是：内网主机主动访问外网网站时，防火墙需要放行外网返回的响应流量，同时拒绝外网主动发起的所有入站连接请求，无状态规则无法区分响应包和主动连接请求，只能通过放行源端口来实现响应，存在被伪造源端口绕过的风险，而有状态规则通过跟踪连接状态，仅放行内网主动发起连接对应的响应包，既保证了正常访问，又阻止了外网主动向内网发起的非法连接。
![无状态规则](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2条|2条|
| 是否需要手动考虑返回方向 |是，必须额外添加源端口规则放行响应包|否，状态检测自动处理响应包|
| 能否区分"新连接"和"响应包" |不能，仅根据端口号放行|能，通过连接跟踪区分 NEW/ESTABLISHED 状态|
| `wan` 能否主动发起到 `lan` 的连接 |能，伪造源端口即可绕过规则|不能，仅允许lan主动发起的连接及其响应|

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
答：有状态规则通过内核的连接跟踪模块（conntrack）记录了lan主动发起的 TCP 连接状态，当wan返回的响应包到达防火墙时，会被识别为属于一个已建立的连接，匹配ESTABLISHED状态的规则，因此即使没有写任何返回方向的规则，响应包也能被自动放行。

2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
答：有状态规则仅允许NEW状态（即lan主动发起）的连接请求进入，而wan主动发起的连接请求是NEW状态但源地址为外网，不匹配规则；而无状态的--sport 38070规则仅基于源端口放行，无法区分包是合法响应还是伪造的新连接请求，wan主机只需从 38070 端口发起连接就能绕过规则，因此无法阻止主动连接。
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 条目中的源地址和源端口（原始方向） |10.0.0.2:50980|
...
答：防火墙放行一个 ESTABLISHED 包时，依据的是 conntrack 表里的连接记录，而不是单纯依赖包头里的字段；两者的区别在于，仅靠包头字段无法区分一个包是合法连接的响应还是伪造的新连接请求，而 conntrack 表里的记录是基于完整的 TCP 三次握手过程建立的，会记录连接的完整状态，只有属于已建立连接的数据包才会被放行，因此能有效防止伪造源端口的非法连接请求，同时自动放行合法连接的双向流量。
...
1. 无状态包过滤和有状态检测防火墙最本质的区别是什么？
   ...
   > 答：无状态包过滤仅根据单个数据包的包头字段独立判断是否放行，不维护任何连接上下文；而有状态检测防火墙会通过 conntrack 模块维护连接状态表，将每个数据包关联到其所属的连接上下文进行判断，这是两者最本质的区别。


2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？

   > 答：将ESTABLISHED,RELATED放在第一条、NEW放在第二条，是为了优先放行已建立或关联连接的后续数据包，提升处理效率；不能调换顺序，若将NEW放在前面，大量已建立连接的数据包需要先匹配NEW规则再被拒绝，会降低性能，且无法保证连接状态规则的优先级。


3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？

   > 答：如果 conntrack 条目因超时被删除，属于这条连接的后续包会被视为NEW状态的包，防火墙会按照针对NEW状态的规则处理，若没有匹配的放行规则，这些包会被默认策略丢弃，导致连接中断。

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？

   > 答：RELATED状态主要描述与已建立连接相关但独立的辅助连接流量，典型场景如 FTP 协议的数据通道，本实验未涉及多连接协议，因此未观察到该状态，在主动模式 FTP 传输文件时，控制通道已建立后，数据通道的连接请求就会被标记为RELATED状态。


5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？

   > 答：“默认拒绝，按需放行” 的安全优势在于它遵循最小权限原则，所有未明确允许的流量都会被拒绝，攻击面最小化，即使存在配置疏漏也不会暴露额外端口；而 “默认允许，事后补堵” 会先开放所有流量，再通过规则限制，容易因遗漏规则或规则被绕过导致安全漏洞，攻击面更大。
