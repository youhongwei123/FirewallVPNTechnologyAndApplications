...
| 测试 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> 10.1.0.1` | 成功 |0% 丢包，能正常 ping 通，测试成功|
| `siteB -> 10.2.0.1` | 成功 |0% 丢包，能正常 ping 通，测试成功|
| `gwA -> 192.0.2.2` | 成功 |0% 丢包，底层 underlay 网关互通正常，测试成功|
| `siteA -> 10.2.0.2` | 失败 |100% 丢包，提示目标网络不可达 ，测试失败|
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功，日志收到GET / HTTP/1.1 200请求，访问正常|
| `siteA -> siteB:2222` | 成功 |成功，日志收到GET / HTTP/1.1 200请求，访问正常|
| `siteA -> siteB ping` | 成功 |成功，0% 丢包，延迟正常，ICMP 互通|
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |放行源 10.1.0.0/4、去往 10.2.0.2、TCP 8080 新连接；放行已有相关 / 已建立连接|A-TO-B-DENY:|DROP|
| `gwB` |放行源 10.1.0.0/4、去往 10.2.0.2、TCP 8080 新连接；放行已有相关 / 已建立连接|VPN-S2S-DENY:|DROP|
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |访问正常放行，iptables 允许规则数据包计数 + 1|无拦截日志|
| `siteA -> siteB:2222` | 失败 |访问被拒绝，100% 丢包|执行journalctl查询A-TO-B-DENY无匹配条目；gwB 侧VPN-S2S-DENY无日志（符合预期）|
| `siteA -> siteB ping` | 失败 |ICMP 目标不可达，全部丢包|执行journalctl查询A-TO-B-DENY无匹配条目；gwB 侧VPN-S2S-DENY无日志（符合预期）|
...
| `gwA` underlay 地址 |192.0.2.51/24|
| `gwB` underlay 地址 |192.2.1.52/24|
| `gwB` LAN 地址 |10.2.0.1/24|
| `siteB` 地址 |10.2.0.2/24|
| underlay 是否连通 |是，两端网关 UDP 51820 互通|

### B. WireGuard 状态

| 项目 | `gwA` | `gwB` |
| :--- | :--- | :--- |
| `wg0` 地址 |10.0.0.1/24|10.0.0.2/24|
| peer `AllowedIPs` |10.2.0.0/24,10.0.0.2/32|10.1.0.0/24,10.0.0.1/32|
| `latest handshake` |有最新握手时间戳|有最新握手时间戳|
| `transfer` 计数 |存在收发字节|存在收发字节|
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功|放行 TCP 8080 NEW 流量 ACCEPT 规则|
| `siteA -> siteB:2222` |失败|LOG 记录 + REJECT 拒绝规则|
| `siteA -> siteB ping` |失败|LOG 记录 + REJECT 拒绝规则|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |公网 underlay 地址 192.2.1.51/52|UDP 51820|WireGuard加密外层隧道，看不到内网 10 段地址|
| `wg0` |内网 10.1.0.0/24、10.2.0.0/24|TCP/ICMP（明文内网包）|解密后原始内网流量，完整业务报文|
| B 站点 LAN |内网 10.1.0.2、10.2.0.2|TCP/ICMP|抵达目标局域网的明文业务流量|
...
答：远程接入 VPN：单个远程用户设备接入企业内网，一对一，客户端是个人终端，网关服务端固定；适用于员工居家办公。站点到站点 (S2S) VPN：两个完整局域网网关互联，两端内网所有主机互通，网关对等；适用于分支机构、多办公场地内网互通。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：AllowedIPs 控制两类行为：①匹配该网段的流量全部从 wg0 隧道转发；②仅接收该网段的对端入站流量。填写对端内网网段才能自动路由跨站点流量，同时过滤非法网段报文，提升安全。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：网关需要转发本地 LAN ↔ wg0 隧道之间的跨网段 IP 报文，ip_forward 关闭时系统丢弃跨网卡转发包，两端内网无法互通。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：站点内普通主机访问异地内网时，数据包需要交给本地 VPN 网关完成加密封装并通过隧道传输，如果默认网关不指向本地 VPN 网关，访问远端内网的流量只会走公网路由，无法进入 WireGuard 隧道。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：wg show 有握手仅代表底层 UDP 隧道连通，业务不通排查顺序：先检查两端网关 ip_forward 是否开启，再核对 AllowedIPs 是否包含对端内网网段，接着查看路由表确认远端网段路由出接口为 wg0，之后检查 iptables FORWARD 规则是否拦截流量，再分别在 wg0、LAN 网卡抓包定位丢包位置，最后确认两端内网网段无冲突、接口 IP 配置正确。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：gwA 与 gwB 同时配置防火墙规则能够实现双向流量管控、双向安全日志审计，形成分层防护，单侧防护失效时另一侧仍可拦截非法流量；只在一侧配置规则不行，对端主动发起的非法流量会绕过单侧防护，直接访问内网，失去访问控制效果。
