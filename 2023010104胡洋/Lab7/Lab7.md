...
| :--- | :--- |
| 只有第一条规则时是否成功 |失败 |
| 第一条规则放行的是什么方向的包 |lan → wan 方向，目的端口为38070的TCP包 |
| 失败的原因 |服务端wan发回的响应包（源端口 38070）被FORWARD默认DROP策略拦截，无法返回给客户端lan |
| 第二条规则匹配的是什么 |wan → lan方向，源端口为38070的TCP包 |
| 加第二条规则后是否成功 |成功 |
| 当前规则共几条 |2条 |
...
本质是无状态规则无法区分连接的请求包与响应包，只能基于端口放行，存在安全漏洞且无法识别完整的 TCP连接上下文。
2. 写出一个无状态规则无法解决、但有状态规则能解决的场景（不限于本实验）：
仅允许内网主机主动访问外网的HTTP服务，但拒绝外网主动向内网发起的连接请求。无状态规则无法区分主动请求的响应和主动发起的连接，而有状态规则可以只放行已建立连接的响应包，阻断外部主动发起的连接。
![无状态规则](stateless.png)
...
| :--- | :--- | :--- |
| 规则条数 |2条 |2条 |
| 是否需要手动考虑返回方向 |是，必须额外放行响应包 |否，由ESTABLISHED,RELATED规则自动处理 |
| 能否区分"新连接"和"响应包" |不能，仅基于端口/IP放行 |能，通过conntrack状态区分 |
| `wan` 能否主动发起到 `lan` 的连接 |能，只要端口匹配就会放行 |不能，仅NEW状态且目的端口为 38070的包才放行，反向主动连接不匹配 |

简答题：

1. 有状态规则没有写任何关于"返回方向"的规则，为什么 `wan` 的响应包仍然能通过 `fw`？
因为有状态规则中ESTABLISHED,RELATED会识别并放行已建立连接的响应包，内核的连接跟踪表会记录 lan主动发起的连接状态，自动允许对应的返回流量。
2. 为什么有状态规则能阻止 `wan` 主动发起到 `lan` 的连接，而无状态规则的 `--sport 38070` 做不到？
有状态规则只允许lan主动发起的新连接，wan主动发起的连接不满足这个条件；而无状态的 --sport 38070 只检查端口，无法区分连接方向，wan从 38070 端口发起的包也会被放行。
![有状态规则](stateful.png)
...
防火墙放行 ESTABLISHED 包时，依据的是 conntrack 表里的记录，而不是单纯包头里的字段。
区别：
包头字段只能看到当前包的源/目的IP、端口、标志位等信息，无法判断这个包属于一个已经建立的连接。
conntrack表会记录连接的完整生命周期状态，防火墙通过查表，确认该包属于一个已建立的连接后才放行，从而实现状态过滤。
...
无状态仅依据单个包的IP/端口/协议放行，不关心连接上下文；有状态通过conntrack记录并识别完整的连接状态（如NEW/ESTABLISHED），能区分请求包和响应包。
   > 答：

2. 有状态规则中，`ESTABLISHED,RELATED` 放在第一条、`NEW` 放在第二条，这个顺序有什么意义？能否调换？
意义是优先放行已建立连接的响应包，效率更高；不能调换，如果先匹配NEW，响应包也会被错误地当作新连接处理，可能被后续规则拦截。
   > 答：

3. 如果 conntrack 条目因超时被删除，属于这条连接的后续包会变成什么状态？防火墙会如何处理它？
后续包会被视为NEW状态；如果没有匹配的NEW规则，防火墙会按默认策略丢弃它，导致连接中断。
   > 答：

4. `RELATED` 状态主要描述什么类型的流量？本实验没有观察到它，你认为在哪种场景下会出现？
RELATED描述和已有连接相关、但独立的辅助连接，比如FTP协议的数据通道、ICMP错误消息（如端口不可达）等场景会出现。
   > 答：

5. "默认拒绝，按需放行"相比"默认允许，事后补堵"在安全上有什么本质优势？
默认拒绝只放行明确允许的流量，攻击面最小；默认允许会暴露所有未被明确禁止的服务，容易被遗漏的漏洞利用，安全边界不可控。
   > 答：
