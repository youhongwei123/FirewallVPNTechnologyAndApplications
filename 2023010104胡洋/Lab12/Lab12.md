...
| 测试项 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `client -> 192.0.2.1` | 成功 |成功（2 个包都收到，0% 丢包） |
| `server -> 10.20.0.1` | 成功 |成功（2 个包都收到，0% 丢包） |
| `client -> 10.20.0.2` | 失败 |失败（提示 “网络不可达”） |
...
| client 路由表中的 `wg0` 路由 |10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1、10.20.0.0/24 dev wg0 scope link |
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `client -> server:8080` | 成功 |成功（返回了完整的目录列表 HTML） |
| `client -> server:2222` | 成功 |成功（返回了完整的目录列表 HTML） |
| `client -> server ping` | 成功 |成功（3 个包全收到，0% 丢包） |
...
| 行号 | target | 匹配条件 | 作用 |
| :--- | :--- | :--- | :--- |
| 1 |ACCEPT |ctstate RELATED,ESTABLISHED，所有协议/接口/地址 |放行已建立或相关的连接，允许响应流量通过 |
| 2 |ACCEPT |tcp、--dport 8080、ctstate NEW，入接口wg0、出接口veth-vpn-lan，源10.10.10.1，目的 10.20.0.2 |放行从 VPN 客户端到后端服务器 8080 端口的新 TCP 连接请求 |
| 3 |LOG |所有流量、入接口wg0，限速avg 5/min burst 10，日志前缀 VPN-DENY |记录所有从VPN接口进入但未被前面规则放行的流量 |
| 4 |REJECT |所有流量、入接口wg0 |拒绝所有未被前面规则放行的VPN流量，并返回 icmp-port-unreachable错误 |
...
| :--- | :--- | :--- | :--- |
| `client -> server:8080` |成功 |规则 1（RELATED,ESTABLISHED）+ 规则 2（ACCEPT tcp dpt:8080） |增加（规则1和规则2的pkt/bytes都有计数） |
| `client -> server:2222` |失败 |规则3（LOG VPN-DENY）+ 规则 4（REJECT） |增加（规则3和规则4的pkt/bytes都更新到了 312） |
| `client -> server ping` |失败 |规则3（LOG VPN-DENY）+ 规则 4（REJECT） |增加（规则3和规则4的计数器同时记录了 ping 的 ICMP 包） |
...
| :--- | :--- | :--- | :--- |
| `client -> vpn:9090` |成功（能正常访问，返回了目录列表 HTML） |失败（curl提示无法连接服务器） |是（产生了带VPN-MGMT-DENY 前缀的日志） |
...
| 访问目标 | 经过的链 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` | `FORWARD` |目标是后端服务器而非 VPN 网关自身，流量需要由网关转发到目标主机，因此经过FORWARD链，且匹配链中放行 8080 端口的规则，请求成功。 |
| `client -> server:2222` | `FORWARD` |目标同样是后端服务器，需要网关转发，走FORWARD链；但未匹配链中放行规则，命中了LOG和REJECT规则，请求被拒绝。 |
| `client -> vpn:9090` | `INPUT` |目标 IP 是 VPN 网关自身，是发给网关的流量，因此进入网关后走INPUT链，被链中针对 9090 端口的规则拦截。 |
...
| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |已建立的连接也被拦截，如curl 8080失败 |缺少对已建立 / 相关连接的放行，响应包无法通过防火墙 |在FORWARD链开头恢复规则：iptables -A FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |看不到被拒绝流量的日志 |包命中REJECT后直接被丢弃，不会继续匹配后面的LOG规则 |将LOG规则移动到REJECT之前，确保被拒绝的流量先经过日志记录 |
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 仍然成功，FORWARD链计数器和日志都无变化 |访问 VPN 本机的流量走INPUT链，不会经过FORWARD链，因此FORWARD中的规则对其无效 |删除FORWARD链中的错误规则，将管理面拦截规则重新添加到INPUT链 |
...
| 测试 | 只建 VPN 时是否成功 |
| :--- | :--- |
| `client -> server:8080` |成功（返回了完整的目录列表 HTML） |
| `client -> server:2222` |成功（返回了完整的目录列表 HTML） |
| `client -> server ping` |成功（3 个包全收到，0% 丢包） |

### C. 加防火墙后的测试

| 测试 | 结果 | 原因 |
| :--- | :--- | :--- |
| `client -> server:8080` |成功 |流量匹配了FORWARD链中放行8080端口的ACCEPT规则，同时后续响应包匹配RELATED,ESTABLISHED规则，因此被正常转发。 |
| `client -> server:2222` |失败 |流量不匹配前面的ACCEPT规则，命中了LOG和REJECT规则，被防火墙拒绝。 |
| `client -> server ping` |失败 |ICMP 流量不匹配ACCEPT规则，同样命中了LOG和REJECT规则，被防火墙拒绝。 |
...
| :--- | :--- |
| 模拟管理服务端口 |9090 |
| 加 INPUT 规则前是否能访问 |能访问（成功返回目录列表 HTML） |
| 加 INPUT 规则后是否能访问 |不能访问（curl 提示无法连接服务器） |
| 管理面拒绝日志前缀 |VPN-MGMT-DENY |
| `client -> vpn:9090` 经过的链 |INPUT（因为目标是 VPN 网关自身的进程，不走 FORWARD 转发链） |
...
| 内层 TCP 连接目的地址 |10.20.0.2 |
| 内层 TCP 目标端口 |8080 |
| 内层连接状态 |TIME_WAIT（连接已完成并等待关闭） |
| 外层 UDP 源地址 |192.0.2.2 |
| 外层 UDP 目的地址 |192.0.2.1 |
| 外层 UDP 目标端口 | 51820|
| WireGuard 隧道状态应看 `conntrack` 还是 `wg show` |wg show（conntrack 只跟踪解密后的内层业务连接，不负责显示 WireGuard 加密隧道本身的状态） |

### G. 故障排查

| 故障 | 现象 | 原因 | 修复方法 |
| :--- | :--- | :--- | :--- |
| 删除 `ESTABLISHED,RELATED` |client -> server:8080 请求失败 / 超时，连接无法建立 |只允许了客户端发起的新请求，但服务器返回的响应包因没有状态规则放行，被默认策略拒绝 |在 FORWARD 链开头恢复规则：iptables -A FORWARD -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT |
| `LOG` 放在 `REJECT` 后面 |访问被拒绝，但看不到预期的日志 |iptables 按顺序匹配，包命中 REJECT后直接被丢弃，不会继续匹配后面的 LOG 规则 |将 LOG 规则移动到 REJECT 之前，确保被拒绝的流量先被记录 |
| 管理面规则错写到 `FORWARD` |client -> vpn:9090 仍然成功，FORWARD 链计数器和日志均无变化 |访问 VPN 网关自身的流量走 INPUT链，不会经过 FORWARD 链，因此 FORWARD 中的规则对其无效 |删除 FORWARD 链中的错误规则，将管理面拦截规则添加到 INPUT 链 |
...
答:AllowedIPs:控制哪些网段的流量走 WireGuard 隧道，是 “路由 + 接收” 的粗粒度控制。
iptables：在网关层面做端口 / 协议级的细粒度访问控制。
只靠 AllowedIPs 无法限制用户访问内网的具体端口和服务，无法实现 “只允许访问 8080，拒绝 2222” 这类策略。
2. 为什么 VPN 用户访问内网服务器时，规则要写在 `FORWARD` 链，而不是 `INPUT` 链？
答:访问内网服务器的包是 “经过网关转发” 的流量，目的地不是网关自身，因此走 FORWARD 链，而非 INPUT链。
3. 本实验中 `ESTABLISHED,RELATED` 规则如果删除，访问 `server:8080` 会出现什么现象？
答:server:8080 的访问会失败：客户端能发请求，但服务器的响应包会被防火墙拦截，导致连接超时。
4. 为什么 LOG 必须放在 REJECT 前面？
答:iptables 按顺序匹配规则，命中 REJECT 后会直接丢弃包，不会再执行后面的 LOG 规则，所以日志规则必须写在前面。
5. underlay 抓包只能看到 UDP 51820，这对通信安全意味着什么？它还能暴露哪些信息？
答:安全意义：内层业务数据被加密，第三方无法读取内容，保证了通信机密性。
暴露信息：能看到公网 IP、端口、流量大小、握手包特征，可识别 VPN 通信行为。
6. 如果一个真实企业允许 VPN 用户访问整个内网 `10.0.0.0/8`，可能带来哪些风险？
答:用户一旦接入 VPN，可访问所有内网资源，可能导致数据泄露、横向渗透、单点被攻陷后全网受影响。
7. `client -> vpn:9090` 和 `client -> server:8080` 分别经过哪条 iptables 链？为什么不同？
答:client -> vpn:9090：走 INPUT 链，目标是网关自身进程。
client -> server:8080：走 FORWARD 链，目标是需要转发的内网服务器。
8. conntrack 能看到 WireGuard 外层 UDP 包和内层 TCP 连接。为什么不能把 WireGuard 隧道本身理解成 conntrack 的 NAT 映射？
答:conntrack 只跟踪解密后的内层业务连接，不维护外层加密隧道状态。
WireGuard 隧道由自身协议维护，不是 NAT 映射，无法通过 conntrack 查看握手和传输状态。
9. 如果 VPN 网关上有 SSH 管理端口，应该允许哪些来源访问？应该如何写日志？
答:只允许可信 IP（如运维管理网段）访问，拒绝所有 VPN 客户端网段。
在 INPUT 链配置规则，添加日志前缀记录访问尝试，便于审计。
10. 当 `wg show` 有握手但业务访问失败时，应该按什么顺序检查服务监听、路由表、防火墙规则、日志和计数器？
答:检查 server 服务是否正常监听；
检查 client 和 vpn 上的路由表；
检查 iptables 规则及计数器；
查看 journalctl 日志，定位被拒绝的流量。
