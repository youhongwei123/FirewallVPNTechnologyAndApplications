...
| `wan` 无法回复的原因 |防火墙（fw）未配置 ICMP响应的NAT回包规则，或未开启相关转发/允许规则，导致请求能到达WAN侧但无法生成并返回ICMP echo reply包。 |
...
Lab7（防火墙连接判断）：conntrack记录了连接的状态，帮助防火墙判断数据包是否属于一个已建立的合法连接，以此决定是否放行。
Lab8（NAT）场景：conntrack额外记录了地址/端口的NAT映射关系（如原始源地址、转换后的地址），这样防火墙收到外网回复包时，才能把目的地址还原回内网主机地址。
说明：conntrack是Linux网络状态管理的核心，它不仅是防火墙状态检测的基础，也是NAT功能的关键支撑，是连接跟踪与地址转换的统一底层模块。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT为每条内网连接生成唯一映射条目（含内网原地址端口、转换后外网地址端口、目标服务器地址端口）。外网回复包到达时，NAT根据包的 “目的地址 + 端口” 匹配映射表，将回复转发给对应内网主机。多台内网主机访问同一外网服务时，NAT会分配不同的外网端口，以此区分连接。
...
| TCP 38070 访问是否成功 |成功 |
| ping 是否成功 |失败 |
| ping 失败的原因 |防火墙规则仅放行：已建立/相关的连接（ESTABLISHED,RELATED）目标端口为38070的新建TCP连接ICMP请求既不属于已放行的状态，也不符合TCP 38070的端口规则，被默认的FORWARD DROP策略丢弃。 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址（如10.0.0.0/8）仅在内网路由，公网路由器不识别/转发其流量，因此无法被直接访问。NAT将内网主机的私有地址转换为路由器的公网地址，使内网主机能以公网身份与外网通信，外网回复也能被NAT映射回内网主机。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source <固定IP>适合固定公网IP场景，性能更高；MASQUERADE 适合动态公网IP（如拨号上网）场景，会自动适配当前接口 IP，但性能略低。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT会影响在载荷中内嵌IP地址/端口的协议，如 FTP、SIP、IPsec 等。因为NAT只修改 IP/端口头，不会修改载荷中的地址信息，导致连接无法正常建立。这类协议通常需要ALG（应用层网关）辅助才能穿透NAT。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：两者都依赖conntrack（连接跟踪表）：状态检测用它判断连接合法性，NAT用它维护地址映射。集成后可共用一套连接跟踪表，实现高效的状态检测与 NAT 地址转换，减少系统开销，同时保障安全性和连接性。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：添加一条允许 ICMP 新建请求的规则：sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -m conntrack --ctstate NEW -j ACCEPT
