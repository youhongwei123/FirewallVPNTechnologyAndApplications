...
| 返回码（Response Code）      |0|
| 回答记录数（Answers）        |4|
| 返回的 IP 地址               |202.89.233.101, 202.89.233.100|
| TTL（若可见）                |117|
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名服务器）的作用就是将人类可读的名称（域名）转换为机器识别的IP地址，供计算机相互连接；计算机网络通信依赖IP而非域名，DNS就是负责将域名转换为IP。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致；用于匹配DNS请求和响应报文，区分不同的DNS查询结果，防止混淆，并辅助抵御DNS劫持或欺诈攻击。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：告诉DNS服务器查询的情况是需要这个域名的IPv4地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：通常使用UDP协议，使用53端口进行传输；当相应数据超过512字节或进行区域传送时会使用TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：1. 该函数是为IPv4设计的，无法兼容IPv6，如果是在IPv6的环境下无法正常工作；2. 该函数只能解析主机名，不能处理服务器名或端口号，导致功能单一缺乏灵活性；3. POSIX.1-2001 已将其标记为过时，POSIX.1-2008 更是将其从规范中移除。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：该域名下的服务采用了负载均衡、高可用性（灾备）或内容分发网络（CDN）技术。通过这种方式，DNS服务器将用户流量分散到多台服务器上，从而提高网站的整体处理能力、访问速度和抗风险能力。
