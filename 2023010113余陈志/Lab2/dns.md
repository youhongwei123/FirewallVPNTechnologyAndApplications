...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用是将人类易记的域名（如 www.yxnu.edu.cn）转换为机器可识别的 IP 地址（如 202.203.86.93）。
访问网站时需先进行 DNS 解析，因为网络通信依赖 IP 地址定位主机，必须先获取目标服务器的 IP 才能建立连接。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致，请求包与响应包的 Transaction ID 均为 0x4b4d。
该字段用于匹配请求与对应的响应，使客户端能区分不同查询的应答，防止乱序或伪造响应。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录表示将域名解析为 IPv4 地址.

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用 UDP 协议，端口 53。
当响应数据超过 512 字节（如 DNSSEC 或大量记录）、或进行区域传输（AXFR/IXFR）时，会改用 TCP 以保证完整性。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：不支持 IPv6，无法处理 AAAA 记录。
非线程安全，内部使用静态缓冲区，在多线程环境下易产生数据竞争。
缺乏可重入性，且功能有限，现代工程建议使用 getaddrinfo()。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常说明该域名采用了 负载均衡 或 高可用部署，不同 IP 对应不同的服务器，用于分散访问压力或提供冗余。
